...
export default function () {
  ...
  let landingRes = http.get("https://uniz.rguktong.in", {
    tags: { step: "landing" },
  });
  ...
  let signinPageRes = http.get("https://uniz.rguktong.in/student/signin", {
    tags: { step: "signin_page" },
  });
  ...
  if (!loginSuccess) {
    console.error(
      `[AUTH_FAIL] Student: ${studentId}, Status: ${loginRes.status}, Body: ${loginRes.body}`,
    );
    sleep(1);
    return;
  }
  ...
  // --- 2. GET PROFILE (/me) ---
  let profileRes = http.get(`${BASE_URL}/profile/student/me`, {
    headers: authHeaders,
  });
  ...
  if (!profileSuccess) {
    console.error(
      `[PROFILE_FAIL] Student: ${studentId}, Status: ${profileRes.status}`,
    );
  }
  ...
  let gradesRes = http.get(
    `${BASE_URL}/academics/grades?studentId=${studentId}&semester=SEM-1&year=E1`,
    { headers: authHeaders, tags: { step: "metadata" } },
  );
  ...
  let attendanceRes = http.get(
    `${BASE_URL}/academics/attendance?studentId=${studentId}&semester=SEM-1&year=E1`,
    { headers: authHeaders, tags: { step: "metadata" } },
  );
