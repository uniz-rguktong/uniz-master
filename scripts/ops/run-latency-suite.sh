...
run python3 "$ROOT/scripts/ops/audit-prod-latency.py"
...
if [[ -z "${TOKEN:-}" ]]; then
  ...
  run python3 "$ROOT/scripts/ops/audit-auth-latency.py"
  header "3/4 Functional student API smoke"
  run bash "$ROOT/scripts/ops/test-student-apis.sh"
fi

header "4/4 Peak load profile"
if [[ "$RUN_LOAD" == "1" ]]; then
  run env CONCURRENCY="$CONCURRENCY" bash "$ROOT/scripts/ops/peak-load-profile.sh"
else
