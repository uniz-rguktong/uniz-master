...
cd "$ROOT_DIR/uniz-auth" && PORT=3001 npx ts-node src/index.ts > "$ROOT_DIR/logs/auth.log" 2>&1 &
echo "Auth Service started on 3001"
sleep 5

cd "$ROOT_DIR/uniz-user" && PORT=3002 npx ts-node src/index.ts > "$ROOT_DIR/logs/user.log" 2>&1 &
echo "User Service started on 3002"
sleep 5

cd "$ROOT_DIR/uniz-outpass" && PORT=3003 npx ts-node src/index.ts > "$ROOT_DIR/logs/outpass.log" 2>&1 &
echo "Outpass Service started on 3003"
sleep 5

cd "$ROOT_DIR/uniz-academics" && PORT=3004 npx ts-node src/index.ts > "$ROOT_DIR/logs/academics.log" 2>&1 &
echo "Academics Service started on 3004"
sleep 5

cd "$ROOT_DIR/uniz-files" && PORT=3005 npx ts-node src/index.ts > "$ROOT_DIR/logs/files.log" 2>&1 &
echo "Files Service started on 3005"
sleep 5

cd "$ROOT_DIR/uniz-mail" && PORT=3006 npx ts-node src/index.ts > "$ROOT_DIR/logs/mail.log" 2>&1 &
echo "Mail Service started on 3006"
sleep 5

cd "$ROOT_DIR/uniz-notifications" && PORT=3007 npx ts-node src/index.ts > "$ROOT_DIR/logs/notification.log" 2>&1 &
echo "Notification Service started on 3007"
sleep 5

cd "$ROOT_DIR/uniz-cron" && PORT=3008 npx ts-node src/index.ts > "$ROOT_DIR/logs/cron.log" 2>&1 &
echo "Cron Service started on 3008"

# Start the Node-based Gateway API (which we built for Docker)
cd "$ROOT_DIR/uniz-gateway" && PORT=3000 npx ts-node src/index.ts > "$ROOT_DIR/logs/gateway.log" 2>&1 &
