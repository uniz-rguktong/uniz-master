...
for path in "${!service_prefixes[@]}"; do
    if [ -d "$path" ]; then
        ...
        gsed -i 's/REDACTED_TURNSTILE_SECRET/1x00000000000000000000000000000000/g' "$path/.env" 2>/dev/null || sed -i '' 's/REDACTED_TURNSTILE_SECRET/1x00000000000000000000000000000000/g' "$path/.env"
