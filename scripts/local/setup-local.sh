...
install_hint_docker() {
  case "$OS" in
    ...
    windows) echo "Prefer WSL2; see docs/local/LOCAL_SETUP.md" ;;
...
info "   Full guide: docs/local/LOCAL_SETUP.md"
info ""
...
if ! $COMPOSE_CMD -f "$COMPOSE_FILE" up -d uniz-redis uniz-postgres; then
  fail "Failed to start Docker containers.\n   Fix: docker logs uniz-postgres — see docs/local/LOCAL_SETUP.md#troubleshooting"
fi
