...
echo "=========================================================="
echo "🛡️  UNIZ MULTI-SERVICE MONOLITHIC SYNC (V6 - SEED FOCUS)"
echo "=========================================================="
...
npx prisma migrate reset --force --skip-seed --skip-generate --schema prisma/monolith.prisma || echo "⚠️  Advisory lock timeout on reset, proceeding with push-only sync..."
...
for SERVICE in "${ALL_SERVICES[@]}"; do
    ...
    if grep -q '"seed":' package.json; then
        ...
        echo "⏭️  No seed script for $SERVICE, skipping."
    fi
