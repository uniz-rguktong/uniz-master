...
#   export TOKEN='...'
#   CONCURRENCY=25  bash scripts/peak-load-profile.sh
#   CONCURRENCY=100 bash scripts/peak-load-profile.sh
#
# VPS synthetic users (no Turnstile, invasive — prod DB):
#   CONCURRENCY=50 node scripts/load_test_core.js
#
...
if [[ -z "${TOKEN:-}" ]]; then
  ...
  echo ""
  python3 "$ROOT/scripts/audit-prod-latency.py"
  exit 0
fi
