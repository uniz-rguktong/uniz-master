...
async function generate() {
  ...
  const academicPrisma = new PrismaClient({
    datasourceUrl: process.env.DATABASE_URL_ACADEMIC,
  });
  ...
  try {
    ...
    console.log(
      `Found ${students.length} students and ${subjects.length} subjects.`,
    );
    ...
    XLSX.utils.book_append_sheet(
      wbAtt,
      XLSX.utils.aoa_to_sheet(attHeaders),
      "Template",
    );
    fs.writeFileSync(
    ...
    console.log("Successfully generated full batch files in tests/data/");
  } finally {
