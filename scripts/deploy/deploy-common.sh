...
#
# Production path (VPS scale): build docker/prod/Dockerfile.service → GHCR → kubectl.
# Local demo path (any OS):    docker/local/compose.yml + Dockerfile.service.local.
# Those two paths must never mix — Compose is not invoked by CI/deploy.
...
resolve_build_paths() {
  ...
  if uses_monorepo_dockerfile "$DIR"; then
    BUILD_CONTEXT="."
    # Production/CI ONLY — never docker/local/* (Compose fast-path).
    DOCKERFILE="docker/prod/Dockerfile.service"
    return
  fi
  ...
  if [[ "${DOCKERFILE:-}" == *".local"* ]] || [[ "${DOCKERFILE:-}" == *"docker/local/"* ]]; then
    echo "[Error] Refusing local Compose paths for production/CI builds: $DOCKERFILE" >&2
    return 1
  fi
}
...
service_should_build() {
  ...
  if [ -n "$CHANGED_FILES" ] && echo "$CHANGED_FILES" | grep -qE '^package\.json$|^package-lock\.json$|^docker/prod/Dockerfile\.service$'; then
    if echo " $MONOREPO_SERVICES " | grep -q " $DIR "; then
      return 0
    fi
  fi
