...
deploy_logic() {
  ...
  if [ -f "scripts/deploy/prisma-migrate-deploy-all.sh" ]; then
  ...
  if [ "$CURRENT_BRANCH" == "main" ]; then
    ...
    if [ "$FORCE_ALL" == "true" ] || echo "$CHANGED_FILES" | grep -qE "^(${LANDING_BACKEND_DIR}/|scripts/deploy(\.sh|/deploy\.sh)|scripts/(deploy/)?refresh-landing-backend-env\.sh)"; then
...
if [ -f "/root/uniz-secrets.env" ]; then
  ...
  ssh -o StrictHostKeyChecking=no root@76.13.241.174 << EOF
    ...
    git clean -fd
    /bin/bash ./scripts/deploy/deploy.sh "remote-trigger"
EOF
fi
