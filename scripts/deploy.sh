    ...
    if [ "$SHOULD_BUILD" == "true" ]; then
      if [ -z "${BUILT_IMAGES[$IMG]}" ]; then
        ...
        if [[ "$DIR" == "uniz-portal" ]]; then
          ...
          BUILD_ARGS="--build-arg NEXT_PUBLIC_VAPID_PUBLIC_KEY=$ORNATE_VAPID_PUBLIC_KEY --build-arg DATABASE_URL=$ORNATE_DATABASE_URL --build-arg DIRECT_URL=$ORNATE_DIRECT_URL --build-arg REDIS_URL=$ORNATE_REDIS_URL --build-arg R2_PUBLIC_DOMAIN=$ORNATE_R2_PUBLIC_DOMAIN --build-arg R2_PUBLIC_URL=$ORNATE_R2_PUBLIC_DOMAIN --build-arg NEXT_PUBLIC_ASSETS_URL=$NEXT_PUBLIC_ASSETS_URL --build-arg NEXTAUTH_URL=$ORNATE_LANDING_NEXTAUTH_URL"
        elif [[ "$DIR" == "ornate-core" ]]; then
          BUILD_ARGS="--build-arg NEXT_PUBLIC_VAPID_PUBLIC_KEY=$ORNATE_VAPID_PUBLIC_KEY --build-arg DATABASE_URL=$ORNATE_DATABASE_URL --build-arg DIRECT_URL=$ORNATE_DIRECT_URL --build-arg REDIS_URL=$ORNATE_REDIS_URL --build-arg NEXTAUTH_URL=$NEXTAUTH_URL"
        fi
