...
async function runRealStudentTest() {
  console.log(
    `🚀 Fasten your seatbelts! Simulating ${CONCURRENCY} real students on UniZ-Core!`,
  );
  ...
  const studentsRaw = runDbQuery(
    "uniz_auth",
    `SELECT username, "passwordHash" FROM "AuthCredential" WHERE role='STUDENT' AND "isDisabled"=false AND "username" NOT LIKE 'LOAD_TEST%' LIMIT ${CONCURRENCY}`,
  );
  ...
  console.log(
    `✅ Real students hijacked, resetted passwords safely: ${successFlows}/${testUsers.length} in ${timeEnd - timeStart}ms`,
  );
    ...
    testUsers.map(async (user) => {
      try {
        ...
        if (loginData.success && (loginData.token || loginData.accessToken)) {
          ...
          const [meRes, gradesRes, attendanceRes, ana1, ana2] =
            await Promise.all([
              ...
              fetch(
                `${ANALYTICS_BASE_URL}/api/analytics/student/${user.username}/attendance`,
                {
                  headers: {
              ...
              fetch(
                `${ANALYTICS_BASE_URL}/api/analytics/student/${user.username}/grades-trend`,
                {
                  headers: {
                    Authorization: `Bearer ${token}`,
                    "x-api-key": ANALYTICS_KEY,
  ...
  console.log(
    `✅ Parallel Data Extracts Completed (/me, /grades, /attendance): ${dataPulls}/${logins}`,
  );
  console.log(
    `✅ Analytics Fetches Completed (Bearer token used): ${analyticsHits}/${logins} in ${timeEnd - timeStart}ms`,
  );

  console.log(`\n🧹 [Rollback] Restoring Real Passwords...`);
  // Build batch restore query
  const restoreQueries = testUsers
    .map(
      (u) =>
        `UPDATE "AuthCredential" SET "passwordHash" = '${u.originalHash}' WHERE username = '${u.username}';`,
    )
    .join(" ");
