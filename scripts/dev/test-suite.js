...
async function run() {
  ...
  log.step("Fetching OTP from Database");
  try {
    const { Client } = require("pg");
    const dbUrl = process.env.AUTH_DATABASE_URL;

    if (!dbUrl) {
      throw new Error("AUTH_DATABASE_URL not found in environment");
    }
    ...
    try {
      const res = await client.query(
        'SELECT otp FROM auth_v2."OtpLog" WHERE username = $1 ORDER BY "createdAt" DESC LIMIT 1',
        ["O210008"],
      );
      otp = res.rows[0]?.otp;
    } catch (sqlErr) {
      log.info(
        `Schema-qualified fetch failed, trying default: ${sqlErr.message}`,
      );
      const res = await client.query(
        'SELECT otp FROM "OtpLog" WHERE username = $1 ORDER BY "createdAt" DESC LIMIT 1',
        ["O210008"],
      );
      otp = res.rows[0]?.otp;
    }
    ...
    if (!otp) throw new Error("No OTP found in database for O210008");
    ...
    log.info(`OTP Bypass failed: ${e.message}, skipping verification steps.`);
  }
