...
try {
  ...
  envFile.split('\n').forEach(line => {
    if (line.startsWith('INTERNAL_SECRET=')) internalSecret = line.split('=')[1].replace(/["']/g, '');
    if (line.startsWith('AUTH_DATABASE_URL=')) authDbUrl = line.split('=')[1].replace(/["']/g, '');
  });
} catch(e) {}
...
async function runLoadTest() {
  console.log(`🚀 Starting UniZ-Core Load Test with ${CONCURRENCY} concurrent users...`);
  ...
  console.log(`✅ [Setup] ${createdCount}/${TEST_USERS_COUNT} test users ready.`);
  ...
  await Promise.all(testUsers.map(async () => {
    try {
  ...
  console.log(`✅ Frontend Hits: ${success}/${TEST_USERS_COUNT} successful in ${timeEnd - timeStart}ms`);
  ...
  await Promise.all(testUsers.map(async (user) => {
    try {
      ...
      const otp = runDbCommand(`SELECT otp FROM "OtpLog" WHERE username='${user.username}' ORDER BY "createdAt" DESC LIMIT 1;`);
      if (!otp) return;
  ...
  timeEnd = Date.now();
  console.log(`✅ Full Reset Flows: ${success}/${TEST_USERS_COUNT} completed in ${timeEnd - timeStart}ms`);
  ...
  console.log(`✅ Parallel Data Fetches(/me, /grades, /attendance): ${dataSuccess}/${TEST_USERS_COUNT} completed in ${timeEnd - timeStart}ms`);


  console.log(`\n🧹 [Cleanup] Removing test users...`);
  runDbCommand(`DELETE FROM "AuthCredential" WHERE username LIKE 'LOAD_TEST_STU_%'; DELETE FROM "OtpLog" WHERE username LIKE 'LOAD_TEST_STU_%';`);
