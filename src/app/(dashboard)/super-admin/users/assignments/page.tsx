import { Building2, Music, Trophy, Heart, Search, UserPlus, RefreshCw, MoreVertical } from 'lucide-react';
...
const ASSIGNMENTS = [
    {
        ...
        icon: Building2,
        admins: [
            { name: 'John Doe', image: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop', email: 'cse@admin.com' }
        ],
        status: 'Active'
    },
    {
        ...
        icon: Building2,
        admins: [
            { name: 'Sarah Conn', image: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&h=100&fit=crop', email: 'ece@admin.com' }
        ],
        status: 'Active'
    },
    {
    ...
    },
    {
        ...
        icon: Trophy,
        admins: [
            { name: 'Mike Ross', image: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=100&h=100&fit=crop', email: 'sports@admin.com' }
        ],
        status: 'Active'
    },
    {
        ...
        icon: Heart,
        admins: [
            { name: 'Rachel Zane', image: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100&h=100&fit=crop', email: 'hho@admin.com' }
        ],
        status: 'Active'
    }
];
        ...
        <div className="p-6 md:p-8 max-w-[1600px] mx-auto">
            <div className="flex flex-col gap-4 mb-8">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-[#1A1A1A]">Portal Assignments</h1>
                        <p className="text-sm text-[#6B7280]">Manage which admins control which portals and departments.</p>
                    </div>
                ...
                <div className="bg-white rounded-[14px] border border-[#E5E7EB] overflow-hidden shadow-sm">
                    <table className="w-full">
                        <thead className="bg-white border-b border-[#F3F4F6]">
                            <tr>
                                <th className="px-6 py-4 text-left text-xs font-semibold text-[#9CA3AF] uppercase tracking-wider">Resource / Portal</th>
                                <th className="px-6 py-4 text-left text-xs font-semibold text-[#9CA3AF] uppercase tracking-wider">Type</th>
                                <th className="px-6 py-4 text-left text-xs font-semibold text-[#9CA3AF] uppercase tracking-wider">Assigned Admins</th>
                                ...
                                <tr key={item.id} className="hover:bg-[#FAFAFA] transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="font-medium text-[#1A1A1A]">{item.resource}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        ...
                                        {item.admins.length > 0 ? (
                                            <div className="flex items-center gap-2">
                                                {item.admins.map((admin: any, idx: any) => (
                                                    <div key={idx} className="flex items-center gap-2 pr-4">
                                                        <Image
                                                            src={admin.image}
                                                            alt={admin.name}
                                                            width={28}
                                                        ...
                                                        <div className="flex flex-col">
                                                            <span className="text-sm text-[#1A1A1A] leading-tight">{admin.name}</span>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium 
                                            ...
                                            <span className={`w-1.5 h-1.5 rounded-full ${item.status === 'Active' ? 'bg-green-600' : 'bg-yellow-600'}`}></span>
                                            {item.status}
                                        </span>
                                    </td>
