...
import { sendCertificateEmail, getParticipationEmailTemplate } from '@/lib/email';
 ...
 * The general proxy rate limit (100/min per IP) is too permissive for this.
 */
...
export async function POST(request: NextRequest) {
    ...
    if (distributionRateLimiter) {
        ...
        if (!rl.success) {
            logger.warn({ adminId: admin.id, remaining: rl.remaining }, 'distribute.participation.rate_limited');
        ...
        if (!hasLoggedDistributionFallback) {
            logger.warn(
                { adminId: admin.id, alert: 'RATE_LIMIT_FALLBACK_MEMORY', layer: 'cert-distribute-participation' },
                '[RATE_LIMIT_FALLBACK_MEMORY] Participation distribution limiter using in-memory fallback (Redis unavailable).'
            );
    ...
    try {
        ...
        const templatePath = path.join(process.cwd(), 'src', 'templates', 'participation.html');
        if (!fs.existsSync(templatePath)) {
            return NextResponse.json({ error: 'Participation template not found' }, { status: 500 });
        }
        ...
        logger.info({ eventId, registrationCount: event.registrations.length }, 'distribute.participation.start');
        ...
        for (const reg of event.registrations) {
            try {
                ...
                errorCount++;
                logger.error({ err, eventId, registrationId: reg.id }, 'distribute.participation.cert.error');
            }
        }

        const durationMs = Math.round(performance.now() - startMs);
        logger.info({ eventId, count, emailCount, errorCount, durationMs }, 'distribute.participation.complete');
