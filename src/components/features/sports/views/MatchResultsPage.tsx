  ...
  useEffect(() => {
    ...
    getSportWinnerAnnouncement(sport.id).then((res) => {
      if (res.success && res.data) {
        ...
        setPositions({
          first: { branch: pos.first?.branch || pos.first || '', email: pos.first?.email || '' },
          second: { branch: pos.second?.branch || pos.second || '', email: pos.second?.email || '' },
          third: { branch: pos.third?.branch || pos.third || '', email: pos.third?.email || '' },
        });
      }
    });
  }, [sport.id, loaded]);

  const updatePos = (place: 'first' | 'second' | 'third', field: 'branch' | 'email', value: string) =>
              ...
              {medals.map(({ label, key, icon, bg, border, text }) => {
                ...
                return (
                  <div key={key} className={`flex-1 flex flex-col items-center py-2 px-1 rounded-xl ${bg} border ${border} shadow-sm transition-transform hover:scale-105`}>
                    ...
                    <div className="font-bold text-[11px] text-[#1A1A1A] mt-1 text-center truncate w-full px-1">{branch}</div>
                  </div>
                );
              })}
            </div>
    ...
    <div className="p-4 md:p-8 animate-page-entrance bg-white min-h-full">
      <div className="mb-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-6">
          <div>
            <h1 className="text-2xl md:text-[28px] font-semibold text-[#1A1A1A] mb-2">Championship Results</h1>
            <p className="text-sm text-[#6B7280]">Official records of all completed matches and victory points.</p>
          </div>
                              ...
                              <div className="flex items-center justify-between gap-1 mb-4 relative z-10 flex-1">
                                ...
                                <div className={`flex flex-col items-center gap-3 flex-1 transition-all duration-500 ${match.winner === match.team1Name ? 'opacity-100 scale-105' : 'opacity-70'}`}>
                                  ...
                                  <div className="text-center w-full">
                                    <div className="font-bold text-slate-800 text-[13px] leading-tight mb-1 truncate px-1">{match.team1Name}</div>
                                    <div className={`text-4xl font-bold tracking-tighter ${match.winner === match.team1Name ? 'text-slate-800' : 'text-slate-300'}`}>{match.score1 || '0'}</div>
                                  </div>
                                </div>
                                ...
                                <div className={`flex flex-col items-center gap-3 flex-1 transition-all duration-500 ${match.winner === match.team2Name ? 'opacity-100 scale-105' : 'opacity-70'}`}>
                                  ...
                                  <div className="text-center w-full">
                                    <div className="font-bold text-slate-800 text-[13px] leading-tight mb-1 truncate px-1">{match.team2Name}</div>
                                    <div className={`text-4xl font-bold tracking-tighter ${match.winner === match.team2Name ? 'text-slate-800' : 'text-slate-300'}`}>{match.score2 || '0'}</div>
                                  </div>
                                </div>
                              </div>
