...
const calendarEvents = [
  { id: 1, name: 'AI/ML Workshop', category: 'Workshops', color: '#8B5CF6', date: 15, time: '10:00 AM', isArchived: false },
  { id: 2, name: 'Tech Quiz', category: 'Quizzes', color: '#F59E0B', date: 18, time: '2:00 PM', isArchived: true },
  { id: 3, name: 'Hackathon', category: 'Hackathons', color: '#EF4444', date: 20, time: '9:00 AM', isArchived: false },
  { id: 4, name: 'Robotics Challenge', category: 'Technical', color: '#3B82F6', date: 22, time: '11:00 AM', isArchived: false },
  { id: 5, name: 'Gaming Tournament', category: 'Fun Games', color: '#10B981', date: 25, time: '3:00 PM', isArchived: true },
  { id: 6, name: 'Project Expo', category: 'Project Expo', color: '#06B6D4', date: 28, time: '10:00 AM', isArchived: false }];
...
export function EventCalendarView({ events = [], selectedFilter, showArchived, showDrafts }: EventCalendarViewProps) {
  ...
  const allMappedEvents = (events.length > 0 ? events.map(e => {
    ...
    return {
      ...
      date: parsedDate && !Number.isNaN(parsedDate.getTime()) ? parsedDate.getDate() : null,
      month: parsedDate && !Number.isNaN(parsedDate.getTime()) ? parsedDate.getMonth() : null,
  ...
  }) : calendarEvents.map(e => ({ ...e, month: currentDate.getMonth(), year: currentDate.getFullYear() })));
  ...
  const getEventsForDay = (day: any) => {
    return allMappedEvents.filter((event: any) => {
      ...
      const isArchived = event.isArchived || event.status?.toUpperCase() === 'CANCELLED' || event.status?.toUpperCase() === 'ARCHIVED';

      const sameDay = event.date === day &&
        (event.month === undefined || event.month === null || event.month === currentDate.getMonth()) &&
        (event.year === undefined || event.year === null || event.year === currentDate.getFullYear());

      if (!sameDay) return false;

      if (showArchived) return isArchived && (selectedFilter === 'All' || event.category === selectedFilter);
      if (showDrafts) return isDraft && (selectedFilter === 'All' || event.category === selectedFilter);

      return !isDraft && !isArchived && (selectedFilter === 'All' || event.category === selectedFilter);
    });
  };
  ...
  const getEventsForExactDate = (date: Date) => {
    return allMappedEvents.filter((event: any) => {
      ...
      const isSameDate = event.date === date.getDate() &&
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E5E7EB] p-6">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto">
          {['Month', 'Week', 'Day'].map((v: any) =>
            <button
              key={v}
              onClick={() => setView(v)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${view === v ?
        ...
        <div className="overflow-x-auto">
          <div className="grid grid-cols-7 gap-px bg-[#E5E7EB] border border-[#E5E7EB] rounded-lg overflow-hidden min-w-200">
            ...
            {emptyDays.map((_: any, index: any) =>
              <div key={`empty-${index}`} className="bg-white p-3 min-h-30"></div>
            )}
            ...
            {days.map((day: any) => {
              ...
              return (
                <div
                  ...
                  <div className={`text-sm font-semibold mb-2 ${isToday ? 'text-[#1A1A1A]' : 'text-[#6B7280]'}`
                  }>
                    {day}
                  </div>
                  <div className="space-y-1">
                    {dayEvents.map((event: any) =>
                      <div
                        key={event.id}
                        onClick={() => showToast(`${event.name} - ${event.time}`, 'info')}
      ...
      {view === 'Week' && (
        <div className="grid grid-cols-1 md:grid-cols-7 gap-3">
          {getWeekDays(currentDate).map((date, index) => {
            ...
            return (
              <div key={index} className={`border border-[#E5E7EB] rounded-lg p-3 bg-white ${isToday ? 'ring-2 ring-[#1A1A1A] ring-inset' : ''}`}>
                <div className="text-xs font-semibold text-[#6B7280] mb-2">
                  {date.toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric' })}
                </div>
                <div className="space-y-1">
                  {dayEvents.length === 0 && <div className="text-xs text-[#9CA3AF]">No events</div>}
                  {dayEvents.map((event: any) => (
                    <div
                      key={event.id}
                      onClick={() => showToast(`${event.name} - ${event.time}`, 'info')}
        ...
        <div className="border border-[#E5E7EB] rounded-lg p-4 bg-white">
          ...
          <div className="space-y-2">
            {getEventsForExactDate(currentDate).length === 0 && (
              <div className="text-sm text-[#9CA3AF]">No events for this day.</div>
            )}
      ...
      <div className="flex flex-wrap items-center gap-4 mt-6 pt-6 border-t border-[#E5E7EB]">
        <div className="text-xs font-semibold text-[#6B7280] w-full sm:w-auto">CATEGORIES:</div>
        {[
