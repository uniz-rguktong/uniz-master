...
const fetcher = async ([key, isArchived, isSent]: [string, boolean, boolean]): Promise<Notification[]> => {
...
export function useNotifications(isArchived = false, isSent = false) {
    ...
    const markAsRead = async (id: string) => {
        ...
        mutate(
            ['notifications', isArchived, isSent],
            (current: Notification[] | undefined) => current?.map((n: any) => (n.id === id ? { ...n, isRead: true } : n)),
            false
        );
    ...
    const toggleReadStatus = async (id: string) => {
        mutate(
            ['notifications', isArchived, isSent],
            (current: Notification[] | undefined) => current?.map((n: any) => (n.id === id ? { ...n, isRead: !n.isRead } : n)),
            false
        );
    ...
    const toggleStarStatus = async (id: string) => {
        mutate(
            ['notifications', isArchived, isSent],
            (current: Notification[] | undefined) => current?.map((n: any) => (n.id === id ? { ...n, isStarred: !n.isStarred } : n)),
            false
        );
    ...
    const toggleArchiveStatus = async (id: string) => {
        ...
        mutate(
            ['notifications', isArchived, isSent],
            (current: Notification[] | undefined) => current?.filter((n: any) => n.id !== id),
            false
        );
