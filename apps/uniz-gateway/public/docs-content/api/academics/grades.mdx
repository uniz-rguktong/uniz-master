...
<Info>
  `GET /academics/grades` is served from a cache layer and typically responds in under 50 ms.
</Info>
...
<ResponseField name="source" type="string">
  `cache` if served from the cache layer, `db` if fetched directly from the database.
</ResponseField>
...
<ParamField body="file" type="file" required>
  The completed Excel file (`.xlsx`) previously downloaded from the template endpoint.
</ParamField>
...
<Tip>
  Poll `GET /academics/grades/upload/progress` with the returned `jobId` to track processing status.
</Tip>
...
<Tip>
  Poll `GET /academics/grades/publish/progress` to monitor email dispatch status.
</Tip>
