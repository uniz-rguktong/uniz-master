...
<ParamField body="date" type="string">
  ISO 8601 date of the class session (e.g. `2026-03-15`). Defaults to today if omitted.
</ParamField>
...
<Tip>
  Poll `GET /academics/attendance/upload/progress` with the returned `jobId` to track processing status.
</Tip>
...
<Warning>
  This endpoint is deprecated. Use the download endpoint (`GET /academics/attendance/download/:semesterId`) to export attendance reports. This endpoint is retained for audit compliance and may be removed in a future version.
</Warning>
