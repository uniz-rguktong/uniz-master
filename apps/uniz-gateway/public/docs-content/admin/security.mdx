...
<Info>
Log in with the `security_admin` username. The Security Portal is role-restricted — other admin roles cannot access the gate control functions.
</Info>
...
<Warning>
Manually updating presence status bypasses the outpass verification check. Only use this endpoint to correct data errors, not as a substitute for the standard check-in/check-out flow.
</Warning>
