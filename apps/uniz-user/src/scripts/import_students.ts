...
async function importStudents() {
  console.log("Starting Batch Student Import...");

  const admissionDataDir =
    "/Users/sreecharandesu/Projects/uniz-master/admissiondata";

  const files = [
    { name: "2020-21_RGUKT AP_ONGOLE(20)F.xlsx", year: "E4", sheet: 1 }, // Changed to E4 as per system logic
  ...
  for (const fileInfo of files) {
    ...
    for (let i = 2; i <= worksheet.rowCount; i++) {
      ...
      try {
        // For speed, we just log and would do a batch prisma call.
        // But for reliability we use upsert.
        // In a real script we would chunk this.
  ...
  console.log(`\n--- IMPORT COMPLETE ---`);
  console.log(`Total Students in System: ${totalImported}`);
}
