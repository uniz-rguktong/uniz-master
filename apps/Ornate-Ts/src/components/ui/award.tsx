...
export function Awards({
    ...
    // Stamp Variant
    if (variant === "stamp") {
        // ... we can strip unused ones, but let's keep it mostly as asked
    ...
    if (variant === "award") {
        const LaurelWreath = () => (
            <svg
                className={cn(
                    "absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 opacity-80 group-hover:opacity-100 transition-opacity duration-500",
                    "w-full h-full scale-[1.3] pointer-events-none fill-[var(--award-color)]"
                )}
                width="892.77"
                height="688.08"
        ...
        return (
            <div
                className={cn(
                    "relative z-0 group",
                    "flex flex-col items-center justify-center p-4 h-full min-h-[160px]",
                    "overflow-visible",
                    className
                )}
            ...
            >
                <LaurelWreath />
                <div className="z-10 px-4 text-center flex flex-col items-center justify-center gap-1.5">
                    {/* Main Title / WINNER label */}
                    <h1 className={cn("text-[10px] sm:text-xs font-bold tracking-[0.2em] text-[var(--award-color)] uppercase drop-shadow-sm")}>{title}</h1>

                    {/* Subtitle / Best UI/UX Design - Break words neatly */}
                    <h2 className={cn("text-xl sm:text-2xl font-black text-white hover:text-[var(--award-color)] transition-colors duration-300 drop-shadow-md leading-tight max-w-[200px]")}>
                        {subtitle}
                    </h2>

                    {/* Organization / Date */}
                    <div className={cn("text-[9px] sm:text-[10px] font-bold tracking-[0.2em] uppercase text-[var(--award-color)] mt-1 break-words max-w-[160px]")}>{description}</div>

                    {/* Recipient / Name */}
                    {recipient && (
                        <p className={cn("text-[10px] sm:text-xs font-bold tracking-widest text-white/90 uppercase mt-0.5")}>{recipient}</p>
                    )}
                </div>
            </div>
        )
