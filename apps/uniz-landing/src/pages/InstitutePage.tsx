...
import type {
  InstituteData,
  InstitutePage as InstitutePageType,
} from "../types/api";
            ...
            {data?.sections && data.sections.length > 0 && (
              <div className="space-y-8">
                {data.sections.map((section, idx) => (
                  ...
                  >
                    <h2 className="text-2xl font-bold text-[#000035] mb-6">
                      {section.title}
                    </h2>
              ...
              <div className="mt-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {data.profiles.map((profile, idx) => (
                    <div
                      key={idx}
                      className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col md:flex-row gap-6 items-start hover:shadow-md transition-shadow"
                    >
                      {profile.photo ? (
                        <img
                          src={profile.photo}
