...
export function DepartmentPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      if (deptCode) {
        setLoading(true);
        const res = await getDepartmentStaff(
          deptCode.toUpperCase() as DeptCode,
        );
        setData(res);
        setLoading(false);
      }
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        {!data || !data.faculties || data.faculties.length === 0 ? (
          <p className="text-slate-500 text-lg">
            No designated faculty information available for this department.
          </p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {data.faculties.map((faculty, index) => (
              ...
              >
                <div className="h-32 bg-gradient-to-r from-navy to-navy w-full flex items-end justify-center pb-4"></div>
                <div className="px-6 pb-6 pt-0 flex flex-col items-center text-center -mt-16 flex-grow">
                    ...
                    <img
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src =
                          ...
                          encodeURIComponent(faculty.name) +
                          "&background=random";
                      }}
                    />
                  ) : (
                  ...
                  <h3 className="text-xl font-bold text-slate-900 mb-1 group-hover:text-[#000035] transition-colors">
                    {faculty.name}
                  </h3>
                  <a
                    href={`mailto:${faculty.email}`}
                    className="text-[#000035] text-sm font-medium hover:underline mb-4 break-all"
                  >
                    {faculty.email}
                  </a>
                  ...
                  {faculty.bio && Object.keys(faculty.bio).length > 0 ? (
                        ...
                        .map(([key, value]) => {
                          ...
                          return (
                            <div key={key} className="flex flex-col">
                              ...
                              <span className="truncate" title={strVal}>
                                {strVal}
                              </span>
                            </div>
                          );
                    ...
                    <div className="mt-auto pt-4 text-sm text-slate-400 italic">
