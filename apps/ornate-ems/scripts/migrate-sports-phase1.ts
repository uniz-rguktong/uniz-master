...
function mapGender(
  ...
  if (upper === "GIRLS" || upper === "FEMALE" || upper === "WOMEN")
    return "FEMALE";
  if (upper === "MIXED" || upper === "CO-ED" || upper === "COED")
    return "MIXED";
  return "MALE";
}
...
function mapFormat(
  raw: string | undefined | null,
): "KNOCKOUT" | "LEAGUE" | "GROUP_KNOCKOUT" {
...
function mapMatchWinner(raw: string | null): "TEAM1" | "TEAM2" | "DRAW" | null {
  if (!raw) return null;
  const lower = raw.toLowerCase().trim();
  if (lower === "team1" || lower === "team_1" || lower === "home")
    return "TEAM1";
  if (lower === "team2" || lower === "team_2" || lower === "away")
    return "TEAM2";
...
async function migrate() {
    ...
    try {
      ...
      const sport = await prisma.sport.create({
        data: {
          ...
          minPlayersPerTeam:
            (event as any).teamSizeMin ?? safeInt(cf.minPlayers),
          maxPlayersPerTeam:
            (event as any).teamSizeMax ?? safeInt(cf.maxPlayers),
  ...
  for (const event of sportEvents) {
    ...
    for (const reg of event.registrations) {
      try {
        ...
        if (!err.message.includes("Unique constraint")) {
          stats.errors.push(
            `SportRegistration ${reg.studentId}: ${err.message}`,
          );
        }
      }
    }
  ...
  for (const event of sportEvents) {
    ...
    if (mappedGender === "MIXED")
      genderVariants.push("Mixed", "mixed", "MIXED", "Co-ed", "co-ed");
  ...
  try {
    ...
    for (const match of matches) {
      ...
      if (sportId) {
        ...
        stats.errors.push(
          `Match ${match.id}: no sport found for "${match.sport}" + "${match.gender}"`,
        );
      }
    }
  } catch (err: any) {
    ...
    console.log(
      `   Skipped match linking (columns may already be migrated): ${err.message}`,
    );
  }
  ...
  console.log(
    `   Fixed ${stats.matchDatesFixed} dates, ${stats.matchWinnersFixed} winners\n`,
  );
