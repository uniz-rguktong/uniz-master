...
async function main() {
    ...
    if (!creator) {
        console.error("❌ Creator 'cse@admin.com' not found. Please run main seed first.");
        return;
    }
    ...
    const existingLogo2 = await prisma.brandLogo.findFirst({ where: { name: 'CSE Department Logo' } });
    if (!existingLogo2) {
        await prisma.brandLogo.create({
            data: {
    ...
    const existingLogo3 = await prisma.brandLogo.findFirst({ where: { name: 'University Logo' } });
    if (!existingLogo3) {
        await prisma.brandLogo.create({
            data: {
