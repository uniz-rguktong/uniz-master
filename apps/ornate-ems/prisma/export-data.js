...
async function exportData() {
  ...
  try {
    ...
    const registrations = await exportTable(
      "Registration",
      "registrations.json",
    );
    const snapshots = await exportTable(
      "AnalyticsSnapshot",
    ...
    const announcements = await exportTable(
      "Announcement",
      "announcements.json",
    );
    const winners = await exportTable(
      "WinnerAnnouncement",
      "winner-announcements.json",
    );
    const tasks = await exportTable("Task", "tasks.json");
    const notifications = await exportTable(
      "Notification",
      "notifications.json",
    );
    ...
    const galleryAlbums = await exportTable(
      "GalleryAlbum",
      "gallery-albums.json",
    );
    const galleryImages = await exportTable(
      "GalleryImage",
      "gallery-images.json",
    );
    ...
    const festSettings = await exportTable(
      "FestSettings",
      "fest-settings.json",
    );
    const certThemes = await exportTable(
      "CertificateTheme",
      "certificate-themes.json",
    );
    const bestStudents = await exportTable(
      "BestOutgoingStudent",
      "best-outgoing-students.json",
    );
