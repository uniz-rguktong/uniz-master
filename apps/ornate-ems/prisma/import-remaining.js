...
async function importRemaining() {
        ...
        try {
            await prisma.auditLog.create({
                data: {
                    ...
                    entityId: r.entityId, metadata: r.metadata || null,
                    performedBy: r.performedBy, ipAddress: r.ipAddress || null,
                    userAgent: r.userAgent || null, timestamp: new Date(r.timestamp)
                }
            });
        } catch (e) { errors.push(`AuditLog: ${e.message}`); }
    }
    ...
    for (const r of snap) {
        try {
            await prisma.analyticsSnapshot.create({
                data: { ...r, snapshotDate: new Date(r.snapshotDate), createdAt: new Date(r.createdAt) }
            });
    ...
    for (const r of pv) {
        try {
            await prisma.promoVideo.create({
                data: { ...r, uploadDate: new Date(r.uploadDate), createdAt: new Date(r.createdAt), updatedAt: new Date(r.updatedAt) }
            });
        } catch (e) { errors.push(`PromoVideo: ${e.message}`); }
    }
    ...
    for (const r of ga) {
        try {
            await prisma.galleryAlbum.create({
                data: { ...r, createdAt: new Date(r.createdAt), updatedAt: new Date(r.updatedAt) }
            });
        } catch (e) { errors.push(`Album: ${e.message}`); }
    }
    ...
    for (const r of bl) {
        ...
        } catch (e) { errors.push(`Logo: ${e.message}`); }
    }
    ...
    for (const r of fs2) {
        try {
            await prisma.festSettings.create({
                data: {
                    id: r.id || 'singleton', festName: r.festName || 'ORNATE 2K26',
                    tagline: r.tagline || null, logoUrl: r.logoUrl || null,
                    brochureUrl: r.brochureUrl || null, description: r.description || null,
                    startDate: r.startDate ? new Date(r.startDate) : null,
                    endDate: r.endDate ? new Date(r.endDate) : null,
                    venue: r.venue || null, registrationOpen: r.registrationOpen ?? true,
                    ...
                    instagramUrl: r.instagramUrl || null, youtubeUrl: r.youtubeUrl || null,
                    supportEmail: r.supportEmail || null, updatedAt: new Date(r.updatedAt)
                }
            });
        } catch (e) { errors.push(`FestSettings: ${e.message}`); }
    }
        ...
        try {
            await prisma.certificateTheme.create({
                data: {
                    ...
                    colors: r.colors || [], style: r.style,
                    createdAt: new Date(r.createdAt),
                    updatedAt: new Date(r.updatedAt || r.createdAt)
                }
            });
        } catch (e) { errors.push(`CertTheme: ${e.message}`); }
    }
    ...
    for (const r of bos) {
        ...
        } catch (e) { errors.push(`BOS: ${e.message}`); }
    }
