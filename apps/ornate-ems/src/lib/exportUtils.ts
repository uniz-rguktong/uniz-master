 ...
 */
export const downloadCSV = (data: any[], filename: string = 'export.csv', headers: Record<string, string> | null = null): void => {
    ...
    // 2. Add Header Row
    csvContent += displayHeaders.map(h => `"${String(h).replace(/"/g, '""')}"`).join(',') + '\n';
 ...
 */
export const exportRegistrationsToCSV = (registrations: ExportRegistration[], filename: string = 'registrations.csv'): void => {
    if (!registrations || !registrations.length) return;

    const exportData = registrations.map(reg => ({
        'Registration ID': reg.registrationId || reg.id?.substring(0, 8).toUpperCase(),
 ...
 */
export const exportTeamsToCSV = (teams: ExportTeam[], filename: string = 'team_registrations.csv'): void => {
    ...
    const exportData = teams.map(team => ({
        ...
        'Members': team.members?.map(m => `${m.name} (${m.studentId})`).join('; ') || 'N/A'
    }));
