    ...
    try {
      ...
      if (options?.rateLimit) {
        ...
        await enforceServerActionRateLimit({
          ...
          identifier: ip,
          config: {
            ...(options.rateLimit.limit !== undefined && {
              limit: options.rateLimit.limit,
            }),
            ...(options.rateLimit.window !== undefined && {
      ...
      if (error instanceof ServerActionRateLimitError) {
        return apiResponse(
          false,
          null,
          `Rate limit exceeded. Try again in ${error.retryAfter}s.`,
          429,
        );
      }
...
async function getServerActionIdentifier(): Promise<string> {
  ...
  try {
    ...
    logger.debug(
      { err: error },
      "Could not resolve server action headers for rate-limit identifier",
    );
  }
...
export function safeFormDataAction<R = any>(
  ...
  return async (formData: FormData): Promise<ActionResponse<R>> => {
    try {
      ...
      logger.error(
        { err: error, action: "FormData mutation" },
        "FormData Action Error",
      );
      if (error instanceof ZodError) {
        return {
          success: false,
