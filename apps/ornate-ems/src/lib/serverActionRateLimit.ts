...
export class ServerActionRateLimitError extends Error {
    ...
    constructor(params: { actionName: string; retryAfter: number; limit: number; remaining: number }) {
...
const inMemoryFallbackStore = new Map<string, { count: number; resetAt: number }>();
let hasLoggedFallbackNotice = false;

const DEFAULT_LIMIT = 30;
const DEFAULT_WINDOW: `${number} ${'s' | 'm' | 'h' | 'd'}` = '1 m';

function parseWindowToMs(window: `${number} ${'s' | 'm' | 'h' | 'd'}`): number {
    const [amountRaw, unitRaw] = window.split(' ') as [string, 's' | 'm' | 'h' | 'd'];
...
function getLimiter(config?: ServerActionRateLimitConfig): Ratelimit | null {
    if (!redis) {
        return null;
    }
...
export async function enforceServerActionRateLimit(input: EnforceServerActionRateLimitInput): Promise<void> {
