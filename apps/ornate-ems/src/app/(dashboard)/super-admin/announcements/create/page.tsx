...
const ANNOUNCEMENTS = [
  {
    ...
    date: "Nov 5, 2025",
    description:
      "The registration deadline for AI/ML Workshop has been extended to November 15, 2025. All interested students can register now.",
    views: 1245,
  ...
  },
  {
    ...
    date: "Nov 4, 2025",
    description:
      "Team formation portal is now open for Hackathon 2025. Maximum team size is 5 members. Form your teams before November 10.",
    views: 892,
  ...
  },
  {
    ...
    date: "Nov 3, 2025",
    description:
      "Tech Quiz Competition venue has been changed from Seminar Hall to Main Auditorium due to increased registrations.",
    views: 567,
  ...
  },
  {
    ...
    date: "Nov 2, 2025",
    description:
      "The schedule for Cultural Night has been released. All participants are requested to check their performance timings.",
    views: 1823,
    audience: "All Students",
    expires: "Nov 12",
  },
  {
    ...
    date: "Nov 1, 2025",
    description:
      "Library will remain open till 10 PM during the fest week to accommodate student preparation needs.",
    views: 456,
    audience: "Everyone",
    expires: "Nov 20",
  },
...
export default function CreateAnnouncementPage() {
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        if (response.success) {
          ...
          showToast(
            response.error || "Failed to load announcement history",
            "error",
          );
        }
      } catch (error) {
  ...
  const handleSaveEdit = async () => {
    ...
    setHistory((prev: any) =>
      prev.map((item: any) => (item.id === editingId ? response.data : item)),
    );
    setEditingId(null);
  ...
  const handleSaveScheduledEdit = () => {
    if (
      !scheduledEditForm.title?.trim() ||
      !scheduledEditForm.description?.trim()
    ) {
      showToast("Title and content are required", "error");
      return;
    }
  ...
  const audienceOptions = [
    ...
    },
    {
      ...
      label: "Club Admins",
      hasSubOptions: true,
      subOptions: CLUBS.map((c) => ({ id: `club_${c.id}`, name: c.name })),
    },
  ...
  const toggleAudience = (audienceId: any) => {
    ...
    setSelectedAudiences((prev: any) => {
      ...
      if (withoutEveryone.includes(audienceId)) {
        const newSelection = withoutEveryone.filter(
          (id: any) => id !== audienceId,
        );
  ...
  const toggleParentCategory = (option: any) => {
    ...
    const allSelected = allSubIds.every((id: any) =>
    ...
    setSelectedAudiences((prev: any) => {
      ...
      if (allSelected) {
        ...
        const newSelection = withoutEveryone.filter(
          (id: any) => !allSubIds.includes(id),
        );
        return newSelection.length === 0 ? ["everyone"] : newSelection;
      } else {
        // Select all sub-options
        const existingWithoutThese = withoutEveryone.filter(
          (id: any) => !allSubIds.includes(id),
        );
  ...
  const isParentSelected = (option: any) => {
    ...
    return option?.subOptions.every((s: any) =>
      selectedAudiences.includes(s.id),
    );
  };

  const isParentPartiallySelected = (option: any) => {
    if (!option.hasSubOptions) return false;
    const someSelected = option?.subOptions.some((s: any) =>
      selectedAudiences.includes(s.id),
    );
    const allSelected = option?.subOptions.every((s: any) =>
      selectedAudiences.includes(s.id),
    );
      ...
      for (const opt of audienceOptions) {
        ...
        if (opt.hasSubOptions) {
          const sub = opt.subOptions?.find(
            (s: any) => s.id === selectedAudiences[0],
          );
    ...
    <div className="p-6 md:p-8 max-w-[1200px] mx-auto">
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <InfoTooltip
                text="Create and send important updates to students and staff"
                size="md"
              />
            </div>
            <p className="text-sm text-[#6B7280]">
              Send push notifications, emails, and portal alerts instantly.
            </p>
          </div>
          <button
      ...
      <div className="mb-10">
        <div className="flex items-center gap-2 mb-4">
          ...
          <InfoTooltip
            text="Compose, audience, distribution, and scheduling in one section"
            size="sm"
          />
        </div>
              ...
              <div className="lg:col-span-2 space-y-6">
                <div>
                  <label className="block text-sm font-medium text-[#1A1A1A] mb-2">
                    Subject / Title <span className="text-[#EF4444]">*</span>
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <h3 className="font-bold text-[#1A1A1A] mb-4 flex items-center gap-2">
                    ...
                    <InfoTooltip
                      text="Choose where this announcement will be displayed"
                      size="sm"
                    />
                  </h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <label className="flex items-center justify-between p-4 border border-indigo-100 bg-indigo-50 rounded-xl cursor-pointer">
                      ...
                      <input
                        ...
                        checked={channelPortalBanner}
                        onChange={(e) =>
                          setChannelPortalBanner(e.target.checked)
                        }
                      />
                    </label>
                    ...
                    <label className="flex items-center justify-between p-4 border border-gray-100 rounded-xl cursor-pointer hover:bg-gray-50">
                      ...
                      <input
                        ...
                        checked={channelPushNotification}
                        onChange={(e) =>
                          setChannelPushNotification(e.target.checked)
                        }
                      />
                    </label>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <InfoTooltip
                          text="Select specific groups or everyone"
                          size="sm"
                        />
                      </div>
                      <p className="text-xs text-[#6B7280]">
                        Select who will receive this
                      </p>
                    </div>
                  </div>
                          ...
                          <div key={option.id}>
                            <div className="flex items-center px-4 py-3 hover:bg-[#F7F8FA] transition-colors border-b border-gray-50 last:border-b-0">
                              ...
                              {option.hasSubOptions && (
                                ...
                                >
                                  <ChevronRight
                                    className={`w-4 h-4 text-gray-400 transition-transform ${expandedCategories.includes(option.id) ? "rotate-90" : ""}`}
                                  />
                                </button>
                              )}
                            </div>
                  ...
                  <div className="mt-4 p-4 bg-[#F7F8FA] rounded-xl border border-[#E5E7EB]">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {getSelectedList().map((item: any, idx: any) => (
                        <span
                          key={idx}
                          className="px-3 py-1.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-xs font-semibold rounded-full shadow-sm"
                        >
                          {item}
                        </span>
                      ))}
                ...
                <div>
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[#6B7280]">
                        When to send this announcement
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 mb-4">
                    <button
                      onClick={handleSendNow}
                      disabled={isSubmitting}
                      className="flex-1 py-2.5 bg-[#1A1A1A] text-white rounded-xl text-sm font-semibold hover:bg-[#2D2D2D] transition-colors disabled:opacity-60"
                    >
                      {isSubmitting ? "Sending..." : "Send Now"}
                    </button>
                    <button
                      onClick={handleScheduleLater}
                      className="flex-1 py-2.5 bg-[#F7F8FA] border border-[#E5E7EB] text-[#6B7280] rounded-xl text-sm font-semibold hover:bg-gray-100 transition-colors"
                    >
                      Schedule later
                    </button>
                  </div>
                  <div className="p-3 bg-[#F7F8FA] rounded-xl border border-[#E5E7EB]">
                    <p className="text-xs text-[#6B7280]">
                      ...
                      <span className="font-bold text-[#1A1A1A]">
                        {selectedAudiences.includes("everyone")
                          ? "2,450"
                          : selectedAudiences.length * 150}{" "}
                        Users
                      </span>
                    </p>
                  </div>
      ...
      <div className="mt-10">
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="flex items-center gap-2">
              ...
              <InfoTooltip
                text="Announcements queued to be sent on selected date and time"
                size="sm"
              />
            </div>
            <p className="text-sm text-[#6B7280]">
              Only scheduled announcements are shown here.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {scheduledAnnouncements.length === 0 ? (
            <div className="bg-[#F4F2F0] rounded-[18px] p-[10px]">
              <div className="bg-white rounded-[14px] p-6 text-sm text-[#6B7280]">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-3">
                          ...
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-semibold ${getTypeColor(announcement.category)}`}
                          >
      ...
      <div className="mt-10">
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-bold text-[#1A1A1A]">
                Updates & Announcements History
              </h2>
              <InfoTooltip text="Log of all sent communications" size="sm" />
            </div>
            <p className="text-sm text-[#6B7280]">
              View and manage all past announcements and updates.
            </p>
          </div>
        </div>
            ...
            <div className="bg-white rounded-[14px] p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-red-500">
                  {history.filter((a) => a.category === "Important").length}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-[14px] p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-emerald-500">
                  {history.filter((a) => a.category === "Events").length}
                </p>
              </div>
            </div>
          </div>
          <div className="bg-[#F4F2F0] rounded-[18px] pt-[5px] px-[5px] pb-[15px]">
            <div className="bg-white rounded-[14px] p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-[#1A1A1A]">
                  {history
                    .reduce((sum: any, a: any) => sum + (a.viewCount || 0), 0)
                    .toLocaleString()}
                </p>
              </div>
            </div>
        ...
        <div className="flex items-center gap-2 mb-6">
          {["All", "Important", "Events", "Updates", "Information"].map(
            (filter: any) => (
              <button
                key={filter}
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-[#E5E7EB]">
                    <div className="flex items-center gap-6">
                      <div className="flex items-center gap-2 text-[#6B7280]">
                        ...
                        <span className="text-sm">
                          {(announcement.viewCount || 0).toLocaleString()} views
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-[#6B7280]">
                        <Layout className="w-4 h-4" />
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setEditForm((prev: any) => ({ ...prev, title: e.target.value }))
              }
            />
          </div>
          <div>
