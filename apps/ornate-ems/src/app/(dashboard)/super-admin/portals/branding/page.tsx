    ...
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">
              Manage logos, banners, and color themes for each portal.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          <h3 className="text-sm font-bold text-[#6B7280] uppercase tracking-wider px-2">
            Select Portal
          </h3>
          {[
            ...
            <button
              key={i}
              onClick={() =>
                setSelectedPortal(p.includes("CSE") ? "cse" : "ece")
              }
              className={`w-full text-left px-4 py-3 rounded-xl text-sm font-medium transition-colors
                                ${
                                  selectedPortal ===
                                  (p.includes("CSE") ? "cse" : "ece")
        ...
        <div className="lg:col-span-3 space-y-8">
          {PORTAL_ASSETS.filter((a) => a.id === selectedPortal).map(
            (brand: any) => (
              <div
                key={brand.id}
                  ...
                  <div className="bg-white rounded-[14px] p-6">
                    ...
                    <div className="flex gap-4">
                      {brand.colors.map((c: any, i: any) => (
                        <div
                          key={i}
                          className="group relative w-16 h-16 rounded-2xl cursor-pointer shadow-sm ring-1 ring-gray-100"
                          style={{ backgroundColor: c }}
                        >
                          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <span className="bg-black/75 text-white text-[10px] px-2 py-1 rounded">
                              {c}
                            </span>
                          </div>
                        </div>
                  ...
                  <div className="bg-white rounded-[14px] p-6">
                    ...
                    <div className="w-full h-48 relative rounded-xl overflow-hidden group">
                      ...
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <button className="bg-white px-4 py-2 rounded-lg text-sm font-semibold hover:scale-105 transition-transform">
                          Change Banner
                        </button>
                      </div>
                    </div>
