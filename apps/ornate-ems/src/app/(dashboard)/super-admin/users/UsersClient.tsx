...
export default function UsersClient({
  ...
  const filteredUsers = activeDataSource.filter((user: any) => {
    ...
    const matchesPortal =
      ...
      (user.portal &&
        user.portal.toString().toLowerCase() ===
          selectedPortal.toString().toLowerCase());
  ...
  const totalCoordinators = users.filter((user: any) =>
    ["Event Coordinator", "Sports Coordinator"].includes(user.role),
  ).length;
    ...
    <div className="p-6 md:p-8 max-w-[1600px] mx-auto">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Manage admins, roles, and permissions across the platform.
            </p>
          </div>
          <div className="flex gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white border border-[#E5E7EB] rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
        ...
        <MetricCard
          ...
          iconBgColor="#F0FDF4"
          iconColor="#10B981"
          subtitle="Active coordinator users"
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="Ornate Committee">
                  Ornate Committee
                </SelectItem>
              </SelectContent>
            </Select>
            ...
            <table className="w-full">
                ...
                <tr>
                  <th className="px-6 py-4 text-left text-xs font-semibold text-[#9CA3AF] uppercase tracking-wider">
                    <div className="flex items-center gap-2">
                      User
                      <InfoTooltip
                        text="Name and registered email address"
                        size="sm"
                      />
                    </div>
                  </th>
              ...
              <tbody>
                {currentUsers.map((user: any, index: number) => (
                  <tr
                    key={user.id}
                    className={`border-b border-[#F3F4F6] hover:bg-[#FAFAFA] transition-colors ${index === currentUsers.length - 1 ? "border-b-0" : ""}`}
                  >
                    ...
                    <td className="px-6 py-4">
                      {isStudentFilter ? (
                        ...
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${user.status === "Active" ? "bg-green-600" : user.status === "Suspended" ? "bg-yellow-600" : "bg-red-600"}`}
                          ></span>
                          {user.status}
                        </span>
                      )}
                    ...
                    <td className="px-6 py-4">
                      {isStudentFilter ? (
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-green-50 text-green-700 ring-1 ring-green-600/20`}
                        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-[#F3F4F6]">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1.5 border border-[#E5E7EB] rounded-lg text-sm text-gray-600 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Previous
              </button>
              <button
                ...
                <div className="space-y-2 max-h-[250px] overflow-y-auto">
                  {selectedUser.registrations.map((reg: any) => (
                    ...
                    >
                      ...
                      <span
                        className={`flex-shrink-0 ml-3 px-2 py-0.5 rounded-full text-xs font-medium
                                                ${
      ...
      <Modal
        ...
        confirmText="Save Changes"
        onConfirm={async () => {
          const result = await updateStudentUser(
            selectedUser.id,
            studentEditFormData,
          );
          if ("error" in result) {
      ...
      <Modal
        ...
        onConfirm={async () => {
          ...
          try {
            const result = await assignEventToStudent(
              selectedUser.id,
              selectedEventId,
            );
            if ("error" in result) {
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          ...
          const resetFn = selectedUser?.isStudent
            ? resetStudentPassword
            : resetAdminPassword;
          ...
          if ("error" in result) {
            ...
            showToast(
              `Password reset successfully. New password: ${tempPass}`,
              "success",
            );
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          const deleteFn = selectedUser?.isStudent
            ? deleteStudentUser
            : deleteAdminUser;
        ...
        confirmLabel={
          selectedUser?.isStudent ? "Delete Student" : "Delete User"
        }
        variant="danger"
      />
    </div>
