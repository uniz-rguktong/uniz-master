...
import { Calendar, Save, Upload, MapPin, Type, FileText, Image as ImageIcon } from 'lucide-react';
import Image from 'next/image';
import { InfoTooltip } from '@/components/InfoTooltip';
import { useToast } from '@/hooks/useToast';
import { updateFestSettings } from '@/actions/festActions';

export default function FestSetupClient({ initialSettings }: { initialSettings: any }) {
    ...
    const [logoPreview, setLogoPreview] = useState(settings?.logoUrl || 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=200&h=200&fit=crop');
    ...
    const handleSave = async () => {
        // Validation
        if (!settings.festName?.trim()) {
            showToast('Fest Name is required', 'error');
            return;
        }
        ...
        if (!settings.venue?.trim()) {
            showToast('Venue Address is required', 'error');
            return;
        }
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-2">
                            <h1 className="text-2xl font-bold text-[#1A1A1A]">Fest Configuration</h1>
                            <InfoTooltip text="Set up the core identity and details of the current fest" size="md" />
                        </div>
                        <p className="text-sm text-[#6B7280]">Define identity, schedule, and core details for the event.</p>
                    </div>
                    <button
                        onClick={handleSave}
                        disabled={isLoading}
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-[#F4F2F0] rounded-[18px] px-[10px] pt-4 pb-[10px] md:pt-6">
                        <h3 className="font-semibold text-[#1A1A1A] flex items-center gap-2 mb-4 ml-[10px]">
                            ...
                            Fest Logo
                            <InfoTooltip text="Displayed on all portals and certificates" size="sm" />
                        </h3>
                        <div className="bg-white rounded-[14px] p-6 text-center">
                            ...
                            <p className="text-xs text-gray-500 mb-4">Recommended: 500x500px PNG</p>
                            <input
                                type="file"
                                ref={logoInputRef}
                    ...
                    <div className="bg-[#F4F2F0] rounded-[18px] px-[10px] pt-4 pb-[10px] md:pt-6">
                        <h3 className="font-semibold text-[#1A1A1A] flex items-center gap-2 mb-4 ml-[10px]">
                            ...
                            Brochure/Rulebook
                            <InfoTooltip text="Downloadable PDF for public viewing" size="sm" />
                        </h3>
                        <div className="bg-white rounded-[14px] p-6 text-center">
                            <input
                                type="file"
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-[#F4F2F0] rounded-[18px] px-[10px] pt-4 pb-[10px] md:pt-6">
                        <h3 className="font-semibold text-[#1A1A1A] flex items-center gap-2 mb-4 ml-[10px]">
                            ...
                            Core Details
                            <InfoTooltip text="Essential information displayed on the landing page" size="sm" />
                        </h3>
                        <div className="bg-white rounded-[14px] p-6 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label className="text-sm font-medium text-[#1A1A1A] mb-2 flex items-center gap-2">
                                        Fest Name
                                        <InfoTooltip text="Official name of the event (e.g., ORNATE 2K26)" size="sm" />
                                    </label>
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setSettings({ ...settings, festName: e.target.value })}
                                ...
                                <div>
                                    <label className="text-sm font-medium text-[#1A1A1A] mb-2 flex items-center gap-2">
                                        Tagline / Theme
                                        <InfoTooltip text="Short slogan or theme for this year" size="sm" />
                                    </label>
                                    <input
                                        type="text"
                            ...
                            <div>
                                <label className="text-sm font-medium text-[#1A1A1A] mb-2 flex items-center gap-2">
                                    Venue Address
                                    <InfoTooltip text="Primary location for map integration" size="sm" />
                                </label>
                                <div className="relative">
