...
export default async function RegistrationAnalyticsPage() {
  ...
  const totalMale = data.reduce(
    (acc: any, curr: any) => acc + (curr.male || 0),
    0,
  );
  const totalFemale = data.reduce(
    (acc: any, curr: any) => acc + (curr.female || 0),
    0,
  );
  const topBranch = data.reduce(
    (max: any, item: any) =>
      (item.total || 0) > (max.total || 0) ? item : max,
    { branch: "N/A", total: 0 },
  );
  const topClub = clubsData.reduce(
    (max: any, item: any) =>
      (item.total || 0) > (max.total || 0) ? item : max,
    { branch: "N/A", total: 0 },
  );
  const topBranchDisplay =
    (topBranch.total || 0) > 0 ? topBranch.branch || "N/A" : "N/A";
  const topClubDisplay =
    (topClub.total || 0) > 0 ? topClub.branch || "N/A" : "N/A";
        ...
        <div className="flex flex-col gap-4 mb-8">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">
              Demographic breakdown and enrollment trends.
            </p>
          </div>
        </div>
