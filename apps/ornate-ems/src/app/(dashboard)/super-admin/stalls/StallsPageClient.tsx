'use client';
import { Store, Plus, X, Save, Pencil, CheckCircle, Clock, IndianRupee, Trash2, Users as UsersIcon, MapPin } from 'lucide-react';
        ...
        <div className="p-6 md:p-8 max-w-[1400px] mx-auto">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-2">
                            <h1 className="text-2xl font-bold text-[#1A1A1A]">Stalls Management</h1>
                            <InfoTooltip text="Track and manage vendor stalls and auction details" size="md" />
                        </div>
                        <p className="text-sm text-[#6B7280]">Manage commercial stalls and allocations after bidding.</p>
                    </div>
                    <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                {displayedStalls.map((stall: any) => (
                    <div key={stall.id} className="group relative bg-white rounded-3xl p-5 border border-gray-100 hover:border-indigo-100 hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-300 flex flex-col h-[240px]">
                        {/* Top row: No & Type */}
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-4">
                            <div className="p-2 rounded-xl bg-gray-50/50 border border-gray-100">
                                ...
                                <div className="text-xs font-bold text-gray-700">{stall.team}</div>
                            </div>
                            <div className="p-2 rounded-xl bg-gray-50/50 border border-gray-100">
                                <div className="flex items-center gap-1 text-[9px] text-gray-400 mb-0.5 font-bold uppercase tracking-tighter">
                                    <IndianRupee className="w-2.5 h-2.5 text-emerald-600" /> Bid
                                </div>
                                <div className="text-xs font-bold text-emerald-600">{stall.price}</div>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-50">
                            <div className="flex items-center gap-2">
                                <div className={`w-2 h-2 rounded-full ${stall.status === 'Allocated' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.3)]' : 'bg-gray-300'}`} />
                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">{stall.status}</span>
                            </div>
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
                        ...
                        <div className="flex items-center justify-between p-5 border-b border-gray-100">
                            <div>
                                ...
                                <p className="text-xs text-gray-500">
                                    {editingStall ? 'Update stall details' : 'Record details of a new stall allocation'}
                                </p>
                            </div>
                            <button
                                onClick={closeModal}
                        ...
                        <div className="p-5 overflow-y-auto max-h-[60vh] space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                {/* Stall Number */}
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-2">Stall No</label>
                                    <input
                                        type="text"
                                        name="stallNo"
                                        placeholder="e.g. 14"
