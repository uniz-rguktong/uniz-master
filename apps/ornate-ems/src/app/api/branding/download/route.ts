...
export async function GET(request: NextRequest) {
    ...
    const filenameParam = request.nextUrl.searchParams.get('filename') || 'logo_file';
    ...
    if (!ALLOWED_PROTOCOLS.has(parsed.protocol) && !urlParam.startsWith('data:')) {
    ...
    try {
        ...
        if (urlParam.startsWith('data:')) {
            ...
            if (!upstream.ok) {
                return NextResponse.json({ error: 'Failed to fetch logo file' }, { status: upstream.status });
            }
        ...
        const initialText = new TextDecoder().decode(buffer.slice(0, 1000)).toLowerCase();
        ...
        if (isSvg && targetFormat === 'png') {
            try {
                ...
                console.log(`[BrandingDownload] Converted SVG to PNG. New filename: ${safeName}`);
            } catch (e) {
        ...
        if (finalBuffer.length === 0) {
            return NextResponse.json({ error: 'Source file is empty' }, { status: 500 });
        }
        ...
        return NextResponse.json({ error: 'Failed to download logo file' }, { status: 500 });
    }
}
