 ...
 */
const distributionRateLimiter = redis
    ? new Ratelimit({
        redis,
        limiter: Ratelimit.slidingWindow(3, '5 m'),
        analytics: true,
        prefix: 'ratelimit:cert-distribute',
    })
    : null;

const DISTRIBUTION_FALLBACK_LIMIT = 3;
const DISTRIBUTION_FALLBACK_WINDOW_MS = 5 * 60 * 1000;
const inMemoryDistributionLimiter = new Map<string, { count: number; resetAt: number }>();
let hasLoggedDistributionFallback = false;

export interface DistributeWinnersRequest {
    eventId: string;
}

const ALLOWED_DISTRIBUTION_ROLES = ['SUPER_ADMIN', 'BRANCH_ADMIN', 'HHO', 'SPORTS_ADMIN', 'BRANCH_SPORTS_ADMIN'];
...
export async function POST(request: NextRequest) {
    ...
    if (distributionRateLimiter) {
        const rl = await distributionRateLimiter.limit(admin.id);
        if (!rl.success) {
            logger.warn({ adminId: admin.id, remaining: rl.remaining }, 'distribute.winners.rate_limited');
        ...
        if (!hasLoggedDistributionFallback) {
            logger.warn(
                { adminId: admin.id, alert: 'RATE_LIMIT_FALLBACK_MEMORY', layer: 'cert-distribute-winners' },
                '[RATE_LIMIT_FALLBACK_MEMORY] Winners distribution limiter using in-memory fallback (Redis unavailable).'
            );
            hasLoggedDistributionFallback = true;
        }

        const now = Date.now();
        const existing = inMemoryDistributionLimiter.get(admin.id);
        const isExpired = !existing || existing.resetAt <= now;
        const nextState = isExpired
            ? { count: 1, resetAt: now + DISTRIBUTION_FALLBACK_WINDOW_MS }
            : { count: existing.count + 1, resetAt: existing.resetAt };

        inMemoryDistributionLimiter.set(admin.id, nextState);

        if (nextState.count > DISTRIBUTION_FALLBACK_LIMIT) {
            const retryAfterSeconds = Math.max(1, Math.ceil((nextState.resetAt - now) / 1000));
            return NextResponse.json(
                { error: 'Too many distribution requests. Please wait a few minutes.' },
                {
                    status: 429,
                    headers: {
                        ...
                        'X-RateLimit-Remaining': String(Math.max(0, DISTRIBUTION_FALLBACK_LIMIT - nextState.count)),
                    },
                }
            );
        }
    ...
    try {
        ...
        if (admin.role === 'BRANCH_ADMIN') {
            ...
            if (eventOwner?.creator?.branch !== admin.branch) {
                return NextResponse.json({ error: 'Forbidden: Event not in your branch' }, { status: 403 });
            }
        }
        ...
        logger.info({ eventId, registrationCount: event.registrations.length }, 'distribute.winners.start');

        for (const reg of event.registrations) {
            let templateName = '';
            let type = '';

            if (reg.rank === 1) { templateName = 'gold.html'; type = 'MERIT_FIRST'; }
            else if (reg.rank === 2) { templateName = 'silver.html'; type = 'MERIT_SECOND'; }
            else if (reg.rank === 3) { templateName = 'bronze.html'; type = 'MERIT_THIRD'; }
            else continue;
            ...
            try {
                ...
                if (!fs.existsSync(templatePath)) {
                    logger.warn({ templateName }, 'distribute.winners: template not found, skipping');
                    continue;
                }
                ...
                if (reg.user && reg.user.email) {
                    const emailHtml = getWinnerEmailTemplate(reg.studentName, event.title, reg.rank!, verificationUrl, reg.id);
                ...
                errorCount++;
                logger.error({ err, eventId, registrationId: reg.id }, 'distribute.winners.cert.error');
            }
        }

        const durationMs = Math.round(performance.now() - startMs);
        logger.info({ eventId, count, emailCount, errorCount, durationMs }, 'distribute.winners.complete');
