...
export function NotificationModal({ isOpen, onClose }: NotificationModalProps) {
  ...
  const [selectedNotification, setSelectedNotification] = useState<NotificationItem | undefined>(notifications[0]);
  ...
  const filteredNotifications = notifications.filter((notif: any) => {
    const matchesFilter =
      filterType === 'all' ? !notif.isArchived :
        filterType === 'unread' ? !notif.isRead && !notif.isArchived :
          filterType === 'starred' ? notif.isStarred && !notif.isArchived :
            filterType === 'archived' ? notif.isArchived : true;
  ...
  const toggleStar = (id: string) => {
    setNotifications((prev: NotificationItem[]) => prev.map((n: any) =>
      n.id === id ? { ...n, isStarred: !n.isStarred } : n
    ));
    // If selected notification is the one being toggled, update it
    if (selectedNotification?.id === id) {
      setSelectedNotification((prev: NotificationItem | undefined) => prev ? { ...prev, isStarred: !prev.isStarred } : undefined);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-6 backdrop-blur-sm">
      <div
        ...
        <div className="p-6 border-b border-[#E5E7EB] flex items-center justify-between bg-white shrink-0">
          <div>
            <h2 className="text-xl font-bold text-[#1A1A1A]">Notifications & Messages</h2>
            ...
            <div className="p-4 grid grid-cols-2 gap-2 shrink-0">
              <div className="bg-white p-3 rounded-lg border border-[#E5E7EB] flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-semibold text-[#1A1A1A]">{unreadCount}</div>
                </div>
              </div>
              <div className="bg-white p-3 rounded-lg border border-[#E5E7EB] flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-semibold text-[#1A1A1A]">{starredCount}</div>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-hide">
                {['all', 'unread', 'starred', 'archived'].map((type: any) =>
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md text-xs font-medium capitalize whitespace-nowrap transition-colors ${filterType === type ?
              ...
              {filteredNotifications.map((notif: any) =>
                <div
                  ...
                  <div className="flex justify-between items-start mb-1">
                    <span className={`text-sm truncate pr-2 ${!notif.isRead ? 'font-bold text-[#1A1A1A]' : 'font-medium text-[#4B5563]'}`}>
                      {notif.from}
                    </span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {notif.isStarred && <Star className="w-3 h-3 text-[#EAB308] fill-[#EAB308] ml-auto" />}
                  </div>
                </div>
              )}
            </div>
            ...
            {selectedNotification ?
              <div className="max-w-3xl mx-auto">
                ...
                <div className="flex items-start justify-between mb-6 pb-6 border-b border-[#E5E7EB]">
                  <div>
                    <h1 className="text-xl font-bold text-[#1A1A1A] mb-2">{selectedNotification.subject}</h1>
                    <div className="flex items-center gap-3">
                      <div
                      ...
                      <div>
                        <div className="font-medium text-[#1A1A1A]">{selectedNotification.from}</div>
                        <div className="text-xs text-[#6B7280]">{selectedNotification.fromDepartment} • {new Date(selectedNotification.timestamp).toLocaleString()}</div>
                      </div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => toggleStar(selectedNotification.id)} className="p-2 hover:bg-[#F3F4F6] rounded-lg">
                      <Star className={`w-5 h-5 ${selectedNotification.isStarred ? 'text-[#EAB308] fill-[#EAB308]' : 'text-[#6B7280]'}`} />
                    </button>
                    <button onClick={() => handleArchive(selectedNotification.id)} className="p-2 hover:bg-[#F3F4F6] rounded-lg text-[#6B7280]">
                ...
                {selectedNotification.attachments && selectedNotification.attachments.length > 0 &&
                  <div className="bg-[#F9FAFB] border border-[#E5E7EB] rounded-lg p-4 mb-6">
                    <div className="text-xs font-semibold text-[#6B7280] uppercase mb-3">Attachments</div>
                    <div className="grid grid-cols-2 gap-3">
                      {selectedNotification.attachments.map((att: { name: string; size: string }, i: number) =>
                        <div key={i} className="flex items-center gap-3 p-2 bg-white border border-[#E5E7EB] rounded-md">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-xs text-[#6B7280]">{att.size}</div>
                          </div>
                        </div>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60] p-8">
          ...
          <div className="bg-white rounded-xl max-w-2xl w-full p-6 shadow-xl">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowComposeModal(false)}><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                <button onClick={() => setShowComposeModal(false)} className="px-4 py-2 border rounded-lg">Cancel</button>
                <button onClick={handleSendMessage} className="px-4 py-2 bg-[#10B981] text-white rounded-lg">Send</button>
              </div>
            </div>
          </div>
        </div>
