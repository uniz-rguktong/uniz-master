import { Users, Trophy, Edit, Eye, UserPlus, Swords, Layers } from 'lucide-react';
...
export function SportCard({ sport, onNavigate, onEdit, onCopy, onArchive, onDelete, onPublish }: SportCardProps) {
      ...
      <div className="w-full bg-[#F4F2F0] rounded-[24px] p-[10px] flex flex-col gap-3 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
        ...
        <div className="flex items-center justify-between px-[12px] py-[4px] mt-[4px]">
          ...
          <div className="flex items-center gap-2">
            <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${sport.type === 'Girls' || sport.type === 'FEMALE'
        ...
        <div className="bg-white rounded-[18px] p-[10px] flex flex-col gap-4 border border-[#E5E7EB]/50 shadow-sm">
          ...
          <div className="relative w-full aspect-video rounded-[14px] overflow-hidden group/thumbnail bg-[#F7F8FA]">
            ...
            <div className={`absolute top-3 left-3 px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider text-white shadow-lg bg-black/60`}>
          ...
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center gap-4 text-[#7A7772]">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[11px] font-medium">{sport.registrations} reg</span>
              </div>
              <div className="flex items-center gap-1.5">
                <Trophy className="w-3.5 h-3.5 opacity-60" />
                <span className="text-[11px] font-medium">
                  {((sport.category === 'Individual' || sport.name.toLowerCase().includes('athletics')) && sport.winnerPoints === 10) ? 5 : (sport.winnerPoints || 0)} pts
                </span>
              </div>
              {sport.matchesCount !== undefined && (
                <div className="flex items-center gap-1.5">
                  <Swords className="w-3.5 h-3.5 opacity-60" />
                  <span className="text-[11px] font-medium">{sport.matchesCount} matches</span>
                </div>
              )}
            </div>
            ...
            <ActionMenu
              actions={[
                { label: 'View Bracket/Polls', icon: 'view', onClick: () => onNavigate('polls-fixtures') },
                { label: 'View Teams', icon: 'users', onClick: () => onNavigate('all-registrations', { eventId: sport.id, filterSport: sport.name }) },
                ...(!isSportsCoordinator ? [
                  { divider: true },
                  { label: 'Edit Competition', icon: 'edit', onClick: () => onEdit(sport) },
                  { label: 'Duplicate Sport', icon: 'copy', onClick: () => onCopy(sport) },
                  ...(sport.isDraft ? [{ label: 'Restore to All Sports', icon: 'check', onClick: () => onPublish(sport) }] : []),
                  { label: sport.isArchived ? 'Restore' : 'Archive', icon: 'archive', onClick: () => onArchive(sport) },
                  { label: 'Delete', icon: 'delete', onClick: () => onDelete(sport), danger: true }
                ] : [])
              ]}
              size="sm"
            />
          ...
          <div className="grid grid-cols-2 gap-2 px-1">
            <div className="flex items-center gap-2 p-2 rounded-xl bg-[#F9FAFB] border border-[#F3F4F6]">
              ...
              <div className="flex flex-col">
                <span className="text-[8px] font-bold text-[#9CA3AF] uppercase tracking-wider">Format</span>
                <span className="text-[11px] font-bold text-[#1A1A1A]">{sport.format || 'Knockout'}</span>
              </div>
            </div>
            <div className="flex items-center gap-2 p-2 rounded-xl bg-[#F9FAFB] border border-[#F3F4F6]">
              <Layers className="w-3 h-3 text-[#1A1A1A]/40" />
              <div className="flex flex-col overflow-hidden">
                <span className="text-[8px] font-bold text-[#9CA3AF] uppercase tracking-wider">Category</span>
                <span className="text-[11px] font-bold text-[#1A1A1A] truncate">{sport.category || 'Team'}</span>
              </div>
            </div>
          </div>
          ...
          {(sport.runnerPoints !== undefined && sport.runnerPoints > 0) && (
            <div className="flex items-center justify-between px-1">
              <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-[#9CA3AF]">RUNNER-UP POINTS</span>
              <span className="text-[10px] font-bold text-[#1A1A1A]">
                {((sport.category === 'Individual' || sport.name.toLowerCase().includes('athletics')) && sport.runnerPoints === 5) ? 3 : sport.runnerPoints} pts
              </span>
            </div>
          )}

          {/* 4b. 2nd RUNNER-UP POINTS */}
          {(sport.category === 'Individual' || sport.name.toLowerCase().includes('athletics') || (sport.secondRunnerPoints !== undefined && sport.secondRunnerPoints > 0)) && (
            <div className="flex items-center justify-between px-1">
              <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-[#9CA3AF]">2nd RUNNER-UP POINTS</span>
              <span className="text-[10px] font-bold text-[#1A1A1A]">
                {((sport.category === 'Individual' || sport.name.toLowerCase().includes('athletics')) && (sport.secondRunnerPoints === 0 || !sport.secondRunnerPoints)) ? 1 : (sport.secondRunnerPoints || 0)} pts
              </span>
            </div>
          )}
          ...
          <div className="flex gap-2 pt-2 border-t border-[#F1F3F5]">
            <button
              onClick={() => onNavigate(isSportsCoordinator ? 'add-registration' : 'polls-fixtures', isSportsCoordinator ? { initialData: sport } : undefined)}
              ...
              {isSportsCoordinator ? (
                <>
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-wider">Add Registration</span>
                </>
              ) : (
                <>
                  <Eye className="w-3.5 h-3.5 text-white group-hover/btn:scale-110 transition-transform" />
                  <span className="text-[10px] font-bold uppercase tracking-wider">Polls and Fixtures</span>
                </>
              )}
            </button>
            <button
              onClick={() => isSportsCoordinator ? onNavigate('polls-fixtures') : onEdit(sport)}
              ...
              {isSportsCoordinator ? (
                <>
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-wider">View Polls</span>
                </>
              ) : (
                <>
