...
export function ReviewLaunchStep({ data }: ReviewLaunchStepProps) {
  ...
  const checklistItems = [
    {
      label: "Sport Identity & Category Set",
      completed: !!data.sportName && !!data.category,
    },
    {
      label: "Registration Rules & Deadlines Valid",
      completed: !!data.deadline,
    },
    {
      label: "Points System & Awards Configured",
      completed: !!data.winnerPoints,
    },
    {
      label: "Brackets Format Selected",
      completed: !!(data.format || data.tournamentType),
    },
    {
      label: "Tournament Poster & Media Ready",
      completed: !!(data.bannerUrl || data.poster) || true,
    }, // Placeholder true
    {
      label: "Primary Coordinator Assigned",
      completed: (data.coordinators?.length ?? 0) > 0,
    },
  ];
    ...
    <div className="space-y-8 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {checklistItems.map((item: any, index: any) => (
              ...
              >
                ...
                >
                  {item.completed ? (
                    <CheckCircle className="w-4 h-4 stroke-[2px]" />
                  ) : (
                    <AlertCircle className="w-4 h-4 stroke-[2px] animate-pulse" />
                  )}
                </div>
                <span
                  className={`text-xs font-semibold ${
                    item.completed ? "text-[#1A1A1A]" : "text-amber-800"
                  }`}
                >
                  {item.label}
                </span>
      ...
      <div>
        <div className="flex items-center justify-between mb-4 px-1">
          ...
          <div className="flex bg-[#F4F2F0] p-1 rounded-lg border border-[#E5E7EB]">
            <button
              ...
              className={`px-4 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all ${previewDevice === "desktop" ? "bg-white text-[#1A1A1A] shadow-sm" : "text-[#7A7772] opacity-60"}`}
            >
              Desktop
            </button>
            <button
              onClick={() => setPreviewDevice("mobile")}
              className={`px-4 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider transition-all ${previewDevice === "mobile" ? "bg-white text-[#1A1A1A] shadow-sm" : "text-[#7A7772] opacity-60"}`}
            >
              Mobile
            </button>
          </div>
        ...
        <div className="flex justify-center p-8 bg-[#F4F2F0] rounded-[24px] border border-[#E5E7EB] shadow-inner relative overflow-hidden group">
          ...
          <div
            className={`transition-all duration-700 ease-out ${previewDevice === "mobile" ? "w-[280px]" : "w-full max-w-[360px]"}`}
          >
            ...
            <div className="bg-white rounded-[20px] overflow-hidden shadow-xl border border-[#E5E7EB]">
              ...
              <div className="p-5">
                <h4 className="text-lg font-bold text-[#1A1A1A] leading-tight mb-4 tracking-tight">
                  {data.sportName || "Tournament Preview"}
                </h4>
                <div className="grid grid-cols-2 gap-4 mb-5">
                  ...
                  <div className="flex items-center gap-2 text-[#7A7772]">
                    ...
                    <span className="text-[10px] font-medium font-geist truncate">
                      {(
                        data.format ||
                        data.tournamentType ||
                        "Format TBD"
                ...
                <div className="pt-4 border-t border-[#F1F3F5] flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold text-[#1A1A1A] tracking-tight">
                      {data.winnerPoints || "0"} Points
                    </span>
                  </div>
                  <div className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[8px] font-bold uppercase tracking-widest border border-emerald-100/50 shadow-sm">
                    OPENED
                  </div>
                </div>
              </div>
      ...
      <div className="p-6 bg-[#FFFBEB] border border-[#FEF3C7] rounded-[24px] flex items-start gap-4">
        ...
        <div className="flex-1">
          <h4 className="text-xs font-bold text-[#92400E] uppercase tracking-widest mb-1">
            Final Confirmation
          </h4>
          <p className="text-[11px] text-[#B45309] leading-relaxed opacity-90">
            Once launched, this competition will go live for all students.
            Registration systems and automated bracket tracking will be
            initialized.
          </p>
        </div>
      </div>
