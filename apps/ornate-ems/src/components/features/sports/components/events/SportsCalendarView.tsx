...
export function SportsCalendarView({
  sports,
  matches,
  selectedFilter,
  showArchived,
  variant = "default",
}: SportsCalendarViewProps) {
  ...
  const getEventsForDate = (targetDate: Date) =>
    filteredItems.filter((item: any) =>
      isSameDate(new Date(item.date), targetDate),
    );
  ...
  const renderMonthView = () => {
    ...
    return (
      <div
        className={`overflow-x-auto ${isSportsAdminVariant ? "rounded-[12px] border border-[#D1D5DB]" : "rounded-[20px] border border-[#E5E7EB] p-1"}`}
      >
        <div
          className={`grid grid-cols-7 gap-px min-w-[800px] ${isSportsAdminVariant ? "bg-[#D1D5DB]" : "bg-[#F3F4F6]"}`}
        >
          {["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"].map((day) => (
            <div
              key={day}
              className={`${isSportsAdminVariant ? "bg-[#F3F4F6] py-3 text-sm font-semibold text-[#6B7280]" : "bg-white py-3 text-center text-[10px] font-bold text-[#6B7280] tracking-wider"} text-center`}
            >
              {day}
            </div>
          ))}
          ...
          {emptyDays.map((_, index) => (
            <div
              key={`empty-${index}`}
              className={`bg-white ${isSportsAdminVariant ? "h-36" : "h-32"}`}
            />
          ))}
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span
                    className={`text-sm font-bold ${isToday && !isSportsAdminVariant ? "text-[#047857]" : "text-[#1A1A1A]"}`}
                  >
                    {day}
                  </span>
                  {dayEvents.length > 0 && (
          ...
          <div className="grid grid-cols-7 gap-px min-w-[800px] bg-[#D1D5DB]">
            {weekDays.map((day) => (
              <div
                ...
                className="bg-[#F3F4F6] py-3 text-center text-sm font-semibold text-[#6B7280]"
              >
            ...
            {weekDays.map((day) => {
              ...
              return (
                <div
                  key={day.toISOString()}
                  className="bg-white p-3 min-h-[138px]"
                >
      ...
      <div className="grid grid-cols-1 md:grid-cols-7 gap-3">
        {weekDays.map((day) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className={`rounded-2xl border p-3 ${isToday ? "border-[#10B981] bg-[#ECFDF5]" : "border-[#E5E7EB] bg-white"}`}
            >
              <div className="mb-3">
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <h2
            className={`${isSportsAdminVariant ? "text-[36px] font-bold" : "text-xl md:text-2xl font-semibold"} text-[#1A1A1A]`}
          >
            {calendarTitle}
          </h2>
          <button
            onClick={previousPeriod}
            className={`${isSportsAdminVariant ? "p-1 border-none" : "p-2 rounded-xl border border-[#E5E7EB] hover:bg-[#F9FAFB]"} transition-colors`}
            aria-label="Previous period"
          >
            <ChevronLeft
              className={`${isSportsAdminVariant ? "w-4 h-4 text-[#6B7280]" : "w-5 h-5 text-[#1A1A1A]"}`}
            />
          </button>
          <button
            onClick={nextPeriod}
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            className={`flex items-center gap-1 p-1 ${isSportsAdminVariant ? "bg-transparent border-none" : "bg-[#F9FAFB] border border-[#E5E7EB]"} rounded-xl`}
          >
            {(["Month", "Week", "Day"] as CalendarView[]).map((mode) => (
              <button
                key={mode}
                onClick={() => setView(mode)}
                className={`px-5 py-2 rounded-xl text-2xs sm:text-sm font-semibold transition-colors ${
                  view === mode
                    ? "bg-[#1A1A1A] text-white"
                    : "text-[#6B7280] bg-[#F3F4F6] hover:text-[#1A1A1A]"
                }`}
              >
                {mode}
              </button>
        ...
        <div className="flex items-center justify-between gap-4 text-xs text-[#6B7280] border-t border-[#E5E7EB] pt-4">
          <div className="flex items-center gap-2">
            ...
            <span>
              ...
              {safeMatches.length > 0 ? "matches" : "events"}
            </span>
          </div>
          <div className="flex items-center gap-2">
        ...
        >
          <div className="p-5 relative">
            ...
            <div className="mb-5 pr-8">
              ...
              >
                {safeMatches.length > 0
                  ? `${selectedEvent.sport || "Match"} ${selectedEvent.round ? `• ${selectedEvent.round}` : ""}`
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              {selectedEvent.team1Name || selectedEvent.team2Name ? (
                <div className="p-3 rounded-xl border border-[#E5E7EB] bg-white sm:col-span-2">
                  ...
                  <div className="text-sm font-semibold text-[#1A1A1A]">
                    {selectedEvent.team1Name || "TBD"} vs{" "}
                    {selectedEvent.team2Name || "TBD"}
                  </div>
                </div>
              ) : null}
            </div>
