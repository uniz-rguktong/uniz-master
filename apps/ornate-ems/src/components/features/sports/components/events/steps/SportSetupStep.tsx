...
export function SportSetupStep({ data, updateData }: SportSetupStepProps) {
  ...
  const handleAddNewSport = () => {
    ...
    const existingMatch = sportOptions.find((item) => item.toLowerCase() === trimmedSport.toLowerCase());
  ...
  const [sportOptions, setSportOptions] = useState<string[]>(() => {
    if (data.sportCategory && !defaultSportOptions.includes(data.sportCategory)) {
      return [...defaultSportOptions, data.sportCategory];
    }
    return defaultSportOptions;
  });

  const athleticsTypes = ['100M', '400M', '800M', '4X100M Relay', 'Shotput', '5K'];
  ...
  const isTeamSizeLessThanOne = category === 'Team' && (data.maxPlayers !== undefined) && maxPlayersValue < 1;
  const isTeamSizeLessThanMin = category === 'Team' && (data.maxPlayers !== undefined) && maxPlayersValue >= 1 && minPlayersValue > 0 && maxPlayersValue <= minPlayersValue;
            ...
            <div className="flex-1 min-w-0">
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  const nextCategory = value === 'Athletics' ? 'Individual' : 'Team';
                  ...
                  const derivedSportName = shouldClearAthleticsType
                    ? value
                    : (athleticsType ? `${value} - ${athleticsType}` : value);

                  updateData({
                    sportCategory: value,
                    athleticsType: shouldClearAthleticsType ? '' : athleticsType,
                ...
                <SelectContent className="rounded-xl border-[#E5E7EB]">
                  {sportOptions.map((item: string) => (
                    <SelectItem key={item} value={item} className="font-medium text-sm py-2 focus:bg-[#F9FAFB]">{item}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value) => {
              ...
              const parsedCurrentMin = Number.parseInt(String(data.minPlayers || ''), 10);
              const parsedCurrentMax = Number.parseInt(String(data.maxPlayers || ''), 10);

              const nextMin = isTeam ? (Number.isNaN(parsedCurrentMin) ? 11 : parsedCurrentMin) : 1;
              const nextMax = isTeam ? (Number.isNaN(parsedCurrentMax) ? 12 : Math.max(parsedCurrentMax, nextMin)) : 1;
            ...
            <SelectContent className="rounded-xl border-[#E5E7EB]">
              {categories.map((cat: any) => (
                <SelectItem key={cat} value={cat} className="font-medium text-sm py-2 focus:bg-[#F9FAFB]">{cat}</SelectItem>
              ))}
            </SelectContent>
          </Select>

          <div className={`grid grid-cols-1 gap-3 mt-4 ${category === 'Individual' ? 'sm:grid-cols-3' : 'sm:grid-cols-2'}`}>
            <div>
              <label className="block text-xs font-medium text-[#6B7280] mb-2">Winner Points</label>
              <input
                type="number"
                min="0"
            ...
            <div>
              <label className="block text-xs font-medium text-[#6B7280] mb-2">Runner Points</label>
              <input
                type="number"
                min="0"
            ...
            {category === 'Individual' && (
              <div>
                <label className="block text-xs font-medium text-[#6B7280] mb-2">2nd Runner Points</label>
                <input
                  type="number"
                  min="0"
          ...
          <div className="flex gap-2 p-1.5 bg-[#F8F9FA] border border-[#E5E7EB] rounded-xl h-12 shadow-sm">
            {genderTypes.map((type: any) => (
              <button
                ...
                className={`flex-1 h-full rounded-lg text-sm font-semibold tracking-wide transition-all border ${genderType === type.value
          ...
          <Select
            ...
            <SelectContent className="rounded-xl border-[#E5E7EB]">
              {formats.map((item: any) => (
                <SelectItem key={item.value} value={item.value} className="font-medium text-sm py-2 focus:bg-[#F9FAFB]">{item.label}</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
