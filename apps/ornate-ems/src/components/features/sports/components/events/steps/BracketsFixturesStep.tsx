...
export function BracketsFixturesStep({
  data,
  updateData,
}: BracketsFixturesStepProps) {
  const [tournamentType, setTournamentType] = useState(
    data.tournamentType || data.format || "KNOCKOUT",
  );
          ...
          {[
            {
              ...
              desc: "Teams are eliminated immediately after a single loss.",
            },
            {
              id: "LEAGUE",
              name: "League",
              desc: "Every team plays against all other teams in the group.",
            },
            {
              id: "GROUP_KNOCKOUT",
              name: "Group Knockout",
              desc: "Group stage followed by knockout rounds.",
            },
          ].map((type: any) => (
            <button
              key={type.id}
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span
                  className={`text-xs font-semibold ${tournamentType === type.id ? "text-[#1A1A1A]" : "text-[#6B7280]"}`}
                >
                  {type.name}
                </span>
                {tournamentType === type.id ? (
