...
export function WinnersAnnouncementPage({
  ...
  useEffect(() => {
    ...
    const transformed = initialWinners.map((w) => ({
      ...
      eventDate:
        w.sport?.createdAt || w.event?.date
          ? new Date((w.sport?.createdAt || w.event?.date) as string | Date)
              .toISOString()
              .split("T")[0]
          : "",
  ...
  const handleDownload = (winner: any) => {
    const dataStr =
      ...
      encodeURIComponent(JSON.stringify(winner, null, 2));
    const downloadAnchorNode = document.createElement("a");
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute(
      "download",
      `${winner.eventName}_winners.json`,
    );
  ...
  const handleTogglePublish = async (winner: any) => {
    ...
    const result = await toggleSportWinnerAnnouncementPublish(
      winner.id,
      newStatus,
    );
    if (result.success) {
  ...
  const handleSave = async (publish = true) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        result = await createSportWinnerAnnouncement(
          formData.eventId,
          newPositions,
        );
      ...
      if (result.success) {
        ...
        if (
          ...
          showToast(
            "Sport certificate auto-distribution is not wired yet in this flow.",
            "warning",
          );
        }
      } else {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">
              Announce and manage tournament winners and official awards
            </p>
          </div>
                ...
                <div className="bg-white rounded-[14px] p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-2xl font-semibold text-[#10B981]">
                      {winnersList.filter((w) => w.isPublished).length}
                    </div>
                  </div>
                </div>
              </div>
            ...
            >
              <div className="px-6 py-4 bg-[#F9FAFB] flex items-center justify-between border-b border-[#E5E7EB]">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium border ${event.isPublished ? "bg-white text-[#6B7280]" : "bg-[#10B981] text-white"}`}
                  >
              ...
              <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                {event.positions.map((pos: any) => (
                  <div
                    key={pos.rank}
                    className="p-4 border rounded-xl bg-white flex items-start gap-4 shadow-sm hover:shadow transition-shadow"
                  >
                    <div className="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center shrink-0 border">
                      {getRankIcon(pos.rank)}
                    </div>
                    <div>
                      <div className="font-bold text-[#1A1A1A]">
                        {pos.teamName || "Individual"}
                      </div>
                      <div className="text-sm text-[#6B7280]">
          ...
          footer={
            <div className="flex gap-4 w-full">
              ...
              <button
                ...
                disabled={isSaving}
                className="flex-1 px-4 py-2 bg-[#10B981] text-white rounded-lg"
              >
                {isSaving
                  ? "Processing..."
                  : editingId
            ...
            {[0, 1, 2].map((idx) => (
              ...
              >
                <div className="text-sm font-bold flex items-center gap-2">
                  {getRankIcon(idx + 1)} {idx + 1} Rank
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <input
                    placeholder="Team/Winner Name"
                ...
                <input
                  placeholder="Tag Members (Gmail/Email required)"
