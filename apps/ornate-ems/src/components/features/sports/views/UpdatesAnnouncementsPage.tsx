...
export function UpdatesAnnouncementsPage() {
  ...
  useEffect(() => {
    if (searchParams.get('create') === 'true' && !isCoordinator) {
      resetForm();
    ...
    <div className="p-4 md:p-8 animate-page-entrance">
      ...
      <div className="mb-6 md:mb-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            <h1 className="text-xl md:text-[28px] font-semibold text-[#1A1A1A] mb-1 md:mb-2">Updates & Announcements</h1>
            <p className="text-[10px] md:text-sm text-[#6B7280]">Manage important updates and announcements for students</p>
          </div>
          ...
          {!isCoordinator && (
            <button
              ...
              className="hidden md:flex items-center justify-center gap-2 px-5 py-3 bg-[#10B981] text-white rounded-xl text-sm font-semibold hover:bg-[#059669] transition-all shadow-sm w-full md:w-auto active:scale-95">
          ...
          {isLoading ? (
            ...
            <>
              <MetricCard
                ...
                iconColor="#3B82F6"
                delay={40}
                tooltip="Total announcements ever created"
              />

              <MetricCard
                title="Total Views"
                value={announcementsList.reduce((sum: any, a: any) => sum + (a.viewCount || 0), 0).toLocaleString()}
                icon={Eye}
                iconBgColor="#F5F3FF"
                iconColor="#8B5CF6"
                delay={80}
                tooltip="Cumulative views across all published announcements"
              />
            </>
          )}
        </div>
