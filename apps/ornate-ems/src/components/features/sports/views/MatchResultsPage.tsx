  ...
  useEffect(() => {
    ...
    getSportWinnerAnnouncement(sport.id).then((res) => {
      if (res.success && res.data) {
        ...
        setPositions({
          first: {
            branch: pos.first?.branch || pos.first || "",
            email: pos.first?.email || "",
          },
          second: {
            branch: pos.second?.branch || pos.second || "",
            email: pos.second?.email || "",
          },
          third: {
            branch: pos.third?.branch || pos.third || "",
            email: pos.third?.email || "",
          },
        });
      }
    });
        ...
        <div className="px-2 pt-1 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="font-bold text-[#1A1A1A] text-base leading-snug truncate">
              {sport.name}{" "}
              <span className="text-[#6B7280] font-medium opacity-80">
                • {genderLabel}
              </span>
            </span>
          </div>
            ...
            <div className="flex gap-2 mb-4">
              {medals.map(({ label, key, icon, bg, border, text }) => {
                ...
                return (
                  <div
                    key={key}
                    className={`flex-1 flex flex-col items-center py-2 px-1 rounded-xl ${bg} border ${border} shadow-sm transition-transform hover:scale-105`}
                  >
                    <span className="text-xl mb-0.5">{icon}</span>
                    <div
                      className={`text-[9px] font-bold uppercase tracking-widest ${text}`}
                    >
                      {label}
                    </div>
                    <div className="font-bold text-[11px] text-[#1A1A1A] mt-1 text-center truncate w-full px-1">
                      {branch}
                    </div>
                  </div>
                );
...
export function MatchResultsPage() {
  ...
  const [selectedAthleticsSubType, setSelectedAthleticsSubType] =
    useState("All");
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (sportsRes.success && sportsRes.sports) {
        const allSports = (sportsRes.sports as SportOption[]).filter(
          (sport) => sport?.id && sport?.name,
        );
        setAvailableSports(
          allSports.filter((s) => !s.name.startsWith("Athletics")),
        );
        setAthleticsSports(
          allSports.filter((s) => s.name.startsWith("Athletics")),
        );
  ...
  const athleticsSubTypes = Array.from(
    new Set(
      athleticsSports.map((s) => getAthleticsSubType(s.name)).filter(Boolean),
    ),
  ).sort();
  ...
  const filteredAthleticsSports =
    selectedAthleticsSubType === "All"
      ? athleticsSports
      : athleticsSports.filter(
          (s) => getAthleticsSubType(s.name) === selectedAthleticsSubType,
        );

  // Group filtered athletics sports by sub-type for display
  const athleticsBySubType = filteredAthleticsSports.reduce<
    Record<string, SportOption[]>
  >((acc, sport) => {
  ...
  const filteredMatches = matches.filter((match) => {
    ...
    if (!isMainSportsAdmin)
      return defaultFields.some((field) =>
        normalizeSearch(field).includes(normalizedQuery),
      );
    const sportsAdminFields = [
      ...defaultFields,
      match.sport,
      match.gender,
      match.venue,
    ...
    return sportsAdminFields.some((field) =>
      normalizeSearch(field).includes(normalizedQuery),
    );
  });
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-6">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">
              Official records of all completed matches and victory points.
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-3 p-1 bg-white border border-[#E5E7EB] rounded-xl shadow-sm h-11">
            <div className="relative pl-3 flex items-center gap-2">
              <Search className="w-4 h-4 text-[#9CA3AF]" />
              <input
                placeholder="Search..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="bg-transparent border-none focus:ring-0 text-xs font-semibold text-[#1A1A1A] w-32 placeholder:text-[#9CA3AF]"
              />
            </div>
            ...
            <Select
              ...
              onValueChange={(v) => {
                if (v === "athletics-nav") {
                  document
                    .getElementById("athletics-section")
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-64 bg-white rounded-[32px] animate-pulse border border-[#E5E7EB]"
              />
            ))}
          </div>
        ) : (
          <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-8">
                  {filteredMatches.map((match: any) => {
                    ...
                    const getDisplaySportName = (sportName: string) => {
                      if (availableSports.some((s) => s.name === sportName))
                        return sportName;
                      const sTemp = sportName.toLowerCase();
                      const found = availableSports.find((s) =>
                        s.name.toLowerCase().includes(sTemp),
                      );
                    ...
                    return (
                      <div
                        key={match.id}
                        className="group animate-card-entrance flex flex-col"
                      >
                        <div className="w-full bg-[#F4F2F0] rounded-[24px] p-4 flex flex-col flex-1 gap-4 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                          ...
                          <div className="bg-white rounded-[20px] overflow-hidden flex flex-col shadow-sm border border-black/5 flex-1 relative">
                            ...
                            <div className="px-[7px] pb-[7px] pt-4 flex-1 flex flex-col">
                              <div className="flex items-center justify-between gap-1 mb-4 relative z-10 flex-1">
                                {/* Team 1 */}
                                <div
                                  className={`flex flex-col items-center gap-3 flex-1 transition-all duration-500 ${match.winner === match.team1Name ? "opacity-100 scale-105" : "opacity-70"}`}
                                >
                                  <div
                                    className={`relative w-16 h-16 rounded-2xl flex items-center justify-center shadow-sm transition-all ${match.winner === match.team1Name ? "bg-gradient-to-br from-yellow-50 to-amber-50 border border-amber-200" : "bg-slate-50 border border-slate-100"}`}
                                  >
                                    {match.winner === match.team1Name && (
                                      <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-amber-100 text-amber-700 text-[9px] font-bold px-2 py-0.5 rounded-full border border-amber-200 uppercase tracking-wider flex items-center gap-1">
                                        <Trophy className="w-2 h-2" /> Win
                                      </div>
                                    )}
                                    <Medal
                                      className={`w-7 h-7 ${match.winner === match.team1Name ? "text-amber-500" : "text-slate-400"}`}
                                    />
                                  </div>
                                  <div className="text-center w-full">
                                    <div className="font-bold text-slate-800 text-[13px] leading-tight mb-1 truncate px-1">
                                      {match.team1Name}
                                    </div>
                                    <div
                                      className={`text-4xl font-bold tracking-tighter ${match.winner === match.team1Name ? "text-slate-800" : "text-slate-300"}`}
                                    >
                                      {match.score1 || "0"}
                                    </div>
                                  </div>
                                ...
                                <div className="flex flex-col items-center justify-center -mt-6 opacity-30">
                                  <span className="text-[12px] font-bold text-slate-400 uppercase tracking-widest italic">
                                    VS
                                  </span>
                                </div>
                                {/* Team 2 */}
                                <div
                                  className={`flex flex-col items-center gap-3 flex-1 transition-all duration-500 ${match.winner === match.team2Name ? "opacity-100 scale-105" : "opacity-70"}`}
                                >
                                  <div
                                    className={`relative w-16 h-16 rounded-2xl flex items-center justify-center shadow-sm transition-all ${match.winner === match.team2Name ? "bg-gradient-to-br from-yellow-50 to-amber-50 border border-amber-200" : "bg-slate-50 border border-slate-100"}`}
                                  >
                                    ...
                                    <Award
                                      className={`w-7 h-7 ${match.winner === match.team2Name ? "text-amber-500" : "text-slate-400"}`}
                                    />
                                  </div>
                                  <div className="text-center w-full">
                                    <div className="font-bold text-slate-800 text-[13px] leading-tight mb-1 truncate px-1">
                                    ...
                                    <div
                                      className={`text-4xl font-bold tracking-tighter ${match.winner === match.team2Name ? "text-slate-800" : "text-slate-300"}`}
                                    >
                                      {match.score2 || "0"}
                                    </div>
                                  </div>
                              ...
                              <div
                                className={`rounded-xl p-3 flex items-center justify-center gap-2 mb-2 ${match.winner === "Draw" ? "bg-amber-50/80 text-amber-700" : "bg-emerald-50/80 text-emerald-700"}`}
                              >
                                {match.winner !== "Draw" && (
            ...
            {showAthletics && (
              <div id="athletics-section" className="scroll-mt-10">
                ...
                <div className="space-y-8">
                  {Object.entries(athleticsBySubType).map(
                    ([subType, sports]) => (
                      <div key={subType}>
                        {/* Sub-type heading */}
            ...
            {filteredMatches.length === 0 &&
              (!showAthletics ||
                Object.keys(athleticsBySubType).length === 0) && (
                <div className="py-24 text-center bg-white rounded-[40px] border border-dashed border-gray-200">
                  ...
                  <p className="text-[#7A7772] text-sm max-w-sm mx-auto font-medium">
                    Results will appear here once matches are marked as
                    'Completed' in the tournament schedule.
                  </p>
                </div>
              )}
          </>
