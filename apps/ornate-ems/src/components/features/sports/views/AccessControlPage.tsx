...
const roles = [
  {
    ...
    description: 'Full administrative access to all sports modules and faculty oversight.',
    users: 2,
    color: '#6366F1',
  ...
  },
  {
    ...
    description: 'Operational management of tournaments, fixtures, and team approvals.',
    users: 8,
    color: '#10B981',
    accent: 'bg-emerald-50 border-emerald-100 text-emerald-600',
    permissions: {
      tournaments: { view: true, create: true, edit: true, delete: false },
  ...
  },
  {
    id: 3,
    name: 'Team Manager',
    description: 'Limited access to team roster management and schedule viewing.',
    users: 14,
    color: '#F59E0B',
...
export function AccessControlPage({ initialUsers = [] as any[], initialLogs = [] as any[] }: AccessControlPageProps) {
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto animate-page-entrance">
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { icon: Users, label: 'Total Entities', value: '24', color: 'indigo', sub: 'Verified across system' },
            { icon: Shield, label: 'Security Roles', value: '3', color: 'emerald', sub: 'Defined access levels' },
            { icon: Fingerprint, label: 'Active Sessions', value: '18', color: 'amber', sub: 'Encrypted connections' },
                      ...
                      <tbody>
                        {usersList.filter(u =>
                          ...
                          <tr key={user.id} className={`border-b border-[#F3F4F6] row-hover-effect hover:bg-[#FAFAFA] transition-all animate-card-entrance ${idx === arr.length - 1 ? 'border-b-0' : ''}`} style={{ animationDelay: `${idx * 40}ms` }}>
                            ...
                            <td className="px-6 py-4">
                              <span className="inline-flex items-center px-2.5 py-1 bg-gray-100 border border-gray-200 rounded-full text-xs font-semibold text-[#1A1A1A]">
                                {user.role}
                              </span>
                            </td>
                            <td className="px-6 py-4">
