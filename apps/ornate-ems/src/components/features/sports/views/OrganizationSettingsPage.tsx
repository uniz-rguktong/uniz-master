...
import { Building2, Mail, Phone, Upload, Trash2, Save, Trophy } from 'lucide-react';
...
export function OrganizationSettingsPage() {
    ...
    const [formData, setFormData] = useState<any>({
        ...
        shortName: "USC",
        coordinator: "Dr. Robert Smith",
        establishedYear: "2010",
        description: "Overseeing all inter-collegiate sports events, tournaments, and athlete development programs.",
        email: "sports.council@university.edu",
    ...
    const handleSave = () => {
        ...
        setTimeout(() => {
            setHasChanges(false);
            showToast("Sports organization settings saved successfully", "success");
        }, 800);
    };
        ...
        <div className="p-4 md:p-8 animate-page-entrance">
            ...
            <div className="mb-6 md:mb-8">
                <div className="flex flex-wrap items-center gap-1 md:gap-2 text-[10px] md:text-sm text-[#6B7280] mb-3">
                    ...
                    <span>Settings</span>
                    <span className="text-[#9CA3AF]">›</span>
                    <span className="text-[#1A1A1A] font-medium whitespace-nowrap">Organization Settings</span>
                </div>

                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-xl md:text-[28px] font-semibold text-[#1A1A1A] mb-1 md:mb-2">Sports Settings</h1>
                        <p className="text-[10px] md:text-sm text-[#6B7280]">Configure sports council details and institutional information</p>
                    </div>

                    <button
                        onClick={handleSave}
                        disabled={!hasChanges}
                        className={`flex items-center justify-center w-full md:w-auto gap-2 px-5 py-3 rounded-lg text-sm font-medium transition-colors shadow-sm ${hasChanges
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-[#F4F2F0] rounded-[18px] p-[10px] animate-card-entrance" style={{ animationDelay: '40ms' }}>
                        <h3 className="text-lg font-semibold text-[#1A1A1A] mb-3 pl-[5px] pt-[15px] pb-[5px] flex items-center gap-2">
                            Basic Information
                            <InfoTooltip text="Core details about the sports council." />
                        </h3>
                    ...
                    <div className="bg-[#F4F2F0] rounded-[18px] p-[10px] animate-card-entrance" style={{ animationDelay: '80ms' }}>
                        <h3 className="text-lg font-semibold text-[#1A1A1A] mb-3 pl-[5px] pt-[15px] pb-[5px] flex items-center gap-2">
                            Contact Information
                            <InfoTooltip text="Official contact channels for sports inquiries." />
                        </h3>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-[#F4F2F0] rounded-[18px] p-[10px]">
                        <h3 className="text-base font-semibold text-[#1A1A1A] mb-3 pl-[5px] pt-[15px] pb-[5px] flex items-center gap-2">
                            Council Logo
                            <InfoTooltip text="Official emblem displayed on all sports portals." />
                        </h3>
                        ...
                        <div className="bg-white rounded-[14px] p-6 shadow-sm">
                            <div className="text-center">
                                ...
                                >
                                    {formData.logo ? (
                                        <img src={formData.logo} alt="Sports Logo" className="w-full h-full object-cover" />
                                    ) : (
                                        <Trophy className="w-12 h-12 text-[#9CA3AF]" />
                                    )}
                                </div>
                    ...
                    <div className="bg-[#F4F2F0] rounded-[18px] p-[10px]">
                        <h3 className="text-base font-semibold text-[#1A1A1A] mb-3 pl-[5px] pt-[15px] pb-[5px] flex items-center gap-2">
                            Sports Overview
                            <InfoTooltip text="Key council metrics." />
                        </h3>
                        ...
                        <div className="bg-white rounded-[14px] overflow-hidden shadow-sm border border-[#E5E7EB]">
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-[#F3F4F6]">
                                    <tr>
                                        <td className="px-6 py-4 text-sm font-medium text-[#6B7280]">Registered Athletes</td>
                                        <td className="px-6 py-4 text-right text-sm font-bold text-[#1A1A1A]">{formData.stats.athletes}</td>
                                    </tr>
                                    <tr>
                                        <td className="px-6 py-4 text-sm font-medium text-[#6B7280]">Annual Tournaments</td>
                                        <td className="px-6 py-4 text-right text-sm font-bold text-[#1A1A1A]">{formData.stats.tournaments}</td>
                                    </tr>
                                    <tr>
                                        <td className="px-6 py-4 text-sm font-medium text-[#6B7280]">Inter-Univ Medals</td>
                                        <td className="px-6 py-4 text-right text-sm font-bold text-[#1A1A1A]">{formData.stats.medalsWon}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
