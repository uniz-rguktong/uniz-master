...
export function PointsTablePage({
  ...
  const boysSports = includeAthletics
    ? [...BASE_SPORTS, "Athletics"]
    : BASE_SPORTS;
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">
              Track departmental leaderboard and standings.
            </p>
          </div>
        </div>
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-[#6B7280]">
                  Per-branch performance across categories
                </p>
              </div>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-225">
                  <thead className="bg-[#F9FAFB] border-b border-[#E5E7EB]">
                    <tr>
                      <th className="text-left px-6 py-4 text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.15em]">
                        Department
                      </th>
                      {boysSports.map((s) => (
                        <th
                          key={s}
                          className="text-center px-4 py-4 text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.12em]"
                        >
                          {s}
                        </th>
                      ))}
                      <th className="text-center px-6 py-4 text-[11px] font-bold text-[#1A1A1A] uppercase tracking-[0.15em]">
                        Total
                      </th>
                    </tr>
                  </thead>
                  <tbody className="bg-white">
                    {BRANCHES.map((branch: any, idx: any) => {
                      ...
                      return (
                        <tr
                          key={branch}
                          className={`border-b border-[#F3F4F6] hover:bg-[#FAFAFA] transition-colors ${idx === BRANCHES.length - 1 ? "border-b-0" : ""}`}
                        >
                          <td className="py-4 px-6 text-[12px] font-bold text-[#1A1A1A]">
                            {branch}
                          </td>
                          {boysSports.map((sport) => {
                            ...
                            const points =
                              typeof rawPoints === "number" ? rawPoints : 0;
                            totalPoints += points;
                            ...
                            return (
                              <td key={sport} className="text-center py-4 px-4">
                                {points > 0 ? (
                                  <span
                                    className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-[10px] font-bold text-white shadow-sm ${
                                      points === 10
                                        ? "bg-amber-500"
                                        : points === 5
                                          ? "bg-emerald-500"
