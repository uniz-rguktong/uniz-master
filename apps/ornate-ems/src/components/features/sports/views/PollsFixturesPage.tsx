...
export function PollsFixturesPage() {
  ...
  const handleDrop = async (e: any, matchId: string, position?: string) => {
    ...
    if (targetMatch && position && (position === 'team1' || position === 'team2') && isFixedWinnerSlot(targetMatch, position as 'team1' | 'team2')) {
      ...
      setFixtures(fixtures.map(round => ({
        ...
        matches: round.matches.map((match: any) => {
          if (match.id === matchId && position) {
            return { ...match, [position]: null, [`${position}Id`]: null, [`${position}Name`]: item.label };
          }
          return match;
        })
      })));
    ...
    <div className="p-4 md:p-8 animate-page-entrance flex flex-col min-h-screen bg-white">
      ...
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <h1 className="text-2xl md:text-[28px] font-semibold text-[#1A1A1A] mb-2">Polls & Fixtures</h1>
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-medium text-[#6B7280] pl-1">Search Branches</span>
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-medium text-[#6B7280] pl-1">Gender</span>
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-medium text-[#6B7280] pl-1">Sport</span>
        ...
        <div className="w-[320px] flex flex-col shrink-0 gap-6">
          <div className="bg-white rounded-[32px] border border-[#E5E7EB] p-6 shadow-sm flex-[2] overflow-hidden flex flex-col">
            <div className="mb-6 lowercase">
              <h3 className="text-xl font-bold tracking-tight mb-1 capitalize">RGUKT Branches</h3>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto pr-1 scrollbar-thin">
              {filteredBranches.map((team: any, idx: any) => (
                <div
                  ...
                  className={`group flex items-center gap-3 p-4 bg-[#F9FAFB] border border-[#E5E7EB] rounded-2xl transition-all animate-card-entrance ${isBranchSportsAdmin
                    ? 'opacity-80'
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="text-[13px] font-bold text-[#1A1A1A] truncate">{team.teamName || team.name}</div>
                    <div className="text-[10px] font-bold text-[#9CA3AF] uppercase tracking-widest">{team.teamCode?.replace('BR-', '') || 'Branch'} Div.</div>
                  </div>
                  {!isBranchSportsAdmin && <GripVertical className="w-4 h-4 text-[#E5E7EB] group-hover:text-[#1A1A1A]" />}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#1A1A1A]/5 rounded-[32px] border border-[#E5E7EB] p-6 shadow-sm flex-1 flex flex-col">
            <div className="mb-4 lowercase">
              <h3 className="text-[15px] font-bold tracking-tight mb-0.5 capitalize text-[#1A1A1A]">Link Winners</h3>
              <p className="text-[9px] font-bold text-[#9CA3AF] uppercase tracking-widest">Define Bracket Flow</p>
            </div>
            ...
            <div className="space-y-3">
              {availableWinnerPlaceholders.map((placeholder: any, idx: any) => (
                <div
                  ...
                  draggable={!isBranchSportsAdmin}
                  onDragStart={(e) => !isBranchSportsAdmin && handleDragStart(e, placeholder, 'placeholder')}
                  className={`group flex items-center gap-3 p-4 bg-[#F9FAFB] border border-[#E5E7EB] rounded-2xl transition-all animate-card-entrance ${isBranchSportsAdmin
                    ? 'opacity-80'
                    : 'hover:bg-white hover:border-[#1A1A1A] cursor-grab active:cursor-grabbing hover:shadow-xl'
                    }`}
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="text-[13px] font-bold text-[#1A1A1A] truncate">{placeholder.label}</div>
                    <div className="text-[10px] font-bold text-[#9CA3AF] uppercase tracking-widest">Winner Link</div>
                  </div>
                  {!isBranchSportsAdmin && <GripVertical className="w-4 h-4 text-[#E5E7EB] group-hover:text-[#1A1A1A]" />}
                </div>
              ))}
        ...
        <div className="flex-1 bg-white rounded-[40px] border border-[#E5E7EB] shadow-sm overflow-hidden flex flex-col relative">
          <div className="p-6 border-b border-[#F3F4F6] flex justify-between items-center bg-[#F9FAFB]/50">
            <div className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-xl border border-[#E5E7EB] shadow-xs">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#1A1A1A]">Official 5-Branch Grid</span>
            </div>
            <div className="flex items-center gap-3">
              ...
              >-</button>
              <span className="text-[10px] font-bold text-[#1A1A1A] w-12 text-center bg-white/50 py-2 rounded-lg">{Math.round(zoom * 100)}%</span>
              <button
          ...
          >
            ...
            >
              {fixtures.map((round: any, rIndex: any) => (
                <div key={rIndex} className="flex flex-col gap-8 h-full justify-center">
                    ...
                    {round.matches.map((match: any, mIndex: any) => (
                      <div key={match.id || `${round.level}-${match.matchId}-${mIndex}`} className="relative">
                          ...
                          <div className="absolute left-full top-1/2 -translate-y-1/2 w-16 h-full pointer-events-none">
                            <svg className="w-full h-full overflow-visible">
                              <path
                                ...
                                strokeWidth="3"
                                strokeDasharray={match.status === 'completed' ? '0' : '6 4'}
                        ...
                        <div
                          ...
                          onDragOver={(e) => handleDragOver(e, match.id, 'match')} // Allow dropping on the match card itself
                        >
                          ...
                          {/* Match Card */}
                          <div className={`relative bg-white border-2 rounded-[28px] overflow-hidden transition-all duration-500 shadow-sm ${match.status === 'completed'
                            ...
                            <div className="relative z-10">
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  ...
                                  >
                                    {match.team1 ? (match.team1.logo || '🎓') : (getSlotDisplayName(match, match.team1Name) && !isTbdLabel(getSlotDisplayName(match, match.team1Name)) ? '🏆' : <Plus className="w-3 h-3 text-[#9CA3AF]" />)}
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <span className={`text-[12px] font-bold tracking-tight ${match.team1 || (getSlotDisplayName(match, match.team1Name) && !isTbdLabel(getSlotDisplayName(match, match.team1Name))) ? 'text-[#1A1A1A]' : 'text-[#9CA3AF] uppercase text-[9px] font-bold tracking-widest opacity-60'}`}>
                                      {match.team1?.teamName || match.team1?.name || (!isTbdLabel(getSlotDisplayName(match, match.team1Name)) ? getSlotDisplayName(match, match.team1Name) : '') || 'Assign Squad'}
                                    </span>
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  ...
                                  >
                                    {match.team2 ? (match.team2.logo || '🎓') : (getSlotDisplayName(match, match.team2Name) && !isTbdLabel(getSlotDisplayName(match, match.team2Name)) ? '🏆' : <Plus className="w-3 h-3 text-[#9CA3AF]" />)}
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <span className={`text-[12px] font-bold tracking-tight ${match.team2 || (getSlotDisplayName(match, match.team2Name) && !isTbdLabel(getSlotDisplayName(match, match.team2Name))) ? 'text-[#1A1A1A]' : 'text-[#9CA3AF] uppercase text-[9px] font-bold tracking-widest opacity-60'}`}>
                                      {match.team2?.teamName || match.team2?.name || (!isTbdLabel(getSlotDisplayName(match, match.team2Name)) ? getSlotDisplayName(match, match.team2Name) : '') || 'Assign Squad'}
                                    </span>
                ...
                {(() => {
                  ...
                  return (
                    <div className="w-64 h-64 relative group">
                      ...
                      <div className={`absolute inset-0 blur-[80px] rounded-full transition-all duration-1000 ${isCompleted ? 'bg-emerald-500/20 scale-125' : 'bg-[#1A1A1A]/5'}`} />

                      <div className={`relative h-full flex flex-col items-center justify-center bg-white border-[4px] rounded-[48px] shadow-2xl p-8 text-center ring-8 transition-all duration-700 ${isCompleted ? 'border-emerald-500 ring-emerald-500/10' : 'border-[#1A1A1A] ring-[#1A1A1A]/5'}`}>
                        {isCompleted ? (
                          <>
                            ...
                            <div className="space-y-1">
                              <h3 className="text-[10px] font-bold text-emerald-600 uppercase tracking-[0.4em]">Branch King</h3>
                          ...
                          <>
                            ...
                            <h3 className="text-[14px] font-bold text-[#1A1A1A] uppercase tracking-[0.3em] mb-1">Branch King</h3>
