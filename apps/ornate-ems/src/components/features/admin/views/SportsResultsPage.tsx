...
export function SportsResultsPage() {
  ...
  const filteredResults = results.filter(
    (result: any) => filterSport === "All" || result.sport === filterSport,
  );
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">
              View match results and championship standings
            </p>
          </div>
        ...
        <div className="space-y-4">
          {isLoading
            ? [...Array(3)].map((_: any, i: any) => (
                <div
                  key={i}
                  className="bg-white rounded-xl border border-[#E5E7EB] p-6 space-y-4 animate-pulse"
                >
                ...
                >
                  ...
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-6">
                      {/* Team 1 */}
                      <div
                        className={`flex-1 text-center py-4 px-6 rounded-lg ${result.team1.result === "won" ? "bg-[#D1FAE5]" : "bg-[#F7F8FA]"}`}
                      >
                        <div
                          className={`text-lg font-bold mb-2 ${result.team1.result === "won" ? "text-[#065F46]" : "text-[#6B7280]"}`}
                        >
                          {result.team1.name}
                        </div>
                        <div
                          className={`text-3xl font-bold ${result.team1.result === "won" ? "text-[#10B981]" : "text-[#1A1A1A]"}`}
                        >
                          {result.team1.score}
                        </div>
                      ...
                      <div className="px-6 text-2xl font-bold text-[#6B7280]">
                        VS
                      </div>

                      {/* Team 2 */}
                      <div
                        className={`flex-1 text-center py-4 px-6 rounded-lg ${result.team2.result === "won" ? "bg-[#D1FAE5]" : "bg-[#F7F8FA]"}`}
                      >
                        <div
                          className={`text-lg font-bold mb-2 ${result.team2.result === "won" ? "text-[#065F46]" : "text-[#6B7280]"}`}
                        >
                          {result.team2.name}
                        </div>
                        <div
                          className={`text-3xl font-bold ${result.team2.result === "won" ? "text-[#10B981]" : "text-[#1A1A1A]"}`}
                        >
                          {result.team2.score}
                        </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-base font-bold text-[#1A1A1A]">
                        {leaderboard.sport} Championship
                      </h3>
                    </div>
                  </div>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-[#E5E7EB] bg-[#F7F8FA]">
                          <th className="text-left py-3 px-6 text-xs font-semibold text-[#6B7280]">
                            RANK
                          </th>
                          <th className="text-left py-3 px-6 text-xs font-semibold text-[#6B7280]">
                            TEAM
                      ...
                      <tbody>
                        {leaderboard.teams.map((team: any) => (
                          ...
                          >
                            ...
                            <td className="py-3 px-6 font-semibold text-[#1A1A1A]">
                              {team.name}
                            </td>
                            <td className="py-3 px-6 text-[#6B7280]">
                              {team.played}
                            </td>
                            <td className="py-3 px-6 text-[#10B981] font-semibold">
                              {team.won}
                            </td>
                            <td className="py-3 px-6 text-[#EF4444] font-semibold">
                              {team.lost}
                            </td>
                            <td className="py-3 px-6 font-bold text-[#1A1A1A]">
                              {team.points}
                            </td>
                          </tr>
                        ))}
