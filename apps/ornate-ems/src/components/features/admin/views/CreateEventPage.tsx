...
export function CreateEventPage({ onNavigate, mode = 'create', initialData }: CreateEventPageProps = {}) {
  ...
  const [eventData, setEventData] = useState(mode === 'create' ? { ...buildCreateDefaults(), ...(initialData || {}) } : (initialData || {}));
  ...
  const validateStep = (step: number, candidateData: Record<string, any> = eventData) => {
    if (step === 1) {
      const name = String(candidateData.eventName || candidateData.title || '').trim();
      const category = String(candidateData.category || '').trim();
      const shortDescription = String(candidateData.shortDescription || '').trim();
      ...
      if (!shortDescription && !isEditMode) return 'Short description is required.';
      if (!eventType && !isEditMode) return 'Participation type is required.';
      if (!hasPrizesSelected && !isEditMode) return 'Tournament style is required.';
      ...
      if (eventType.toLowerCase().includes('team')) {
        ...
        if (max < min) return 'Team maximum size must be greater than or equal to minimum size.';
      }
    }

    if (step === 2) {
      const category = String(candidateData.category || '').trim().toLowerCase();
      ...
      if (category !== 'sports' && coordinators.length === 0) return 'At least one coordinator is required.';
    ...
    if (step === 3) {
      const registrationStatus = String(candidateData.registrationStatus || '').trim();
      ...
      if (maxParticipants <= 0) return 'Maximum participants must be a positive integer.';
  ...
  const validateAllStepsForPublish = () => {
    ...
    if (publishVenueCapacity > 0 && publishMaxParticipants > publishVenueCapacity) {
  ...
  const nextStep = () => {
    if (mode !== 'edit') {
      ...
      const validationError = validateStep(currentStep, validationCandidateData);
      if (validationError) {
  ...
  const saveEvent = async (status = 'PUBLISHED') => {
    ...
    try {
      ...
      formData.append('capacity', toText(eventData.capacity || eventData.maxParticipants));
      ...
      const backendMaxParticipants = parsePositiveInt(eventData.maxParticipants || eventData.capacity);
