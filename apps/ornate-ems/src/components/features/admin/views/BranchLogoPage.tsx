...
import { Upload, Image as ImageIcon, CheckCircle, Trash2, Edit2, Download, Search, Plus } from 'lucide-react';
...
const INITIAL_BRANCH_LOGOS = [
  ...
  },
  {
    ...
    logoUrl: 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=300&h=300&fit=crop',
...
export function BranchLogoPage({ user }: BranchLogoPageProps) {
  ...
  const handleCreateLogo = () => {
    ...
    if (logos.find(l => l.branchName === normalizedBranch)) {
      showToast(`A logo for ${normalizedBranch} already exists. Please edit the existing one.`, 'error');
      return;
    }
  ...
  const handleUpdateLogo = () => {
    ...
    if (logos.find(l => l.branchName === normalizedBranch && l.id !== selectedLogo.id)) {
      showToast(`A logo for ${normalizedBranch} already exists.`, 'error');
      return;
    }

    setLogos(logos.map(l => l.id === selectedLogo.id ? {
      ...l,
      branchName: normalizedBranch,
      fullName,
    ...
    <div className="p-8 animate-page-entrance">
      ...
      <div className="mb-8">
        <div className="flex flex-wrap items-center gap-2 text-sm text-[#6B7280] mb-3">
          ...
          <span className="whitespace-nowrap">›</span>
          <span className="text-[#1A1A1A] font-medium whitespace-nowrap">Branch Logos</span>
        </div>

        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            <h1 className="text-xl md:text-[28px] font-semibold text-[#1A1A1A] mb-2">Sports Branch Logos</h1>
            <p className="text-sm text-[#6B7280]">
              {userBranch ? `Manage the official logo for the ${userBranch} branch` : 'Manage branch logos for sports events and results'}
            </p>
          </div>

          {!hasBranchLogo && (
            <button
              onClick={() => { resetForm(); if (userBranch) setBranchName(userBranch); setShowUploadModal(true); }}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {isLoading ? (
          ...
          filteredLogos.map((logo: any) => (
            <div key={logo.id} className="bg-[#F4F2F0] rounded-[24px] p-2 group">
              <div className="bg-white rounded-[18px] p-6 shadow-sm border border-[#E5E7EB] h-full flex flex-col">
                <div className="relative aspect-square mb-4 bg-[#F9FAFB] rounded-xl border border-dashed border-[#E5E7EB] flex items-center justify-center overflow-hidden">
                  <Image src={logo.logoUrl} alt={logo.branchName} width={120} height={120} className="object-contain" />
                  <div className={`absolute top-2 right-2 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${logo.status === 'active' ? 'bg-[#D1FAE5] text-[#10B981]' : 'bg-[#F3F4F6] text-[#6B7280]'
                    }`}>
                    {logo.status}
                  </div>
                </div>
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-[#1A1A1A]">{logo.branchName}</h3>
          ...
          <div className="col-span-full py-20 text-center bg-[#F9FAFB] rounded-2xl border border-dashed border-[#E5E7EB]">
            ...
            <h3 className="text-base font-bold text-[#1A1A1A]">No logos found</h3>
      ...
      >
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#1A1A1A] mb-1">Branch Code (e.g. CSE) *</label>
            <input
              type="text"
              value={branchName}
          ...
          <div>
            <label className="block text-sm font-medium text-[#1A1A1A] mb-1">Full Branch Name *</label>
            <input
              type="text"
              value={fullName}
          ...
          <div>
            <label className="block text-sm font-medium text-[#1A1A1A] mb-1">Logo File *</label>
            <div
            ...
            >
              {logoPreview ? (
                <div className="flex flex-col items-center">
                  <Image src={logoPreview} alt="Preview" width={80} height={80} className="object-contain mb-2" />
                  <p className="text-xs text-[#10B981] font-medium">Click to change</p>
                </div>
              ) : (
                <>
                  <Upload className="w-8 h-8 text-[#9CA3AF] mx-auto mb-2" />
                  <p className="text-sm text-[#6B7280]">Click to upload logo</p>
                  <p className="text-xs text-[#9CA3AF] mt-1">PNG, SVG, or JPG (max 2MB)</p>
                </>
              )}
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-[#1A1A1A] mb-2">Initial Status</label>
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" checked={status === 'active'} onChange={() => setStatus('active')} className="w-4 h-4" />
                <span className="text-sm">Active</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="radio" checked={status === 'inactive'} onChange={() => setStatus('inactive')} className="w-4 h-4" />
      ...
      >
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#1A1A1A] mb-1">Branch Code *</label>
            <input
              type="text"
              value={branchName}
              onChange={(e) => setBranchName(e.target.value)}
          ...
          <div>
            ...
            >
              <Image src={logoPreview} alt="Preview" width={60} height={60} className="object-contain mx-auto mb-2" />
      ...
      >
        <div className="text-center py-4">
          ...
          <h3 className="text-lg font-bold text-[#1A1A1A] mb-2">Delete Branch Logo?</h3>
          <p className="text-sm text-[#6B7280]">
            Are you sure you want to delete the logo for <span className="font-bold text-[#1A1A1A]">{selectedLogo?.branchName}</span>?
