...
import { Calendar, Clock, MapPin, Users, Plus, Edit, Trash2, AlertCircle } from 'lucide-react';
...
export function MatchSchedulingPage() {
  ...
  const matchesByDate = matches.filter((match: any) => match.date === selectedDate);
    ...
    <div className="p-8 animate-page-entrance">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl md:text-[28px] font-semibold text-[#1A1A1A] mb-2">Match Scheduling</h1>
            <p className="text-sm text-[#6B7280]">Manage match schedules, venues, and timings</p>
          </div>
            ...
            <div className="bg-white rounded-lg border border-[#E5E7EB] p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-2xl font-semibold text-[#10B981]">{stats.scheduled}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg border border-[#E5E7EB] p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-2xl font-semibold text-[#F59E0B]">{stats.pending}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg border border-[#E5E7EB] p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-2xl font-semibold text-[#6B7280]">{stats.completed}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 mb-6">
          <label className="text-sm font-medium text-[#1A1A1A]">Select Date:</label>
          <input
            type="date"
            value={selectedDate}
          ...
          <div className="text-sm text-[#6B7280]">
            {matchesByDate.length} {matchesByDate.length === 1 ? 'match' : 'matches'} scheduled
          </div>
        </div>
      </div>
            ...
            {matchesByDate.
              ...
              map((match, index) =>
                <div
                  ...
                  style={{ borderColor: match.sportColor, animationDelay: `${index * 100 + 200}ms` }}>
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <span className="text-sm font-semibold text-[#6B7280]">{match.round}</span>
                        <span
                          className={`px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${match.status === 'scheduled' ?
                      ...
                      <div className="text-sm text-[#6B7280] mb-3">{match.championship}</div>

                      {/* Teams */}
                      <div className="flex items-center gap-4 mb-4">
                        <div className="flex-1 text-center py-2 px-4 bg-white rounded-lg border border-[#E5E7EB]">
                          <div className="text-sm font-bold text-[#1A1A1A]">{match.team1}</div>
                        </div>
                        <div className="text-xs font-bold text-[#6B7280]">VS</div>
                        <div className="flex-1 text-center py-2 px-4 bg-white rounded-lg border border-[#E5E7EB]">
                          <div className="text-sm font-bold text-[#1A1A1A]">{match.team2}</div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-4 gap-4">
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <div>
                            ...
                            <div className="font-semibold text-[#1A1A1A]">{match.time}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 text-sm">
                          <Clock className="w-4 h-4 text-[#6B7280]" />
                          <div>
                            ...
                            <div className="font-semibold text-[#1A1A1A]">{match.duration}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <div>
                            ...
                            <div className="font-semibold text-[#1A1A1A]">{match.venue}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 text-sm">
                          <Users className="w-4 h-4 text-[#6B7280]" />
                          <div>
                            <div className="text-xs text-[#6B7280]">Referee</div>
                            <div className="font-semibold text-[#1A1A1A]">{match.referee}</div>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-8">
          <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-[#E5E7EB]">
              <h3 className="text-xl font-semibold text-[#1A1A1A]">Schedule New Match</h3>
            </div>
