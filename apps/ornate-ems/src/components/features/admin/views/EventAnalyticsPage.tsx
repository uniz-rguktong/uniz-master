...
export function EventAnalyticsPage() {
    ...
    async function fetchAnalytics() {
      ...
      try {
        ...
        if (result.success && result.data && result.data.events) {
          ...
          const spot = mockEventData.reduce((acc: any, e: any) => acc + e.spotRegistrations, 0);
          const rev = mockEventData.reduce((acc: any, e: any) => acc + e.revenue, 0);
          const totalReg = online + spot;
          const attRate = totalReg > 0 ? Math.round(mockEventData.reduce((acc: any, e: any) => acc + (e.attendance / (e.onlineRegistrations + e.spotRegistrations) * 100), 0) / mockEventData.length) : 0;

          setSummaryMetrics({
            totalOnlineRegistrations: online,
            totalSpotRegistrations: spot,
            totalRevenue: rev,
  ...
  const { totalOnlineRegistrations, totalSpotRegistrations, totalRevenue, avgAttendanceRate } = useMemo(() => {
    const onlineRegs = filteredEvents.reduce((sum: any, event: any) => sum + event.onlineRegistrations, 0);
    const spotRegs = filteredEvents.reduce((sum: any, event: any) => sum + event.spotRegistrations, 0);
    ...
    return {
      ...
      totalSpotRegistrations: spotRegs,
  ...
  const handleEditEvent = (event: any) => {
    ...
    setEditFormData({
      spotRegistrations: event.spotRegistrations,
  ...
  const handleExportReport = (options: any) => {
    ...
    if (options.format === 'csv' || options.format === 'excel') {
      const headers = ['Event Name', 'Category', 'Online Registrations', 'Spot Registrations', 'Total Registrations', 'Attendance', 'Revenue'];
      ...
      const rows = filteredEvents.map(event => [
        ...
        event.onlineRegistrations + event.spotRegistrations,
        event.attendance,
        `$${event.revenue}`
      ]);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          ...
          <div className="flex flex-row items-center gap-3">
            {isMounted ? (
              ...
              <div className="flex-1 sm:w-[180px] sm:flex-none bg-white border border-[#E5E7EB] text-[#1A1A1A] h-[42px] rounded-md px-3 py-2 flex items-center justify-between opacity-50 cursor-not-allowed">
          ...
          <>
            ...
            <div className="animate-card-entrance" style={{ animationDelay: '80ms' }}>
              <MetricCard
                ...
                iconBgColor="#F5F3FF"
                iconColor="#8B5CF6"
                trend={trends.spotRegistrations ? { ...trends.spotRegistrations } : null}
                className="!pb-[25px]"
