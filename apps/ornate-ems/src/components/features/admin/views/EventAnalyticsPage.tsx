...
export function EventAnalyticsPage() {
    ...
    async function fetchAnalytics() {
      ...
      try {
        ...
        if (result.success && result.data && result.data.events) {
          ...
          const online = mockEventData.reduce(
            (acc: any, e: any) => acc + e.onlineRegistrations,
            0,
          );
          const offline = mockEventData.reduce(
            (acc: any, e: any) => acc + e.offlineRegistrations,
            0,
          );
          const rev = mockEventData.reduce(
            (acc: any, e: any) => acc + e.revenue,
            0,
          );
          ...
          const attRate =
            totalReg > 0
              ? Math.round(
                  mockEventData.reduce(
                    (acc: any, e: any) =>
                      acc +
                      (e.attendance /
                        (e.onlineRegistrations + e.offlineRegistrations)) *
                        100,
                    0,
  ...
  const {
    ...
    const onlineRegs = filteredEvents.reduce(
      (sum: any, event: any) => sum + event.onlineRegistrations,
      0,
    );
    const offlineRegs = filteredEvents.reduce(
      (sum: any, event: any) => sum + event.offlineRegistrations,
      0,
    );
    const revenue = filteredEvents.reduce(
      (sum: any, event: any) => sum + event.revenue,
      0,
    );
          ...
          <>
            ...
            >
              <MetricCard
                ...
                iconColor="#10B981"
                trend={
                  trends.attendanceRate ? { ...trends.attendanceRate } : null
                }
                className="!pb-[25px]"
            ...
            <div className="space-y-5 p-5">
              {isLoading
                ...
                : Object.values(categoryPerformance)
                    ...
                    .map((cat: any) => {
                      const percentage =
                        (cat.totalRegistrations / maxCategoryRegistrations) *
                        100;
                      return (
