...
interface UpdatesAnnouncementsPageProps {
  ...
  enforceExpiryAfterCreation?: boolean;
}

export function UpdatesAnnouncementsPage({
  autoOpenCreate = false,
  enforceExpiryAfterCreation = false,
}: UpdatesAnnouncementsPageProps) {
  ...
  const handleSave = async () => {
    ...
    if (enforceExpiryAfterCreation) {
      const expiry = new Date(formData.expiryDate);
      const editingAnnouncement = editingId
        ? announcementsList.find((item: any) => item.id === editingId)
        : null;
      const createdReference = editingAnnouncement
        ? new Date(editingAnnouncement.createdDate)
        : new Date();
      ...
      const createdDateOnly = new Date(
        createdReference.getFullYear(),
        createdReference.getMonth(),
        createdReference.getDate(),
      ).getTime();

      if (!(expiryDateOnly > createdDateOnly)) {
        showToast(
          "Expiration date should be greater than the creation date",
          "error",
        );
        return;
      }
  ...
  const filteredAnnouncements = announcementsList.filter((ann: any) => {
    const matchesCategory =
      filterCategory === "All" || ann.category === filterCategory;
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">
              Manage important updates and announcements for students
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="Global (All Users)">
                      Global (All Admins & Students)
                    </SelectItem>
                    ...
                    <SelectItem value="Registered Participants">
                      Registered Participants
                    </SelectItem>
                    <SelectItem value="Workshop Participants">
                      Workshop Participants
                    </SelectItem>
                    <SelectItem value="Paid Event Registrants">
                      Paid Event Registrants
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
