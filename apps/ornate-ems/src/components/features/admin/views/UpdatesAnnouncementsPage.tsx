...
export function UpdatesAnnouncementsPage({ autoOpenCreate = false, enforceExpiryAfterCreation = false }: UpdatesAnnouncementsPageProps) {
  ...
  const handleSave = async () => {
    ...
    if (enforceExpiryAfterCreation) {
      ...
      const editingAnnouncement = editingId ? announcementsList.find((item: any) => item.id === editingId) : null;
      const createdReference = editingAnnouncement ? new Date(editingAnnouncement.createdDate) : new Date();

      const expiryDateOnly = new Date(expiry.getFullYear(), expiry.getMonth(), expiry.getDate()).getTime();
      const createdDateOnly = new Date(createdReference.getFullYear(), createdReference.getMonth(), createdReference.getDate()).getTime();

      if (!(expiryDateOnly > createdDateOnly)) {
        showToast('Expiration date should be greater than the creation date', 'error');
        return;
      }
    }
    ...
    <div className="p-8 animate-page-entrance">
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            <h1 className="text-[28px] font-semibold text-[#1A1A1A] mb-2">Updates & Announcements</h1>
