  ...
  return (
    <div className="p-6 md:p-8 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between mb-6 gap-4">
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filterGender === "all" ? "bg-[#1A1A1A] text-white" : "bg-white border border-[#E5E7EB] text-[#6B7280] hover:bg-[#F7F8FA]"}`}
          >
            All
          </button>
          <button
            onClick={() => setFilterGender("male")}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filterGender === "male" ? "bg-[#1A1A1A] text-white" : "bg-white border border-[#E5E7EB] text-[#6B7280] hover:bg-[#F7F8FA]"}`}
          >
            Male Students
          </button>
          <button
            onClick={() => setFilterGender("female")}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filterGender === "female" ? "bg-[#1A1A1A] text-white" : "bg-white border border-[#E5E7EB] text-[#6B7280] hover:bg-[#F7F8FA]"}`}
          >
            Female Students
          </button>
        </div>
        ...
        >
          <SelectTrigger className="w-[140px] px-4 py-2 bg-white border border-[#E5E7EB] rounded-lg text-sm font-medium">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {isLoading
          ? [...Array(4)].map((_, i) => (
              <div
                key={i}
                className="bg-[#F4F2F0] rounded-[18px] p-[10px] py-[16px] animate-pulse"
              >
                {/* Skeleton UI */}
              </div>
            ))
          : filteredStudents.map((student: any) => (
              <div
                key={student.id}
                className="bg-[#F4F2F0] rounded-[18px] p-[10px] py-[16px] h-full flex flex-col"
              >
                  ...
                  <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-3 text-center sm:text-left w-full sm:w-auto">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 text-sm text-[#6B7280] mb-2">
                        {student.rollNumber} • {student.branch} - Year{" "}
                        {student.year}
                      </div>
                      <div
                ...
                <div className="bg-white rounded-[14px] p-5 flex-1 flex flex-col">
                  <div className="grid grid-cols-2 gap-4 mb-5 pb-5 border-b border-[#E5E7EB]">
                    ...
                    <div className="text-center">
                      ...
                      <div className="text-xs text-[#6B7280]">Award Year</div>
                    </div>
                  </div>
                      ...
                      {student.placementStatus === "Placed" &&
                        student.package && (
                          <div className="text-right">
                            ...
                            <div className="text-lg font-extrabold text-[#1A1A1A] tracking-tight">
                              {student.package}{" "}
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, awardYear: e.target.value })
                }
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, placementStatus: status })
                    }
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                      formData.placementStatus === status
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                onClick={handleSave}
                className="px-4 py-2 bg-[#10B981] text-white rounded"
              >
                Save
              </button>
            </div>
      ...
      <ConfirmDialog
        ...
        message={`Remove ${deleteStudent?.name}?`}
        variant="danger"
      />
    </div>
  );
