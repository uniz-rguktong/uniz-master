...
export function AllRegistrationsPage({ initialSearchQuery = '', initialRegistrations = [] }: AllRegistrationsPageProps = {}) {
    ...
    async function loadData() {
      try {
        ...
        const [regResult, trendsResult, teamResult] = await Promise.all([
          !initialRegistrations.length ? getAllRegistrations() : Promise.resolve({ success: true, registrations: initialRegistrations }),
          getAllTrends(),
          getAllTeams()
        ]);
        ...
        if ((teamResult as any).teams) {
          setTeamRegistrations((teamResult as any).teams.map((t: any) => ({
            id: t.id,
            teamId: t.teamCode,
            teamName: t.teamName,
  ...
  const offlineRegistrations = totalRegistrations - onlineRegistrations || Math.floor(totalRegistrations * 0.18);
