    ...
    const fetchData = async () => {
      ...
      try {
        if (initialUsers.length === 0) {
          ...
          if (usersRes.success) setUsersList((usersRes as any).users);
        }

        if (initialLogs.length === 0) {
          const logsRes = await getAuditLogs();
          if (logsRes.success) setLogsList((logsRes as any).logs);
        }
      } catch (error) {
    ...
    <div className="p-4 md:p-8 animate-page-entrance">
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl md:text-[28px] font-semibold text-[#1A1A1A] mb-2">Access Control & Roles</h1>
            <p className="text-sm text-[#6B7280]">Manage system permissions and user access levels.</p>
          </div>
        </div>
      </div>
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-[12px] mt-[10px] mb-[16px]">
                        <div className="flex items-center gap-4">
                          ...
                          <span className="px-3 py-1 bg-white border border-gray-100 rounded-full text-xs font-semibold text-[#6B7280]">
                            {usersList.filter(u => u.role === role.name).length} Members
                          </span>
                        </div>
