...
import { Search, Download, Mail, CheckCircle, UserCheck, Award, Trophy, Users, RefreshCw, Globe, Target, DollarSign } from 'lucide-react';
...
export function ConfirmedRegistrationsPage({ initialData = [], initialEvents = [] }: ConfirmedRegistrationsPageProps = {}) {
  ...
  const onlineRegistrationsCount = data.filter(r => r.transactionId || r.paymentAmount > 0).length || Math.floor(totalRegistrationsCount * 0.82);
  const spotRegistrationsCount = totalRegistrationsCount - onlineRegistrationsCount || Math.floor(totalRegistrationsCount * 0.18);
  const totalRevenueValue = data.reduce((sum: any, reg: any) => sum + (reg.paymentAmount || 0), 0) || (onlineRegistrationsCount * 250);
  const avgAttendanceRateValue = 92;

  const totalConfirmed = data.length;
  const attendanceMarkedCount = data.filter((r: any) => r.attendanceMarked).length;
  const certificatesIssuedCount = data.filter((r: any) => r.certificateIssued).length;
  ...
  const handleExportData = () => {
    ...
    exportRegistrationsToCSV(filteredRegistrations, 'confirmed_registrations.csv');
  ...
  return (
    <div className="p-8 animate-page-entrance">
      ...
      <div className="mb-8">
        ...
        <div className="md:hidden flex flex-col gap-3 mb-6">
          {selectedRegistrations.length > 0 &&
            <button
              onClick={handleBulkEmail}
              className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#10B981] text-white rounded-lg text-sm font-medium hover:bg-[#059669] transition-colors w-full">
              <Mail className="w-4 h-4" />
              Email Selected ({selectedRegistrations.length})
            </button>
          }
        ...
        <div className="flex flex-row items-center justify-between mb-4 px-3 mt-[-4px] gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            {selectedRegistrations.length > 0 &&
              <button
                onClick={handleBulkEmail}
                title={`Email Selected (${selectedRegistrations.length})`}
                className="flex items-center justify-center gap-1.5 p-2 sm:px-3 sm:py-1.5 bg-[#10B981] text-white rounded-md hover:bg-[#059669] transition-colors text-sm font-medium">
                <Mail className="w-4 h-4" />
            ...
            <table className="w-full min-w-[1000px] table-fixed">
              <thead className="bg-white border-b border-[#F3F4F6]">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-[15%]">
                    Registration ID ↑
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-[22%]">
                    Student Details ↑
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-[15%]">
                    Branch
                  ...
                  <th className="px-4 py-3 text-right text-xs font-bold text-gray-500 uppercase tracking-wider w-[8%]">
                    Actions
                  </th>
                </tr>
              </thead>
                ...
                {isLoading ? (
                  [...Array(5)].map((_: any, i: any) => (
                    <tr key={i} className="border-b border-[#F3F4F6]">
                      ...
                      <td className="px-4 py-4"><Skeleton width={80} height={24} borderRadius={20} /></td>
                      <td className="px-4 py-4"><Skeleton width={80} height={24} borderRadius={20} /></td>
                  ...
                  filteredRegistrations.map((registration: any, index: any) =>
                    <tr
                      ...
                      <td className="px-4 py-4">
                        <div className="font-medium text-sm text-[#1A1A1A] truncate">{registration.department}</div>
                      </td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4">
                        <button
                          ...
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded text-xs font-medium transition-all ${registration.attendanceMarked
                            ? 'bg-[#D1FAE5] text-[#065F46] hover:bg-[#A7F3D0]'
                            : 'bg-white border border-[#E5E7EB] text-[#3B82F6] hover:bg-gray-50'
                            }`}>
                          <CheckCircle className={`w-3 h-3 ${registration.attendanceMarked ? 'opacity-100' : 'opacity-40'}`} />
                          {registration.attendanceMarked ? 'Marked' : 'Mark'}
                        </button>
                      </td>
                      <td className="px-4 py-4">
                        <button
                          onClick={() => handleIssueCertificate(registration.id)}
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded text-xs font-medium transition-all ${registration.certificateIssued
                            ? 'bg-[#FEF3C7] text-[#92400E] hover:bg-[#FDE68A]'
                            : 'bg-white border border-[#E5E7EB] text-[#F59E0B] hover:bg-gray-50'
                            }`}>
                          <Award className={`w-3 h-3 ${registration.certificateIssued ? 'opacity-100' : 'opacity-40'}`} />
                          {registration.certificateIssued ? 'Issued' : 'Issue'}
                        </button>
                      </td>
      ...
      <Modal
        ...
        confirmText="Send Email"
        onConfirm={() => {
          showToast(`Email sent to ${selectedRegistrations.length} students`, 'success');
