...
export function PromoVideoLogoPage() {
  ...
  const [videoUploadMethod, setVideoUploadMethod] = useState(
    ...
    input.onchange = (e: any) => {
      ...
      if (file) {
        if (type === "image") {
          ...
          showToast(
            "Direct video upload is disabled in this environment. Please use YouTube/Vimeo URL.",
            "warning",
          );
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">
              Manage promotional content and branding assets
            </p>
          </div>
              ...
              <>
                ...
                >
                  <MetricCard
                    title="Active Logos"
                    value={
                      logos.filter((l: any) => l.status === "active").length
                    }
                    icon={CheckCircle}
                    iconBgColor="#F0FDF4"
                    iconColor="#10B981"
