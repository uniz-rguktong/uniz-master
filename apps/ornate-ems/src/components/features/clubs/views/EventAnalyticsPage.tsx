...
export function EventAnalyticsPage() {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (res.success) {
          ...
          showToast(
            "error" in res ? res.error : "Failed to load analytics",
            "error",
          );
        }
      } catch (error) {
  ...
  const totalOnlineRegistrations = filteredEvents.reduce(
    (sum: any, event: any) => sum + event.onlineRegistrations,
    0,
  );
  const totalOfflineRegistrations = filteredEvents.reduce(
    (sum: any, event: any) => sum + (event.offlineRegistrations || 0),
    0,
  );
  const totalRevenue = filteredEvents.reduce(
    (sum: any, event: any) => sum + (event.revenue || 0),
    0,
  );
    ...
    filteredEvents.length > 0
      ? Math.round(
          filteredEvents.reduce((sum: any, event: any) => {
            const totalRegs =
              event.onlineRegistrations + (event.offlineRegistrations || 0);
            return (
              sum + (totalRegs > 0 ? (event.attendance / totalRegs) * 100 : 0)
            );
  ...
  const handleEditEvent = (event: any) => {
    ...
    setEditFormData({
      offlineRegistrations: event.offlineRegistrations || 0,
  ...
  const handleSaveEdit = () => {
    if (selectedEvent) {
      ...
      showToast(
        "Event data updated locally (Backend persistence pending schema update)",
        "success",
      );
  ...
  const handleExportReport = (options: any) => {
    ...
    if (options.format === "csv" || options.format === "excel") {
      const exportData = filteredEvents.map((event) => ({
        ...
        "Offline Registrations": event.offlineRegistrations,
        "Total Registrations":
          event.onlineRegistrations + event.offlineRegistrations,
        Attendance: event.attendance,
        Revenue: event.revenue,
        "Attendance Rate": `${event.onlineRegistrations + event.offlineRegistrations > 0 ? Math.round((event.attendance / (event.onlineRegistrations + event.offlineRegistrations)) * 100) : 0}%`,
      }));

      downloadCSV(
        exportData,
        `event_analytics_report_${new Date().toISOString().split("T")[0]}.csv`,
      );
    }

    showToast(
      `${options.format.toUpperCase()} report exported successfully!`,
      "success",
    );
  };
  ...
  const categoryPerformance = filteredEvents.reduce(
    (acc: any, event: any) => {
  ...
  const maxCategoryRegistrations = Math.max(
    ...Object.values(categoryPerformance).map((c: any) => c.totalRegistrations),
    0,
  );
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            <h1 className="text-[28px] font-semibold text-[#1A1A1A] mb-2">
              Event Performance Reports & Analytics
            </h1>
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 px-3 mt-[-4px] gap-4">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-bold text-[#7A7772] uppercase tracking-widest opacity-70">
              Event Performance
            </h2>
                  ...
                  : filteredEvents
                      ...
                      .map((event, index, array) => {
                        ...
                        const attendanceRate =
                          totalRegs > 0
                            ? Math.round((event.attendance / totalRegs) * 100)
                            : 0;
            ...
            <div className="space-y-5 p-5">
              {isLoading
                ...
                : Object.values(categoryPerformance).map((cat: any) => {
                    const percentage =
                      (cat.totalRegistrations / maxCategoryRegistrations) * 100;
                    return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-[#6B7280]">
                            {event.totalRegs} registrations • {event.attendance}{" "}
                            attended
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-semibold text-[#10B981]">
                          {Math.round(
                            (event.attendance / event.totalRegs) * 100,
                          )}
                          %
                        </div>
        ...
        >
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditFormData({
                    ...editFormData,
                    offlineRegistrations: parseInt(e.target.value) || 0,
                  })
                }
            ...
            <div className="bg-[#F7F8FA] rounded-lg p-3 text-sm text-[#6B7280]">
              <p>
                💡 After each event, update the offline registrations and
                attendance numbers to maintain accurate analytics.
              </p>
            </div>
          </div>
        </Modal>
