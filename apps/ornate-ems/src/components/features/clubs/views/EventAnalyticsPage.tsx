...
export function EventAnalyticsPage() {
  ...
  const totalOfflineRegistrations = filteredEvents.reduce((sum: any, event: any) => sum + (event.offlineRegistrations || 0), 0);
  ...
  const avgAttendanceRate = filteredEvents.length > 0 ? Math.round(
    filteredEvents.reduce((sum: any, event: any) => {
      const totalRegs = event.onlineRegistrations + (event.offlineRegistrations || 0);
  ...
  const handleExportReport = (options: any) => {
    ...
    if (options.format === 'csv' || options.format === 'excel') {
      const exportData = filteredEvents.map(event => ({
        ...
        'Total Registrations': event.onlineRegistrations + event.offlineRegistrations,
