...
export function BrandingPage() {
    ...
    const handleSaveVideo = async () => {
        ...
        const videoData = {
            ...
            thumbnail: getThumbnailUrl({ url: videoUrl, platform: videoPlatform }) || 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=600&h=400&fit=crop',
            duration: '0:00'
        };
        ...
        try {
            if (editingVideo) {
                ...
                if (result.success) {
                    setPromoVideos(prev => prev.map(v => v.id === editingVideo.id ? result.video : v));
    ...
    const handleSaveLogo = async () => {
        ...
        let finalUrl = editingLogo?.url || 'https://images.unsplash.com/photo-1611162617474-5b21e879e113?w=300&h=300&fit=crop';
        ...
        try {
            if (editingLogo) {
                ...
                if (result.success) {
                    setLogos(prev => prev.map(l => l.id === editingLogo.id ? result.logo : l));
        ...
        <div className="p-8 animate-page-entrance">
            ...
            <div className="mb-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                    <div>
                        <h1 className="text-[28px] font-semibold text-[#1A1A1A] mb-2">HHO Branding Assets</h1>
                        <p className="text-sm text-[#6B7280]">Manage HHO's promotional content and visual identity</p>
                    </div>
                ...
                <div className="inline-flex items-center p-1 bg-[#F4F2F0] rounded-lg border border-[#E5E7EB]/50">
                    <button onClick={() => handleTabChange('logos')} className={`flex items-center gap-2 px-6 py-2 text-sm font-medium rounded-[7px] transition-all duration-200 ${activeTab === 'logos' ? 'bg-white text-[#1A1A1A] shadow-sm' : 'text-[#6B7280] hover:text-[#1A1A1A]'}`}>
                        <ImageIcon className="w-4 h-4" /> Brand Logos
                    </button>
                    <button onClick={() => handleTabChange('videos')} className={`flex items-center gap-2 px-6 py-2 text-sm font-medium rounded-[7px] transition-all duration-200 ${activeTab === 'videos' ? 'bg-white text-[#1A1A1A] shadow-sm' : 'text-[#6B7280] hover:text-[#1A1A1A]'}`}>
            ...
            {activeTab === 'videos' &&
                <div className="space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                        {isLoading ? <><MetricCardSkeleton /><MetricCardSkeleton /><MetricCardSkeleton /><MetricCardSkeleton /></> : (
                            <>
                                ...
                                <div className="animate-card-entrance" style={{ animationDelay: '120ms' }}>
                                    <MetricCard
                                        title="Active Videos"
                                        value={promoVideos.filter(v => v.status === 'active').length}
                                        icon={Play}
                                        iconBgColor="#F0FDF4"
                                        iconColor="#10B981"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        {isLoading ? <><VideoCardSkeleton /><VideoCardSkeleton /></> : promoVideos.map(video => <VideoCard key={video.id} video={{ ...video, thumbnail: getThumbnailUrl(video) }} onPreview={() => handleVideoPreview(video)} onEdit={() => handleVideoEdit(video)} onDelete={() => handleVideoDelete(video)} />)}
                    </div>
                </div>
            }
            ...
            {activeTab === 'logos' &&
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {isLoading ? <><LogoCardSkeleton /><LogoCardSkeleton /><LogoCardSkeleton /></> : logos.map(logo => <LogoCard key={logo.id} logo={logo} onPreview={() => handleLogoPreview(logo)} onDownload={() => handleLogoDownload(logo)} onEdit={() => handleLogoEdit(logo)} onDelete={() => handleLogoDelete(logo)} />)}
                    </div>
                </div>
            }
            ...
            {showVideoPreviewModal && selectedVideo && (
                <Modal isOpen={true} onClose={() => setShowVideoPreviewModal(false)} title={selectedVideo.title} size="xl">
                    <div className="aspect-video w-full bg-black rounded-b-xl overflow-hidden">
                        <iframe src={getEmbedUrl(selectedVideo) || undefined} className="w-full h-full" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen></iframe>
                    </div>
                </Modal>
            )}
                                ...
                                <div>
                                    ...
                                    <Select value={logoType} onValueChange={setLogoType}>
                                        ...
                                        <SelectContent>
                                            ...
                                            <SelectItem value="Secondary Logo">Secondary Logo</SelectItem>
            ...
            {
                showDeleteModal && (
                    <Modal isOpen={true} onClose={() => setShowDeleteModal(false)} title="Confirm Deletion" onConfirm={confirmDelete} confirmText="Delete" confirmButtonClass="bg-rose-500 hover:bg-rose-600">
                        <div className="p-8"><p className="text-[#6B7280]">Are you sure you want to delete this {deleteType}? This action cannot be undone.</p></div>
                    </Modal>
                )
            }
            {/* Logo Preview Modal */}
