...
export function BasicDetailsStep({ data, updateData }: BasicDetailsStepProps) {
  ...
  const [teamSize, setTeamSize] = useState(() => {
    ...
    return {
      min:
        ts.min === undefined || ts.min === null || isNaN(Number(ts.min))
          ? ""
          : Number(ts.min),
      max:
        ts.max === undefined || ts.max === null || isNaN(Number(ts.max))
          ? ""
          : Number(ts.max),
    };
  });
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          >
            <SelectTrigger className="w-full h-11.5 bg-white border border-[#E5E7EB] rounded-lg text-sm focus:ring-2 focus:ring-[#1A1A1A] disabled:bg-[#F7F8FA] disabled:cursor-not-allowed">
              <SelectValue placeholder="Select a sub-category" />
            </SelectTrigger>
            <SelectContent>
              {category &&
                subCategories[category]?.map((subCat: any) => (
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 p-4 border border-[#E5E7EB] rounded-lg cursor-pointer hover:bg-[#F7F8FA] transition-colors">
              <input
                ...
                onChange={() => {
                  ...
                  const isCompetition =
                    currentType.includes("competition") || data.hasPrizes;
            ...
            <label className="flex items-center gap-3 p-4 border border-[#E5E7EB] rounded-lg cursor-pointer hover:bg-[#F7F8FA] transition-colors">
              ...
              <div>
                ...
                <div className="text-xs text-[#6B7280]">
                  Participants compete in groups
                </div>
              </div>
            </label>
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 p-4 border border-[#E5E7EB] rounded-lg cursor-pointer hover:bg-[#F7F8FA] transition-colors">
              <input
                ...
                checked={
                  data.hasPrizes === true ||
                  Boolean(data.eventType?.includes("competition"))
                }
                onChange={() => {
              ...
              <div>
                ...
                <div className="text-xs text-[#6B7280]">
                  Event includes winner ranking and prizes
                </div>
              </div>
            </label>
            ...
            <label className="flex items-center gap-3 p-4 border border-[#E5E7EB] rounded-lg cursor-pointer hover:bg-[#F7F8FA] transition-colors">
              ...
              <div>
                ...
                <div className="text-xs text-[#6B7280]">
                  Workshop, Seminar, or Learning session
                </div>
              </div>
            </label>
          </div>
      ...
      <div>
        ...
        <div className="flex items-center justify-between mt-2">
          <span className="text-xs text-[#6B7280]">
            This will appear on event cards
          </span>
          <span className="text-xs text-[#6B7280]">
      ...
      <div>
        ...
        <div className="flex items-center gap-2 mt-2">
          ...
          <span className="text-xs text-[#6B7280]">
            This will be visible on the event detail page
          </span>
        </div>
      </div>
    </div>
