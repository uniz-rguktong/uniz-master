...
export function RegistrationSettingsStep({ data, updateData }: RegistrationSettingsStepProps) {
  ...
  const [maxParticipants, setMaxParticipants] = useState(
    data.maxParticipants || (data as any).capacity || (data as any).maxCapacity || ''
  );
  ...
  const [customFields, setCustomFields] = useState<CustomField[]>(
    Array.isArray(data.customFields)
      ? data.customFields
      : (Array.isArray((data.customFields as any)?.registrationFields) ? (data.customFields as any).registrationFields : [])
  );
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-medium text-[#1A1A1A]">Custom Registration Fields</h3>
          <button
        ...
        <div className="space-y-3 mb-4">
          <div className="p-3 bg-[#F7F8FA] border border-[#E5E7EB] rounded-lg text-xs text-[#6B7280]">
            Default fields: Full Name, Roll Number, Email, Phone Number, Branch, Year
          </div>

          {customFields.map((field: any, index: any) =>
            <div key={index} className="p-4 border border-[#E5E7EB] rounded-lg bg-white">
              <div className="flex items-start gap-3">
      ...
      {!isPaid && (
        <div>
          <h3 className="text-sm font-medium text-[#1A1A1A] mb-4">Confirmation Settings</h3>
          <div className="space-y-3">
            <label className="flex items-center justify-between p-4 border border-[#E5E7EB] rounded-lg">
              <div>
                <div className="font-medium text-[#1A1A1A]">Auto-Confirmation</div>
                <div className="text-xs text-[#6B7280] mt-1">Automatically confirm registrations</div>
              </div>
              <input type="checkbox" className="w-5 h-5 rounded border-[#E5E7EB]" defaultChecked />
            </label>

            <label className="flex items-center justify-between p-4 border border-[#E5E7EB] rounded-lg">
              <div>
                <div className="font-medium text-[#1A1A1A]">Send Confirmation Email</div>
                <div className="text-xs text-[#6B7280] mt-1">Email confirmation to participants</div>
              </div>
              <input type="checkbox" className="w-5 h-5 rounded border-[#E5E7EB]" defaultChecked />
            </label>
            ...
            <label className="flex items-center justify-between p-4 border border-[#E5E7EB] rounded-lg">
              <div>
                <div className="font-medium text-[#1A1A1A]">Send SMS Notification</div>
                <div className="text-xs text-[#6B7280] mt-1">SMS confirmation to participants</div>
              </div>
              <input type="checkbox" className="w-5 h-5 rounded border-[#E5E7EB]" />
            </label>
          </div>
        </div>
      )}
