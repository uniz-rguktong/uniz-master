...
export function ReviewPublishStep({
  ...
  const checklistItems: ChecklistItem[] = [
    {
      label: "Event name and description added",
      completed: !!(
        data.eventName ||
        (data.title && (data.description || data.detailedDescription))
      ),
    },
    {
      label: "Venue and timing set",
      completed: !!(
        data.venue &&
    ...
    {
      label: "Registration settings configured",
      completed: !!data.registrationStatus,
    },
    { label: "Poster uploaded", completed: !!(data.poster || data.posterUrl) },
    {
      label: "Coordinator contact visible",
      completed: !!(data.coordinators && data.coordinators.length > 0),
      warning: !(data.coordinators && data.coordinators.length > 0),
    },
  ];
    ...
    <div className="space-y-8">
          ...
          <div className="space-y-3">
            {checklistItems.map((item: any, index: any) => (
              ...
              >
                ...
                <span
                  className={`text-sm ${item.warning ? "text-[#92400E]" : "text-[#1A1A1A]"}`}
                >
                  {item.label}
                </span>
              </div>
      ...
      <div>
        ...
        <div className="border-2 border-[#E5E7EB] rounded-xl p-6 bg-[#F7F8FA]">
          <div
            className={`bg-white rounded-lg overflow-hidden shadow-lg mx-auto ${previewDevice === "mobile" ? "max-w-sm" : ""}`}
          >
            <div className="aspect-video bg-linear-to-br from-[#3B82F6] to-[#8B5CF6] flex items-center justify-center text-white overflow-hidden relative">
              {data.poster || data.posterUrl ? (
                <img
                  src={data.poster || data.posterUrl}
                  alt="Poster"
                ...
                <div className="flex items-center gap-2">
                  ...
                  {data.startDate ||
                    (data.date
                      ...
                      : "Date TBD")}{" "}
                  • {formatTimeTo12h(data.startTime) || "Time TBD"}
                </div>
                <div className="flex items-center gap-2">
                  <span>👥</span> Capacity:{" "}
                  {data.maxParticipants || data.maxCapacity || "Unlimited"}
                </div>
              </div>
            </div>
          </div>
