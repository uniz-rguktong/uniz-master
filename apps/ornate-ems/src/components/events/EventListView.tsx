...
const EventDetailsModal = dynamic(() => import('@/components/EventDetailsModal').then(mod => mod.EventDetailsModal), {
  loading: () => null
});
import { useState, useEffect } from 'react';
import { useRouter, usePathname } from 'next/navigation';
import { updateEventStatus, deleteEvent, duplicateEvent } from '@/actions/eventActions';
...
export function EventListView({ events, searchQuery, selectedFilter, onNavigate, showArchived, showDrafts, sortBy }: EventListViewProps) {
  ...
  useEffect(() => {
    if (events) {
      const mappedEvents = events.map((e: any) => ({
        ...
        thumbnail: e.posterUrl || 'https://images.unsplash.com/photo-1677442136019-21780ecad995?w=100&h=60&fit=crop',
        dateTime: e.date ? new Date(e.date).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short', hour12: true }) : 'TBD',
        venue: e.venue || 'TBD',
        ...
        isArchived: (e.status || '').toUpperCase() === 'CANCELLED' || (e.status || '').toUpperCase() === 'ARCHIVED',
  ...
  const basePath = pathname.startsWith('/hho') ? '/hho' :
    pathname.startsWith('/super-admin') ? '/super-admin' :
      pathname.startsWith('/clubs-portal') ? '/clubs-portal' :
        pathname.startsWith('/sports') ? '/sports' :
          pathname.startsWith('/branch-admin') ? '/branch-admin' : '/dashboard';
  ...
  const handleArchive = async (event: MappedEvent) => {
    const isCurrentlyArchived = event.status === 'CANCELLED' || event.status === 'ARCHIVED';
    const newStatus = isCurrentlyArchived ? 'PUBLISHED' : 'CANCELLED';

    // Optimistic UI update
    setEventsList(eventsList.map(e =>
      e.id === event.id ? { ...e, status: newStatus, isArchived: !isCurrentlyArchived } : e
    ));

    const result = await updateEventStatus(event.id, newStatus);

    if (result.success) {
      showToast(`Event ${isCurrentlyArchived ? 'restored' : 'archived'} successfully`, isCurrentlyArchived ? 'success' : 'success');
    } else {
      // Revert on failure
      setEventsList(eventsList.map(e =>
        e.id === event.id ? { ...e, status: event.status, isArchived: isCurrentlyArchived } : e
      ));
  ...
  const filteredEvents = eventsList.filter((event: any) => {
    ...
    const isArchived = status === 'CANCELLED' || status === 'ARCHIVED' || event.isArchived;
    ...
    return matchesSearch && matchesFilter && status !== 'DRAFT' && status !== 'CANCELLED' && status !== 'ARCHIVED';
  }).sort((a, b) => {
    switch (sortBy) {
      case 'newest-first':
        return new Date(b.originalData.createdAt || b.originalData.updatedAt || b.originalData.date || 0).getTime()
          - new Date(a.originalData.createdAt || a.originalData.updatedAt || a.originalData.date || 0).getTime();
      case 'name': return a.name.localeCompare(b.name);
      case 'registrations': return b.registrations - a.registrations;
      case 'capacity': return b.capacity - a.capacity;
      case 'status': return a.status.localeCompare(b.status);
      case 'date':
      default:
        return new Date(a.originalData.date || 0).getTime() - new Date(b.originalData.date || 0).getTime();
    }
  });
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-[#E5E7EB] bg-[#F7F8FA]">
            <th className="text-left py-3 px-4 md:px-6 lg:pe-6 xl:pe-12 text-xs font-semibold text-[#6B7280] tracking-wide whitespace-nowrap lg:whitespace-normal">
              EVENT NAME ↓
            </th>
            <th className="text-left py-3 px-4 md:px-6 text-xs font-semibold text-[#6B7280] tracking-wide whitespace-nowrap lg:whitespace-normal">
              CATEGORY
            </th>
            <th className="text-left py-3 px-4 md:px-6 text-xs font-semibold text-[#6B7280] tracking-wide whitespace-nowrap lg:whitespace-normal">
              DATE & TIME
        ...
        <tbody>
          {filteredEvents.map((event: any) =>
            <tr
              ...
              <td className="py-4 px-4 md:px-6 lg:pe-6 xl:pe-12">
                <div className="flex items-center gap-2 sm:gap-3">
                  <img
                    src={event.thumbnail}
                    alt={event.name}
                    className="w-10 h-7 sm:w-16 sm:h-10 object-cover rounded shrink-0" />

                  <span className="text-[13px] md:text-sm font-medium text-[#1A1A1A] whitespace-nowrap lg:whitespace-normal md:min-w-[120px]">{event.name}</span>
                </div>
              </td>
              <td className="py-4 px-4 md:px-6 whitespace-nowrap lg:whitespace-normal">
                <span
                  className="inline-block px-2.5 py-1 rounded-full text-[11px] md:text-xs font-semibold text-white"
                  style={{ backgroundColor: event.categoryColor }}>

                  {event.category}
                </span>
              </td>
              <td className="py-4 px-4 md:px-6 text-[13px] md:text-sm text-[#6B7280] whitespace-nowrap lg:whitespace-normal">{event.dateTime}</td>
              <td className="py-4 px-4 md:px-6 text-[13px] md:text-sm text-[#6B7280] whitespace-nowrap lg:whitespace-normal lg:min-w-[120px]">{event.venue}</td>
              ...
              <td className="py-4 px-4 md:px-6 text-[13px] md:text-sm text-[#6B7280] whitespace-nowrap lg:whitespace-normal w-fit">{event.lastModified}</td>
                  ...
                  <ActionMenu
                    actions={[
                      ...
                      { divider: true, onClick: () => { } },
                      {
                        ...
                        icon: 'users',
                        onClick: () => handleNavigation('all-registrations', { eventId: event.id })
                      },
                      {
                        label: event.isArchived ? 'Restore Event' : 'Archive Event',
                        icon: 'archive',
