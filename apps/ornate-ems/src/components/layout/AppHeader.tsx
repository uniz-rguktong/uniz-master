...
import { Menu, Search, Bell, User as UserIcon, Send, Plus, Clock, LogOut, ChevronDown } from 'lucide-react';
...
export function AppHeader({ onMenuClick, user, onLogout }: AppHeaderProps) {
    ...
    const getBreadcrumb = () => {
        ...
        const breadcrumbParts = parts
            ...
            .map(p => readabilityMap[p] || p.charAt(0).toUpperCase() + p.slice(1).replace(/-/g, ' '));

        return breadcrumbParts.length > 0 ? breadcrumbParts.join(' › ') : 'Dashboard';
    };
    ...
    const getRoleBadge = () => {
        ...
        if (user?.role === 'CLUB_COORDINATOR') {
            const clubId = user.clubId || user.branch || 'CLUB';
            const clubDisplay = clubId.toLowerCase() === 'techexcel' ? 'TECHXCEL' : clubId.toUpperCase();
            return `${clubDisplay} COORDINATOR`;
        }
    ...
    const getRoleBasePath = (): string => {
        switch (user?.role) {
            ...
            case 'CLUB_COORDINATOR': return '/clubs-portal';
            case 'HHO': return '/hho';
            case 'SPORTS_ADMIN':
                return '/sports';
    ...
    const quickProfilePath = isMainSportsAdmin ? '/sports/admin-profile' : (isHho ? '/hho/profile' : `${basePath}/settings/profile`);
    ...
    const headerSearchOptions = useMemo<SearchSuggestion[]>(() => {
        ...
        for (const section of roleSections) {
            for (const item of section.items) {
                const cleanedSectionTitle = section.title.replace(/&/g, 'and').toLowerCase();
    ...
    const searchSuggestions = useMemo(() => {
        ...
        const ranked = headerSearchOptions
            ...
            .filter((entry): entry is { option: SearchSuggestion; score: number } => entry !== null)
    ...
    return (
        <header className={`bg-[#F4F2F0] border-b border-[#E5E7EB] h-[88px] shrink-0 flex flex-col justify-center z-40 relative ${isEventCoordinator ? 'px-6 md:px-12 lg:px-20' : 'px-4 md:px-8'}`}>
            <div className="flex items-center justify-between gap-3 min-w-0">
                ...
                <div className={`flex items-center gap-3 min-w-0`}>
                    ...
                    {true && (
                        <div className="sm:hidden flex flex-col justify-center min-w-0">
                            <h2 className="text-lg font-bold text-[#1A1A1A] truncate leading-tight">
                                {user?.role === 'SUPER_ADMIN' ? 'Mission Control' : 'Welcome back'}
                            </h2>
                            <p className="text-[10px] text-[#6B7280] font-bold uppercase tracking-widest truncate mt-0.5">
                                {getRoleBadge()}
                            </p>
                        </div>
                    )}
                    ...
                    <div className="hidden sm:block min-w-0">
                        <h2 className="text-lg md:text-lg font-bold md:font-semibold text-[#1A1A1A] truncate">
                            {user?.role === 'SUPER_ADMIN' ? `Mission Control, ${userName}` :
                                isBranchSportsAdmin ? 'Welcome back' :
                                    `Welcome back, ${userName}`}
                        </h2>
                        <div className="flex items-center gap-2 mt-1 min-w-0">
                            <div className="text-sm md:text-xs text-[#6B7280] font-semibold md:font-medium truncate max-w-[32vw] lg:max-w-[42vw]">{getBreadcrumb()}</div>
                                        ...
                                        <div className="max-h-72 overflow-y-auto">
                                            {searchSuggestions.map((suggestion, index) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="text-sm font-medium text-[#1A1A1A]">{suggestion.label}</div>
                                                        <div className="text-xs text-[#6B7280] mt-0.5">{suggestion.description}</div>
                                                    </button>
                                                );
                                            })}
                                        </div>
                                    ) : (
                                        <div className="px-4 py-3 text-sm text-[#6B7280]">No related pages found</div>
                                    )}
                                </div>
                            )}
                        </form>
                ...
                <div className="flex items-center gap-3 md:gap-4 flex-shrink-0">
                    ...
                    {user?.role === 'SUPER_ADMIN' ? (
                        ...
                        /* EVENT_COORDINATOR: no action buttons here, user section moved after notifications below */
                        null
                    ) : (isMainSportsAdmin || isBranchSportsAdmin) ? (
                        <>
                            {/* Sports Admin: Icon on Mobile */}
                            <Link
                                href="/sports/all-updates?create=true"
                                className="flex items-center justify-center w-10 h-10 bg-white border border-[#E5E7EB] text-[#1A1A1A] rounded-lg hover:bg-[#F3F4F6] transition-colors shadow-sm md:hidden"
                            >
                                <Send className="w-4 h-4" />
                            </Link>

                            {/* Sports Admin: Button on Desktop */}
                            <Link
                                href="/sports/all-updates?create=true"
                                className="hidden md:flex items-center gap-2 px-3 lg:px-4 py-2 bg-white border border-[#E5E7EB] text-[#1A1A1A] rounded-lg text-sm font-medium hover:bg-[#F3F4F6] transition-colors shadow-sm"
                            >
                                <Send className="w-4 h-4" />
                        ...
                        <>
                            {/* Actions for BRANCH_ADMIN, CLUB_COORDINATOR, HHO */}
                            <Link
                                href={isHho ? '/hho/updates?create=true' : `${basePath}/content/announcements?create=true`}
                                className="flex items-center justify-center w-10 h-10 bg-white border border-[#E5E7EB] text-[#1A1A1A] rounded-lg hover:bg-[#F3F4F6] transition-colors shadow-sm xl:hidden"
                            >
                                <Send className="w-4 h-4" />
                            </Link>
                            ...
                            <Link
                                href={isHho ? '/hho/updates?create=true' : `${basePath}/content/announcements?create=true`}
                                className="hidden xl:flex items-center gap-2 px-3 md:px-4 py-2 bg-white border border-[#E5E7EB] text-[#1A1A1A] rounded-lg text-sm font-medium hover:bg-[#F3F4F6] transition-colors shadow-sm"
                            >
                                <Send className="w-4 h-4" />
                                <span>Send Update</span>
                            </Link>

                            {basePath !== '/sports-coordinator' && (
                                <>
                                    <Link
                                        href={isHho ? '/hho/add-event?mode=create' : `${basePath}/events/create`}
                                        className="flex items-center justify-center w-10 h-10 bg-[#10B981] text-white rounded-lg hover:bg-[#059669] transition-colors shadow-sm md:hidden"
                                    >
                                        <Plus className="w-4 h-4" />
                                    </Link>
                    ...
                    {/* Notifications */}
                    <NotificationPopover onNavigate={() => router.push(`${basePath}/notifications`)} />
                        ...
                        <div className="relative">
                            ...
                            >
                                ...
                                <div className="text-left hidden md:block">
                                    <div className="text-sm font-semibold text-[#1A1A1A] leading-tight">{userName}</div>
                                    <div className="text-[11px] text-[#6B7280] font-medium">COORDINATOR</div>
                                </div>
