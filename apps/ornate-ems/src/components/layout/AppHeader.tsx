...
import { Menu, Search, Bell, User, Send, Plus, Clock, LogOut, ChevronDown } from 'lucide-react';
        ...
        <header className={`bg-[#F4F2F0] border-b border-[#E5E7EB] h-[88px] shrink-0 flex flex-col justify-center z-40 relative ${isEventCoordinator ? 'px-6 md:px-12 lg:px-20' : 'px-4 md:px-8'}`}>
            <div className="flex items-center justify-between gap-3 min-w-0">
                ...
                <div className={`flex items-center gap-3 min-w-0`}>
                    ...
                    <div className="hidden sm:block min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1 min-w-0">
                            <div className="text-xs text-[#6B7280] font-medium truncate max-w-[32vw] lg:max-w-[42vw]">{getBreadcrumb()}</div>
                ...
                <div className="flex items-center gap-3 md:gap-4 flex-shrink-0">
                    ...
                    ) : (user?.role === 'EVENT_COORDINATOR' || isBranchSportsAdmin) ? (
                        /* EVENT_COORDINATOR/BRANCH_SPORTS_ADMIN: no action buttons here, user section moved after notifications below */
                        null
                    ) : (
                        <>
                            {/* Actions for BRANCH_ADMIN, CLUB_COORDINATOR, HHO, SPORTS_ADMIN */}
                            <Link
                                href={isMainSportsAdmin ? '/sports/all-updates?create=true' : (isHho ? '/hho/updates?create=true' : `${basePath}/content/announcements?create=true`)}
