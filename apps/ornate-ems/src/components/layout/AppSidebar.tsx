...
import { ChevronDown, ChevronRight, LogOut, X, UserCircle } from "lucide-react";
...
export function AppSidebar({
  ...
  const getBranding = () => {
    ...
    if (roleKey === "CLUB_COORDINATOR") {
      ...
      // Simple formatting: TechExcel
      const displayName = clubName.charAt(0).toUpperCase() + clubName.slice(1);
      return {
    ...
    const details = (branch && branchMap[branch]) || {
      initials: "OR",
      name: "Ornate Dashboard",
    };
  ...
  const getRoleDisplay = () => {
    ...
    if (user?.role === "CLUB_COORDINATOR") {
      ...
      // If branding.name is already 'TechExcel', use it.
    ...
    <>
      ...
      >
        ...
        <div className="h-[88px] shrink-0 border-b border-[#E5E7EB] bg-[#F4F2F0] px-4 flex flex-col justify-center relative overflow-hidden">
          ...
          >
            <div
              className={`shrink-0 w-9 h-9 ${branding.color} rounded-lg flex items-center justify-center text-white font-bold text-sm shadow-sm`}
            >
              {branding.initials}
            </div>
          ...
          <nav className="flex-1 overflow-y-auto p-4 bg-[#F4F2F0] custom-scrollbar space-y-6">
            {navItems.map((section: any, idx: number) => (
              <div key={idx}>
                ...
                {section.title &&
                  (section.key ? (
                    ...
                    >
                      ...
                      <ChevronRight
                        className={`w-3 h-3 transition-transform duration-200 ${expandedSections.includes(section.key) ? "rotate-90" : ""}`}
                      />
                    </button>
                  ) : (
                ...
                {/* Section Items */}
                <div
                  className={`space-y-1 ${section.key && !expandedSections.includes(section.key) ? "hidden" : ""}`}
                >
                  {section.items.map((item: any, itemIdx: number) => {
                    ...
                    return (
                      ...
                      >
                        <Icon
                          className={`w-4 h-4 ${active ? "text-white" : "text-[#9CA3AF]"}`}
                        />
