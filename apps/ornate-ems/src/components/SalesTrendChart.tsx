...
interface SalesTrendChartProps {
  data?: (Partial<SalesTrendDatasets> | Record<string, SalesTrendItem[]>) | undefined;
}
...
export function SalesTrendChart({ data }: SalesTrendChartProps) {
  ...
  const hasTrendData = datasets.Weekly.length > 0 || datasets.Monthly.length > 0 || datasets.Yearly.length > 0;
  const maxValue = points.length > 0 ? Math.max(...points.map((p: any) => p.registered)) : 0;
  const scaleMax = Math.ceil(maxValue / 100) * 100 || 100;
  const yTicks = 5;

  const totalRegistered = points.reduce((sum: any, p: any) => sum + p.registered, 0);
  const totalAttended = points.reduce((sum: any, p: any) => sum + p.attended, 0);
  const overallRate =
    totalRegistered === 0 ? 0 : Math.round(totalAttended / totalRegistered * 1000) / 10;
  ...
  const hoveredXPercent =
    hoveredIndex === null || points.length <= 1 ? 0 : hoveredIndex / (points.length - 1);
  const hoveredAttendedYPercent =
    hoveredIndex === null ?
      0 :
      (() => {
        const innerH = chart.height - chart.padding.top - chart.padding.bottom;
        const attendedValue = attendedSeries[hoveredIndex] ?? 0;
        const y = chart.padding.top + (1 - attendedValue / (scaleMax || 1)) * innerH;
  ...
  return (
    <div className="bg-[#F4F2F0] rounded-[18px] p-[10px] h-full flex flex-col transition-colors duration-300">
      <div className="flex items-center justify-between mt-[10px] mb-[16px] px-[12px]">
        ...
        <div className="flex items-center justify-center mb-5 bg-transparent p-1.5 rounded-full cursor-pointer">
      ...
      <div className="bg-white rounded-[16px] p-4 md:p-8 shadow-sm relative flex-1 flex flex-col min-h-[400px] md:min-h-[500px]">
        {!hasTrendData && (
          <div className="mb-4 text-center">
            <p className="text-sm text-[#9CA3AF]">No participation data available from database.</p>
          </div>
        )}
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex flex-wrap items-start justify-between gap-6 mb-[32px]">
            ...
            <div className="flex flex-wrap items-center gap-4 md:gap-8 ml-auto pb-1">
              ...
              <div className="flex bg-[#F4F2F0] p-1 rounded-full">
                {(['Weekly', 'Monthly', 'Yearly'] as const).map((range: any) =>
                  <button
                    key={range}
                    onClick={() => setActiveRange(range)}
                    className={`px-3 md:px-5 py-1.5 rounded-full text-[10px] font-bold transition-all duration-200 ${activeRange === range ?
          ...
          {/* Chart Area */}
          <div className="flex gap-4 md:gap-10 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
            {/* Y Axis Labels */}
            <div className="flex flex-col justify-between h-[280px] md:h-[320px] text-[10px] md:text-[11px] font-bold text-[#9A9792] pb-[32px] sticky left-0 bg-white z-10 pr-2">
              {Array.from({ length: yTicks + 1 }).map((_: any, idx: any) => {
                const value = scaleMax / yTicks * (yTicks - idx);
                return (
                  <span
                    key={idx}
                    className={idx === yTicks ? 'flex items-end' : 'flex items-center'}>
            ...
            {/* Grid + Bars */}
            <div className="flex-1 min-w-[500px] md:min-w-0">
              <div className="relative">
                <div className="relative h-[320px]">
                  ...
                  <div
                    ...
                    onMouseMove={(e) => {
                      ...
                      setHoveredIndex(getHoveredIndexFromX(e.clientX, rect, points.length));
                    }}
                    ...
                    <svg
                      ...
                      <defs>
                        <linearGradient id="attendedArea" x1="0" x2="0" y1="0" y2="1">
                      ...
                      {Array.from({ length: yTicks + 1 }).map((_: any, idx: any) => {
                        const y =
                          chart.padding.top +
                          idx / yTicks * (chart.height - chart.padding.top - chart.padding.bottom);
                      ...
                      {attendedPath &&
                        <path
                          d={`${attendedPath} L ${(
                            ...
                            toFixed(2)} ${(chart.height - chart.padding.bottom).toFixed(2)} L ${chart.padding.left.toFixed(
                              2
                      ...
                      {points.map((p: any, i: any) => {
                        const innerW = chart.width - chart.padding.left - chart.padding.right;
                        const innerH = chart.height - chart.padding.top - chart.padding.bottom;
                        const x =
                          chart.padding.left + (
                            points.length === 1 ? innerW / 2 : i / (points.length - 1) * innerW);
                        const yReg = chart.padding.top + (1 - p.registered / (scaleMax || 1)) * innerH;
                        const yAtt = chart.padding.top + (1 - p.attended / (scaleMax || 1)) * innerH;
                    ...
                    {hoveredPoint &&
                      <div
                        ...
                        <div className="text-[11px] font-bold text-[#9A9792] mb-2">
                          {hoveredPoint.label}{' '}
                          {activeRange === 'Yearly' ? '' : activeRange === 'Monthly' ? '2025' : ''}
                        </div>
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[12px] text-[#3A3A3A] font-medium">Attended</span>
                            </div>
                            <span className="text-[12px] font-semibold text-[#1F1F1F]">
                          ...
                          <div className="flex items-center justify-between pt-1 border-t border-dashed border-[#E6E3DE] mt-1">
                            <span className="text-[11px] text-[#9A9792]">Participation rate</span>
                            <span className="text-[12px] font-semibold text-[#4F8A6B]">
                              {(
                                hoveredPoint.registered === 0 ?
                                  0 :
                                  Math.round(hoveredPoint.attended / hoveredPoint.registered * 1000) /
                                  10).
                                toFixed(1)}
                              %
                            </span>
                ...
                <div
                  ...
                  style={{ paddingLeft: chart.padding.left, paddingRight: chart.padding.right }}>

                  {points.map((p: any, idx: any) =>
                    <span
                      key={p.label}
                      className="text-[11px] font-bold text-[#9A9792] uppercase tracking-wider inline">
                      {p.label}
                    </span>
                  )}
                </div>
