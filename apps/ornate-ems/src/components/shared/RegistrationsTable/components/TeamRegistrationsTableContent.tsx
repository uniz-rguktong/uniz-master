                        ...
                        <thead className="bg-white border-b border-[#F3F4F6]">
                            <tr>
                                ...
                                {columns.map((col: any) => (
                                    <th
                                        key={col.key}
                                        className="px-4 py-3 text-left text-[10px] font-medium text-[#9CA3AF] uppercase tracking-wider"
                                    >
                                        {col.label}
                                    </th>
                                ))}
                            ...
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan={columns.length + 2} className="text-center py-12">
                                        <p className="text-sm text-[#6B7280]">No team registrations found</p>
                                    </td>
                                </tr>
                            ) : (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-4 text-right">
                                            <div className="flex items-center justify-end">
                                                <ActionMenu
                                                    actions={[
                                                        { label: 'View Details', icon: 'view', onClick: () => onViewDetails(team) },
                                                        { label: 'Send Email', icon: 'send', onClick: () => onSendEmail(team) },
                                                        { label: 'Delete', icon: 'delete', onClick: () => onDelete(team.id), danger: true }
                                                    ]}
                                                />
                                            </div>
                                        </td>
