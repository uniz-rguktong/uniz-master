...
export default function CertificatesPage({
    ...
    async function fetchData() {
      try {
        ...
        if (eventsRes.success && eventsRes.data) {
          ...
          showToast(
            eventsRes.error || `Failed to fetch ${entityLabelLower}s`,
            "error",
          );
        }
    ...
    if (selectedEventId && certificateThemes.length > 0) {
      ...
      if (event) {
        if (event.certificateTheme) {
          const theme = certificateThemes.find(
            (t) => t.id === event.certificateTheme,
          );
        ...
        setIsConfigured(
          event.certificateStatus === "CONFIGURED" ||
            event.certificateStatus === "DISTRIBUTED",
        );
          ...
          prev.map((e) =>
            e.id === selectedEventId
              ? {
                  ...
                  certificateTheme: selectedTheme.id,
                }
              : e,
          ),
        );
  ...
  const handleDistribute = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showToast(
          result.message || `Certificates distributed successfully!`,
          "success",
        );
        setEvents((prev) =>
          prev.map((e) =>
            e.id === selectedEventId
              ? { ...e, certificateStatus: "DISTRIBUTED" }
              : e,
          ),
        );
      } else {
  ...
  const selectedEventTitle =
    events.find((e) => e.id === selectedEventId)?.title || "Not selected";
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">
              Select themes and manage certificate distribution for{" "}
              {entityLabelLower}s
            </p>
          </div>
        </div>
              ...
              {isLoading ? (
                ...
                >
                  <SelectTrigger className="w-full px-4 py-3 bg-white border border-[#E5E7EB] rounded-xl text-sm focus:ring-2 focus:ring-[#1A1A1A]">
                    <SelectValue
                      placeholder={`Choose a ${entityLabelLower}...`}
                    />
                  </SelectTrigger>
                  <SelectContent className="">
                    ...
                    {events.length === 0 && (
                      <SelectItem value="no-events" disabled className="">
                        No {entityLabelLower}s found
                      </SelectItem>
                    )}
                  </SelectContent>
                </Select>
                ...
                {isLoading ? (
                  <>
                    {[1, 2, 3].map((i: any) => (
                      <div
                        key={i}
                        className="rounded-xl border-2 border-transparent overflow-hidden bg-[#F9FAFB]"
                      >
          ...
          <div className="bg-[#F4F2F0] rounded-[24px] p-[15px]">
            ...
            <div className="bg-white rounded-[18px] shadow-sm border border-[#E5E7EB] p-6">
                ...
                {isLoading ? (
                  ...
                  <>
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium text-[#1A1A1A] truncate max-w-[150px]">
                        {selectedEventTitle}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span
                        className={
                          Object.values(templates).every(Boolean)
                            ? "text-[#10B981] font-medium"
                            : "text-[#F59E0B] font-medium"
                        }
                      >
                        {Object.values(templates).filter(Boolean).length === 4
                          ? "Ready (4/4)"
                          : `Pending (${Object.values(templates).filter(Boolean).length}/4)`}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-[#6B7280]">Status</span>
                      <span
                        className={`font-medium ${distributionStatus === "DISTRIBUTED" ? "text-green-600" : "text-orange-500"}`}
                      >
                        {distributionStatus}
                      </span>
                    </div>
              ...
              <button
                ...
                className={`w-full mt-3 py-4 border-2 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 ${
                  !isConfigured
              ...
              >
                {isDistributing ? (
                  ...
                  <Send
                    className={`w-5 h-5 ${!isConfigured ? "text-[#9CA3AF]" : "text-[#3B82F6]"}`}
                  />
                )}
                {distributionStatus === "DISTRIBUTED"
                  ? "Re-distribute Certificates"
