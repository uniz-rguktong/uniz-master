...
export default function NotificationsPage({
    ...
    const [selectedAudience, setSelectedAudience] = useState<'club' | 'admin'>('admin');
    ...
    const currentHook = filter === 'archived' ? archived : (filter === 'sent' ? sent : inbox);
    ...
    const allMessages = useMemo(
        ...
        [inbox.notifications, sent.notifications, archived.notifications, sentArchived.notifications]
    );
    ...
    const isLoading =
        filter === 'all' || filter === 'starred'
            ? (inbox.isLoading || sent.isLoading || archived.isLoading || sentArchived.isLoading)
            : filter === 'archived'
    ...
    const loadRecipientOptions = useCallback(async (audience: 'club' | 'admin') => {
    ...
    return (
        <div className="p-4 md:p-8 animate-page-entrance min-h-screen">
            ...
            <div className="mb-8">
                {backPath && (
                    <Link href={backPath} className="inline-flex items-center gap-2 text-sm text-[#6B7280] mb-3 hover:text-[#1A1A1A] transition-colors group">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex-1">
                        ...
                        <p className="text-sm text-[#6B7280]">Manage messages, alerts, and announcements.</p>
                    </div>

                    <div className="flex flex-wrap items-center gap-3">
                        <button
                            onClick={() => setIsComposeOpen(true)}
                            className="hidden md:flex items-center gap-2 px-5 py-2.5 bg-[#10B981] text-white rounded-xl text-sm font-medium hover:bg-[#059669] transition-all shadow-sm shadow-emerald-500/20 active:scale-95"
                        >
            ...
            <button
                onClick={() => setIsComposeOpen(true)}
                className="flex md:hidden items-center justify-center gap-2 px-5 py-3 mb-6 bg-[#10B981] text-white rounded-xl text-sm font-semibold hover:bg-[#059669] transition-all shadow-sm w-full active:scale-95"
            >
            ...
            <div className="bg-[#F4F2F0] rounded-[18px] p-[10px]">
                {/* Toolbar */}
                <div className="flex flex-row items-center justify-between gap-3 px-3 md:px-[12px] mt-[10px] mb-[16px]">
                    <div className="flex-1 relative max-w-xs">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#9CA3AF]" />
                        <input
                            type="text"
                            placeholder="Search messages..."
                            className="pl-10 pr-2 py-2 bg-white border border-[#E5E7EB] rounded-xl text-xs sm:text-sm w-full focus:outline-none focus:ring-2 focus:ring-[#1A1A1A] transition-all"
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                        />
                    </div>

                    <div className="flex items-center gap-2 md:pb-0 w-auto justify-end shrink-0">
                        <div className="min-w-[120px] sm:min-w-[140px] shrink-0">
                            <Select value={filter} onValueChange={setFilter}>
                                ...
                                <SelectContent className="">
                                    <SelectItem value="all" className="">All Messages</SelectItem>
                                    <SelectItem value="unread" className="">Unread Only</SelectItem>
                                    <SelectItem value="starred" className="">Starred Only</SelectItem>
                                    <SelectItem value="sent" className="">Sent Messages</SelectItem>
                                    <SelectItem value="archived" className="">Archived</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
                    </div>
                ...
                <div className="bg-white rounded-[14px] border border-[#E5E7EB] overflow-hidden shadow-sm min-h-[500px] flex flex-col">
                    {isLoading ? (
                            ...
                            <div className="divide-y divide-[#F3F4F6]">
                                {filteredNotifications.map((notif: any) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-row md:flex-row items-center gap-4 shrink-0 w-full md:w-auto">
                                            <button
                                            ...
                                            >
                                                <Star className={`w-5 h-5 ${notif.isStarred ? 'fill-current' : ''}`} />
                                            </button>

                                            <div className={`w-12 h-12 rounded-[14px] flex items-center justify-center font-bold text-lg shadow-sm relative group-hover:scale-105 transition-transform ${notif.type === 'admin' ? 'bg-[#1A1A1A] text-white' :
                                                notif.type === 'club' ? 'bg-[#10B981] text-white' :
                                                    notif.type === 'student' ? 'bg-[#3B82F6] text-white' :
                                                        'bg-[#EF4444] text-white'
                                                }`}>
                                                {(filter === 'sent' ? notif.recipientName : notif.senderName)?.charAt(0) || '?'}
                                            ...
                                            <div className="flex md:hidden flex-row gap-2 ml-auto">
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); handleToggleRead(notif.id); }}
                                                    className="p-2 bg-white border border-[#E5E7EB] text-[#6B7280] rounded-xl hover:text-[#3B82F6] hover:border-[#3B82F6] transition-all active:scale-95"
                                                ...
                                                <button
                                                    ...
                                                    className="p-2 bg-white border border-[#E5E7EB] text-[#6B7280] rounded-xl hover:text-[#EF4444] hover:border-[#EF4444] transition-all active:scale-95"
                                                    title="Archive"
                                                >
                                        ...
                                        <div className="flex-1 min-w-0 w-full">
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-1.5">
                                                <div className="flex items-center flex-wrap gap-2">
                                                    <h3 className={`text-sm font-bold ${notif.isRead || filter === 'sent' ? 'text-[#4B5563]' : 'text-[#1A1A1A]'}`}>
                                                        {filter === 'sent' ? `To: ${notif.recipientName || 'Recipient'}` : notif.senderName}
                                                    </h3>
                                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${getPriorityStyles(notif.priority)}`}>
                                                        {notif.priority}
                                                    </span>
                                                </div>
                                            ...
                                            <div className="flex items-center flex-wrap gap-2 mb-2">
                                                <span className="text-[11px] font-semibold text-[#3B82F6] uppercase tracking-wide">
                                                    {filter === 'sent' ? (notif.recipientRole || 'Recipient') : notif.senderRole}
                                                </span>
                                                ...
                                                <span className="text-[11px] font-medium text-[#6B7280] uppercase tracking-wide">
                                                    {filter === 'sent' ? (notif.recipientBranch || 'Main') : notif.senderBranch}
                                                </span>
                                            </div>

                                            <p className={`text-sm line-clamp-2 md:line-clamp-1 leading-relaxed ${notif.isRead ? 'text-[#6B7280]' : 'text-[#4B5563] font-medium'}`}>
                                                {notif.message}
                                            </p>
                                        </div>

                                        <div className="hidden md:flex items-center gap-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity self-end md:self-center">
                                            <button
                                                onClick={(e) => { e.stopPropagation(); handleToggleRead(notif.id); }}
                        ...
                        <div className="flex flex-col items-center justify-center py-40">
                            ...
                            <h3 className="text-lg font-bold text-[#1A1A1A] mb-1">No messages found</h3>
                            <p className="text-sm text-[#6B7280]">Your search did not match any notifications.</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            {selectedNotification && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
                    <div className="absolute inset-0 bg-[#1A1A1A]/80 backdrop-blur-xl" onClick={() => setSelectedNotification(null)} />
                        ...
                        <div className="p-5 sm:p-8">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5 mb-6">
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-lg shadow-lg shadow-gray-200 border-2 border-white ${selectedNotification.type === 'admin' ? 'bg-[#1A1A1A] text-white' :
                                        selectedNotification.type === 'club' ? 'bg-[#10B981] text-white' :
                                            'bg-[#3B82F6] text-white'
                                        }`}>
                                        {(filter === 'sent' ? selectedNotification.recipientName : selectedNotification.senderName)?.charAt(0) || '?'}
                                    </div>
                                    <div>
                                        <h2 className="text-lg sm:text-xl font-bold text-[#1A1A1A] tracking-tight">
                                            {filter === 'sent' ? (selectedNotification.recipientName || 'Recipient') : selectedNotification.senderName}
                                        </h2>
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="px-2 py-0.5 bg-blue-50 text-[#3B82F6] text-[10px] font-bold uppercase tracking-wider rounded-md border border-blue-100">
                                                {filter === 'sent' ? (selectedNotification.recipientRole || 'Recipient') : selectedNotification.senderRole}
                                            </span>
                                            <span className="text-[10px] font-medium text-[#9CA3AF] uppercase tracking-wider">
                                                {filter === 'sent' ? (selectedNotification.recipientBranch || 'Main') : selectedNotification.senderBranch}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                <div className="text-left sm:text-right">
                                    ...
                                    <span className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${getPriorityStyles(selectedNotification.priority)}`}>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button
                                        onClick={() => {
                                            ...
                                            showToast(filter === 'archived' ? "Notification restored to inbox" : "Notification moved to archive", "info");
                                        }}
                                    ...
                                    >
                                        {filter === 'archived' ? <RotateCcw className="w-4 h-4" /> : <Archive className="w-4 h-4" />}
                                    </button>
                                </div>
            ...
            >
                <div className="space-y-6">
                                ...
                                {[
                                    {
                                        ...
                                        label: 'Admins',
                                        icon: Shield,
                                        activeClass: 'bg-[#1A1A1A] text-white shadow-lg shadow-black/10 border-transparent',
                                        inactiveClass: 'bg-white text-[#6B7280] border-[#E5E7EB] hover:border-[#1A1A1A] hover:text-[#1A1A1A]'
                                    },
                                    {
                                        id: 'club' as const,
                                        label: 'Clubs',
                                        icon: Building,
                                        activeClass: 'bg-[#10B981] text-white shadow-lg shadow-emerald-500/20 border-transparent',
                                        inactiveClass: 'bg-white text-[#6B7280] border-[#E5E7EB] hover:border-[#10B981] hover:text-[#10B981]'
                                    }
                                ].map(type => (
                                    <button
                                        key={type.id}
                                        ...
                                        className={`relative flex flex-col items-center justify-center gap-3 py-6 rounded-2xl border transition-all duration-300 ${selectedAudience === type.id
                                            ? `${type.activeClass} scale-[1.02]`
                                            : `${type.inactiveClass} hover:shadow-sm`
                                            }`}
                                    >
                                        <div className={`p-2 rounded-xl ${selectedAudience === type.id ? 'bg-white/20' : 'bg-[#F9FAFB]'
                                            }`}>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-[#1A1A1A]">Recipient</label>
                        {isSuperAdmin ? (
                            <Select
                                value={selectedRecipient?.id || ''}
                                onValueChange={(value) => {
                                    const recipient = recipientOptions.find((entry: any) => entry.id === value) || null;
                            ...
                            >
                                <SelectTrigger className="w-full h-[46px] bg-white border border-[#E5E7EB] rounded-xl text-sm focus:ring-2 focus:ring-[#10B981]">
                                    <SelectValue placeholder={selectedAudience === 'admin' ? 'Select admin recipient' : 'Select club recipient'} />
                                </SelectTrigger>
                                <SelectContent>
                                    {recipientOptions.map((recipient: any) => (
                                        <SelectItem key={recipient.id} value={recipient.id}>
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-[#1A1A1A]">Message Content</label>
                        <textarea
                            rows={6}
                    ...
                    <div className="flex justify-end gap-3 mt-4">
                        ...
                        <button
                            ...
                            disabled={isSending || !selectedRecipient || !composeSubject.trim() || !composeMessage.trim()}
