  ...
  async (email: string) => {
    ...
    const eventScope: Prisma.EventWhereInput = isAdmin
      ...
      : isClubCoordinator
        ...
        : {
            OR: [{ creatorId: adminId }, { creator: { branch: branch } }],
          };

    const registrationScope = { event: eventScope };

    const [totalEvents, statusGroups, revenueGroups] = await Promise.all([
    ...
    const totalRegistrations = (
      statusGroups as { status: string; _count: { _all: number } }[]
    ).reduce((sum: number, curr) => sum + curr._count._all, 0);
...
export async function getAllTrends(): Promise<{
  ...
  try {
    ...
    const trends: AllTrends = {
      ...
      totalRegistrations: calculateTrend(
        current.totalOnlineRegistrations,
        yesterday.totalOnlineRegistrations,
      ),
      activeParticipants: calculateTrend(
        current.activeParticipants,
        yesterday.activeParticipants,
      ),
      completionRate: calculateTrend(
        current.completionRate,
        yesterday.completionRate,
      ),
      confirmedRegistrations: calculateTrend(
        current.confirmedRegistrations,
        yesterday.confirmedRegistrations,
      ),
      pendingRegistrations: calculateTrend(
        current.pendingRegistrations,
        yesterday.pendingRegistrations,
      ),
      waitlistRegistrations: calculateTrend(
        current.waitlistRegistrations,
        yesterday.waitlistRegistrations,
      ),
      onlineRegistrations: calculateTrend(
        current.totalOnlineRegistrations,
        yesterday.totalOnlineRegistrations,
      ),
      offlineRegistrations: calculateTrend(
        current.totalOfflineRegistrations,
        yesterday.totalOfflineRegistrations,
      ),
      totalRevenue: calculateTrend(
        current.totalRevenue,
        yesterday.totalRevenue,
      ),
      avgAttendanceRate: calculateTrend(
        current.avgAttendanceRate,
        yesterday.avgAttendanceRate,
      ),
      totalAttendance: calculateTrend(
        current.totalAttendance,
        yesterday.totalAttendance,
      ),
      pendingRevenue: calculateTrend(
        current.pendingRevenue,
        yesterday.pendingRevenue,
      ),
    };
 ...
 */
export async function createDailySnapshot(): Promise<
  { success: true; data: AnalyticsSnapshot } | { success: false; error: string }
> {
  const session = await getServerSession(authOptions);
  if (!session) {
