...
function getDashboardSourceLabel(
  creator:
    | { role?: string | null; branch?: string | null; clubId?: string | null }
    | null
    | undefined,
) {
...
const getCachedCoordinatorEvents = unstable_cache(
  async (userId: string, branch?: string | null) => {
    ...
    const countsByEventId = registrationCounts.reduce(
      (acc, item) => {
      ...
      {} as Record<
        string,
        { total: number; pending: number; attended: number }
      >,
    );
...
const getCachedEventForCoordinator = unstable_cache(
  async (eventId: string) => {
    ...
    return {
      ...
      assignedCoordinatorIds: event.assignedCoordinators.map(
...
const getCachedSportForCoordinator = unstable_cache(
  async (sportId: string) => {
    ...
    return {
      ...
      date:
        sport.registrationDeadline?.toISOString() || new Date().toISOString(),
...
export async function getCoordinatorEvents(): Promise<{
  success?: boolean;
  ...
  return executeAction(async () => {
    ...
    if (
      !user ||
      (user.role !== "EVENT_COORDINATOR" && user.role !== "SUPER_ADMIN")
    ) {
      throw new Error("Unauthorized");
    }

    const events = await getCachedCoordinatorEvents(
      user.id,
      user.branch || null,
    );
...
export async function getCoordinatorEventDetails(eventId: string): Promise<{
  success?: boolean;
  ...
  return executeAction(async () => {
    ...
    const isAssigned = eventData.assignedCoordinatorIds.some(
      (id: string) => id === user.id,
    );
    const isBranchScopedAccess =
      (user.role === "BRANCH_ADMIN" || user.role === "BRANCH_SPORTS_ADMIN") &&
      !!user.branch &&
      (!eventData.coordinatorBranch ||
        eventData.coordinatorBranch === user.branch);
