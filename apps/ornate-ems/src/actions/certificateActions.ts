...
import { sendCertificateEmail, getParticipationEmailTemplate, getWinnerEmailTemplate } from "@/lib/email";
 ...
 */
export async function getEventsForCertificates(filterCategory: string | null = null): Promise<CertificateActionResponse<FormattedCertEvent[]>> {
    ...
    return executeAction(async () => {
        ...
        const formattedEvents = events.map(e => ({
            ...
            certificateTemplates: e.certificateTemplates ? JSON.parse(JSON.stringify(e.certificateTemplates)) : null,
        ...
        return { success: true, data: formattedEvents } as CertificateActionResponse<FormattedCertEvent[]>;
    }, 'getEventsForCertificates');
}

export async function getSportsForCertificates(): Promise<CertificateActionResponse<FormattedCertEvent[]>> {
    const session = await getServerSession(authOptions);
    if (!session || !session.user?.email) return { success: false, error: 'Unauthorized' };
    ...
    return executeAction(async () => {
        ...
        if (!['SUPER_ADMIN', 'SPORTS_ADMIN', 'BRANCH_SPORTS_ADMIN'].includes(admin.role)) {
        ...
        const formattedSports = sports.map((sport) => ({
            ...
            certificateTemplates: sport.certificateTemplates ? JSON.parse(JSON.stringify(sport.certificateTemplates)) : null,
            certificateIssuedAt: sport.certificateIssuedAt ? sport.certificateIssuedAt.toISOString() : null,
            registrationsCount: sport._count.registrations || 0,
        }));

        return { success: true, data: formattedSports } as CertificateActionResponse<FormattedCertEvent[]>;
...
export async function distributeSportCertificates(sportId: string): Promise<CertificateActionResponse> {
    const session = await getServerSession(authOptions);
    if (!session || !session.user?.email) return { success: false, error: 'Unauthorized' };
    ...
    return executeAction(async () => {
        ...
        if (sport.winnerAnnouncement?.isPublished && sport.winnerAnnouncement.positions) {
        ...
        for (const reg of sport.registrations) {
            try {
                ...
                const rank = winnerMap.get(reg.studentName.trim().toLowerCase())
                    || winnerMap.get(reg.studentId.trim().toLowerCase())
                    || null;

                const isWinner = rank !== null && rank >= 1 && rank <= 3;
                const certType = isWinner
                    ? (rank === 1 ? 'MERIT_FIRST' : rank === 2 ? 'MERIT_SECOND' : 'MERIT_THIRD')
                    : 'PARTICIPATION';
                ...
                try {
                    ...
                    logger.error({ err: qrErr, registrationId: reg.id }, 'Sport cert QR generation failed');
                }

                const emailHtml = isWinner
                    ? getWinnerEmailTemplate(reg.studentName, sportTitle, rank!, verificationUrl, reg.id)
                    : getParticipationEmailTemplate(reg.studentName, sportTitle, verificationUrl, reg.id);

                const subject = isWinner
                    ? `Certificate of Achievement: ${sportTitle}`
                    : `Certificate of Participation: ${sportTitle}`;

                const result = await sendCertificateEmail(reg.email, subject, emailHtml, attachments);
                if (result.success) emailsSent++;

                certsProcessed++;
            } catch (err) {
                errors++;
                logger.error({ err, sportId, registrationId: reg.id }, 'Failed to process sport certificate');
            }
        }
        ...
        logger.info({ sportId, certsProcessed, emailsSent, errors }, 'distributeSportCertificates.complete');
        await revalidateCertificates();
        return {
            success: true,
 ...
 */
export async function saveCertificateConfiguration(eventId: string, { themeId, templates }: { themeId: string, templates: Prisma.InputJsonValue }): Promise<CertificateActionResponse> {
    const session = await getServerSession(authOptions);
    if (!session || !session.user?.email) return { success: false, error: "Unauthorized" };
    ...
    return executeAction(async () => {
        ...
        const canAccess =
            ...
            (admin.role === 'BRANCH_ADMIN' && eventCheck.creator?.branch === admin.branch);
 ...
 */
export async function getCertificateThemes(): Promise<CertificateActionResponse<CertTheme[]>> {
    return executeAction(async () => {
        const session = await getServerSession(authOptions);
        if (!session?.user?.email) {
            return { success: false, error: 'Unauthorized' } as CertificateActionResponse<CertTheme[]>;
        }
        ...
        const formattedThemes = themes.map(t => ({
            id: t.id,
            name: t.name,
            preview: t.preview || 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=800&h=450&fit=crop',
            style: t.style || 'Classic',
            colors: t.colors || []
        }));

        return { success: true, data: formattedThemes } as CertificateActionResponse<CertTheme[]>;
 ...
 */
export async function distributeCertificates(eventId: string): Promise<CertificateActionResponse> {
    const session = await getServerSession(authOptions);
    if (!session || !session.user?.email) return { success: false, error: "Unauthorized" };
    ...
    return executeAction(async () => {
        ...
        const eventCheck = await prisma.event.findUnique({
            where: { id: eventId },
            select: { creatorId: true, title: true, creator: { select: { branch: true } } }
        });
        ...
        if (!winnerAnnouncement) {
            ...
            if (sport) {
                ...
                if (finalMatch && finalMatch.winner) {
                    const winnerName = finalMatch.winner === 'TEAM1' ? finalMatch.team1Name : finalMatch.team2Name;
                    const runnerName = finalMatch.winner === 'TEAM1' ? finalMatch.team2Name : finalMatch.team1Name;
            ...
            if (!autoAnnouncement) {
                return {
                    success: false,
                    error: "Certificate distribution blocked. Please create and publish a Winner Announcement, or ensure the Tournament Final Match is marked as Completed."
                };
            }
        ...
        if (winnerAnnouncement) {
            ...
            const extractBranch = (memberList: string[] | undefined): string | null => {
                ...
                if (firstWord && ['CSE', 'ECE', 'EEE', 'MECH', 'CIVIL'].includes(firstWord)) return firstWord;
                return null;
            };

            positions.forEach(pos => {
                if (pos && String(pos.rank) === "1") winnerBranch = extractBranch(pos.members as string[] | undefined);
                if (pos && String(pos.rank) === "2") runnerBranch = extractBranch(pos.members as string[] | undefined);
            });
        ...
        await revalidateCertificates();
        return { success: true, message: "Certificates distributed successfully to winners and participants." } as CertificateActionResponse;
...
async function distributeParticipationCertificates(eventId: string) {
    ...
    for (const reg of event.registrations) {
        ...
        try {
            ...
            logger.error({ err, registrationId: reg.id }, 'Failed to process participation cert');
        }
    }
}
...
async function distributeWinnerCertificates(eventId: string) {
            ...
            if (rankStr && members && Array.isArray(members)) {
                for (const member of members) {
                    ...
                    if (identifier.includes('@')) {
                        const user = await prisma.user.findUnique({ where: { email: identifier }, select: { id: true } });
                        if (user) {
                    ...
                    else if (/^[A-Za-z]\d{6}$/.test(identifier) || /^\d{7}$/.test(identifier)) {
                        await prisma.registration.updateMany({
                            where: {
                                eventId,
    ...
    for (const winner of winners) {
        try {
            ...
            logger.error({ err, winnerId: winner.id }, 'Failed to process winner cert');
        }
    }
}
