...
const getCachedAllTeams = unstable_cache(
  async (
    adminRole: string,
    adminBranch: string | null | undefined,
    adminClubId: string | null | undefined,
  ) => {
...
> {
  ...
  try {
    ...
    if (!teams) {
      return {
        success: false,
        error: "Team model not found in database client",
      };
    }
...
> {
  ...
  try {
    ...
    if (!team) {
      ...
      if (!prisma.team)
        return { success: false, error: "Team model not found" };
