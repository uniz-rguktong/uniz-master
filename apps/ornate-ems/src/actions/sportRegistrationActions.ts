...
import {
  ...
  TeamRole,
  UserRole,
  type SportRegistration,
} from "@/lib/generated/client";
import { SportRegistrationSchema } from "@/lib/schemas/sportSchemas";
import {
  sendEmail,
  getSportRegistrationEmailTemplate,
  getIndividualSportRegistrationEmailTemplate,
} from "@/lib/email";
...
export async function createSportRegistration(
  input: CreateSportRegistrationInput,
) {
  ...
  return executeAction(async () => {
    ...
    if (registration.email && !registration.email.endsWith("@offline.local")) {
      try {
        ...
        console.error(
          "Failed to send individual registration email:",
          emailError,
        );
      }
    }
...
export async function bulkCreateSportRegistrations(
  sportId: string,
  registrations: CreateSportRegistrationInput[],
) {
  const user = await getAuthenticatedUser();
  if (!user) return { success: false, error: "Unauthorized" };
...
export async function updateSportRegistrationStatus(
  registrationId: string,
  status: string,
) {
  const user = await getAuthenticatedUser();
...
export async function getSportRegistrationStats(sportId: string) {
  ...
  return executeAction(async () => {
    const [total, byStatus, byBranch] = await Promise.all([
      ...
      prisma.sportRegistration.groupBy({
        by: ["status"],
        where: { sportId },
        _count: { _all: true },
      }),
...
export async function createSportOfflineTeamRegistration(
  ...
  return executeAction(async () => {
    ...
    const normalizedIds = members
      .map((m) => (m.rollNumber || "").trim().toUpperCase())
      .filter(Boolean);
    ...
    if (sport.category !== "TEAM")
      throw new Error("This sport supports only individual registrations");
    ...
    const captainStudentId =
      (captain.rollNumber || "").trim() || `TEAM-${Date.now()}`;
    const existing = await prisma.sportRegistration.findFirst({
    ...
    if (existing)
      throw new Error(
        `Captain ${captainStudentId} is already registered for this sport`,
      );
...
export async function updateSportOfflineTeamRegistration(
  ...
  return executeAction(async () => {
    ...
    if (!registration) throw new Error("Registration not found");
    if (!registration.teamMember?.teamId)
      throw new Error("Linked team not found for this registration");
    if (!sport) throw new Error("Sport not found");
    if (sport.category !== "TEAM")
      throw new Error("This sport supports only individual registrations");
    if (sport.minPlayersPerTeam && members.length < sport.minPlayersPerTeam) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.teamMember.createMany({
        data: members.map((member, index) => ({
          ...
          status: MemberStatus.ACCEPTED,
          sportRegistrationId:
            member.role === "Captain" ? normalizedRegistrationId : null,
        })),
      });
    });
