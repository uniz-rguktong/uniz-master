...
export interface SportSpotTeamRegistrationInput {
    sportId: string;
    teamName: string;
    members: SportTeamMemberInput[];
}

export interface UpdateSportSpotTeamRegistrationInput {
...
export async function createSportRegistration(input: CreateSportRegistrationInput) {
    ...
    return executeAction(async () => {
        ...
        if (registration.email && !registration.email.endsWith('@spot.local')) {
            try {
...
// ────────────────────────────────────────────────────────────
// Sport Spot Team Registration (for Sports Admin)
// ────────────────────────────────────────────────────────────
...
export async function createSportSpotTeamRegistration(
    data: SportSpotTeamRegistrationInput
) {
    const user = await getAuthenticatedUser();
    if (!user) return { success: false, error: 'Unauthorized' };
    ...
    return executeAction(async () => {
        ...
        await createAuditLog({
            action: 'CREATE_SPOT_TEAM_SPORT_REGISTRATION',
            entityType: 'SPORT_REGISTRATION',
        ...
        if (leaderUser.email && !leaderUser.email.endsWith('@spot.local')) {
            try {
    ...
    }, 'createSportSpotTeamRegistration') as any;
}

export async function updateSportSpotTeamRegistration(
    data: UpdateSportSpotTeamRegistrationInput
) {
    const user = await getAuthenticatedUser();
    if (!user) return { success: false, error: 'Unauthorized' };
    ...
    return executeAction(async () => {
        ...
        await createAuditLog({
            action: 'UPDATE_SPOT_TEAM_SPORT_REGISTRATION',
            entityType: 'SPORT_REGISTRATION',
    ...
    }, 'updateSportSpotTeamRegistration') as any;
}
