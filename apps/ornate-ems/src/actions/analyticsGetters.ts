...
import { UserRole, RegistrationStatus, PaymentStatus, Prisma } from '@/lib/generated/client'
import { getAuthenticatedUser } from '@/lib/auth-helpers'
import { getCategoryColor, getEventStatus } from '@/lib/constants'
import { executeAction } from '@/lib/api-utils';

const CLUB_DISPLAY_ORDER = ['TechXcel', 'Sarvasrijana', 'Artix', 'Kaladharini', 'KhelSaathi', 'ICRO', 'Pixelro'];
...
export interface AnalyticsTrends {
    ...
    spotRegistrations: TrendValue | null;
...
export async function getEventAnalytics(): Promise<{ success?: boolean; data?: AnalyticsData; error?: string }> {
    return executeAction(async () => {
        ...
        const data = await unstable_cache(
            async () => {
                const whereClause: Prisma.EventWhereInput = role === UserRole.SUPER_ADMIN ? {} :
                    role === UserRole.CLUB_COORDINATOR && clubId ? { creator: { clubId: clubId } } :
                        role === UserRole.SPORTS_ADMIN ? { creator: { role: UserRole.SPORTS_ADMIN } } :
                            role === UserRole.HHO ? { creator: { role: UserRole.HHO } } :
                                {
                                    OR: [
                                        { creatorId: adminId },
                ...
                let totalSpotRegistrations = 0;
                let totalRevenue = 0;
                let totalAttendance = 0;

                const eventPerformanceData: EventPerformance[] = events.map(event => {
                    const registrations = event.registrations;
                    const attendedRegs = registrations.filter(r => r.status === RegistrationStatus.ATTENDED);
                    const paidRegs = registrations.filter(r => r.paymentStatus === PaymentStatus.PAID);

                    const onlineRegs = registrations.filter(r => r.transactionId || (r.amount > 0 && r.paymentStatus === PaymentStatus.PAID));
                    const spotRegs = registrations.length - onlineRegs.length;

                    const onlineCount = onlineRegs.length;
                    const spotCount = spotRegs;
                    const attendanceCount = attendedRegs.length;
                    const revenue = paidRegs.reduce((sum: any, r: any) => sum + (r.amount || 0), 0);

                    totalOnlineRegistrations += onlineCount;
                    totalSpotRegistrations += spotCount;
                ...
                const totalRegs = totalOnlineRegistrations + totalSpotRegistrations;
                const avgAttendanceRate = totalRegs > 0
                    ? Math.round((totalAttendance / totalRegs) * 100)
                    : 0;

                const trends: AnalyticsTrends = {
                    onlineRegistrations: yesterdaySnapshot ? calculateTrend(totalOnlineRegistrations, yesterdaySnapshot.totalOnlineRegistrations) : null,
                    spotRegistrations: yesterdaySnapshot ? calculateTrend(totalSpotRegistrations, yesterdaySnapshot.totalSpotRegistrations) : null,
                    revenue: yesterdaySnapshot ? calculateTrend(totalRevenue, yesterdaySnapshot.totalRevenue) : null,
                    attendanceRate: yesterdaySnapshot ? calculateTrend(avgAttendanceRate, yesterdaySnapshot.avgAttendanceRate) : null,
                };
            ...
            ['event-analytics-v4', adminId, role, clubId || 'no-club', branch || 'no-branch'],
            {
...
export async function createAnalyticsSnapshot(): Promise<{ success?: boolean; snapshot?: any; error?: string }> {
    return executeAction(async () => {
        const session = await getServerSession(authOptions);
        if (!session) throw new Error('Unauthorized');

        const result = await getEventAnalytics();
        if (!result.success || !result.data) throw new Error('Failed to get current analytics');
        ...
        const snapshot = await prisma.analyticsSnapshot.upsert({
            ...
            update: {
                ...
                totalSpotRegistrations: summary.totalSpotRegistrations,
...
export async function getRegistrationAnalytics(): Promise<{ success?: boolean; data?: any; error?: string }> {
    return executeAction(async () => {
        const session = await getServerSession(authOptions);
        ...
        const data = await unstable_cache(
            async () => {
                ...
                const clubRows = await prisma.$queryRaw<{ clubId: string | null; total: number }[]>`
                ...
                const mappedData = Object.entries(branchAggregates).map(([branch, total]) => {
