 ...
 */
export async function bulkImportRegistrations(data: BulkImportRecord[]): Promise<BulkImportResponse> {
        ...
        for (const record of data) {
            try {
                ...
                if (!email || (!eventName && !eventId)) {
                    throw new Error('Missing required fields: Email and (Event Name or Event ID)');
                }

                // Find event
                let targetEventId = eventId;
                if (!targetEventId && eventName) {
                    const event = events.find(e => e.title.toLowerCase() === eventName.toLowerCase());
                ...
                if (Object.values(RegistrationStatus).includes(recordStatus as RegistrationStatus)) {
                ...
                await prisma.registration.create({
                    data: {
                        ...
                        paymentStatus: record.paymentAmount ? PaymentStatus.PAID : PaymentStatus.PENDING
                    }
                });
 ...
 */
export async function getExportData(filters: ExportFilters = {}): Promise<BulkExportResponse> {
    const session = await getServerSession(authOptions);
    ...
    return executeAction(async () => {
        ...
        if (eventId && eventId !== 'all' && eventId !== 'All Events') where.eventId = eventId;
        if (status && status !== 'all' && status !== 'All Statuses') {
            const statusStr = (status.split(' ')[0] ?? '').toUpperCase();
            if (Object.values(RegistrationStatus).includes(statusStr as RegistrationStatus)) {
        ...
        const user = session.user as { role?: string; clubId?: string; branch?: string };
        const adminRole = user.role;
        const adminClubId = user.clubId;
        const adminBranch = user.branch;

        if (adminRole === 'CLUB_COORDINATOR' && adminClubId) {
            where.event = { creator: { clubId: adminClubId } } as Prisma.EventWhereInput;
        } else if (adminBranch && adminRole !== 'SUPER_ADMIN') {
            where.event = { creator: { branch: adminBranch } } as Prisma.EventWhereInput;
        }
        ...
        const registrations = await prisma.registration.findMany({
            ...
            select: {
                ...
                user: { select: { name: true, email: true, phone: true, branch: true, currentYear: true } }
            },
