...
async function assertTeamScope(user: AuthUser, teamId: string): Promise<void> {
  ...
  if (
    (user.role === ROLES.BRANCH_ADMIN ||
      user.role === ROLES.BRANCH_SPORTS_ADMIN) &&
    user.branch
  ) {
    if (event.creator?.branch === user.branch) return;
    throw new Error(
      "Unauthorized: This team belongs to an event outside your branch",
    );
  }
  ...
  if (user.role === ROLES.CLUB_COORDINATOR && user.clubId) {
    ...
    throw new Error(
      "Unauthorized: This team belongs to an event outside your club",
    );
  }
 ...
 */
async function assertBulkTeamScope(
  user: AuthUser,
  teamIds: string[],
): Promise<void> {
  if (
  ...
  for (const team of teams) {
    ...
    if (
      ...
      if (event.creator?.branch !== user.branch) {
        throw new Error(
          "Unauthorized: One or more teams belong to events outside your branch",
        );
      }
      continue;
    }
    if (user.role === ROLES.CLUB_COORDINATOR && user.clubId) {
      if (event.creator?.clubId !== user.clubId) {
        throw new Error(
          "Unauthorized: One or more teams belong to events outside your club",
        );
      }
      continue;
    }
    if (user.role === ROLES.EVENT_COORDINATOR) {
      ...
      if (!isAssigned) {
        throw new Error(
          "Unauthorized: You are not assigned to one or more of these events",
        );
      }
      continue;
    }
      ...
      if (members && members.length > 0) {
        await tx.teamMember.createMany({
          data: members.map((member) => ({
            ...
            email:
              member.email || `temp-${Date.now()}-${Math.random()}@example.com`, // Frontend might not provide email for all members?
