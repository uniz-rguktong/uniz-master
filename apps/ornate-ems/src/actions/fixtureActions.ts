...
const collectMatchesToPromoteLive = (
  matches: Array<{
    id: string;
    date: Date | null;
    time: string | null;
...
async function resolveSportIds(
  sport: string,
  gender: string,
): Promise<string[]> {
...
async function resolveSingleSportId(
  sport: string,
  gender: string,
): Promise<string | null> {
...
async function assertFixtureMutationAccess(
  user: AuthUser,
  sportId?: string | null,
): Promise<void> {
...
async function handleMatchProgression(
  match: Match & { sport: { id: string; name: string; gender: string } },
) {
  ...
  if (match.status === MatchStatus.COMPLETED) {
    ...
    if (match.matchId === "R1-M1") {
      ...
      if (remaining.length >= 1) {
        await prisma.match.updateMany({
          ...
          data: {
            team1Id: remaining[0]!.id,
            team1Name: remaining[0]!.teamName,
          },
        });
      }
      if (remaining.length >= 2) {
        await prisma.match.updateMany({
          ...
          data: {
            team1Id: remaining[1]!.id,
            team1Name: remaining[1]!.teamName,
          },
        });
      }
      if (remaining.length >= 3) {
        await prisma.match.updateMany({
          ...
          data: {
            team2Id: remaining[2]!.id,
            team2Name: remaining[2]!.teamName,
          },
        });
      }
    ...
    if (match.matchId === "GF-M1") {
      const runnerUpName =
        match.winner === MatchWinner.TEAM1 ? match.team2Name : match.team1Name;
...
export async function getMatches(
  sport: string,
  gender: string,
): Promise<FixtureActionResponse<Match[]>> {
  ...
  return executeAction(async () => {
    ...
    return {
      success: true,
      data: sorted.map(formatMatchForUi),
    } as FixtureActionResponse<Match[]>;
  }, "getMatches");
}
...
export async function getCompletedMatches(
  ...
  return executeAction(async () => {
    ...
    return {
      success: true,
      data: matches.map(formatMatchForUi),
    } as FixtureActionResponse<Match[]>;
  }, "getCompletedMatches");
}
...
const REQUIRED_MATCH_IDS = BRACKET_STRUCTURE.flatMap((r) =>
...
export async function initializeTournament(
  ...
  return executeAction(async () => {
    ...
    if (!sportId) {
      return {
        success: false,
        error: "Sport not found for the selected filters.",
      } as FixtureActionResponse<Match[]>;
    }
    ...
    const existingTemplateIds = new Set(
      existing.filter((m) => m.matchId).map((m) => m.matchId),
    );
    const allBracketSlotsExist = REQUIRED_MATCH_IDS.every((id) =>
      existingTemplateIds.has(id),
    );
    ...
    if (allBracketSlotsExist) {
      ...
      return {
        success: true,
        data: existing.map(formatMatchForUi),
      } as FixtureActionResponse<Match[]>;
    }
...
export async function updateMatch(
  ...
  return executeAction(async () => {
    ...
    const explicitCompleted =
      ...
      normalizeStatusForDb(data.status) === MatchStatus.COMPLETED;
    if (explicitCompleted) {
      const nextDate =
        (updateData.date as Date | null | undefined) !== undefined
          ? (updateData.date as Date | null)
          : current.date;
      const nextTime =
        updateData.time !== undefined
          ? (updateData.time as string | null)
          : current.time;
      const nextVenue =
        updateData.venue !== undefined
          ? (updateData.venue as string | null)
          : current.venue;
      const nextScore1 =
        updateData.score1 !== undefined
          ? (updateData.score1 as string | null)
          : current.score1;
      const nextScore2 =
        updateData.score2 !== undefined
          ? (updateData.score2 as string | null)
          : current.score2;
      const nextWinner =
        updateData.winner !== undefined
          ? (updateData.winner as MatchWinner | null)
          : current.winner;

      const hasScheduling = Boolean(
        nextDate && (nextTime || "").trim() && (nextVenue || "").trim(),
      );
      const hasResult = Boolean(
        (nextScore1 || "").trim() && (nextScore2 || "").trim() && nextWinner,
      );
      if (!hasScheduling) {
        throw new Error(
          "Cannot mark match as completed without date, time and venue",
        );
      }
      if (!hasResult) {
        throw new Error(
          "Cannot mark match as completed without score and winner",
        );
      }
      ...
      updateData.status = hasScheduling
...
export async function createMatch(
  ...
  return executeAction(async () => {
    const sportId =
      data.sportId ||
      (data.sport
        ? await resolveSingleSportId(data.sport, data.gender || "All")
        : null);
...
export async function getTournamentStandings(): Promise<FixtureActionResponse> {
  ...
  return executeAction(async () => {
    ...
    const sports = Array.from(
      new Set(activeSports.map((s) => s.name).filter(Boolean)),
    );

    type GenderStandings = Record<string, Record<string, number>>;
    const standings: { Boys: GenderStandings; Girls: GenderStandings } = {
      Boys: {},
      Girls: {},
    };
    ...
    for (const bp of branchPoints) {
      ...
      pointsObj[bp.branch] =
        (pointsObj[bp.branch] || 0) + (bp.points + bp.manualAdjustment);
    }
...
export async function getOverallBranchStandings(): Promise<FixtureActionResponse> {
  ...
  return executeAction(async () => {
    ...
    const groupedMap = new Map(
      grouped.map((g) => [
        g.branch,
        (g._sum.points || 0) + (g._sum.manualAdjustment || 0),
      ]),
    );
...
export async function getUpcomingMatches(
  ...
  return executeAction(async () => {
    const matches = await prisma.match.findMany({
      where: {
        status: {
          in: [MatchStatus.PENDING, MatchStatus.SCHEDULED, MatchStatus.LIVE],
        },
      },
      select: {
        id: true,
