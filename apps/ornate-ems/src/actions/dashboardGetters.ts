...
async function getScopedEventIds(
  ...
  if (role === "SUPER_ADMIN") {
    ...
    eventScope = {
      OR: [{ creatorId: userId }, { creator: { branch: branch ?? null } }],
    };
  }
...
export async function getDashboardStats() {
  return executeAction(async () => {
    ...
    const regTrendPercent =
      result.prevMonthRegistrationCount > 0
        ? Math.round(
            ((result.registrationCount - result.prevMonthRegistrationCount) /
              result.prevMonthRegistrationCount) *
              100,
          )
        : 0;
...
export async function getSalesTrendData() {
  return executeAction(async () => {
    ...
    return await unstable_cache(
      async () => {
        const eventIds = await getScopedEventIds(
          userId,
          role,
          clubId ?? null,
          branch ?? null,
    ...
    const data = await unstable_cache(
      async () => {
        ...
        return events.map((event: any) => {
          const validRegistrations = event.registrations.filter(
...
export async function getHHODashboardData() {
  return executeAction(async () => {
    ...
    const result = await unstable_cache(
        ...
        const [statsResult, transactions, events] = await Promise.all([
          (async () => {
            const [
              ...
              prisma.registration.count({
                where: { event: hhoScope, status: "ATTENDED" },
              }),
              prisma.registration.count({
                where: { event: hhoScope, status: "PENDING" },
              }),
              prisma.registration.count({
                where: { event: hhoScope, status: "WAITLISTED" },
              }),
              prisma.registration.aggregate({
                where: { event: hhoScope, paymentStatus: "PAID" },
                _sum: { amount: true },
              }),
            ]);
            return {
      ...
      {
        tags: ["dashboard-stats", "hho-data", "events", "registrations"],
        revalidate: 60,
      },
    )();
