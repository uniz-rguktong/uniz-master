...
function buildSportRegistrationWhere(user: { role: string; id: string; branch?: string | null }) {
...
const getCachedSportTeamRegistrations = unstable_cache(
    async (userId: string, role: string, branch: string | null) => {
        ...
        } else if (!['SUPER_ADMIN', 'SPORTS_ADMIN', 'BRANCH_SPORTS_ADMIN'].includes(role)) {
        ...
        return sportTeams.map((sportTeam) => {
            const captain =
                sportTeam.team.members.find((member) => member.role === TeamRole.LEADER) ||
            ...
            return {
                ...
                viceCaptain:
                    sportTeam.team.members.find((member) => member.role === TeamRole.VICE_CAPTAIN)?.name ||
                    'N/A',
                yearLevel: captain?.year || 'N/A',
                status: (captain?.sportRegistration?.status || sportTeam.team.status).toLowerCase(),
...
export async function getAllSportRegistrations() {
    return executeAction(async () => {
        ...
        const registrations = await getCachedAllSportRegistrations(user.id, user.role, user.branch || null);
...
export async function getSportTeamRegistrations() {
    return executeAction(async () => {
        ...
        const teams = await getCachedSportTeamRegistrations(user.id, user.role, user.branch || null);
