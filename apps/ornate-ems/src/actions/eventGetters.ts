...
function buildEventWhereClause(user: AuthUser): Prisma.EventWhereInput {
    ...
    if (role === UserRole.SPORTS_ADMIN || role === UserRole.BRANCH_SPORTS_ADMIN) return { creator: { role: UserRole.SPORTS_ADMIN } };
    if (role === UserRole.CLUB_COORDINATOR) return { creator: { clubId: user.clubId ?? null } };
...
const getCachedCalendarEvents = unstable_cache(
    async (userId: string, role: string, clubId: string | null, branch: string | null) => {
        ...
        return events.map(e => ({
            ...
            time: e.time ? formatTimeTo12h(e.time) : (e.startTime ? `${formatTimeTo12h(e.startTime)}${e.endTime ? ` - ${formatTimeTo12h(e.endTime)}` : ''}` : 'TBD'),
...
export async function getEvents(): Promise<{ success?: boolean; data?: EventBase[]; error?: string }> {
...
export async function getEventsListForFilter(): Promise<{ data?: EventFilterItem[], success?: boolean, error?: string }> {
    return executeAction(async () => {
        const user = await getAuthenticatedUser();
        if (!user) {
...
export async function getEventById(eventId: string): Promise<{ success?: boolean; data?: EventDetails; error?: string }> {
    return executeAction(async () => {
        const user = await getAuthenticatedUser();
        if (!user) throw new Error('Unauthorized');
...
export async function getWinners(): Promise<{ success?: boolean; data?: any[]; error?: string }> {
    return executeAction(async () => {
        const user = await getAuthenticatedUser();
        if (!user) throw new Error('Unauthorized');
