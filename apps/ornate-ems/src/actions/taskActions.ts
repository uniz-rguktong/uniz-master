...
// Create a new task
export async function createTask(formData: FormData): Promise<TaskActionResponse<TaskData>> {
...
// Update a task
export async function updateTask(taskId: string, updates: Partial<Pick<TaskData, 'title' | 'description' | 'assignedTo' | 'priority' | 'status'>> & { date?: string | Date; time?: string }): Promise<TaskActionResponse<TaskData>> {
    const session = await getServerSession(authOptions);
    if (!session?.user?.id) {
        return { success: false, error: 'Unauthorized' };
    }
    ...
    return executeAction(async () => {
        ...
        const isSameBranch = session.user!.role === 'BRANCH_ADMIN' && (task.creator?.branch || '') === (session.user!.branch || '');
        ...
        if (updates.date !== undefined) {
            safeUpdates.date = typeof updates.date === 'string' ? new Date(updates.date) : updates.date;
        }
...
export async function getTasks(): Promise<TaskActionResponse<TaskData[]>> {
    try {
        ...
        if (!admin) {
            return { success: false, error: 'Admin not found', data: [] as TaskData[] };
        }
        ...
        return { success: false, error: 'Failed to fetch tasks', data: [] as TaskData[] };
    }
}
