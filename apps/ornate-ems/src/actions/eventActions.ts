...
function extractCoordinatorContacts(raw: string | null): CoordinatorContact[] {
  ...
  for (const item of parsed) {
    ...
    if (typeof maybeObject.id === "string" && maybeObject.id.length > 0)
      continue;
    ...
    const name =
      typeof maybeObject.name === "string" && maybeObject.name.trim().length > 0
        ? maybeObject.name.trim()
        : email.split("@")[0]!;

    const phone =
      typeof maybeObject.phone === "string" &&
      maybeObject.phone.trim().length > 0
...
export async function createEvent(
  ...
  return executeAction(async () => {
    ...
    const rawData = {
      ...
      teamSizeMin: formData.get("teamSizeMin")
        ? Number(formData.get("teamSizeMin"))
        : undefined,
      teamSizeMax: formData.get("teamSizeMax")
        ? Number(formData.get("teamSizeMax"))
        : undefined,
      ...
      maxCapacity:
        formData.get("maxParticipants") || formData.get("capacity")
          ? Number(formData.get("maxParticipants") || formData.get("capacity"))
          : 100,
      ...
      fee: getOptional("fee"),
      price:
        formData.get("isPaid") === "true"
          ? Number(formData.get("fee")) || 0
          : 0,
      posterUrl: getOptional("posterUrl"),
      coordinatorIds: extractCoordinatorIds(coordinatorPayload),
      eligibility: formData.get("eligibility")
        ? JSON.parse(formData.get("eligibility") as string)
        : [],
      ...
      additionalImages: formData.get("additionalImages")
        ? JSON.parse(formData.get("additionalImages") as string)
        : [],
      prizes:
        formData.get("hasPrizes") === "false"
          ? []
          : formData.get("prizes")
            ? JSON.parse(formData.get("prizes") as string)
            : [],
    ...
    const normalizedStatus =
      parseEventStatus(validatedData.status) || EventStatus.DRAFT;
    ...
    const newEvent = await prisma.event.create({
      data: {
        ...
        eligibility:
          (validatedData.eligibility as Prisma.InputJsonValue) ??
          Prisma.JsonNull,
        customFields:
          (validatedData.customFields as Prisma.InputJsonValue) ??
          Prisma.JsonNull,
        documents:
          (validatedData.documents as Prisma.InputJsonValue) ?? Prisma.JsonNull,
        additionalImages: (validatedData.additionalImages as string[]) ?? [],
        prizes:
          (validatedData.prizes as Prisma.InputJsonValue) ?? Prisma.JsonNull,
    ...
    if (coordinatorIds && coordinatorIds.length > 0) {
      await notifyAssignedCoordinators(
        coordinatorIds,
    ...
    if (coordinatorContacts.length > 0) {
      await Promise.allSettled(
        coordinatorContacts.map((coordinator) =>
          inviteCoordinator(newEvent.id, coordinator),
        ),
      );
    }
...
export async function updateEvent(
  ...
  return executeAction(async () => {
    ...
    const { coordinatorIds, ...eventFields } = validatedData;
    const normalizedStatus =
      parseEventStatus(validatedData.status) || existingEvent.status;
    ...
    const previousCoordinatorIds = new Set(
      (existingEvent.assignedCoordinators || []).map((c) => c.id),
    );
    const newlyAssignedIds = (coordinatorIds || []).filter(
      (id: string) => !previousCoordinatorIds.has(id),
    );
    ...
    if (coordinatorContacts.length > 0) {
      await Promise.allSettled(
        coordinatorContacts.map((coordinator) =>
          inviteCoordinator(eventId, coordinator),
        ),
      );
    }
...
export async function deleteEvent(eventId: string): Promise<ActionResponse> {
  ...
  return executeAction(async () => {
    ...
    const isSports =
      ...
      (event.creator.role === "SPORTS_ADMIN" || event.category === "Sports");
    const isSameBranch =
      session.user.role === "BRANCH_ADMIN" &&
      event.creator.branch === session.user.branch;
    const isSameClub =
      session.user.role === "CLUB_COORDINATOR" &&
      event.creator.clubId === session.user.clubId;
...
export async function duplicateEvent(
  ...
  return executeAction(async () => {
    ...
    const { id, createdAt, updatedAt, creatorId, status, ...eventData } =
      originalEvent;
