...
function resolveEntityId(admin: {
  role: string;
  clubId: string | null;
  branch: string | null;
}): string | null {
...
function buildEntity(
  ...
  return {
    id: entityId,
    name:
      (prefs.displayName as string) ||
      meta?.name ||
      admin.clubId ||
      admin.branch ||
      entityId,
    shortName:
      (prefs.shortName as string) ||
      meta?.shortName ||
      entityId.slice(0, 3).toUpperCase(),
    coordinator: admin.name || "Admin",
    establishedYear: String(
      (prefs.establishedYear as number) || meta?.establishedYear || "2020",
    ),
...
function buildAdminWhereClause(
  ...
  if (entityId === "branch_sports_admin")
    return { role: "BRANCH_SPORTS_ADMIN" };
  ...
  async (): Promise<ClubEntity[]> => {
    try {
      ...
      for (const [entityId, admin] of entityAdminMap) {
        entities.push(
          buildEntity(admin, entityId, memberCountByAdmin.get(admin.id) || 0),
        );
      }
