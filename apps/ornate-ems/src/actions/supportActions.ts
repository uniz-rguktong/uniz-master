...
export async function submitSupportTicket(formData: FormData): Promise<SupportActionResponse> {
    ...
    return executeAction(async () => {
        ...
        if (!subject || !message) {
            return { success: false, error: 'Subject and message are required fields.' };
        }

        // 1. Identification: Determine who is sending the ticket
        let sender: { id: string; name: string | null; role?: string; branch?: string | null } | null = await prisma.admin.findUnique({
        ...
        if (!supportAgent) {
            return { success: false, error: 'Support service currently unavailable (No admins found).' };
        }
