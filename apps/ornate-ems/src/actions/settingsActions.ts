 ...
 */
export async function getHHOSettings(): Promise<
  SettingsActionResponse<HHOSettingsData>
> {
 ...
 */
export async function updateHHOSettings(formData: {
  name?: string;
  phone?: string;
  description?: string;
  coordinator?: string;
  ...
  return executeAction(async () => {
    ...
    if (formData.description !== undefined)
      updateData.bio = formData.description;
    if (formData.coordinator !== undefined)
      updateData.designation = formData.coordinator;
 ...
 */
export async function getHHOProfile(): Promise<
  SettingsActionResponse<HHOProfileData>
> {
  const session = await getServerSession(authOptions);
  if (!session || !session.user?.email) {
    return { success: false, error: "Unauthorized" };
 ...
 */
export async function updateHHOProfile(formData: {
  name?: string;
  phone?: string;
  bio?: string;
  designation?: string;
