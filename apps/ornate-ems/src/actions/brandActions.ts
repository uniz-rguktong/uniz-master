...
export async function getPromoVideos(): Promise<
  BrandActionResponse<PromoVideo[]>
> {
  try {
...
): Promise<BrandActionResponse<PromoVideo>> {
  const session = await getServerSession(authOptions);
  if (!session || !session.user?.email) {
    return { success: false, error: "Unauthorized" };
  }
  ...
  return executeAction(async () => {
    ...
    const isAuthorized =
      (admin.role === "HHO" ||
        ...
        videoCheck?.creatorId === admin.id) &&
      admin.role !== "BRANCH_SPORTS_ADMIN";
    if (!videoCheck || !isAuthorized) {
      return {
        success: false,
        error:
          "Unauthorized: You do not have permission or have view-only access",
      };
    }
...
): Promise<BrandActionResponse<BrandLogo>> {
  const session = await getServerSession(authOptions);
  if (!session || !session.user?.email) {
    return { success: false, error: "Unauthorized" };
  }
