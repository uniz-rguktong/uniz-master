...
export async function getBestOutgoingStudents() {
    return executeAction(async () => {
        ...
        if (admin.role !== 'SUPER_ADMIN' && admin.role !== 'HHO' && admin.role !== 'SPORTS_ADMIN' && admin.role !== 'BRANCH_SPORTS_ADMIN' && admin.branch) {
            queryScope.where = {
                OR: [
                    {
                        branch: {
...
export async function createBestOutgoingStudent(formData: BestOutgoingStudentInput) {
    const session = await getServerSession(authOptions);
    ...
    return executeAction(async () => {
        const admin = await prisma.admin.findUnique({ where: { email: session.user.email! }, select: { id: true, role: true, branch: true } });
        ...
        const student = await prisma.bestOutgoingStudent.create({
            data: {
                ...
                branch: (admin.role === 'BRANCH_ADMIN' ? (admin.branch || data.branch) : (data.branch || admin.branch || '')).trim(),
...
export async function updateBestOutgoingStudent(formData: BestOutgoingStudentInput) {
    const session = await getServerSession(authOptions);
    if (!session || !['SUPER_ADMIN', 'BRANCH_ADMIN'].includes(session.user.role)) {
        throw new Error('Unauthorized');
    }
    ...
    return executeAction(async () => {
        const admin = await prisma.admin.findUnique({ where: { email: userEmail }, select: { id: true, role: true, branch: true } });
        if (!admin) throw new Error('Admin not found');
        ...
        // 1. Validate required fields
        if (!data.name || data.name.trim() === '') throw new Error('Student name is required');
        if (!data.rollNumber || data.rollNumber.trim() === '') throw new Error('Roll number is required');
        if (!data.branch || data.branch.trim() === '') throw new Error('Branch is required');
        if (!data.gender || data.gender.trim() === '') throw new Error('Gender is required');
        if (!awardYear) throw new Error('Award Year is required');
        if (!achievements || (Array.isArray(achievements) && achievements.length === 0) || (typeof achievements === 'string' && achievements.trim() === '')) throw new Error('Achievements are required');

        // 2. Normalize numeric fields
        const normalizedCGPA = parseFloat(String(rating ?? cgpa ?? '0'));
        if (isNaN(normalizedCGPA)) throw new Error('Invalid CGPA/Rating value provided');

        const normalizedAwardYear = parseInt(String(awardYear));
        if (isNaN(normalizedAwardYear)) throw new Error('Invalid Award Year value provided');
        ...
        if (admin.role === 'BRANCH_ADMIN') {
            ...
            if (existing.isOverall) {
                throw new Error('Only Super Admin can modify overall best outgoing students');
            }
...
export async function deleteBestOutgoingStudent(id: string) {
    ...
    return executeAction(async () => {
        ...
        if (admin.role === 'BRANCH_ADMIN') {
            ...
            if (existing.isOverall) {
                throw new Error('Only Super Admin can delete overall best outgoing students');
            }
...
export async function toggleStudentPublish(id: string, isPublished: boolean) {
    ...
    return executeAction(async () => {
        ...
        if (admin.role === 'BRANCH_ADMIN') {
            ...
            if (existing.isOverall) {
                throw new Error('Only Super Admin can publish overall best outgoing students');
            }
