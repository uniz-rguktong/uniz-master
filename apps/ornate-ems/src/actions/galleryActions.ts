...
function resolveAlbumVisibility(
  ...
  const match = (tags || []).find((tag) =>
    tag.startsWith(VISIBILITY_TAG_PREFIX),
  );
...
function buildAlbumTags(
  ...
  const tagsWithoutVisibility = (existingTags || []).filter(
    (tag) => !tag.startsWith(VISIBILITY_TAG_PREFIX),
  );
  const resolvedVisibility = visibility || DEFAULT_GALLERY_VISIBILITY;
  return [
    ...tagsWithoutVisibility,
    `${VISIBILITY_TAG_PREFIX}${resolvedVisibility}`,
  ];
}
...
export async function getGalleryAlbums(): Promise<
  GalleryActionResponse<FormattedGalleryAlbum[]>
> {
  try {
...
export async function createGalleryAlbum(
  ...
  return executeAction(async () => {
    ...
    if (admin.role === "BRANCH_SPORTS_ADMIN")
      throw new Error(
        "Unauthorized: Branch Sports Admins have view-only access",
      );
...
export async function updateGalleryAlbum(
  ...
  return executeAction(async () => {
    ...
    const isAuthorized =
      (admin.role === "SUPER_ADMIN" || albumCheck.creatorId === admin.id) &&
    ...
    if (data.visibility !== undefined)
      updateData.tags = buildAlbumTags(albumCheck.tags, data.visibility);
...
export async function getAlbumPhotos(
  albumId: string,
): Promise<GalleryActionResponse<FormattedGalleryImage[]>> {
  try {
  ...
  return executeAction(async () => {
    ...
    if (admin.role !== "SUPER_ADMIN") {
      ...
      if (unauthorized.length > 0) {
        return {
          success: false,
          error: "Unauthorized: Some photos belong to albums you do not own",
        };
      }
    }
...
export async function updateGalleryImage(
  ...
  try {
    ...
    const isSportsAdminScoped =
      (admin.role === "SPORTS_ADMIN" || admin.role === "BRANCH_SPORTS_ADMIN") &&
      image.album.creator?.role === "SPORTS_ADMIN";

    if (
      admin.role === "BRANCH_SPORTS_ADMIN" ||
      (!isSuperAdmin && !isOwner && !isSportsAdminScoped)
    ) {
      return { success: false, error: "Unauthorized: View-only access" };
    }
