...
export async function updateStudentUser(userId: string, data: StudentUpdateInput) {
    const session = await getServerSession(authOptions);
    if (!session?.user || session.user.role !== 'SUPER_ADMIN') {
        return { error: 'Unauthorized' };
    }
    ...
    return executeAction(async () => {
        ...
        return {
            ...
            data: events.map(e => ({
                ...
                isFull: e.maxCapacity ? e._count.registrations >= e.maxCapacity : false
            }))
        };
...
export async function assignEventToStudent(studentId: string, eventId: string) {
    ...
    return executeAction(async () => {
        ...
        const event = await prisma.event.findUnique({
            where: { id: eventId },
            select: { id: true, title: true, maxCapacity: true, registrationOpen: true, price: true }
        });
        ...
        const registrationStatus = (event.maxCapacity && activeCount >= event.maxCapacity) ? 'WAITLISTED' : 'CONFIRMED';
