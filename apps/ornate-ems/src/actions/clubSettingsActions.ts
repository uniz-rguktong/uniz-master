...
export async function getClubSettings(): Promise<
  ClubSettingsActionResponse<ClubSettingsData>
> {
  ...
  try {
    ...
    const adminData: ClubSettingsData = {
      ...
      establishedYear:
        (preferences.establishedYear as number) || new Date().getFullYear(),
...
export async function updateClubSettings(formData: {
  name?: string;
  shortName?: string;
  coordinator?: string;
  establishedYear?: number;
...
}): Promise<ClubSettingsActionResponse> {
  const session = await getServerSession(authOptions);
  if (!session || !session.user?.email) {
    return { success: false, error: "Unauthorized" };
  }
  ...
  return executeAction(async () => {
    ...
    const currentPrefs =
      (currentAdmin?.preferences as Record<string, unknown>) || {};
...
export async function getAdminProfile(): Promise<
  ClubSettingsActionResponse<AdminProfileData>
> {
  const session = await getServerSession(authOptions);
  if (!session || !session.user?.email) {
    return { success: false, error: "Unauthorized" };
  ...
  return executeAction(async () => {
    ...
    if (data.notificationSettings !== undefined)
      updateData.notificationSettings =
        data.notificationSettings as Prisma.InputJsonValue;
    ...
    if (data.newPassword) {
      ...
      if (!adminAuth?.password) {
        return {
          success: false,
          error: "Account security error. Please contact support.",
        };
      }
