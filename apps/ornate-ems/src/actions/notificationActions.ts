...
function getNotificationCenterUrlForRole(role?: string | null): string {
    switch (role) {
        case 'SUPER_ADMIN':
        ...
        case 'CLUB_COORDINATOR':
            return '/clubs-portal/notifications';
        case 'SPORTS_ADMIN':
        case 'BRANCH_SPORTS_ADMIN':
            return '/sports/notifications';
        case 'HHO':
            return '/hho/notifications';
...
export async function getNotifications(isArchived = false, isSent = false) {
    try {
        ...
        let dbUser: { id: string; email: string; role: string } | null = await prisma.admin.findFirst({
        ...
        if (!dbUser) {
            logger.error({ email: session.user.email }, '[getNotifications] User not found in ANY table');
 ...
 */
async function resolveSessionUserId(session: { user?: { email?: string | null } }): Promise<string | null> {
...
export async function markAsRead(id: string) {
    ...
    return executeAction(async () => {
        ...
        if (result.count === 0) return { success: false, error: 'Notification not found' };
...
export async function sendNotification(data: SendNotificationInput) {
    ...
    if (!session.user?.email) return { success: false, error: 'Unauthorized: No email in session' };

    return executeAction(async () => {
        const sender: { id: string; name: string | null; role: string; branch: string | null } | null = await prisma.admin.findUnique({
        ...
        // Fetch recipient details for sent history and push targeting.
        const recipient: { name: string | null; role: string; email: string } | null = await prisma.admin.findUnique({
        ...
        const recipientName = data.recipientName || recipient?.name || 'Recipient';
        const recipientRole = data.recipientRole || recipient?.role || 'Recipient';
        ...
        // Best-effort push delivery: DB notification succeeds even if push queue is unavailable.
        if (recipient?.email) {
            try {
                const { sendWebPush } = await import('@/lib/notifications/notification-worker');
                const compactMessage = message.length > 160 ? `${message.slice(0, 157)}...` : message;
                const notificationUrl = getNotificationCenterUrlForRole(recipientRole);
                await sendWebPush(recipient.email, {
                    title: `${sender.name || 'Admin'} sent you a message`,
                    body: compactMessage,
                    url: notificationUrl,
                ...
                logger.warn({ err: error, recipientId }, 'Push enqueue failed; in-app notification already persisted');
            }
        }
 ...
 */
export async function searchRecipients(query: string, type: 'admin' | 'student' | 'club') {
    try {
        const session = await getServerSession(authOptions)
        if (!session) return { success: false, error: 'Unauthorized' }
        ...
        if (type === 'admin') {
            const adminWhere = query.trim().length > 0
                ? {
                    OR: [
            ...
            results = await prisma.admin.findMany({
                ...
                take: 200,
                select: { id: true, name: true, branch: true, role: true, clubId: true }
            })
            ...
            const clubWhere = query.trim().length > 0
                ? {
