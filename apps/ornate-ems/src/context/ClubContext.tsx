...
export function ClubProvider({ children, initialEntity, initialEntities }: ClubProviderProps) {
    ...
    const [selectedClubId, setSelectedClubId] = useState<string>(initialEntity.id);
    ...
    const updateClubDetails = (newDetails: Partial<ClubEntity>) => {
        setEntities(prev => ({
            ...prev,
            [selectedClubId]: { ...prev[selectedClubId], ...newDetails } as ClubEntity
        }));
    };
