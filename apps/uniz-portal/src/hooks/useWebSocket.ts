...
export const useWebSocket = (
  ...
  const attemptReconnect = () => {
    if (reconnectAttributes.attempts >= reconnectAttributes.maxAttempts) {
      console.warn(
        "Max WebSocket reconnect attempts reached. Falling back to API polling only.",
      );
      return;
    }
    ...
    console.log(
      `Attempting WebSocket reconnect in ${delay}ms... (Attempt ${reconnectAttributes.attempts + 1}/${reconnectAttributes.maxAttempts})`,
    );
