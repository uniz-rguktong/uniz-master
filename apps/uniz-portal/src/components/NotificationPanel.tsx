...
const CAREERS: NotificationItem[] = [
    { id: '1', title: 'Recruitment of Assistant Professors (Contract) - CSE & ECE Depts', dateRange: 'Apply by 15-11-2025', type: 'career', color: 'border-l-red-500', link: '#' },
    { id: '2', title: 'Walk-in Interview for Residential Medical Officer @ Campus Hospital', dateRange: '20-11-2025', type: 'career', color: 'border-l-blue-500', link: '#' },
    { id: '3', title: 'JRF Position available in Dept of Physics (SERB Project)', dateRange: 'Apply by 30-11-2025', type: 'career', color: 'border-l-yellow-500', link: '#' },
];


const Scroller = ({ children, height = "h-[300px]", isPaused }: { children: React.ReactNode, height?: string, isPaused: boolean }) => {
        ...
        fetch("/api/v1/cms/notifications", requestOptions)
            ...
            .then((result) => {
                if (result.success && result.notifications) {
                    ...
                    // Process Tenders
                    const tendersData = result.notifications.tenders.map((item: any, index: number) => {
                        const created = new Date(item.createdAt);
                        const deadline = new Date(item.deadline);

                        const formatDate = (d: Date) => `${String(d.getDate()).padStart(2, '0')}-${String(d.getMonth() + 1).padStart(2, '0')}-${d.getFullYear()}`;

                        const colors = ['border-l-red-500', 'border-l-purple-500', 'border-l-yellow-500', 'border-l-blue-500', 'border-l-green-500'];
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow-lg border border-slate-100 lg:col-span-2 overflow-hidden flex flex-col h-[500px]">
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div className="flex items-center gap-3">
                        <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Announcements</h3>
                        <Megaphone className="w-5 h-5 text-pink-500 animate-pulse" />
                    </div>
                    <div className="flex items-center gap-2">
                        <button onClick={toggleAnnouncePause} className="p-1.5 rounded hover:bg-slate-200 text-slate-500 transition-colors">
                ...
                >
                    <Scroller height="h-full" isPaused={isAnnouncePaused}>
                        {announcements.map((item) => (
                            <div key={item.id} onClick={() => window.open(item.link, '_blank')} className="flex gap-6 group cursor-pointer p-4 rounded-lg hover:bg-slate-50 border-b border-slate-50 last:border-0 transition-colors">
                                <div className="flex flex-col items-center justify-center min-w-[80px] h-[80px] bg-[#800000]/5 text-[#800000] rounded-lg border border-[#800000]/10">
                                    ...
                                    <span className="text-[10px] font-bold uppercase">{new Date(item.date.split('-').reverse().join('-')).toLocaleString('default', { month: 'short' })}</span>
            ...
            <div className="bg-white rounded-xl shadow-lg border border-slate-100 col-span-1 overflow-hidden flex flex-col h-[500px]">
                {/* Header */}
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <h3 className="text-xl font-black text-slate-800 uppercase tracking-tight">Notifications</h3>
                    <div className="flex items-center gap-2">
                        <button onClick={toggleNotifyPause} className="p-1.5 rounded hover:bg-slate-200 text-slate-500 transition-colors">
                ...
                <div className="flex border-b border-slate-100">
                    <button
                        ...
                        className={cn(
                            ...
                            activeTab === 'tenders' ? "border-[#800000] text-[#800000] bg-[#800000]/5" : "border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-50"
                        )}
                    >
                        Tenders
                    </button>
                    <button
                        ...
                        className={cn(
                            "flex-1 py-3 text-sm font-bold uppercase tracking-wider transition-colors border-b-2",
                            activeTab === 'careers' ? "border-[#800000] text-[#800000] bg-[#800000]/5" : "border-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-50"
                        )}
                    >
                        Careers
                    </button>
                ...
                >
                    <Scroller height="h-full" isPaused={isNotifyPaused}>
                        {(activeTab === 'tenders' ? tenders : CAREERS).map((item) => (
                            <div key={item.id} onClick={() => window.open(item.link, '_blank')} className={cn("group cursor-pointer flex justify-between items-center p-3 mb-2 rounded border border-slate-100 hover:shadow-md transition-all bg-white border-l-4", item.color)}>
