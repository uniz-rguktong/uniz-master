        ...
        `}
            >
                    ...
                    {!isCollapsed && (
                        <div className="flex items-center space-x-2.5">
                            ...
                            <div className="flex flex-col">
                                <span className="font-bold text-slate-800 text-lg tracking-tight">Ongole</span>
                                <span className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">Student Portal</span>
                            </div>
                        </div>
                    )}
                ...
                <div className="mt-auto border-t border-slate-200">
                    {/* Profile Section */}
                    <div className={`border-b border-slate-200 bg-slate-50/30 ${isCollapsed ? 'py-4 px-2' : 'p-4'}`}>
                        {!isCollapsed ? (
                            <div className="flex items-center px-3 py-2.5 rounded-lg bg-white border border-slate-100 shadow-sm hover:shadow-md transition-all duration-200 group">
                                <div className="w-9 h-9 bg-gradient-to-br from-slate-100 to-slate-200 rounded-full flex items-center justify-center overflow-hidden border border-slate-200">
                                    {userData?.profile_url ? (
                                        ...
                                        <span className="text-slate-700 font-bold text-sm">{userData?.name?.charAt(0) || 'S'}</span>
                                    )}
                                </div>
                                <div className="flex-1 min-w-0 ml-3">
                                    <p className="text-sm font-bold text-slate-800 truncate">{userData?.name || 'Student'}</p>
                                    <p className="text-[11px] text-slate-500 font-medium truncate uppercase tracking-wider">Member</p>
                                </div>
                                <div className="w-2 h-2 bg-green-500 rounded-full ml-2 ring-4 ring-green-50" title="Online" />
                            </div>
                        ) : (
                                ...
                                <div className="relative group cursor-pointer">
                                    <div className="w-10 h-10 bg-white shadow-sm rounded-full flex items-center justify-center overflow-hidden border border-slate-200 hover:border-blue-400 transition-colors">
                                        {userData?.profile_url ? (
                                            ...
                                            <span className="text-slate-800 font-bold text-sm">{userData?.name?.charAt(0) || 'S'}</span>
                                        )}
                                    </div>
