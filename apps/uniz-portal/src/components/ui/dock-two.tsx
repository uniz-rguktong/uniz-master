...
const DockIconButton = React.forwardRef<HTMLButtonElement, DockIconButtonProps>(
    ({ icon: Icon, label, onClick, isActive, className, activeColor, hoverColor }, ref) => {
        return (
            <motion.button
                ref={ref}
...
const Dock = React.forwardRef<HTMLDivElement, DockProps>(
    ({ items, className }, ref) => {
        return (
            <div ref={ref} className={cn("inline-flex flex-col items-center bg-white border border-slate-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-1.5 rounded-2xl gap-2", className)}>
                {items.map((item) => (
