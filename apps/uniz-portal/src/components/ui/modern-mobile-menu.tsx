...
const InteractiveMenu: React.FC<InteractiveMenuProps> = ({ items, accentColor }) => {
    ...
    // Sync activeIndex with isActive prop
    const activePropIndex = useMemo(() => {
        return finalItems.findIndex(item => item.isActive);
    }, [finalItems]);

    const [activeIndex, setActiveIndex] = useState(activePropIndex >= 0 ? activePropIndex : 0);

    useEffect(() => {
        if (activePropIndex >= 0 && activePropIndex !== activeIndex) {
            setActiveIndex(activePropIndex);
        }
    }, [activePropIndex]);
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(setLineWidth, 100); // Small delay to ensure refs are ready
                ...
                return (
                    ...
                    >
                        <div className="menu__icon">
                            <IconComponent className="icon" size={22} />
                        </div>
                        <strong
