...
export default function RequestCard({
  ...
  const StatusBadge = () => {
    if (isApproved)
      return (
        <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-black text-white">
          <CheckCircle className="w-3.5 h-3.5" /> Approved
        </span>
      );
    if (isRejected)
      return (
        <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-neutral-100 text-neutral-600">
          <XCircle className="w-3.5 h-3.5" /> Rejected
        </span>
      );
    return (
      <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-white border border-neutral-200 text-neutral-900">
  ...
  return (
    <div className="group bg-white border border-neutral-100 hover:border-black/10 rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] hover:shadow-lg transition-all duration-300 p-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start gap-3 mb-4 border-b border-neutral-50 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-bold text-neutral-900 capitalize text-lg">
              {type} Request
            </h4>
      ...
      {/* Content Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 text-sm mb-4">
        ...
        <div>
          ...
          <div className="inline-flex items-center px-2 py-1 rounded-md bg-neutral-50 text-neutral-900 font-bold border border-neutral-100/50">
            {formatDuration(
              calculateDuration(
      ...
      {request.approval_logs && request.approval_logs.length > 0 && (
        <div className="pt-4 border-t border-neutral-50">
          ...
          <div className="relative pl-3 space-y-3 border-l border-neutral-100 ml-1">
            {request.approval_logs.map((log: any, i: number) => (
              <div key={i} className="text-xs relative">
                <span
                  className={`absolute -left-[17px] top-1 w-2 h-2 rounded-full border-2 border-white ring-1 ring-neutral-100 ${
                    log.action === "approve"
                      ? "bg-black"
                      : log.action === "reject"
                        ? "bg-red-500"
