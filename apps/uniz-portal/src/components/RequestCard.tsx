        ...
        <div className="group bg-white border border-neutral-100 hover:border-black/10 rounded-xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] hover:shadow-lg transition-all duration-300 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8 text-sm mb-4">
                <div className="col-span-1 md:col-span-2">
                    <p className="text-xs font-bold text-neutral-400 uppercase tracking-widest mb-1">Reason</p>
                ...
                <div>
                    <p className="text-xs font-bold text-neutral-400 uppercase tracking-widest mb-1">Total Time</p>
                ...
                {isRejected && request.message && (
                    <div className="col-span-full p-4 bg-red-50/50 border border-red-100 rounded-lg text-red-900 text-sm">
                        <span className="font-bold block mb-1 flex items-center gap-2"><XCircle className="w-3 h-3" /> Rejection Reason</span>
                        {request.message}
                    </div>
                )}
            </div>
                    ...
                    <div className="relative pl-3 space-y-3 border-l border-neutral-100 ml-1">
                        {request.approval_logs.map((log: any, i: number) => (
                            <div key={i} className="text-xs relative">
                                <span className={`absolute -left-[17px] top-1 w-2 h-2 rounded-full border-2 border-white ring-1 ring-neutral-100 ${log.action === 'approve' ? 'bg-black' :
