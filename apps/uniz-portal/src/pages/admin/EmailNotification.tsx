...
export default function EmailNotification() {
  ...
  const sendNotification = async () => {
    ...
    if (target === "userIds")
      payload.filter = { ids: filterValue.split(",").map((id) => id.trim()) };
    ...
    try {
      ...
      intervalRef.current = setInterval(async () => {
        if (abortRef.current) {
          ...
          setProgress((prev) =>
            prev
              ? { ...prev, status: "failed", percentage: prev.percentage }
              : null,
          );
          return;
        }
        try {
          const progRes = await fetch(
            `${BASE_URL}/admin/notify/progress?processId=${pid}`,
            {
          ...
          {progress && (
            <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm animate-in fade-in slide-in-from-bottom-2">
              ...
              <div className="flex justify-between text-xs text-slate-500 mb-1">
                <span>
                  {progress.processedRecords} / {progress.totalRecords} sent
                </span>
