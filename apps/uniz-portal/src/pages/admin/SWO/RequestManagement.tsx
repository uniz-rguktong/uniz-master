...
export default function RequestManagement({ type }: RequestManagementProps) {
    ...
    const fetchRequests = async () => {
        ...
        try {
            ...
            const baseUrl = type === "outing" ? GET_OUTING_REQUESTS : GET_OUTPASS_REQUESTS;
            const url = `${baseUrl}?page=1&limit=50${searchQuery ? `&search=${searchQuery}` : ""}`;

            const data = await apiClient<any>(url);
            if (data && data.success) {
                const fetchedRequests = type === "outing" ? data.outings : data.outpasses;
    ...
    const handleAction = async (action: "approve" | "reject" | "forward", id: string) => {
        let message = "";
        if (action === "approve") {
            message = prompt("Enter approval message:", "Parent verified. Fine to go.") || "";
            if (message === "") return;
        } else if (action === "reject") {
            message = prompt("Enter rejection comment:", "Emergency not valid.") || "";
            if (message === "") return;
        }
        ...
        try {
            ...
            const body = action === "forward" ? { id } : (action === "approve" ? { message } : { comment: message });
                ...
                <div className="bg-white rounded-[32px] border border-slate-100 p-24 flex flex-col items-center justify-center text-center space-y-6 shadow-sm border-dashed min-h-[400px]">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-slate-400 font-medium max-w-sm mx-auto leading-relaxed">
                            No pending {type} requests found. New applications will appear here in real-time.
                        </p>
                    </div>
                </div>
            ) : (
                        ...
                        >
                            ...
                            <div className="flex-1 space-y-6">
                                ...
                                <div className="bg-slate-50/50 rounded-2xl p-6 border border-slate-100">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-3">Reason</span>
                                ...
                                <div className="grid grid-cols-2 gap-6 pb-2">
                                    <div className="space-y-1.5">
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block">From</span>
                                        <p className="text-slate-900 font-bold text-[13px] truncate" title={request.from_time || request.from_day}>
                                            {request.from_time || request.from_day}
                                        </p>
                                    </div>
                                    <div className="space-y-1.5">
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block">To</span>
                                        <p className="text-slate-900 font-bold text-[13px] truncate" title={request.to_time || request.to_day}>
                                ...
                                <div className="grid grid-cols-2 gap-6 pb-2">
                                    ...
                                    <div className="space-y-1.5">
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block text-right">Submitted</span>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                ...
                                >
                                    {actionLoading === request._id ? <Loader2 size={16} className="animate-spin" /> : <CheckCircle2 size={16} />}
                                    Approve
                                </button>
                                <button
                                ...
                                >
                                    {actionLoading === request._id ? <Loader2 size={16} className="animate-spin" /> : <Ban size={16} />}
                                    Reject
                                </button>
                                <button
                                ...
                                >
                                    {actionLoading === request._id ? <Loader2 size={16} className="animate-spin" /> : <Forward size={16} />}
                                    Forward
                                </button>
                            </div>
                        </div>
