...
export default function StudentBulkSection() {
    ...
    const handleExport = async () => {
        setLoading(true);
        const token = localStorage.getItem("admin_token");
        const url = ADMIN_STUDENT_EXPORT(exportParams.branch, exportParams.year, exportParams.fields);
        try {
        ...
        <div className="p-8 space-y-10 animate-in fade-in duration-700 pb-20 text-slate-900">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div className="flex flex-col gap-2">
                    <h2 className="text-4xl font-black tracking-tight text-slate-900 leading-none">Student Bulk Operations</h2>
                    <p className="text-slate-500 font-medium">Bulk onboard students or extract batch records to Excel.</p>
                </div>
                    ...
                    <div className="lg:col-span-7 space-y-8">
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="font-black text-xl tracking-tight">Step 1: Get the Framework</h3>
                                    <p className="text-slate-500 text-sm font-medium">Download the standard template to ensure data compatibility.</p>
                                </div>
                            </div>
                            <button
                        ...
                        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="font-black text-xl tracking-tight">Step 2: Sync Records</h3>
                                    <p className="text-slate-500 text-sm font-medium">Upload your filled records to update or create student profiles.</p>
                                </div>
                            </div>
                            ...
                            `}>
                                ...
                                />
                                <label htmlFor="bulk-student-file" className="cursor-pointer flex flex-col items-center gap-4">
                                    <div className={`p-6 rounded-3xl transition-all ${file ? 'bg-slate-900 text-white animate-pulse' : 'bg-white text-slate-300 shadow-sm'}`}>
                                        <Upload size={32} />
                                    </div>
                                    <div>
                                        <p className="text-lg font-black text-slate-900">{file ? file.name : "Choose XLSX File"}</p>
                                        <p className="text-slate-400 font-medium text-xs mt-1">Maximum 5000 records per upload</p>
                                    </div>
                                </label>
                                {file && (
                                    <button onClick={() => setFile(null)} className="absolute top-6 right-6 p-2 bg-white rounded-full shadow-lg text-red-500 hover:scale-110 transition-all">
                                        <X size={16} />
                                    </button>
                                )}
                            </div>
                        ...
                        <div className="sticky top-24">
                            {progress || uploadId ? (
                                <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl space-y-8 animate-in slide-in-from-right-8 duration-500">
                                    <div className="flex items-center justify-between">
                                        <div className="p-3 bg-white/10 rounded-2xl">
                                            <History size={24} className={uploadId ? "animate-spin" : ""} />
                                        </div>
                                        <div className="text-right">
                                            <p className="text-[10px] font-black uppercase tracking-widest text-white/40">Status</p>
                                            <p className="font-black text-xl tracking-tight capitalize">{progress?.status || 'Processing'}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between text-xs font-black uppercase tracking-widest text-white/60">
                                            <span>Progress</span>
                                            <span>{Math.round(((progress?.processed || 0) / (progress?.total || 1)) * 100)}%</span>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 pt-4">
                                        <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                                            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/30">Processed</p>
                                            <p className="text-2xl font-black mt-1">{progress?.processed || 0}</p>
                                        </div>
                                        <div className="bg-white/5 p-4 rounded-2xl border border-white/5">
                                            <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/30">Issues</p>
                                            <p className="text-2xl font-black mt-1 text-red-400">{progress?.failed || 0}</p>
                                        </div>
                                    </div>
                                    ...
                                    {progress?.status === "completed" && (
                                        <div className="flex items-center gap-3 p-4 bg-emerald-500/20 text-emerald-400 rounded-2xl border border-emerald-500/20">
                                            ...
                                            <p className="text-xs font-bold leading-tight">Database synchronization finalized successfully.</p>
                                        </div>
                                    )}
                                </div>
                            ) : (
                                <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 text-center space-y-6 flex flex-col items-center justify-center">
                                    ...
                                    <div>
                                        <h4 className="text-xl font-black text-slate-900 tracking-tight">Active Operation Monitor</h4>
                                        <p className="text-slate-400 text-sm font-medium mt-2 max-w-[240px] mx-auto">Real-time feedback on your background sync tasks will appear here.</p>
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 text-slate-900 animate-in slide-in-from-right-8 duration-500">
                    <div className="max-w-4xl mx-auto space-y-10">
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <h3 className="text-3xl font-black tracking-tight">Filtering Engine</h3>
                                <p className="text-slate-500 font-medium">Export selective student cohorts with specific data fields.</p>
                            </div>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Branch Focus</label>
                                <select
                                    value={exportParams.branch}
                                    onChange={(e) => setExportParams({ ...exportParams, branch: e.target.value })}
                            ...
                            <div className="md:col-span-2 space-y-3">
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Data Columns (Comma Separated)</label>
                                <input
                                    type="text"
                                    value={exportParams.fields}
                                    onChange={(e) => setExportParams({ ...exportParams, fields: e.target.value })}
                                    className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-slate-900/5 outline-none font-bold text-slate-900"
                                />
                            </div>
                        </div>
                        ...
                        <div className="pt-6">
                            ...
                            <p className="text-center text-slate-400 text-xs font-bold uppercase tracking-widest mt-6 italic">Secure export. All activity is logged in the system audits.</p>
                        </div>
                    </div>
                </div>
            )}
