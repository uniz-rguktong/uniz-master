...
export default function UploadHealthAnalytics({
  hideHeader = false,
}: {
  hideHeader?: boolean;
}) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (
        ...
        console.log(
          "Upload health analytics fetching skipped on non-production host:",
  ...
  const delta =
    Number(latest?.success_rate_percent) -
    Number(previous?.success_rate_percent);

  const highValue = Math.max(
    ...filteredData.map((d) => Number(d.success_rate_percent) || 0),
  );
  const lowValue = Math.min(
    ...filteredData.map((d) => Number(d.success_rate_percent) || 0),
  );

  const totalSyncs = filteredData.reduce(
  ...
  const avgSuccess =
    filteredData.length > 0
      ? filteredData.reduce(
          (acc, curr) => acc + (Number(curr?.success_rate_percent) || 0),
          0,
        ) / filteredData.length
      : 0;
              ...
              <div className="flex items-center gap-4">
                ...
                {delta !== 0 && (
                  ...
                  >
                    <TrendingUp
                      size={14}
                      className={cn(delta < 0 && "rotate-180")}
                    />
                    <span>
                      {delta > 0 ? "+" : ""}
                      {delta.toFixed(1)}%
