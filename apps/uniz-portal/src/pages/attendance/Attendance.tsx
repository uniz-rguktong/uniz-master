  ...
  return (
    <div className="min-h-screen bg-white font-sans text-black">
      <div className="max-w-6xl mx-auto px-4 pt-8 pb-4">
        <h1 className="text-3xl font-black tracking-tighter text-black mb-2">Attendance</h1>
        <p className="text-neutral-500 font-medium text-sm">Track your daily attendance and semester progress.</p>
      </div>
      <div className="max-w-6xl mx-auto px-4 py-4">
        {/* Selection Criteria */}
        <div className="mb-8 bg-white p-6 rounded-3xl border border-neutral-200 shadow-sm">
          <h2 className="text-lg font-bold mb-6 flex items-center gap-2">
            <div className="p-1.5 bg-black rounded-md text-white"><CheckCircle size={16} /></div>
            Select Criteria
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="relative group">
              <label className="text-[10px] font-bold uppercase tracking-widest text-neutral-500 mb-1.5 block ml-0.5">Academic Year</label>
              <div
              ...
              {showDropdown && (
                <div className="absolute z-20 mt-1 w-full bg-white border border-neutral-200 rounded-lg shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                  {years.map((year) => (
                    <div
                      key={year}
                      className={`p-3 cursor-pointer text-sm font-medium hover:bg-neutral-50 transition-colors ${selectedYear === year ? 'bg-black text-white hover:bg-black' : 'text-neutral-600'
                        }`}
                      onClick={() => {
            ...
            <div>
              <label className="text-[10px] font-bold uppercase tracking-widest text-neutral-500 mb-1.5 block ml-0.5">Semester</label>
              <div className="relative">
                ...
                <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-neutral-500" size={16} />
              </div>
            </div>
            ...
            <div className="flex items-end">
              <button
                ...
                className={`w-full h-[46px] flex items-center justify-center font-bold text-sm rounded-lg transition-all duration-300 shadow-md hover:shadow-lg hover:-translate-y-0.5 ${isLoading ? 'bg-neutral-100 text-neutral-400 cursor-not-allowed shadow-none' : 'bg-black text-white hover:bg-neutral-800'
                  }`}
              ...
              >
                {isLoading ? (
                  ...
                  <span className="flex items-center gap-1.5">View Attendance</span>
                )}
              </button>
            </div>
          </div>
        ...
        {resultsFetched && attendanceData && attendanceData.success && !isLoading && (
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-8 duration-500">
            ...
            <div className="flex items-end justify-between border-b pb-2 border-neutral-200">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-widest text-neutral-400 block mb-0.5">Attendance For</span>
                <h2 className="text-xl font-black tracking-tight">{selectedYear} - {selectedSemester}</h2>
              </div>
            </div>

            {/* Content */}
            {(!attendanceData.attendance || !Array.isArray(attendanceData.attendance) || attendanceData.attendance.length === 0) ? (
              <div className="bg-neutral-50 rounded-xl p-8 text-center border border-neutral-200">
                ...
                <h3 className="text-lg font-bold mb-2">Attendance Not Available</h3>
              ...
              <div className="glass-panel rounded-xl overflow-hidden border border-neutral-200 shadow-sm mt-4">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-neutral-200 bg-neutral-50/50">
                        <th className="px-4 py-2 text-left text-[10px] font-bold uppercase tracking-widest text-neutral-500">Subject</th>
                        <th className="px-4 py-2 text-center text-[10px] font-bold uppercase tracking-widest text-neutral-500">Total Classes</th>
                        <th className="px-4 py-2 text-center text-[10px] font-bold uppercase tracking-widest text-neutral-500">Attended</th>
                        <th className="px-4 py-2 text-center text-[10px] font-bold uppercase tracking-widest text-neutral-500">Percentage</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-neutral-100">
                      {Array.isArray(attendanceData.attendance) && attendanceData.attendance.map((record, index) => (
                        <tr key={index} className="group hover:bg-neutral-50 transition-colors">
                          <td className="px-4 py-2 font-bold text-xs text-neutral-900">
                            {typeof record.subject === 'string' ? record.subject : record.subject?.name || 'Unknown Subject'}
                          </td>
                          <td className="px-4 py-2 text-center text-neutral-600 font-medium text-xs">{record.totalClasses}</td>
                          <td className="px-4 py-2 text-center text-neutral-600 font-medium text-xs">{record.attendedClasses}</td>
                          <td className="px-4 py-2 text-center">
                            <span className={`inline-block px-2 py-0.5 rounded-md text-[10px] font-bold ${parseFloat(record.attendancePercentage) >= 75
                              ? 'bg-black text-white'
                              : parseFloat(record.attendancePercentage) >= 65
                ...
                <div className="px-6 py-4 bg-neutral-900 text-white mt-0">
                  <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    <div>
                      <h3 className="text-lg font-black">Overall Attendance</h3>
                    </div>
                    {/* Summary calculation if available, or just a placeholder */}
                    <div className="flex items-center gap-6">
                      <div className="text-right flex items-center gap-2">
                        <p className="text-neutral-400 font-bold text-[10px] uppercase tracking-widest">Calculated Avg :</p>
                        <p className="text-2xl font-black text-white">
                          {(attendanceData.attendance.reduce((acc, curr) => acc + (parseFloat(curr?.attendancePercentage || '0') || 0), 0) / (attendanceData.attendance.length || 1)).toFixed(1)}%
                        </p>
                      </div>
                    </div>
                  </div>
