                                ...
                                <table className="w-full text-sm text-left">
                                    <thead className="bg-white text-neutral-400 border-b border-neutral-50 text-[10px] uppercase tracking-wider font-bold">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 font-bold text-center">Credit</th>
                                    ...
                                    <tbody className="divide-y divide-neutral-50">
                                        {gradesBySemester[sem].map((g: Grade) => (
                                            <tr key={g.id} className="hover:bg-neutral-50/50 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-center text-neutral-500">{g.subject.credits}</td>
                                                <td className="px-4 py-3 text-right font-bold text-black">{g.grade}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <div className="bg-white border border-neutral-100 rounded-xl overflow-hidden shadow-sm h-fit">
                                ...
                                <div className="divide-y divide-neutral-50">
                                    {attendanceBySemester[sem].map((att: Attendance) => {
                                        ...
                                        return (
                                            <div key={att.id} className="px-4 py-3 flex items-center justify-between hover:bg-neutral-50/50 transition-colors">
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-2 h-2 rounded-full ${isLow ? 'bg-red-500' : 'bg-green-500'}`}></div>
                                                    <span className="font-medium text-sm text-neutral-900">{att.subject.name}</span>
                                                </div>
                                                <div className="text-right">
                                                    <span className={`font-bold text-sm block ${isLow ? 'text-red-600' : 'text-black'}`}>{percentage}%</span>
