        ...
        <div key={sem} className="space-y-10 group/sem">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-1.5 h-10 bg-zinc-900 rounded-full" />
              <div>
          ...
          <div className="grid lg:grid-cols-2 gap-10">
            ...
            {gradesBySemester[sem] && (
              <div className="space-y-4">
                ...
                <div className="bg-white border border-zinc-100 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl hover:shadow-zinc-200/40 transition-all duration-700">
                  <div className="divide-y divide-zinc-50">
                    {gradesBySemester[sem].map((g: Grade) => (
                      <div
                        key={g.id}
                        className="p-6 md:p-8 flex items-center justify-between group hover:bg-zinc-50/50 transition-colors"
                      >
                        <div className="flex flex-col gap-1.5 min-w-0">
                          <span className="text-base font-semibold text-zinc-900 group-hover:text-zinc-900 transition-colors tracking-tight leading-none">
                            {g.subject.name}
                          </span>
                        ...
                        <div className="flex items-center gap-6 shrink-0">
                          <div
                            className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-semibold ${g.grade === 0 ? "bg-rose-50 text-rose-500" : "bg-zinc-900 text-white shadow-xl shadow-zinc-100"}`}
                          >
                            {g.grade === 0 ? "F" : g.grade}
                          </div>
                        </div>
                  ...
                  <div className="divide-y divide-zinc-100">
                    {attendanceBySemester[sem].map((att: Attendance) => {
                      ...
                      return (
                        <div
                          key={att.id}
                          className="p-6 md:p-8 flex flex-col gap-5 hover:bg-zinc-50/50 transition-colors"
                        >
                          <div className="flex items-center justify-between gap-4">
                            ...
                            <span
                              className={`text-xl font-semibold tabular-nums tracking-tighter ${isLow ? "text-rose-500" : "text-zinc-900"}`}
                            >
                              {percentage}%
                            </span>
                          </div>
                          <div className="w-full h-2 bg-zinc-100 rounded-full overflow-hidden">
                            <motion.div
                              ...
                              className={`h-full rounded-full ${isLow ? "bg-rose-500" : "bg-zinc-900"}`}
                            />
                          </div>
                        </div>
                      );
