        ...
        <div key={sem} className="space-y-10 group/sem">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-2 px-4 py-2 bg-slate-50 border border-slate-100 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
              Performance Score:{" "}
              <span className="text-navy-900">High Resolution</span>
            </div>
          </div>
                ...
                <div className="bg-white border border-slate-100 rounded-[32px] overflow-hidden shadow-sm hover:shadow-xl hover:shadow-slate-200/40 transition-all duration-700">
                  <div className="divide-y divide-slate-50">
                    {gradesBySemester[sem].map((g: Grade) => (
                      <div
                        key={g.id}
                        className="p-6 md:p-8 flex items-center justify-between group hover:bg-slate-50/50 transition-colors"
                      >
                        ...
                        <div className="flex items-center gap-6 shrink-0">
                          <div
                            className={`w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-black ${g.grade === 0 ? "bg-rose-50 text-rose-500" : "bg-navy-900 text-white shadow-xl shadow-navy-100"}`}
                          >
                            {g.grade === 0 ? "F" : g.grade}
                          </div>
                        </div>
                  ...
                  <div className="divide-y divide-slate-100">
                    {attendanceBySemester[sem].map((att: Attendance) => {
                      ...
                      return (
                        <div
                          key={att.id}
                          className="p-6 md:p-8 flex flex-col gap-5 hover:bg-slate-50/50 transition-colors"
                        >
                          <div className="flex items-center justify-between gap-4">
                            ...
                            <span
                              className={`text-xl font-black tabular-nums tracking-tighter ${isLow ? "text-rose-500" : "text-navy-900"}`}
                            >
                              {percentage}%
                            </span>
                          </div>
