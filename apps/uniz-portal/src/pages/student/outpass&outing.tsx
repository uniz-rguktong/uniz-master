...
import { Clock, Calendar, AlertCircle, Plus, CheckCircle, XCircle, Loader2 } from "lucide-react";
...
export default function Outpass_Outing({ request }: requestProps) {
    ...
    const fetchHistory = async () => {
        try {
            ...
            const response = await fetch("https://api.uniz.rguktong.in/api/v1/requests/history", requestOptions);
    ...
    const pendingCount = requests.filter(r => !r.isApproved && !r.isRejected && !r.isExpired).length;

    const getStatusParams = (req: any) => {
        if (req.isApproved || req.is_approved) return { label: 'Approved', color: 'bg-green-100 text-green-700', icon: <CheckCircle className="w-4 h-4" /> };
        if (req.isRejected || req.is_rejected) return { label: 'Rejected', color: 'bg-red-100 text-red-700', icon: <XCircle className="w-4 h-4" /> };
        if (req.isExpired || req.is_expired) return { label: 'Expired', color: 'bg-gray-100 text-gray-700', icon: <Clock className="w-4 h-4" /> };
        return { label: 'Pending', color: 'bg-yellow-100 text-yellow-700', icon: <Clock className="w-4 h-4 animate-pulse" /> };
    };
            ...
            <div className="space-y-6">
                {requests.length === 0 ? (
                    <div className="text-center py-20 bg-white rounded-[2rem] border border-neutral-100 border-dashed">
                        ...
                        <h3 className="text-xl font-bold text-black mb-2">No Requests Found</h3>
                        <p className="text-neutral-500 font-medium">You haven't made any {request} requests yet.</p>
                    </div>
                ) : (
                    <>
                        <div className="grid gap-6 md:grid-cols-2">
                            {requests.map((req: any) => {
                                ...
                                return (
                                    <div key={req._id} className="bg-white rounded-2xl p-4 border border-neutral-200 shadow-sm hover:shadow-md transition-all group">
                                        <div className="flex items-start justify-between mb-3">
                                            <div className="flex items-center gap-3">
                                                <div className="p-2.5 bg-neutral-100 rounded-xl group-hover:bg-black group-hover:text-white transition-colors">
                                                    {request === 'outing' ? <Clock className="w-5 h-5" /> : <Calendar className="w-5 h-5" />}
                                                </div>
                                                <div>
                                                    <h5 className="font-bold text-black text-base leading-tight">
                                                        {request === 'outing' ? 'Outing' : 'Outpass'}
                                                    </h5>
                                                    <p className="text-xs text-neutral-500 font-medium mt-0.5">ID: {req.studentId?.slice(-6) || 'N/A'}</p>
                                                </div>
                                            </div>
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex items-center gap-2 text-sm bg-neutral-50 p-2.5 rounded-lg border border-neutral-100">
                                                <div className="flex-1">
                                                    <p className="text-xs text-neutral-400 font-bold uppercase tracking-wider mb-0.5">Time Window</p>
                                                    <p className="font-bold text-neutral-800 text-xs sm:text-sm">
                                                        {request === 'outing' && <span className="mr-1.5 text-neutral-500">{dateStr} •</span>}
                                                        {timeRange}
                                                    </p>
                                                </div>
                                                {durationText && (
                                                    <div className="text-right border-l border-neutral-200 pl-2.5 min-w-[50px]">
                                                        <p className="text-xs text-neutral-400 font-bold uppercase tracking-wider mb-0.5">Dur.</p>
                                                        <p className="font-bold text-black">{durationText}</p>
                                                    </div>
                                                )}
                                            </div>

                                            <div>
                                                <p className="text-xs text-neutral-400 font-bold uppercase tracking-wider mb-1">Reason</p>
                                                <p className="text-sm font-medium text-neutral-700 line-clamp-1">{req.reason}</p>
                                            </div>
                                        </div>
                                    </div>
                                );
