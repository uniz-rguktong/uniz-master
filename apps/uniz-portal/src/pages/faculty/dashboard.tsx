...
export default function FacultyDashboard() {
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC]">
      {/* ── Sidebar ── */}
      <aside
        className={`bg-white border-r border-zinc-100 transition-all duration-300 z-50 ${
          isSidebarOpen ? "w-72" : "w-20"
        } hidden md:flex flex-col h-screen sticky top-0 shadow-sm`}
      >
        <div className="p-6 flex items-center gap-4 border-b border-zinc-50 shrink-0">
          <div className="w-12 h-12 bg-black rounded-xl flex items-center justify-center shrink-0 shadow-lg">
            <BookOpen className="w-6 h-6 text-white" />
          </div>
          {isSidebarOpen && (
            <div className="flex flex-col animate-in fade-in slide-in-from-left-2 duration-300">
              <span className="font-bold text-zinc-900 text-lg tracking-tight leading-none">
                Faculty Portal
              </span>
              <span className="text-[9px] uppercase tracking-[0.25em] text-zinc-400 font-bold mt-1.5 leading-none">
        ...
        <nav className="flex-1 p-3 space-y-1 mt-4 overflow-y-auto">
          {navItems.map((item) => (
            ...
            >
              <item.icon
                size={20}
                className={`shrink-0 ${activeTab === item.id ? "text-white" : "text-zinc-400"}`}
              />
              {isSidebarOpen && (
                <span
              ...
              {isSidebarOpen && activeTab === item.id && (
                <div className="ml-auto w-1.5 h-1.5 rounded-full bg-zinc-400 animate-pulse" />
              )}
            </button>
          ))}
        </nav>

        <div className="p-4 border-t border-zinc-50 shrink-0">
          <button
      ...
      <main className="flex-1 overflow-y-auto max-h-screen">
        {/* Header */}
        <header className="bg-white/80 backdrop-blur-md sticky top-0 z-40 border-b border-zinc-100 h-[72px] px-8 flex justify-between items-center">
          <button
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className="p-2.5 hover:bg-zinc-100 rounded-xl transition-all hidden md:flex text-zinc-400"
          >
          ...
          <div className="flex items-center gap-4 ml-auto">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-[0.14em] mt-1.5">
                {isHOD ? "Head of Department" : "Faculty"} ·{" "}
                {profile?.Department}
              </p>
            </div>
            <div className="w-11 h-11 rounded-xl bg-black flex items-center justify-center font-bold text-white shadow-sm overflow-hidden">
              {profile?.ProfileUrl ? (
                <img
...
function OverviewSection({
  ...
  return (
    <div className="p-8 space-y-8 pb-20 animate-in fade-in duration-500">
      {/* Hero Card */}
      <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 rounded-3xl py-8 px-10 text-white shadow-xl relative overflow-hidden">
        <div className="relative z-10 space-y-3">
          {isHOD && (
            <span className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 rounded-full border border-white/10 text-[9px] font-bold uppercase tracking-[0.14em] text-zinc-300">
      ...
      {/* Quick Actions */}
      <div>
        <h2 className="text-xs font-bold uppercase tracking-[0.14em] text-zinc-400 mb-5">
          Quick Access
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[
            ...
            <motion.button
              ...
              whileHover={{ y: -3 }}
              onClick={() => setActiveTab(item.id as ActiveTab)}
              className="group relative flex flex-col items-start p-6 bg-white border border-zinc-100 rounded-2xl hover:shadow-xl hover:border-black/10 transition-all text-left"
            >
              <div className="p-3 rounded-xl bg-zinc-50 group-hover:bg-black group-hover:text-white transition-colors duration-300 mb-4">
      ...
      {!loading && profile && (
        <div className="bg-white rounded-2xl border border-zinc-100 p-8 shadow-sm">
          <h2 className="text-sm font-bold text-zinc-400 uppercase tracking-[0.14em] mb-6 flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              ...
              <div key={f.label} className="space-y-1.5">
                <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-[0.14em] flex items-center gap-1.5">
  ...
  return (
    <div className="p-8 pb-20 space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        ...
        {!isEditing ? (
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-black text-white rounded-xl text-sm font-bold hover:bg-zinc-800 transition-all shadow-sm"
          >
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 bg-zinc-100 text-zinc-600 rounded-xl text-sm font-bold hover:bg-zinc-200 transition-all"
            >
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex items-center gap-2 px-5 py-2.5 bg-black text-white rounded-xl text-sm font-bold hover:bg-zinc-800 transition-all disabled:opacity-50"
            >
              {saving ? (
      ...
      <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm overflow-hidden">
        {/* Profile header */}
        <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 p-8 flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-zinc-400 text-sm mt-1 uppercase tracking-[0.14em] font-bold">
              {profile.Designation || "Faculty"} · {profile.Department}
            </p>
            {profile.Role?.toLowerCase() === "hod" && (
              <span className="inline-flex items-center gap-1.5 mt-3 px-3 py-1 bg-amber-500/20 text-amber-300 border border-amber-500/30 rounded-full text-[10px] font-bold uppercase tracking-[0.14em]">
          ...
          <div className="space-y-2">
            ...
            {isEditing ? (
              <input
                ...
                className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl font-semibold text-zinc-900 outline-none focus:ring-4 focus:ring-black/5 focus:border-black transition-all"
              />
            ) : (
              <p className="font-semibold text-zinc-900 text-base">
      ...
      <div className="bg-white rounded-2xl border border-zinc-100 shadow-sm overflow-hidden p-8 space-y-8">
        <div className="flex items-center gap-3">
          ...
          {[
            ...
            return (
              <div key={key} className="space-y-3">
                <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-[0.14em] block">
                  {key}
                </label>
                {isEditing ? (
                  <textarea
                    ...
                    rows={4}
                    className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl font-medium text-zinc-900 outline-none focus:ring-4 focus:ring-black/5 focus:border-black transition-all text-sm leading-relaxed"
                  />
                ) : values.length > 0 ? (
                  <ul className="space-y-2">
                    {values.map((v: string, i: number) => (
                      <li
                        key={i}
                        className="text-zinc-600 text-sm flex items-start gap-2 leading-relaxed"
                      >
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex bg-zinc-100 p-1 rounded-full">
          <button
            ...
            className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-[0.14em] transition-all ${searchMode === "id" ? "bg-white text-black shadow" : "text-zinc-500 hover:text-black"}`}
          >
            By ID
          </button>
          <button
            ...
            className={`px-6 py-2 rounded-full text-[10px] font-bold uppercase tracking-[0.14em] transition-all ${searchMode === "filter" ? "bg-white text-black shadow" : "text-zinc-500 hover:text-black"}`}
          >
            By Filter
          </button>
        </div>
        ...
        >
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full h-12 pl-12 pr-5 bg-white border border-zinc-200 rounded-full focus:ring-4 focus:ring-black/5 focus:border-black outline-none transition-all font-semibold text-zinc-900 text-sm shadow-sm"
            />
          </div>
          <button
            disabled={loading}
            type="submit"
            className="h-12 px-8 bg-black text-white rounded-full font-bold uppercase tracking-[0.14em] text-[10px] hover:bg-zinc-800 transition-all flex items-center gap-2 disabled:opacity-50"
          >
            {loading ? (
        ...
        <div className="flex flex-wrap gap-4 items-center">
          <div className="relative">
            ...
            <select
              ...
              className="bg-white border border-zinc-200 pl-10 pr-8 h-12 rounded-full font-bold text-[11px] uppercase tracking-[0.14em] text-zinc-600 outline-none focus:ring-4 focus:ring-black/5 focus:border-black shadow-sm min-w-[160px] appearance-none cursor-pointer"
            >
              {departments.map((d) => (
          ...
          <div className="relative">
            <select
              ...
              className="bg-white border border-zinc-200 pl-4 pr-8 h-12 rounded-full font-bold text-[11px] uppercase tracking-[0.14em] text-zinc-600 outline-none focus:ring-4 focus:ring-black/5 focus:border-black shadow-sm min-w-[140px] appearance-none cursor-pointer"
            >
      ...
      >
        <div className={`space-y-3 ${selectedStudent ? "lg:col-span-5" : ""}`}>
          {searchResults.length > 0 && (
            <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-zinc-400 px-2">
          ...
          {searchResults.map((std) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`w-11 h-11 rounded-2xl flex items-center justify-center transition-colors font-bold uppercase ${selectedStudent?.username === std.username ? "bg-black text-white" : "bg-zinc-50 text-zinc-400 group-hover:bg-black group-hover:text-white"}`}
                >
                  {std.name?.[0] || "S"}
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[9px] font-bold uppercase tracking-[0.14em] text-black bg-zinc-100 px-2 py-0.5 rounded-md">
                      {std.username}
                    </span>
              ...
              <ChevronRight
                size={16}
                className={`text-zinc-300 transition-transform ${selectedStudent?.username === std.username ? "rotate-90 text-black" : ""}`}
              />
            </div>
          ))}

          {searchResults.length === 0 && !loading && (
            <div className="p-20 flex flex-col items-center gap-4 bg-white rounded-2xl border border-zinc-100 text-center">
              <div className="p-5 bg-zinc-50 rounded-2xl border border-zinc-100">
                <Users size={36} className="text-zinc-300" />
              </div>
              <div>
                <p className="font-bold text-zinc-900">No students yet</p>
        ...
        <AnimatePresence>
          {selectedStudent && (
            ...
            >
              {detailLoading ? (
                <div className="p-8 bg-white rounded-2xl border border-zinc-100 animate-pulse space-y-4">
                    ...
                    <div className="flex items-center gap-5">
                      <div className="w-16 h-16 rounded-2xl bg-white/10 border-2 border-white/20 flex items-center justify-center">
                        {selectedStudent.profile_url ? (
                          ...
                          <span className="text-3xl font-semibold text-white uppercase opacity-40">
                      ...
                      <div>
                        ...
                        <p className="text-zinc-400 text-sm font-bold uppercase tracking-[0.14em] mt-1">
                          {selectedStudent.username}
                        </p>
                        <span
                          className={`inline-block mt-2 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-[0.14em] ${selectedStudent.is_suspended ? "bg-red-500/20 text-red-300" : "bg-emerald-500/20 text-emerald-300"}`}
                        >
                  ...
                  <div className="p-8 grid grid-cols-2 gap-6 text-sm">
                    {[
                      ...
                      <div key={k} className="space-y-1">
                        <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-[0.14em]">
                          {k}
                        </p>
      ...
      <div className="flex items-center gap-3 mb-8">
        {(["current", "otp", "new"] as PwdStep[]).map((s, i) => (
          <div key={s} className="flex items-center gap-3">
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${step === s ? "bg-black text-white" : i < ["current", "otp", "new"].indexOf(step) ? "bg-zinc-200 text-zinc-500" : "border-2 border-zinc-200 text-zinc-300"}`}
            >
              {i + 1}
            </div>
            <span
              className={`text-sm font-medium hidden sm:block ${step === s ? "text-zinc-900" : "text-zinc-400"}`}
            >
              {stepLabels[s]}
            </span>
            {i < 2 && (
          ...
          {step === "current" && (
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="px-4 py-3 bg-zinc-50 border border-zinc-100 rounded-xl font-bold text-zinc-900">
                  {username || "Not found"}
                </div>
              </div>
              <div className="space-y-3 pt-2">
                <button
                  ...
                  className="w-full h-12 bg-black text-white rounded-xl font-bold text-sm hover:bg-zinc-800 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
                ...
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className="w-full h-12 bg-zinc-900 text-white rounded-xl font-bold text-sm hover:bg-black transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {loading && <Loader2 className="w-4 h-4 animate-spin" />}
                    Change Password
                  </button>
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full px-4 py-4 bg-zinc-50 border border-zinc-200 rounded-xl font-bold text-2xl text-center text-zinc-900 outline-none focus:ring-4 focus:ring-black/5 focus:border-black transition-all tracking-[0.14em]"
                />
              </div>
              <div className="flex gap-3 pt-2">
                <button
                  onClick={() => setStep("current")}
                  className="flex-1 h-12 bg-zinc-100 text-zinc-600 rounded-xl font-bold text-sm hover:bg-zinc-200 transition-all"
                >
                  Back
                </button>
                <button
                  ...
                  className="flex-[2] h-12 bg-black text-white rounded-xl font-bold text-sm hover:bg-zinc-800 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  {loading && <Loader2 className="w-4 h-4 animate-spin" />}
                  Verify OTP
                </button>
              ...
              <button
                onClick={requestOtp}
                disabled={loading}
                className="w-full text-center text-xs text-zinc-400 hover:text-zinc-900 font-medium transition-colors"
              >
                Resend OTP
              </button>
            </motion.div>
