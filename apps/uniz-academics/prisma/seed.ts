...
const subjectsData: any = {
  ...
  'E4': {
    ...
    'Sem - 2': {
      'CSE': {
        names: ["Elective-VI", "Open Elective-III", "Open Elective-IV", "Project-II", "Community Service"],
        credits: [3, 3, 3, 6, 2, 0, 0, 0, 0],
        hide: [6, 7, 8, 9]
      },
      'ECE': {
...
async function main() {
  ...
  try {
    ...
    console.log(" Deleted old academic records and subjects.");
  } catch (e) {
      ...
      Object.entries(branchData).forEach(
        ([branchKey, details]: [string, any]) => {
          ...
          names.forEach((name: string, index: number) => {
            ...
            allSubjects.push({
              id: code, // Using code as ID for stability
              code,
              name,
  ...
  // 4. Seed Faculty Profiles (Rich Data)
  ...
  for (const f of facultyProfiles) {
    // Clean data for Prisma (remove field "username", "contact" not in model)
    const { username, contact, role, ...cleanData } = f;
    const finalRole = (role === "teacher" || !role) ? "FACULTY" : role;

    await (prisma as any).faculty.upsert({
      where: { email: cleanData.email },
      update: { ...cleanData, role: finalRole },
      create: { ...cleanData, role: finalRole },
    });
  }
