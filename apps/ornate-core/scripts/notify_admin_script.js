...
async function notifyAdmins(winner, runnerUp, sport, gender) {
    ...
    if (winnerBranch) branchesToNotify.push({ branch: winnerBranch, type: 'Winner', team: winner });
    if (runnerUpBranch) branchesToNotify.push({ branch: runnerUpBranch, type: 'Runner-up', team: runnerUp });
    ...
    for (const item of branchesToNotify) {
        ...
        if (admin) {
             ...
             console.log(`   -> Notification sent to ${item.branch} Admin (${admin.email})`);
        } else {
