 ...
 */
export function hasPermission(user: User, action: PermissionAction, resource?: any): boolean {
    ...
    switch (action) {
        ...
        case 'delete:event':
            ...
            if (role === ROLES.BRANCH_ADMIN && resource.creator?.branch === user.branch) return true;

            // Club Coordinator can edit/delete events from their club
            if (role === ROLES.CLUB_COORDINATOR && resource.creator?.clubId === user.clubId) return true;
        ...
        case 'manage:sport_registrations':
            return ([
                ROLES.BRANCH_ADMIN,
            ] as string[]).includes(role) || isMainSportsAdmin || isBranchSportsAdmin;
 ...
 */
export function assertPermission(user: User, action: PermissionAction, resource?: any) {
    if (!hasPermission(user, action, resource)) {
        throw new Error(`Unauthorized: You do not have permission to perform '${action}'.`);
    }
}
