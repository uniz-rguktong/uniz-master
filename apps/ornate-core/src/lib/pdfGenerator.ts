...
// ── Configuration ────────────────────────────────────────────────
const RENDER_TIMEOUT_MS = parseInt(process.env.PDF_RENDER_TIMEOUT_MS || '30000');
...
export function embedLocalImages(htmlContent: string): string {
    return htmlContent.replace(/src="((?:\.\/|\.\.\/|\/)[^"]+\.(png|jpg|jpeg|gif|svg|webp))"/gi, (match: string, imgPath: string, ext: string) => {
        try {
            ...
            logger.warn({ imgPath, err: error.message }, 'embedLocalImages: failed to embed image');
        }
        return match;
    });
}
