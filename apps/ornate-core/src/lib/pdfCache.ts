...
// ── Configuration ────────────────────────────────────────────────
const CACHE_DIR = process.env.PDF_CACHE_DIR || path.join(os.tmpdir(), 'ornate-pdf-cache');
...
function ensureCacheDir(): void {
    ...
    try {
        ...
        logger.error({ err, dir: CACHE_DIR }, 'pdfCache: failed to create cache directory');
    }
}
...
export function getCachedPdf(registrationId: string): Buffer | null {
    ...
    try {
        ...
        if (stat.size < MIN_VALID_SIZE) {
            logger.warn({ registrationId, size: stat.size }, 'cache.corrupt.size — removing');
