...
export function inlineImages(html: string): string {
    ...
    html = html.replace(/src="\.\/components\/([^"]+)"/g, (match: string, filename: string) => {
        const componentDir = path.join(process.cwd(), 'src', 'templates', 'components');
        const filePath = path.join(componentDir, filename);
        return inlineImageAtPath(filePath, match);
    });

    // Pattern 2: broader relative/absolute paths with image extensions
    html = html.replace(/src="((?:\.\/|\.\.\/|\/)[^"]+\.(png|jpg|jpeg|gif|svg|webp))"/gi, (match: string, imgPath: string, ext: string) => {
...
function inlineImageAtPath(absolutePath: string, fallback: string): string {
    ...
    try {
        ...
        logger.error({ path: absolutePath, err: e }, 'inlineImages: failed to inline');
        return fallback;
    }
}
