...
const SW_META_CACHE = "uniz-push-meta";
...
export async function syncSubscriptionWithBackend(
  ...
  const subscriptionJson =
    "toJSON" in subscription && typeof subscription.toJSON === "function"
      ? subscription.toJSON()
      : (subscription as { endpoint: string; keys?: Record<string, string> });
...
export async function syncSubscriptionToBackend(
  ...
  const targetRegistration = registration || (await navigator.serviceWorker.ready);
  ...
  await syncSubscriptionWithBackend(subscription, username, notificationServiceUrl);
  window.localStorage.setItem(PUSH_LAST_SYNC_KEY, username.trim().toLowerCase());
  return { synced: true };
}
...
export async function registerForPushNotifications(
  ...
  if (!username) return { subscribed: false, reason: "Username is required" };
  return initPushNotifications(username);
}
