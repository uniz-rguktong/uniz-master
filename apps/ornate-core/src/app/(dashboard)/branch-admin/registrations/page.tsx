...
import { deleteRegistration, announceWinnersFromRegistrations, assignWinnerPrizeAndAnnounce } from '@/actions/registrationActions';

interface BranchAdminRegistrationsPageProps {
    searchParams: Promise<Record<string, string | string[] | undefined>>;
}

export default async function Page({ searchParams }: BranchAdminRegistrationsPageProps) {
    const resolvedParams = await searchParams;
    const query = (Array.isArray(resolvedParams?.q) ? resolvedParams?.q[0] : resolvedParams?.q) || '';
    const page = Number(resolvedParams?.page) || 1;
    const limit = Number(resolvedParams?.limit) || 10;
    const status = (Array.isArray(resolvedParams?.status) ? resolvedParams?.status[0] : resolvedParams?.status) || 'all';
    const eventId = (Array.isArray(resolvedParams?.eventId) ? resolvedParams?.eventId[0] : resolvedParams?.eventId) || 'all';
    ...
    const teams = allRegsResult.data?.teams || [];
