        ...
        <div className="p-6 md:p-8 max-w-[1600px] mx-auto">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-3">
                            ...
                            <h1 className="text-2xl font-bold text-[#1A1A1A]">Live Operations</h1>
                        </div>
                        <p className="text-sm text-[#6B7280] ml-6">Real-time control center for currently active events.</p>
                    </div>
                </div>
            </div>
            ...
            {liveEvents.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-white border border-dashed border-gray-200 rounded-2xl">
                    <p className="text-gray-500 font-medium">No events are currently live.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {liveEvents.map((event: any) => (
                        <div key={event.id} className="bg-white border border-[#E5E7EB] rounded-[18px] p-6 shadow-sm relative overflow-hidden">
                            <div className="flex justify-between items-start mb-6">
                                ...
                                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${event.status === 'Running' ? 'bg-green-50 text-green-700 border-green-100' : 'bg-yellow-50 text-yellow-700 border-yellow-100'
                                    }`}>
                                    {event.status}
                                </span>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-6">
                                <div className="bg-gray-50 rounded-xl p-4 text-center">
                                    ...
                                    <div className="text-2xl font-bold text-[#1A1A1A]">{event.participants}</div>
