'use client';
import { Search, Filter, MoreVertical, Heart, Download, Trash2, Shield } from 'lucide-react';
...
const ALL_UPLOADS = [
    { id: 1, src: 'https://images.unsplash.com/photo-1511578314322-379afb476865?w=500&h=500&fit=crop', event: 'Inauguration', user: 'Pro Cam 1', status: 'Approved', likes: 120 },
    { id: 2, src: 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=500&h=500&fit=crop', event: 'Crowd Shot', user: 'Student Cam', status: 'Approved', likes: 85 },
    { id: 3, src: 'https://images.unsplash.com/photo-1505373877841-8d25f7d46678?w=500&h=500&fit=crop', event: 'Code Marathon', user: 'Branch Admin', status: 'Approved', likes: 45 },
    { id: 4, src: 'https://images.unsplash.com/photo-1543353071-873f17a7a088?w=500&h=500&fit=crop', event: 'Dance Comp', user: 'Pro Cam 2', status: 'Pending', likes: 0 },
    { id: 5, src: 'https://images.unsplash.com/photo-1501281668745-f7f57925c3b4?w=500&h=500&fit=crop', event: 'Decorations', user: 'Volunteer', status: 'Flagged', likes: 0 },
];
            ...
            <div className="flex flex-col gap-4 mb-8">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-[#6B7280]">Central repository of all event photos and videos.</p>
                    </div>
                    <div className="flex gap-2">
                        <button
                            onClick={() => showToast('Media upload modal will open here', 'info')}
                    ...
                    <div key={item.id} className="group relative break-inside-avoid">
                        <div className="relative aspect-4/5 bg-gray-100 rounded-xl overflow-hidden shadow-sm">
                            ...
                            <div className="absolute inset-0 bg-linear-to-t from-black/80 via-transparent to-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-4">
                                <div className="flex justify-between items-start">
                                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider
                                        ${item.status === 'Approved' ? 'bg-green-500 text-white' :
                                            item.status === 'Pending' ? 'bg-yellow-500 text-white' : 'bg-red-500 text-white'}`}>
                                        {item.status}
                                    </span>
                                ...
                                <div>
                                    <p className="text-white text-sm font-bold truncate">{item.event}</p>
                                    <p className="text-white/70 text-xs truncate">by {item.user}</p>
