...
export const POST = apiHandler(async (request: NextRequest) => {
  ...
  if (activeRegistrationsCount >= event.maxCapacity) {
    ...
    if (!currentUser) {
      ...
      if (admin) {
        return apiResponse(
          false,
          null,
          "Admins are not allowed to register for events",
          403,
        );
      }
  ...
  if (!userId) {
    const existingUser = await prisma.user.findFirst({
      where: {
        OR: [{ email: data.email }, { phone: data.phone || null }],
      },
    });
  ...
  const registration = await prisma.registration.create({
    data: {
      ...
      paymentStatus:
        event.price > 0 && registrationStatus !== "WAITLISTED"
          ? "PENDING"
          : "PAID",
    },
  });
