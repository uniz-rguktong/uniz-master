...
export async function GET(
    ...
    try {
        ...
        const isAdmin = session.user?.email
            ? await prisma.admin.findUnique({ where: { email: session.user.email }, select: { id: true } })
            : null;
        if (!isOwner && !isAdmin) {
            return NextResponse.json({ error: 'Forbidden: You can only download your own certificates' }, { status: 403 });
        }
        ...
        switch (registration.certificateType) {
            ...
            case 'MERIT_THIRD':
                ...
                break;
            default:
                if (registration.rank === 1) { templateName = 'gold.html'; rankText = '1st'; }
                else if (registration.rank === 2) { templateName = 'silver.html'; rankText = '2nd'; }
                else if (registration.rank === 3) { templateName = 'bronze.html'; rankText = '3rd'; }
                else { templateName = 'participation.html'; }
        }
        ...
        html = html
            ...
            .replace(/{{date}}/g, new Date(registration.event.date).toLocaleDateString())
