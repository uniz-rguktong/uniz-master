...
} from "@/lib/serverActionRateLimit";
...
function validateMagicBytes(buffer: Buffer, mimeType: string): boolean {
  ...
  if (mimeType === "image/heic" || mimeType === "image/heif") {
    ...
    return (
      brandMarker === "ftyp" &&
      ["heic", "heix", "hevc", "mif1", "msf1"].includes(majorBrand)
    );
  }
  ...
  try {
    ...
    if (contentTypeHeader.includes("multipart/form-data")) {
      ...
      if (!validateMagicBytes(fileBuffer, file.type)) {
        console.log(
          `[Upload] Mismatch detected for ${file.name}. Declared: ${file.type}. Sniffing...`,
        );
    ...
    return NextResponse.json(
      {
        error:
          "Upload must use multipart/form-data. JSON presigned URL requests are no longer supported.",
      },
      { status: 400 },
    );
  } catch (error: any) {
