    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 px-4 py-12">
            <div className="max-w-xl w-full text-center mb-10">
                <h1 className="text-4xl font-extrabold text-gray-900 mb-4 tracking-tight">Certificate Verification</h1>
                <p className="text-lg text-gray-600">
                    Enter your Certificate ID (UUID) or registerd Email Address to verify and download your certificates.
                </p>
            </div>
            ...
            <div className="max-w-xl w-full bg-white shadow-xl rounded-2xl p-8 border border-gray-100">
                ...
                {results.length > 0 && (
                    <div className="space-y-4">
                        ...
                        {results.map((reg: any) => (
                            <div key={reg.id} className="flex items-center justify-between p-4 bg-gray-50 hover:bg-indigo-50 rounded-lg border border-gray-100 transition-colors group">
                                <div className="text-left">
                                    <p className="font-bold text-gray-900">{reg.event.title}</p>
                                    <p className="text-sm text-gray-500">
                                        {new Date(reg.event.date).toLocaleDateString()} &bull; {reg.rank ? `Rank: ${reg.rank}` : 'Participation'}
                                    </p>
                                </div>
                                <Link
                                    href={`/verify/${reg.id}`}
