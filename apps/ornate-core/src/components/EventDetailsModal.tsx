import { MapPin, Calendar, Users, IndianRupee, Clock, Building2, Trophy, UserCheck, FileText, X } from 'lucide-react';
    ...
    return (
        <Modal
            ...
            <div className="space-y-6">
                ...
                {event.posterUrl && (
                    <div className="relative w-full h-48 rounded-xl overflow-hidden">
                        ...
                        <div className="absolute top-3 left-3 px-3 py-1.5 bg-white/90 backdrop-blur-sm rounded-lg">
                            <span className="text-xs font-bold text-[#1A1A1A]">{event.category}</span>
                        </div>
                    </div>
                )}
                ...
                {/* Basic Information */}
                <div>
                    <h3 className="text-xl font-bold text-[#1A1A1A] mb-4">{event.title}</h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-sm font-medium text-[#1A1A1A]">{formatDate(event.date)}</div>
                                </div>
                            </div>
                            <div className="flex items-start gap-3">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-sm font-medium text-[#1A1A1A]">{event.venue}</div>
                                </div>
                            </div>
                            <div className="flex items-start gap-3">
                                <Building2 className="w-5 h-5 text-[#6B7280] mt-0.5" />
                                <div>
                                    <div className="text-xs text-[#6B7280] mb-1">Location Type</div>
                                    <div className="text-sm font-medium text-[#1A1A1A] capitalize">{event.locationType || 'Physical'}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {event.description && (
                    <div>
                        <h4 className="text-sm font-semibold text-[#1A1A1A] mb-3">Description</h4>
                ...
                <div>
                    <h4 className="text-sm font-semibold text-[#1A1A1A] mb-3">Registration Details</h4>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-sm font-medium text-[#1A1A1A]">{event.maxCapacity} participants</div>
                                </div>
                            </div>
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-sm font-medium text-[#1A1A1A]">
                                        {event.registrationsCount || event._count?.registrations || 0}
                                    </div>
                                </div>
                            </div>
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div className="text-sm font-medium text-[#1A1A1A]">{event.fee || 'Free'}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-[10px] font-black text-[#1A1A1A]">
                                    {Math.round(((event.registrationsCount || event._count?.registrations || 0) / event.maxCapacity) * 100)}%
                                </span>
                            </div>
                ...
                {event.eventType && (
                    <div>
                        <h4 className="text-sm font-semibold text-[#1A1A1A] mb-3">Event Type</h4>
                        <div className="bg-[#F7F8FA] rounded-xl p-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <div className="text-xs text-[#6B7280] mb-1">Type</div>
                                    <div className="text-sm font-medium text-[#1A1A1A] capitalize">{event.eventType}</div>
                                </div>
                                {event.eventType === 'team' && event.teamSizeMin && event.teamSizeMax && (
                                    <div>
                ...
                {/* Coordinators */}
                {event.assignedCoordinators && Array.isArray(event.assignedCoordinators) && event.assignedCoordinators.length > 0 && (
                    <div>
                        <h4 className="text-sm font-semibold text-[#1A1A1A] mb-3">Event Coordinators</h4>
                        <div className="bg-[#F7F8FA] rounded-xl p-4 space-y-3">
                            {event.assignedCoordinators.map((coord: any) => (
                                <div key={coord.id} className="flex items-start gap-3 p-3 bg-white rounded-lg">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="text-xs text-[#6B7280] mt-1">{coord.email}</div>
                                        <div className="text-xs text-[#6B7280]">{coord.phone}</div>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {/* Eligibility */}
                {event.eligibility && Array.isArray(event.eligibility) && event.eligibility.length > 0 && (
                    <div>
                        <h4 className="text-sm font-semibold text-[#1A1A1A] mb-3">Eligibility Criteria</h4>
                        <div className="bg-[#F7F8FA] rounded-xl p-4">
                ...
                {event.prizes && (
                    <div>
                        <h4 className="text-sm font-semibold text-[#1A1A1A] mb-3">Prizes & Recognition</h4>
                        <div className="bg-[#F7F8FA] rounded-xl p-4">
                                ...
                                {event.prizes.first && (
                                    <div className="p-3 bg-gradient-to-br from-[#FEF3C7] to-[#FDE68A] rounded-lg">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs font-bold text-[#92400E]">1st Prize</span>
                                        </div>
                                        <div className="text-sm font-semibold text-[#92400E]">₹{event.prizes.first.amount}</div>
                                    </div>
                                )}
                                {event.prizes.second && (
                                    <div className="p-3 bg-gradient-to-br from-[#E5E7EB] to-[#D1D5DB] rounded-lg">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs font-bold text-[#1F2937]">2nd Prize</span>
                                        </div>
                                        <div className="text-sm font-semibold text-[#1F2937]">₹{event.prizes.second.amount}</div>
                                    </div>
                                )}
                                {event.prizes.third && (
                                    <div className="p-3 bg-gradient-to-br from-[#FED7AA] to-[#FDBA74] rounded-lg">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs font-bold text-[#7C2D12]">3rd Prize</span>
                                        </div>
                                        <div className="text-sm font-semibold text-[#7C2D12]">₹{event.prizes.third.amount}</div>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="bg-[#F7F8FA] rounded-xl p-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="flex items-center gap-2">
                                <div className={`w-2 h-2 rounded-full ${event.registrationOpen ? 'bg-[#10B981]' : 'bg-[#EF4444]'}`} />
                                <span className="text-sm text-[#1A1A1A]">
                                    Registration {event.registrationOpen ? 'Open' : 'Closed'}
                                </span>
                            </div>
                            <div className="flex items-center gap-2">
                                <div className={`w-2 h-2 rounded-full ${new Date(event.date) > new Date() ? 'bg-[#3B82F6]' : 'bg-[#6B7280]'}`} />
                                <span className="text-sm text-[#1A1A1A]">
