...
import { updateAdminProfile, updatePassword } from '@/actions/adminProfileActions';
...
export default function ProfilePage({ initialProfile, variant = 'admin', fullHeight = true }: ProfilePageProps) {
    ...
    const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (url) {
                ...
                showToast('Photo uploaded successfully. Save changes to keep it.', 'success');
            } else {
    ...
    const handleSave = async () => {
        ...
        try {
            if (activeTab === 'profile') {
                ...
                Object.entries(profileData).forEach(([key, value]) => formData.append(key, value as string));
    ...
    return (
        <div className={`p-4 md:p-8 animate-page-entrance ${fullHeight ? 'min-h-screen' : 'min-h-full'}`}>
            ...
            <div className="mb-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex-1">
                        <h1 className="text-2xl md:text-[28px] font-semibold text-[#1A1A1A] mb-2">Account Settings</h1>
                        <p className="text-sm text-[#6B7280]">Manage your profile and security settings.</p>
                    </div>
                </div>
            </div>
                    ...
                    <div className="bg-[#F4F2F0] rounded-[18px] p-2 flex flex-col gap-2">
                        {tabs.map((tab: any) => {
                            ...
                            return (
                                ...
                                >
                                    <Icon className={`w-4 h-4 ${activeTab === tab.id ? 'text-[#10B981]' : ''}`} />
                                    {tab.label}
                                </button>
                            );
                        })}
                                ...
                                <>
                                        ...
                                        <div className="w-24 h-24 md:w-32 md:h-32 rounded-3xl overflow-hidden border-4 border-[#F3F4F6] shadow-md group-hover:shadow-lg transition-all">
                                            {profileData.profilePicture ? (
                                                ...
                                                <div className="w-full h-full bg-gradient-to-br from-[#1A1A1A] to-[#4B5563] flex items-center justify-center text-white text-3xl md:text-4xl font-black">
                                                    {(profileData.firstName[0] || initialProfile?.name?.[0] || 'A').toUpperCase()}{(profileData.lastName[0] || '')?.toUpperCase()}
                                                </div>
                                            )}
                                        </div>
                                        <button
                                    ...
                                    <div className="flex-1 text-center md:text-left">
                                        <div className="flex flex-col md:flex-row md:items-center gap-3 mb-2 justify-center md:justify-start">
                                            <h2 className="text-xl md:text-2xl font-bold text-[#1A1A1A]">{profileData.firstName} {profileData.lastName}</h2>
                                            <span className="inline-flex w-fit mx-auto md:mx-0 px-2 py-0.5 bg-[#DBEAFE] text-[#3B82F6] rounded text-xs font-semibold uppercase tracking-wider border border-[#DBEAFE]/50">
                                ...
                                <div className="animate-page-entrance">
                                    ...
                                    {activeTab === 'profile' && (
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                                            ...
                                            <div className="space-y-2">
                                                <label className="text-sm font-medium text-[#1A1A1A]">Phone Reference</label>
                                            ...
                                            <div className="md:col-span-2 space-y-2">
                                                <label className="text-sm font-medium text-[#1A1A1A]">Institutional Bio</label>
                                                <textarea
                                                    rows={4}
                                                    name="bio"
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="relative">
                                                        <input
                                                            ...
                                                            className="w-full px-4 py-2.5 bg-white border border-[#E5E7EB] rounded-xl text-sm font-medium text-[#1A1A1A] focus:outline-none focus:ring-2 focus:ring-[#10B981] transition-all"
                                                        />
                                                        <button onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#9CA3AF] hover:text-[#1A1A1A] transition-colors">
