...
import type { Registration, TeamRegistration, ColumnDefinition } from '../types';
        ...
        <div className="bg-[#F4F2F0] rounded-[18px] px-[10px] py-[24px] animate-card-entrance">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-[16px] px-[12px] mt-[-4px] gap-4">
                <div className="flex items-center gap-2">
                    <h2 className="text-xs font-medium text-[#9CA3AF] uppercase tracking-wide text-[14px]">{title}</h2>
                            ...
                            <tr>
                                ...
                                {columns.map((col: any) => (
                                    <th
                                        ...
                                        className={`px-4 py-3 text-left text-[10px] font-medium text-[#9CA3AF] uppercase tracking-wider ${col.sortable !== false ? 'cursor-pointer hover:bg-[#F9FAFB]' : ''} transition-colors ${col.className || ''}`}
                                    >
                                        {col.label}
                                        {col.sortable !== false && getSortIndicator(col.key)}
                                    </th>
                                ))}
                                <th className="px-4 py-3 text-right text-[10px] font-medium text-[#9CA3AF] uppercase tracking-wider">
                                    Actions
                                </th>
                            </tr>
                        </thead>
                            ...
                            {isLoading ? (
                                [...Array(5)].map((_: any, i: any) => (
                                    <tr key={i} className="border-b border-[#F3F4F6]">
                                        <td className="px-4 py-4">{showSelection ? <Skeleton width={20} height={20} borderRadius={4} /> : null}</td>
                                        {columns.map((col: any) => (
                                            <td key={col.key} className="px-4 py-4"><Skeleton width={100} height={16} /></td>
                                        ))}
                                    ...
                                    >
                                        ...
                                        {columns.map((col: any) => (
                                            <td key={col.key} className={`px-4 py-4 ${col.className || ''}`}>
                                                {col.render ? col.render(registration) : (registration as any)[col.key]}
                                            </td>
                                        ))}
                                        <td className="px-4 py-4 text-right">
                                            <div className="flex items-center justify-end">
                                                <ActionMenu
                                                    actions={[
                                                        { label: 'View Details', icon: 'view', onClick: () => onViewDetails(registration) },
                                                        { label: 'Send Email', icon: 'send', onClick: () => onSendEmail(registration) },
                                                        { label: 'Delete', icon: 'delete', onClick: () => onDelete(registration.id), danger: true }
                                                    ]}
                                                />
                                            </div>
                                        </td>
                ...
                {registrations.length > 0 && (
                    <div className="px-6 py-4 border-t border-[#F3F4F6] flex items-center justify-between bg-white">
                        <div className="text-sm text-[#6B7280]">
                            Showing <span className="font-medium text-[#1A1A1A]">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-medium text-[#1A1A1A]">{(currentPage - 1) * itemsPerPage + registrations.length}</span> of <span className="font-medium text-[#1A1A1A]">{pagination.total}</span> results
                        </div>
                        <div className="flex items-center gap-2">
                            <button
