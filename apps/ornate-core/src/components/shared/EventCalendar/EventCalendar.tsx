...
import type { CalendarViewType, CalendarEvent, EventCalendarConfig } from './types';
...
interface EventCalendarProps {
    ...
    fetchEvents?: () => Promise<{ success: boolean; data?: CalendarEvent[]; error?: string }>;
}
...
export default function EventCalendar({
    ...
    const [viewType, setViewType] = useState<CalendarViewType>(config.defaultView || 'month');
        ...
        <div className="p-4 md:p-8 animate-page-entrance">
            <div className="mb-8">
                <div className="flex flex-wrap items-center gap-2 text-sm text-[#6B7280] mb-3">
                    <span>Dashboard</span>
                    <span>›</span>
                    <span className="text-[#1A1A1A] font-medium">{config.title || 'Event Calendar'}</span>
                </div>
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl md:text-[28px] font-semibold text-[#1A1A1A] mb-2">{config.title || 'Event Calendar'}</h1>
                        <p className="text-sm text-[#6B7280]">{config.description || 'View and manage scheduled events'}</p>
                    </div>
            ...
            <div className="bg-[#F4F2F0] rounded-[24px] p-2 mb-8 animate-card-entrance">
                <div className="flex flex-col lg:flex-row items-center justify-between gap-4 px-2 py-2">
                    <div className="flex flex-wrap items-center justify-center gap-2 md:gap-3">
                        ...
                        <div className="px-3 md:px-6 py-2 bg-white border border-[#E5E7EB] rounded-lg min-w-[140px] md:min-w-[200px] text-center font-bold text-base md:text-lg">
                    ...
                    <div className="flex items-center gap-1 bg-white border border-[#E5E7EB] rounded-xl p-1 w-full md:w-auto justify-center">
                        {(['month', 'week', 'list'] as CalendarViewType[]).map((view: any) => (
                            <button
                                key={view}
                                onClick={() => setViewType(view)}
                                className={`flex-1 md:flex-initial px-4 md:px-6 py-2 rounded-lg text-sm font-semibold transition-all capitalize ${viewType === view ? 'bg-[#1A1A1A] text-white shadow-md' : 'text-[#6B7280] hover:bg-[#F7F8FA]'
                                    }`}
                            >
                                {view}
                            </button>
