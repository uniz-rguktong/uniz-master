...
export function NotificationPopover({
  ...
  const [selectedNotification, setSelectedNotification] = useState<Record<
    string,
    any
  > | null>(null);
  ...
  const [selectedRecipient, setSelectedRecipient] = useState<Record<
    string,
    any
  > | null>(null);
  ...
  const notifications: Array<Record<string, any>> = (
    rawNotifications as Array<Record<string, any>>
  ).map((n) => ({
    ...n,
    from:
      filterType === "sent"
        ? `To: ${n.recipientName || "Recipient"}`
        : n.senderName,
    fromDepartment:
      filterType === "sent" ? n.recipientRole || "Recipient" : n.senderRole,
    departmentColor:
      n.type === "admin"
        ? "#8B5CF6"
        : n.type === "system"
            ...
            <div className="flex-1 overflow-y-auto min-h-[400px]">
                    ...
                    <div className="flex gap-2">
                      {["all", "unread", "starred", "sent"].map((type: any) => (
                        <button
                          ...
                          className={`px-2.5 py-1 rounded text-xs font-medium capitalize border transition-colors ${
                            filterType === type
                      ...
                      filteredNotifications.map((notif: any) => (
                        ...
                        >
                          <div className="flex justify-between items-start mb-1">
                            <span
                              className={`text-sm truncate pr-6 ${!notif.isRead ? "font-bold text-[#1A1A1A]" : "font-medium text-[#4B5563]"}`}
                            >
                              {notif.from}
                            </span>
                          ...
                          <div className="absolute right-2 bottom-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            >
                              <Star
                                className={`w-3.5 h-3.5 ${notif.isStarred ? "text-[#EAB308] fill-[#EAB308]" : "text-[#9CA3AF]"}`}
                              />
                            </button>
                            <button
              ...
              {view === "detail" && selectedNotification && (
                <div className="p-5">
                  ...
                  <h4 className="font-bold text-[#1A1A1A] mb-3 text-base">
                    ...
                    <div className="mb-6 space-y-2">
                      ...
                      {selectedNotification.attachments.map(
                        (att: any, i: number) => (
                          <div
                            key={i}
                            className="flex items-center gap-3 p-2 bg-[#F9FAFB] border border-[#E5E7EB] rounded-lg"
                          >
                  ...
                  <div className="flex gap-2 mt-4 pt-4 border-t border-[#E5E7EB]">
                    ...
                    >
                      ...
                      {filterType === "sent" ? "Message Again" : "Reply"}
                    </button>
                    <button
