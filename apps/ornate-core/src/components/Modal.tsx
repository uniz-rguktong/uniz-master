...
export function Modal({
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-9999" style={{ zIndex: 9999 }}>
      <div className={`bg-[#F4F2F0] rounded-[24px] w-full ${sizeClasses[size]} shadow-[0px_25px_50px_-12px_rgba(0,0,0,0.25)] flex flex-col max-h-[95vh] md:max-h-[90vh] animate-modal-entrance overflow-hidden border border-white/20 mx-2 md:mx-0`}>
            ...
            <div className="flex flex-col gap-1">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <h2 className="text-[18px] md:text-[22px] font-bold text-[#1A1A1A] tracking-tight truncate">{title}</h2>
                  {tooltipText &&
              ...
              {description && (
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-relaxed max-w-[80%]">
                  {description}
                </p>
              )}
            </div>
        ...
        <div className={`bg-[#F4F2F0] flex-1 overflow-hidden px-3 md:px-5 ${!modalFooter ? 'pb-5 md:pb-8' : 'pb-1 md:pb-2'}`}>
