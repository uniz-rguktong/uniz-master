  ...
  return (
    ...
    >
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-[20px] w-full px-5 py-4 md:px-6 md:py-5 shadow-sm">
        <div
          className={`flex items-center ${compact ? "gap-3 md:gap-4" : "gap-4 md:gap-[20px]"}`}
        >
          {/* Icon Container */}
          <div
          ...
          >
            <Icon
              className={`${compact ? "w-5 h-5 md:w-[22px] md:h-[22px]" : "w-6 h-6 md:w-[26px] md:h-[26px]"}`}
          ...
          <div className="flex-1 flex flex-col gap-[4px] min-w-0">
            <div className="flex items-center gap-2 min-w-0">
              <p
                className={`${compact ? "text-[12px] md:text-[13px]" : "text-[14px] md:text-[15px]"} font-medium text-[#6B7280] leading-none truncate`}
              >
                {title}
              </p>
              {(infoText || tooltip) && (
                <span className="shrink-0">
                  <InfoTooltip text={(infoText || tooltip) as string} />
                </span>
              )}
            </div>
            <p
              className={`${compact ? "text-[20px] md:text-[22px]" : "text-[24px] md:text-[28px]"} font-bold text-[#1A1A1A] leading-tight truncate`}
            >
              {value}
            </p>
          </div>
      ...
      {trend && (
        <div className="flex items-center justify-end gap-[4px] px-2 h-[20px]">
          {trend.isPositive ? (
            <TrendingUp
              className="w-[14px] h-[14px] text-[#22C55E]"
              strokeWidth={2}
            />
          ) : (
            <TrendingDown
              className="w-[14px] h-[14px] text-[#EF4444]"
              strokeWidth={2}
            />
          )}
          <span
