...
export function EventInformationStep({
  data,
  updateData,
}: EventInformationStepProps) {
  ...
  const [startTime, setStartTime] = useState(
    data.startTime ||
      (data.date ? new Date(data.date).toTimeString().slice(0, 5) : ""),
  );
  ...
  const [endTime, setEndTime] = useState(
    data.endTime ||
      (data.date
        ? new Date(new Date(data.date).getTime() + 3600000)
            .toTimeString()
            .slice(0, 5)
        : ""),
  );
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input
            ...
            placeholder="e.g., Seminar Hall A, Lab 301"
      ...
      <div>
        ...
        {isSameDayInvalidTime && (
          <div className="mt-2 text-[11px] font-medium text-[#DC2626]">
            If start date and end date are the same, start time should be less
            than end time.
          </div>
        )}
      </div>
        ...
        <div>
          ...
          {coordinators.length === 0 && (
            <div className="mb-3 p-3 bg-[#FEF3C7] border border-[#FDE68A] rounded-lg">
              <p className="text-xs text-[#92400E]">
                ⚠️ At least one coordinator is required to publish this event.
                Coordinators will receive an email invitation.
              </p>
            </div>
          )}
          ...
          <div className="space-y-3">
            {coordinators.map((coord: any, index: any) => (
              <div
                key={index}
                className="p-4 bg-[#F7F8FA] border border-[#E5E7EB] rounded-lg"
              >
            ...
            {coordinators.length === 0 && (
              <div className="text-center py-6 border-2 border-dashed border-[#E5E7EB] rounded-lg">
                ...
                <p className="text-xs text-[#9CA3AF] mt-1">
                  Click "Add Coordinator" to invite someone
                </p>
              </div>
            )}
          </div>
      ...
      >
        <div className="mb-4 p-3 bg-[#EFF6FF] border border-[#BFDBFE] rounded-lg">
          <p className="text-xs text-[#1E40AF]">
            📧 An email invitation will be sent to the coordinator when this
            event is published.
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                onChange={(e) => {
                  ...
                  if (e.target.checked) {
                    newEligibility = [
                      ...
                      "Final Year",
                      ...eligibility.filter(
                        (i) => i.includes("Branch") || i.includes("Alumni"),
                      ),
                    ];
                  } else {
                    newEligibility = eligibility.filter(
                      (i) =>
                        ![
                          "All Years",
                          "First Year",
            ...
            {["First Year", "Second Year", "Third Year", "Final Year"].map(
              (year) => (
                <label
                  key={year}
                  className={`flex items-center gap-2 p-3 border border-[#E5E7EB] rounded-lg cursor-pointer hover:bg-[#F7F8FA] transition-colors ${eligibility.includes("All Years") ? "opacity-50 cursor-not-allowed bg-[#F9FAFB]" : "bg-white"}`}
                >
                  <input
                    type="checkbox"
      ...
      <div>
        ...
        <textarea
          ...
          defaultValue={
            data.rules ||
            `1. Participants must bring their college ID cards.
      ...
      {(data as any).hasPrizes && (
        <div className="animate-in fade-in slide-in-from-top-4 duration-300">
          ...
          <div className="space-y-4">
            {[
              { position: "1st", color: "#F59E0B" },
              { position: "2nd", color: "#9CA3AF" },
              { position: "3rd", color: "#CD7F32" },
            ].map(({ position, color }) => (
              <div
                key={position}
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <input
                    type="text"
                    placeholder="Prize Title (e.g., Gold Medal)"
