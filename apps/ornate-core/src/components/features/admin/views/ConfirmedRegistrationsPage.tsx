...
export function ConfirmedRegistrationsPage({
  ...
  const filteredRevenue = filteredRegistrations.reduce(
  ...
  const filteredAttendance =
    filteredRegistrations.length > 0
      ? Math.round(
          (filteredRegistrations.filter((r) => r.attendanceMarked).length /
  ...
  const visiblePageWindow = Math.max(
    1,
    Math.min(totalPages, paginationWindowSize || totalPages),
  );
  const pageEnd = Math.min(
    totalPages,
    Math.max(visiblePageWindow, currentPage),
  );
  ...
  const handleExportData = () => {
    ...
    exportRegistrationsToCSV(
      ...
      `confirmed_registrations_${new Date().toISOString().split("T")[0]}.csv`,
    );
    showToast(
      `Exported ${filteredRegistrations.length} confirmed records to CSV`,
      "success",
    );
  };
