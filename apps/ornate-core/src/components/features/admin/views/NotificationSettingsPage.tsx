...
const notificationCategories = [
  {
    ...
    description:
      "Alerts for creation, modification, and participant registration flows.",
  ...
  useEffect(() => {
    ...
    setSettingsState({
      ...
      categories: defaults.categories.map((category: any) => {
        const existingCategory = (initialSettings?.categories || []).find(
          (entry: any) => entry.id === category.id,
        );
        if (!existingCategory) return category;
        return {
          ...category,
          settings: category.settings.map((setting: any) => {
            const existingSetting = (existingCategory.settings || []).find(
              (entry: any) => entry.id === setting.id,
            );
        ...
        <div className="lg:col-span-12 space-y-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-[12px] mt-[10px] mb-[16px]">
            <div className="flex items-center gap-4">
              ...
              <p className="text-sm text-[#6B7280] hidden md:block">
                Manage global delivery platforms
              </p>
            </div>
          </div>
              ...
              >
                <div className="bg-white rounded-[16px] w-full p-6 border border-gray-100 flex flex-col justify-between transition-all shadow-sm hover:shadow-md flex-1">
                  <div className="flex justify-between items-start mb-4">
                    <div
                      className={`${channel.bg} w-12 h-12 rounded-xl flex items-center justify-center ${channel.text}`}
                    >
          ...
          {isLoading ? (
            ...
            notificationCategories.map((category: any) => {
              const categorySettings = settingsState.categories?.find(
                (entry: any) => entry.id === category.id,
              );
              return (
                <div
                  key={category.id}
                  className="bg-[#F4F2F0] rounded-[18px] pt-2 pr-2 pb-6 pl-2"
                >
                  {/* Header Bar */}
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-[12px] mt-[10px] mb-[16px]">
                    <div className="flex items-center gap-4">
                      <div
                        className={`px-3 py-1.5 rounded-md text-xs font-semibold uppercase tracking-wider border ${category.accent}`}
                      >
                        {category.name}
                      </div>
                      <p className="text-sm text-[#6B7280] hidden md:block">
                        ...
                        <tbody>
                          {category.settings.map((setting: any, idx: any) => {
                            const currentSetting =
                              categorySettings?.settings?.find(
                                (entry: any) => entry.id === setting.id,
                              ) || setting;
                            return (
                              <tr
                                key={setting.id}
                                className={`border-b border-[#F3F4F6] row-hover-effect hover:bg-[#FAFAFA] transition-colors ${idx === category.settings.length - 1 ? "border-b-0" : ""}`}
                              >
