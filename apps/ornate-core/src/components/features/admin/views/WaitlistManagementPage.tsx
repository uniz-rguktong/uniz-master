...
export function WaitlistManagementPage({
  ...
  const filteredRegistrations = data.filter((reg: any) => {
    ...
    const matchesPriority =
      !selectedPriority || reg.priority === selectedPriority;

    return matchesSearch && matchesCategory && matchesEvent && matchesPriority;
  });

  const filteredOnline = filteredRegistrations.filter(
    (r) => r.transactionId || r.paymentAmount > 0,
  ).length;
  const filteredOffline = filteredRegistrations.length - filteredOnline;
  const filteredHighPriority = filteredRegistrations.filter(
    (r) => r.priority === "high",
  ).length;
  const filteredPotentialRevenue = filteredRegistrations.reduce(
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
            <button
              onClick={() => {
                ...
                showToast(
                  `Exported ${filteredRegistrations.length} waitlist records to CSV`,
                  "success",
                );
              }}
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
              ...
              <Select
                ...
                onValueChange={(val) =>
                  setSelectedPriority(val === "all" ? null : val)
                }
              >
                  ...
                  <div className="flex-1 w-full text-center sm:text-left">
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                      ...
                      <div className="bg-[#F7F8FA] rounded-lg p-3">
                        ...
                        <div
                          className={`text-xs ${registration.seatsAvailable > 0 ? "text-[#10B981]" : "text-[#EF4444]"}`}
                        >
                          {registration.seatsAvailable > 0
                            ? "Available"
                            : "Full"}
                        </div>
                      </div>
