...
import { getEventRegistrations, type EventAttendee } from '@/actions/registrationGetters';
...
export function AttendanceView({ eventId, backPath, backLabel }: AttendanceViewProps) {
            ...
            <div className="mb-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex-1">
                        <h1 className="text-xl md:text-[28px] font-bold text-[#1A1A1A] mb-2 flex items-center gap-3">
                            Live Attendance Tracking
                        </h1>
                        <p className="text-sm text-[#6B7280]">Mark and monitor real-time attendance for your event</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-[#F4F2F0] rounded-[22px] p-[10px]">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-[12px] mt-[10px] mb-[16px]">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
                        {/* Status Filters - Always in one line, potentially separate from tools on mobile */}
                        <div className="flex items-center bg-white border border-[#E5E7EB] rounded-full p-1 shadow-sm w-full sm:w-auto justify-center sm:justify-start">
                            {['all', 'present', 'absent', 'not-marked'].map((t: any) => (
                                <button
                                    key={t}
                                    onClick={() => setFilter(t)}
                                    className={`flex-1 sm:flex-none px-3 sm:px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all whitespace-nowrap ${filter === t
                        ...
                        <div className="flex items-center justify-between sm:justify-end gap-2 w-full sm:w-auto">
                            ...
                            <button
                                ...
                                className="p-2.5 bg-white border border-[#E5E7EB] rounded-xl hover:bg-[#F3F4F6] text-[#6B7280] active:scale-95 transition-all shadow-sm"
                                title="Refresh Data"
                            >
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    <thead className="bg-[#F9FAFB] border-b border-[#F3F4F6]">
                                        <tr>
                                            <th className="px-6 py-4 text-left text-[10px] font-bold text-[#9CA3AF] uppercase tracking-wider">Student Details</th>
                                            <th className="px-6 py-4 text-left text-[10px] font-bold text-[#9CA3AF] uppercase tracking-wider">Department</th>
                                            <th className="px-6 py-4 text-left text-[10px] font-bold text-[#9CA3AF] uppercase tracking-wider">Actions</th>
                                            <th className="px-6 py-4 text-left text-[10px] font-bold text-[#9CA3AF] uppercase tracking-wider">Status</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-[#F3F4F6]">
                                        {paginatedStudents.length > 0 ? (
                                                ...
                                                >
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <div className="text-sm font-bold text-[#1A1A1A]">{student.name}</div>
                                                                <div className="text-xs font-medium text-[#6B7280]">{student.roll}</div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${student.status === 'present' ? 'bg-[#ECFDF5] text-[#10B981]' :
                                                            student.status === 'absent' ? 'bg-[#FEE2E2] text-[#EF4444]' :
                                                                'bg-gray-100 text-[#6B7280]'
                                                            }`}>
                                                            <div className={`w-1.5 h-1.5 rounded-full ${student.status === 'present' ? 'bg-[#10B981]' :
                                                                student.status === 'absent' ? 'bg-[#EF4444]' :
                                                                    'bg-[#6B7280]'
                                                                }`} />
                                                            {student.status === 'not-marked' ? 'Pending' : student.status.replace('-', ' ')}
                                                        </span>
                                                    </td>
                                                </tr>
                                            ))
                                            ...
                                            <tr>
                                                <td colSpan={4} className="py-20 text-center text-[#6B7280] text-sm">
                                        ...
                                        >
                                            <div className="flex items-start justify-between mb-6">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <div className="text-base font-bold text-[#1A1A1A] line-clamp-1">{student.name}</div>
                                                        <div className="text-xs font-semibold text-[#6B7280]">{student.roll}</div>
                                                        <div className="mt-1 text-[10px] font-bold text-blue-600 uppercase tracking-widest">{student.department}</div>
                                                    </div>
                                                </div>
                                                <div className={`w-3 h-3 rounded-full shadow-sm animate-pulse ${student.status === 'present' ? 'bg-[#10B981]' :
                                                    student.status === 'absent' ? 'bg-[#EF4444]' :
                                                        'bg-[#E5E7EB]'
                                                    }`} />
                                            </div>
                                            ...
                                            <div className="flex gap-3 mt-auto">
                                                <button
                                                    onClick={() => handleMarkAttendance(student.id, 'present')}
                                                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-bold transition-all shadow-sm ${student.status === 'present'
                                                ...
                                                <button
                                                    onClick={() => handleMarkAttendance(student.id, 'absent')}
                                                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-bold transition-all shadow-sm ${student.status === 'absent'
                ...
                {filteredStudents.length > ITEMS_PER_PAGE && (
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-[12px] mt-[16px] mb-[6px]">
                        <p className="text-xs font-semibold text-[#6B7280]">
                            Showing <span className="text-[#1A1A1A]">{(safePage - 1) * ITEMS_PER_PAGE + 1}</span>
                            –<span className="text-[#1A1A1A]">{Math.min(safePage * ITEMS_PER_PAGE, filteredStudents.length)}</span>
                            {' '}of <span className="text-[#1A1A1A]">{filteredStudents.length}</span> students
                        </p>
                        <div className="flex items-center gap-1.5">
                            ...
                            <div className="flex items-center gap-1 mx-2">
                                {Array.from({ length: totalPages }, (_, i) => i + 1)
                                    ...
                                    .map((item: any, i: any) =>
                                        item === 'ellipsis' ? (
                                            <span key={`ellipsis-${i}`} className="px-1 text-[#9CA3AF] text-xs font-bold">...</span>
                                        ) : (
                                            <button
                                                key={item}
                                                onClick={() => setCurrentPage(item)}
                                                className={`w-9 h-9 rounded-xl text-xs font-bold transition-all active:scale-90 ${safePage === item
