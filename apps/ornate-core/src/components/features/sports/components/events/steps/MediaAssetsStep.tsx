...
import { Upload, X, File, Youtube, Link as LinkIcon, FileText } from 'lucide-react';
...
export function MediaAssetsStep({ data, updateData }: MediaAssetsStepProps) {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      {/* Primary Poster */}
      <div>
        <label className="block text-sm font-medium text-[#1A1A1A] mb-4 px-1">Official Tournament Poster</label>
        ...
        {poster ? (
          <div className="relative max-w-2xl aspect-video rounded-2xl overflow-hidden border border-[#E5E7EB] group mx-auto md:mx-0 shadow-sm">
            <Image src={poster as string} alt="Poster Preview" fill className="object-cover" />
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
              ...
              <button
                onClick={() => { setPoster(null); updateData({ poster: null, bannerUrl: null }); }}
          ...
          >
            <div className="flex flex-col items-center">
              ...
              <h3 className="text-sm font-semibold text-[#1A1A1A] mb-1">Official Competition Poster</h3>
              <p className="text-xs text-[#6B7280] mb-5 max-w-xs leading-relaxed">Ensure a 16:9 ratio for the best dashboard visibility.</p>
      ...
      <div className="space-y-4">
        <label className="block text-sm font-medium text-[#1A1A1A] px-1">Teaser / Video Link</label>
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`px-6 border rounded-lg text-xs font-bold uppercase tracking-wider transition-all shadow-sm ${showPreview
      ...
      <div className="bg-[#F4F2F0] rounded-[18px] p-6 border border-[#E5E7EB]">
        <div className="flex items-center justify-between mb-4 px-1">
          <h4 className="text-xs font-bold text-[#6B7280] uppercase tracking-widest opacity-80">Rulebook & Guidelines</h4>
        ...
        {rulebook ? (
          <div className="p-4 bg-white border border-[#E5E7EB] rounded-xl flex items-center justify-between shadow-sm animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-semibold text-[#1A1A1A] leading-tight mb-0.5">{rulebook.name}</div>
                <div className="text-[10px] text-[#9CA3AF] font-bold uppercase tracking-wider">{rulebook.size}</div>
              </div>
            </div>
            <div className="flex gap-1">
              <button className="p-2 hover:bg-[#F9FAFB] rounded-lg transition-all" title="View Link"><LinkIcon className="w-4 h-4 text-[#9CA3AF]" /></button>
              <button
                onClick={() => { setRulebook(null); updateData({ rulebook: null }); }}
          ...
          <div className="py-8 border-2 border-dashed border-[#E5E7EB] rounded-xl bg-white/50 flex flex-col items-center justify-center text-[#9CA3AF]">
            ...
            <p className="text-[10px] font-bold uppercase tracking-widest">No rulebook uploaded</p>
          </div>
        )}
      </div>
    </div>
