...
export function SportForm({
  ...
  const getInitialState = () => {
    ...
    return {
      ...
      sportCategory:
        ...
        initialData.customFields?.sportCategory ||
        "Cricket",
      ...
      const fetchFullDetails = async () => {
        ...
        if (res.success && (res as any).data) {
          ...
          setSportData((prev: any) => ({
            ...
            sportCategory:
              fullEvent.sportCategory ||
              fullEvent.customFields?.sportCategory ||
              prev.sportCategory ||
              "Cricket",
            athleticsType:
              fullEvent.athleticsType ||
              fullEvent.customFields?.athleticsType ||
              "",
            ...
            genderType: (() => {
              ...
              const g =
                fullEvent.gender ||
                fullEvent.customFields?.genderType ||
                fullEvent.customFields?.type ||
                "MALE";
            ...
            maxTeamsPerBranch:
              fullEvent.maxTeamsPerBranch ?? fullEvent.maxCapacity ?? null,
            ...
            deadline: fullEvent.registrationDeadline
              ...
              : "",
            winnerPoints:
              fullEvent.winnerPoints ??
              fullEvent.customFields?.winnerPoints ??
              0,
            runnerupPoints:
              fullEvent.runnerUpPoints ??
              fullEvent.customFields?.runnerupPoints ??
              0,
            participationPoints:
              fullEvent.participationPoints ??
              fullEvent.customFields?.participationPoints ??
              0,
            awards: fullEvent.awards ||
              fullEvent.customFields?.awards || [
                "Certificate of Excellence",
                "Medal",
              ],
            eligibility:
              fullEvent.eligibility ||
              fullEvent.customFields?.eligibility ||
              [],
  ...
  const nextStep = () => {
    if (currentStep === 1) {
      if (
        !sportData.sportCategory ||
        !sportData.category ||
        !sportData.genderType ||
        !sportData.format
      ) {
    ...
    if (currentStep === 2) {
      const hasDeadline = Boolean(
        String(
          sportData.deadline || sportData.registrationDeadline || "",
        ).trim(),
      );
      if (!hasDeadline) {
  ...
  const submitForm = async (targetStatus: string) => {
    ...
    try {
      ...
      formData.append(
        "format",
        sportData.format || sportData.tournamentType || "KNOCKOUT",
      );
      if (sportData.notes) formData.append("description", sportData.notes);
      if (sportData.sportCategory)
        formData.append("sportCategory", sportData.sportCategory);
      if (sportData.athleticsType)
        formData.append("athleticsType", sportData.athleticsType);
      if (sportData.deadline)
        formData.append("registrationDeadline", sportData.deadline);
      if (sportData.rules) formData.append("rules", sportData.rules);
      if (sportData.matchDuration)
        formData.append("matchDuration", String(sportData.matchDuration));
      if (sportData.maxTeamsPerBranch)
        formData.append(
          "maxTeamsPerBranch",
          String(sportData.maxTeamsPerBranch),
        );
      if (sportData.minPlayers)
        formData.append("minPlayersPerTeam", String(sportData.minPlayers));
      if (sportData.maxPlayers)
        formData.append("maxPlayersPerTeam", String(sportData.maxPlayers));
      if (Array.isArray(sportData.eligibility))
        formData.append("eligibility", JSON.stringify(sportData.eligibility));
      if (Array.isArray(sportData.awards))
        formData.append("awards", JSON.stringify(sportData.awards));
      ...
      formData.append(
        "participationPoints",
        String(resolvedSecondRunnerPoints),
      );
      ...
      if (result.success) {
        showToast(
          mode === "edit" ? "Updated successfully" : "Launched successfully",
          "success",
        );
        if (onSuccess) onSuccess();
        if (onNavigate) onNavigate("all-sports");
      } else {
  ...
  return (
    <div className="space-y-6">
      {/* Stepper */}
      <div className="bg-[#F8F9FA] rounded-3xl border border-[#E5E7EB] px-6 py-4">
        <div className="flex items-center w-full gap-0">
          {steps.map((step, idx) => (
            <div key={step.id} className="flex items-center flex-1 min-w-0">
              <div className="flex items-center gap-3 min-w-0">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-base font-semibold transition-all border ${
                    currentStep === idx + 1
                ...
                <div className="min-w-0">
                  <div
                    className={`text-sm font-semibold leading-tight truncate ${currentStep === idx + 1 ? "text-[#1A1A1A]" : "text-[#6B7280]"}`}
                  >
                    {step.name}
                  </div>
                  <div className="text-xs text-[#9CA3AF] leading-tight mt-0.5 truncate">
                    Step {idx + 1} of {steps.length}
                  </div>
                </div>
              </div>
              {idx < steps.length - 1 && (
                <div className="h-px flex-1 bg-[#D1D5DB] mx-3" />
              )}
            </div>
          ))}
        </div>
      ...
      <div className="bg-[#F8F9FA] rounded-2xl p-6 border border-gray-100">
        <h3 className="text-sm font-bold text-[#1A1A1A] mb-4 uppercase tracking-wider">
          {steps[currentStep - 1]?.name}
        </h3>
        <div
          className={mode === "view" ? "pointer-events-none opacity-80" : ""}
        >
          <CurrentStepComponent
            data={sportData}
            hideCoordinatorSection={
              currentStep === 2 ? (hideStep2Coordinators ?? false) : undefined
            }
            updateData={(patch: any) =>
              setSportData({ ...sportData, ...patch })
            }
          />
        </div>
      </div>
