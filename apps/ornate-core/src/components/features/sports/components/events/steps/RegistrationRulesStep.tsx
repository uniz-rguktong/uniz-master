...
export function RegistrationRulesStep({
  data,
  updateData,
  hideCoordinatorSection = false,
}: RegistrationRulesStepProps) {
  const [deadline, setDeadline] = useState(data.deadline || "");
  const [maxTeamsPerBranch, setMaxTeamsPerBranch] = useState(
    data.maxTeamsPerBranch || 1,
  );
          ...
          {eligibilityOptions.map((option: any) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                />
                <span
                  className={`text-xs font-semibold ${eligibility.includes(option) ? "text-[#1A1A1A]" : "text-[#6B7280]"}`}
                >
                  {option}
                </span>
              </div>
            ...
            <div className="bg-white rounded-[14px] border border-[#E5E7EB] p-4 shadow-sm">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {coordinators.map((coord: any, index: any) => (
                  <div
                    key={index}
                    className="flex items-center justify-between p-3 bg-white rounded-xl border border-[#E5E7EB] group hover:border-[#1A1A1A]/30 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] font-medium text-[#6B7280] uppercase tracking-wider">
                          {coord.role}
                        </div>
                      </div>
                    </div>
            ...
            <form onSubmit={handleAddCoordinator} className="space-y-4 pt-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-white border border-[#E5E7EB] rounded-lg text-sm focus:ring-2 focus:ring-[#1A1A1A] outline-none"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-[#1A1A1A] uppercase tracking-wider mb-2">
              ...
              <div>
                ...
                <input
                  ...
                  type="text"
                  required
                  placeholder="e.g. Physical Director, Student Lead"
                  className="w-full px-4 py-3 bg-white border border-[#E5E7EB] rounded-lg text-sm focus:ring-2 focus:ring-[#1A1A1A] outline-none"
                />
              </div>
              <div className="flex justify-end gap-3 pt-4 border-t border-[#E5E7EB]">
                <button
                  ...
                  className="px-5 py-2 rounded-lg text-sm font-semibold text-[#6B7280] hover:bg-[#F3F4F6] transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 bg-[#1A1A1A] text-white rounded-lg text-sm font-semibold hover:bg-[#2D2D2D] transition-colors shadow-sm"
                >
                  Assign
                </button>
              </div>
