...
export function RegistrationRulesStep({ data, updateData, hideCoordinatorSection = false }: RegistrationRulesStepProps) {
  ...
  const [maxTeamsPerBranch, setMaxTeamsPerBranch] = useState(data.maxTeamsPerBranch ?? '');
  ...
  useEffect(() => {
    ...
    setMaxTeamsPerBranch(data.maxTeamsPerBranch ?? '');
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (raw === '') {
                setMaxTeamsPerBranch('');
                updateData({ maxTeamsPerBranch: null });
                return;
              }
              const val = parseInt(raw, 10);
              if (!isNaN(val)) {
                setMaxTeamsPerBranch(val);
