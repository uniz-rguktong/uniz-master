...
export function UpcomingMatches({ data, isLoading }: UpcomingMatchesProps) {
  ...
  const filteredMatches =
    activeFilter === "Today" ? matches.filter((m) => m.isToday) : matches;
    ...
    <div className="bg-[#F4F2F0] rounded-[24px] p-[10px] h-full flex flex-col">
        ...
        <div className="flex gap-2">
          {["All", "Today"].map((filter: any) => (
            <button
              ...
              className={`px-3 py-1 rounded-full text-[10px] font-bold transition-all uppercase tracking-wider ${
                activeFilter === filter
                  ? "bg-[#1A1A1A] text-white shadow-sm"
                  : "bg-white/50 text-[#6B7280] hover:bg-white"
              }`}
            >
              {filter}
            </button>
      ...
      <div className="bg-white rounded-[18px] border border-[#E5E7EB] p-5 md:p-8 shadow-sm flex-1 flex flex-col overflow-hidden">
        <div className="space-y-4 overflow-y-auto pr-1">
          {isLoading ? (
            [1, 2, 3].map((i: any) => (
              <div
                key={i}
                className="p-4 rounded-2xl border border-[#E5E7EB] bg-gray-50/50 space-y-3"
              >
            ...
            filteredMatches.map((match: any, index: any) => (
              <div
                key={index}
                className="p-4 rounded-2xl border border-[#E5E7EB] hover:border-[#10B981] transition-all group cursor-pointer bg-gray-50/50"
              >
            ...
            <div className="flex flex-col items-center justify-center py-12 text-[#9CA3AF]">
              ...
              <p className="text-sm font-medium">
                No matches scheduled for {activeFilter.toLowerCase()}
              </p>
            </div>
          )}
        </div>
        ...
        <Link
          ...
          className="w-full mt-6 py-3 border border-[#E5E7EB] rounded-xl text-sm font-bold text-[#6B7280] hover:bg-[#F9FAFB] hover:text-[#1A1A1A] transition-all uppercase tracking-wider text-center"
        >
          View All Schedule
        </Link>
      </div>
