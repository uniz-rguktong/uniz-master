  ...
  const handleArchiveCategory = async (category: any) => {
    try {
      ...
      if (res.success) {
        ...
        showToast(
          `Category ${category.isArchived ? "restored" : "archived"} successfully`,
          "success",
        );
        router.refresh();
        fetchData();
                ...
                <button
                  onClick={() => {
                    const firstCategoryName =
                      displayCategories[0]?.name || "All Categories";
          ...
          {!isLoading && displayCategories.length === 0 && (
            <div className="text-center py-20 bg-[#F7F8FA] rounded-2xl border border-dashed border-[#E5E7EB]">
              ...
              <p className="text-sm text-[#6B7280] mb-6">
                Create a new category to start managing your gallery.
              </p>
              <button
