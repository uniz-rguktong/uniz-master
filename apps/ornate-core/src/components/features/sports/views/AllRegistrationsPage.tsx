...
import { getAllSportRegistrations, getSportTeamRegistrations } from '@/actions/sportRegistrationGetters';
...
export function AllRegistrationsPage({
  ...
  const getBaseSport = (name: string) => {
    const list = ['Badminton', 'Cricket', 'Volleyball', 'Basketball', 'Football', 'Table Tennis', 'Tennis', 'Kabaddi', 'Chess', 'Athletics', 'Carroms'];
    const found = list.find(s => name.toLowerCase().includes(s.toLowerCase()));
    return found || 'Sport';
  };
  ...
  const onlineRegistrationsCount = registrations.filter(r => r.transactionId || r.paymentAmount > 0).length || Math.floor(totalRegistrations * 0.82);
  const spotRegistrationsCount = totalRegistrations - onlineRegistrationsCount || Math.floor(totalRegistrations * 0.18);
  const totalRevenueValue = registrations.reduce((sum: any, reg: any) => sum + (reg.paymentAmount || 0), 0) || (onlineRegistrationsCount * 250);
  const avgAttendanceRateValue = 92;

  const confirmedCount = registrations.filter((r: any) => r.status === 'confirmed').length;
  const pendingCount = registrations.filter((r: any) => r.status.includes('pending')).length;
  ...
  const getStatusBadge = (status: any) => {
    const styles = {
      ...
      'pending-payment': { bg: '#FFFBEB', text: '#D97706', label: 'Pending Payment' },
      'pending-requirements': { bg: '#FEF2F2', text: '#DC2626', label: 'Pending Verification' },
      ...
      'attended': { bg: '#ECFDF5', text: '#059669', label: 'Attended' },
    ...
    (async () => {
      if (bulkDeleteActive) {
        const results = await Promise.all(
          selectedRegistrations.map((id: string) => deleteCoordinatorRegistration(id))
        );
        ...
        const result = await deleteCoordinatorRegistration(registrationToDelete);
        if (result.success) {
  ...
  const handleExportData = () => {
    ...
    exportRegistrationsToCSV(filteredRegistrations as any, `registrations_export_${new Date().toISOString().split('T')[0]}.csv`);
    showToast(`Exported ${filteredRegistrations.length} records to CSV`, 'success');
  };
  ...
  const handleBulkExport = () => {
    ...
    exportRegistrationsToCSV(selectedData as any, `selected_registrations_${selectedData.length}.csv`);
    showToast(`Exported ${selectedData.length} records to CSV`, 'success');
  };

  const handleTeamExport = () => {
    const exportSource = selectedTeamIds.length > 0
    ...
    showToast(`Exported ${exportSource.length} team registration${exportSource.length > 1 ? 's' : ''}`, 'success');
  };
    ...
    <div className="p-4 md:p-8 animate-page-entrance">
      ...
      <div className="mb-6 md:mb-8">
        <div className="flex flex-wrap items-center gap-1 md:gap-2 text-[10px] md:text-sm text-[#6B7280] mb-3">
          ...
          <span className="text-[#9CA3AF]">›</span>
          <span className="text-[#1A1A1A] font-medium whitespace-nowrap">Individual Registrations</span>
        </div>

        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 md:mb-8 gap-4 md:gap-6">
          <div>
            <h1 className="text-xl md:text-[28px] font-semibold text-[#1A1A1A] mb-1 md:mb-2">Participation Registry</h1>
            <p className="text-[10px] md:text-sm text-[#6B7280]">Oversee and validate every athlete enrollment for the session.</p>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
              <div className="flex flex-row items-center gap-2 w-full sm:w-auto">
                <Select value={filterBranch} onValueChange={handleFilterBranchChange}>
                  <SelectTrigger className="flex-1 sm:w-[160px] h-[42px] bg-[#F7F8FA] border border-[#E5E7EB] rounded-lg text-xs sm:text-sm font-medium text-[#1A1A1A]">
                    <SelectValue placeholder={isBranchPortal ? 'Category' : 'Branch'} />
                  </SelectTrigger>
                  <SelectContent>
                    {isBranchPortal ? (
                      <>
                ...
                <Select value={filterSport} onValueChange={handleFilterSportChange}>
                  <SelectTrigger className="flex-1 sm:w-[180px] h-[42px] bg-[#F7F8FA] border border-[#E5E7EB] rounded-lg text-xs sm:text-sm font-medium text-[#1A1A1A]">
      ...
      <div className="bg-[#F4F2F0] rounded-[18px] px-[10px] py-[24px] pt-[24px] pr-[10px] pb-[10px] pl-[10px] animate-card-entrance mb-8" style={{ animationDelay: '200ms' }}>
        {/* Header Bar */}
        <div className="flex items-center justify-between mb-[16px] px-[12px] mt-[-4px] gap-2 md:gap-4">
          <div className="flex items-center gap-2">
            <h2 className="text-[11px] md:text-sm font-medium text-[#9CA3AF] uppercase tracking-wide">Individual Registrations</h2>
            <InfoTooltip text="List of all individual student registrations for sports events." />
          </div>
          {selectedRegistrations.length > 0 ?
            <>
              <div className="flex items-center gap-2 w-auto">
                <span className="text-[10px] md:text-sm text-[#1A1A1A] font-medium bg-[#F3F4F6] px-2 py-1 rounded w-auto text-center hidden md:inline-block">
            ...
            <div className="flex items-center gap-2 w-auto">
              <button
                ...
                className="flex items-center justify-center gap-1.5 px-2 md:px-3 py-1.5 bg-white border border-[#E5E7EB] rounded-md text-[10px] md:text-sm font-medium text-[#1A1A1A] hover:bg-[#F9FAFB] transition-colors whitespace-nowrap">
            ...
            <table className="w-full min-w-[1000px]">
              <thead className="bg-[#F9FAFB] border-b border-[#E5E7EB]">
                <tr>
                  ...
                  <th
                    className="px-6 py-4 text-left text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.15em]">
                    Branch & Section
                  </th>
                  <th
                  ...
                  paginatedRegistrations.map((registration: any, index: any) =>
                    <tr
                      ...
                      <td className="px-6 py-5">
                        <div className="font-bold text-[14px] text-[#1A1A1A] leading-tight tracking-tight">{registration.studentName}</div>
                        <div className="text-[11px] font-medium text-[#9CA3AF] mt-1">{registration.rollNumber} • {registration.year}</div>
                      </td>
                      <td className="px-6 py-5">
                        <div className="font-bold text-[13px] text-[#1A1A1A]">{registration.branch || 'CSE'}</div>
                        <div className="text-[11px] font-medium text-[#9CA3AF] mt-1">Section {registration.section || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-5">
                        <div className="font-bold text-[13px] text-[#1A1A1A] tracking-tight">{getBaseSport(registration.eventName)}</div>
                        <div className="text-[10px] text-[#6B7280] font-medium mt-0.5">{registration.eventName}</div>
                      </td>
                      <td className="px-6 py-5">
                        <div className="text-[13px] font-bold text-[#1A1A1A]">
                          {isMounted ? new Date(registration.registrationDate).toLocaleDateString('en-US', {
                            month: 'short',
                            day: 'numeric',
                            year: 'numeric'
                          }) : ''}
                        ...
                        <div className="text-xs text-[#6B7280]">
                          {isMounted ? new Date(registration.registrationDate).toLocaleTimeString('en-US', {
                            hour: '2-digit',
                            minute: '2-digit',
                            hour12: true
                          }) : ''}
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-6 py-4 bg-[#F9FAFB] border-t border-[#E5E7EB]">
              <div className="text-sm text-[#6B7280] font-medium">
                Showing <span className="text-[#1A1A1A] font-bold">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="text-[#1A1A1A] font-bold">{Math.min(currentPage * itemsPerPage, sortedRegistrations.length)}</span> of <span className="text-[#1A1A1A] font-bold">{sortedRegistrations.length}</span> results
              </div>
      ...
      <div className="bg-[#F4F2F0] rounded-[18px] px-[10px] py-[24px] pr-[10px] pb-[10px] pl-[10px] animate-card-entrance mb-8" style={{ animationDelay: '300ms' }}>
        {/* Header Bar */}
        <div className="flex items-center justify-between mb-[16px] px-[12px] mt-[-4px] gap-2 md:gap-4">
          <div className="flex items-center gap-2">
            <h2 className="text-[11px] md:text-sm font-medium text-[#9CA3AF] uppercase tracking-wide">Team Registrations</h2>
          ...
          <div className="flex items-center gap-2 w-auto">
            {selectedTeamIds.length > 0 && (
              <div className="flex items-center gap-2">
                <span className="text-[10px] md:text-xs font-semibold text-[#6B7280] bg-white px-2 py-1 rounded-md border border-[#E5E7EB] hidden md:inline-block">
        ...
        <div className="bg-white rounded-[12px] border border-[#E5E7EB] overflow-hidden shadow-sm">
                ...
                <tr>
                  <th className="px-6 py-4">
                    <Checkbox
                      ...
                      onChange={() => setSelectedTeamIds(selectedTeamIds.length === teams.length ? [] : teams.map(t => t.id))}
                      size="sm"
                    />
                  </th>
                  <th className="px-6 py-4 text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.15em] text-left">Team Name</th>
                  <th className="px-6 py-4 text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.15em] text-left">Captain</th>
                  <th className="px-6 py-4 text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.15em] text-left">Student ID</th>
                  <th className="px-6 py-4 text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.15em] text-left">Year</th>
                  <th className="px-6 py-4 text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.15em] text-left">Branch</th>
                  <th className="px-6 py-4 text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.15em] text-left">Section</th>
                  <th className="px-6 py-4 text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.15em] text-left">Sport</th>
                  <th className="px-6 py-4 text-[11px] font-bold text-[#6B7280] uppercase tracking-[0.15em] text-right">Actions</th>
                </tr>
              </thead>
                ...
                {isLoading ? (
                  [...Array(3)].map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      <td colSpan={9} className="px-6 py-4 h-16 bg-gray-50/50" />
                    </tr>
                  ))
                ) : teams.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="px-6 py-12 text-center text-sm text-[#6B7280]">
                      No team registrations found
                    </td>
                  </tr>
                ) : (
                  paginatedTeams.map((team: any, index: number) => (
                    <tr key={team.id} className="hover:bg-[#FCFBFB] transition-colors group">
                      <td className="px-6 py-4">
                        <Checkbox
                          checked={selectedTeamIds.includes(team.id)}
                          onChange={() => setSelectedTeamIds(prev => prev.includes(team.id) ? prev.filter(id => id !== team.id) : [...prev, team.id])}
                          size="sm"
                        />
                      </td>
                      <td className="px-6 py-4">
                        <div className="font-semibold text-sm text-[#1A1A1A]">{team.teamName}</div>
                        <div className="text-[10px] font-bold text-orange-600 uppercase tracking-tighter mt-0.5">{team.members?.length || 0} Members</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm font-medium text-[#1A1A1A]">{team.captain}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-[13px] font-bold text-[#1A1A1A] tracking-tight">{team.captainRoll || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-[#6B7280] font-medium">{team.captainYear || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-[#6B7280] font-medium">{team.captainBranch || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm text-[#6B7280] font-medium">{team.captainSection || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="font-bold text-[13px] text-[#1A1A1A] tracking-tight">{getBaseSport(team.sport)}</div>
                        <div className="text-[10px] text-[#6B7280] font-medium mt-0.5">{team.sport}</div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end">
                          <ActionMenu
                            actions={[
                              { label: 'View Details', icon: 'view', onClick: () => handleViewDetails({ ...team, studentName: team.captain, rollNumber: team.captainRoll, email: 'N/A', phone: 'N/A', year: team.captainYear }) },
                              { label: 'Send Email', icon: 'send', onClick: () => showToast(`Email sent to captain of ${team.teamName}`, 'success') },
                              {
          ...
          {totalPagesTeams > 1 && (
            <div className="flex items-center justify-between px-6 py-4 bg-[#F9FAFB] border-t border-[#E5E7EB]">
              <div className="text-sm text-[#6B7280] font-medium">
                Showing <span className="text-[#1A1A1A] font-bold">{(currentPageTeams - 1) * itemsPerPageTeams + 1}</span> to <span className="text-[#1A1A1A] font-bold">{Math.min(currentPageTeams * itemsPerPageTeams, filteredTeams.length)}</span> of <span className="text-[#1A1A1A] font-bold">{filteredTeams.length}</span> results
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  {[...Array(totalPagesTeams)].map((_, i) => (
                    <button
                      ...
                      className={`w-9 h-9 rounded-xl text-xs font-bold transition-all ${currentPageTeams === i + 1
                        ? 'bg-[#1A1A1A] text-white shadow-md'
                        : 'bg-white border border-[#E5E7EB] text-[#6B7280] hover:bg-gray-50'}`}
                    >
                      {i + 1}
                ...
                <button
                  onClick={() => setCurrentPageTeams(prev => Math.min(prev + 1, totalPagesTeams))}
      ...
      {selectedTeamIds.length > 0 && (
        <div className="mt-4 p-4 bg-white rounded-xl border border-blue-100 flex flex-wrap items-center justify-between gap-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="text-sm font-semibold text-[#1A1A1A]">{selectedTeamIds.length} Teams Selected</div>
              <div className="text-[11px] text-[#6B7280]">Bulk actions available</div>
            </div>
          </div>

          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              onClick={() => showToast(`Sending emails to captains of ${selectedTeamIds.length} teams...`, 'info')}
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="bg-[#F7F8FA] rounded-xl p-4 space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1 uppercase tracking-wider font-bold">Name</div>
                    <div className="text-sm font-medium text-[#1A1A1A]">{selectedRegistration.studentName}</div>
                  </div>
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1 uppercase tracking-wider font-bold">Student ID</div>
                    <div className="text-sm font-medium text-[#1A1A1A]">{selectedRegistration.rollNumber}</div>
                  </div>
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1 uppercase tracking-wider font-bold">Email</div>
                    <div className="text-sm font-medium text-[#1A1A1A]">{selectedRegistration.email}</div>
                  </div>
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1 uppercase tracking-wider font-bold">Phone</div>
                    <div className="text-sm font-medium text-[#1A1A1A]">{selectedRegistration.phone}</div>
                  </div>
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1 uppercase tracking-wider font-bold">Year</div>
                    <div className="text-sm font-medium text-[#1A1A1A]">{selectedRegistration.year}</div>
                  </div>
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1 uppercase tracking-wider font-bold">Branch</div>
                    <div className="text-sm font-medium text-[#1A1A1A]">{selectedRegistration.branch || 'N/A'}</div>
                  </div>
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1 uppercase tracking-wider font-bold">Section</div>
                    <div className="text-sm font-medium text-[#1A1A1A]">{selectedRegistration.section || 'N/A'}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-sm font-semibold text-[#1A1A1A]">Team Roster</h4>
              ...
              <div className="bg-white border border-[#E5E7EB] rounded-xl overflow-hidden">
                <table className="w-full text-left">
                  <thead className="bg-[#F9FAFB] border-b border-[#E5E7EB]">
                    <tr>
                      <th className="px-4 py-2.5 text-[10px] font-bold text-[#6B7280] uppercase tracking-wider">Full Name</th>
                      <th className="px-4 py-2.5 text-[10px] font-bold text-[#6B7280] uppercase tracking-wider">Student ID</th>
                      <th className="px-4 py-2.5 text-[10px] font-bold text-[#6B7280] uppercase tracking-wider">Role</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#F1F3F5]">
                    {selectedRegistration.members ? selectedRegistration.members.map((member: any, i: any) => (
                      <tr key={i} className="hover:bg-[#F9FAFB] transition-colors">
                        <td className="px-4 py-3 text-sm font-medium text-[#1A1A1A]">{member.name}</td>
                        <td className="px-4 py-3 text-sm text-[#6B7280]">{member.id}</td>
                        <td className="px-4 py-3">
                          <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${member.role === 'Captain'
                      ...
                      <tr>
                        <td className="px-4 py-3 text-sm font-medium text-[#1A1A1A]">{selectedRegistration.studentName}</td>
                        <td className="px-4 py-3 text-sm text-[#6B7280]">{selectedRegistration.rollNumber}</td>
                        <td className="px-4 py-3">
            ...
            {/* Event Information */}
            <div>
              <h4 className="text-sm font-semibold text-[#1A1A1A] mb-3">Tournament Details</h4>
              <div className="bg-[#F7F8FA] rounded-xl p-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1 uppercase tracking-wider font-bold">Sport</div>
                    <div className="text-sm font-medium text-[#1A1A1A]">{selectedRegistration.eventName}</div>
                  </div>
                  <div>
                    <div className="text-xs text-[#6B7280] mb-1 uppercase tracking-wider font-bold">Category</div>
