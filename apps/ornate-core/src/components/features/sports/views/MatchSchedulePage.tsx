...
export function MatchSchedulePage() {
  ...
  const isSportsAdminRoute =
    pathname?.startsWith("/sports/") &&
    (session?.user?.role === "SPORTS_ADMIN" ||
      session?.user?.role === "BRANCH_SPORTS_ADMIN");
  ...
  const [activeScheduleModalMatch, setActiveScheduleModalMatch] = useState<
    any | null
  >(null);
  ...
  const fetchInitialData = async () => {
    ...
    if (sportsRes.success && sportsRes.sports) {
      ...
      const sports = (sportsRes.sports as SportOption[]).filter(
        (sport) =>
          sport?.id && sport?.name && !sport.name.startsWith("Athletics"),
      );
  ...
  useEffect(() => {
    if (
      ...
      !availableSports.some((sport) => sport.id === selectedSport)
    ) {
  ...
  const isPlaceholderTeam = (name: string | null | undefined) => {
    ...
    return (
      !normalized || normalized === "TBD" || normalized.startsWith("WINNER OF ")
    );
  };
    ...
    for (const match of matches) {
      ...
      const candidates = matches
        .filter((candidate: any) => {
          ...
          if (
            candidate.sportId !== match.sportId ||
            candidate.gender !== match.gender
          )
            return false;

          const candidateRoundOrder =
            candidate.roundOrder ?? Number.MAX_SAFE_INTEGER;
          ...
          if (candidateRoundOrder === currentRoundOrder) {
            const candidateMatchOrder =
              candidate.matchOrder ?? Number.MAX_SAFE_INTEGER;
            const currentMatchOrder =
              match.matchOrder ?? Number.MAX_SAFE_INTEGER;
          ...
          return (
            t1.toLowerCase() === winnerPlaceholder ||
            t2.toLowerCase() === winnerPlaceholder
          );
        })
        .map((candidate: any) => ({
          match: candidate,
      ...
      const startedByStatus =
        nextStatus === "live" || nextStatus === "completed";
      const startedBySchedule =
        (nextMatch.startsAt as Date).getTime() <= now.getTime();
      lockMap[match.id] =
        startedByStatus || startedBySchedule
  ...
  const selectedSportLabel =
    selectedSport === "All"
      ? "All Sports"
      : availableSports.find((sport) => sport.id === selectedSport)?.name ||
        selectedSport;

  const resetForm = () => {
    const selectedSportMeta = availableSports.find(
  ...
  const handleSaveMatch = async () => {
    ...
    const isDuplicate = matches.some((m) => {
      ...
      if (
        ...
        m.gender === newMatch.gender
      ) {
        const teamsInNew = [newMatch.team1Name, newMatch.team2Name];
        return (
          teamsInNew.includes(m.team1Name) || teamsInNew.includes(m.team2Name)
        );
      }
      return false;
    });
    ...
    if (isDuplicate) {
      showToast(
        `One of these teams is already scheduled for ${newMatch.round}`,
        "error",
      );
      return;
    }
  ...
  const handleRecordResult = (match: any) => {
    ...
    const canEditResult = isSportsAdminRoute
  ...
  const handleSaveMatchResult = async (match: any) => {
    ...
    if (isSportsAdminRoute) {
      ...
      if (
        ...
        showToast(
          "Please enter both scores and select a winning team",
          "error",
        );
        return;
      }
  ...
  const handleSaveScheduleDetails = async () => {
    ...
    const result = await updateMatch(activeScheduleModalMatch.id, {
      ...
      status:
        activeScheduleModalMatch.status === "completed"
          ? "completed"
          : "scheduled",
    });
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-6">
          <div className="space-y-1">
            ...
            <p className="text-sm text-[#6B7280]">
              Synchronized schedule tracking across all sports departments.
            </p>
          </div>
      ...
      <div className="bg-white rounded-[24px] md:rounded-[40px] border border-[#E5E7EB] p-4 md:p-8 shadow-sm">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h3 className="text-lg md:text-2xl font-semibold text-[#1A1A1A] tracking-tight">
              {selectedGender === "All" ? "Combined" : selectedGender}{" "}
              {selectedSport === "All" ? "Matches" : selectedSportLabel}
            </h3>
            <p className="text-sm text-[#6B7280] mt-1">Tournament Fixtures</p>
          </div>
        </div>
        ...
        {filteredMatches.length === 0 ? (
          <div className="py-24 text-center">
            ...
            <h4 className="text-[11px] font-bold text-[#1A1A1A] uppercase tracking-[0.2em] opacity-70">
              No Squads Deployed
            </h4>
            <p className="text-[13px] text-[#6B7280] font-medium mt-2 max-w-sm mx-auto">
              No matches found for{" "}
              <span className="font-bold text-gray-900">
                {selectedGender} {selectedSportLabel}
              </span>
            ...
            {filteredMatches.map((match: any, index: any) => (
              ...
              >
                ...
                {(() => {
                  ...
                  const scoreEditLockReason = isSportsAdminRoute
                    ? resultEditLocksByMatchId[match.id]
                    : null;
                  const isResultEditLocked = Boolean(scoreEditLockReason);
                  const currentStatus = (match.status || "").toLowerCase();
                  const isFinalRound =
                    (match.round || "").toLowerCase() === "finals";
                  const hasFinalPlaceholders =
                    isFinalRound &&
                    (isPlaceholderTeam(match.team1Name) ||
                      isPlaceholderTeam(match.team2Name));
                  const isPendingFinalPlaceholder =
                    hasFinalPlaceholders && currentStatus === "pending";
                  const showScheduleDetails =
                    isScheduleCompleted && !isPendingFinalPlaceholder;
                  return (
                    <div className="flex flex-col lg:flex-row items-center gap-8">
                      ...
                      <div className="shrink-0 text-center lg:text-left w-36">
                        <div className="inline-flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-[#F9FAFB] border border-[#E5E7EB] mb-4 transition-colors">
                          <div
                            className={`w-2 h-2 rounded-full ${match.status === "completed" ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.5)] animate-pulse"}`}
                          />
                          <span className="text-[10px] font-black text-[#1A1A1A] uppercase tracking-wider">
                            {match.status || "Pending"}
                          </span>
                        </div>
                        <div className="text-sm font-semibold text-[#1A1A1A] leading-tight pl-1 bg-[#F9FAFB] rounded-lg py-1 w-fit">
                          {match.round}
                        </div>
                      </div>
                      ...
                      <div className="flex-1 flex flex-col md:flex-row items-center justify-center gap-6 py-5 px-6 bg-[#F9FAFB] rounded-[20px] border border-[#E5E7EB] transition-all duration-300">
                        <div className="flex-1 text-center md:text-right">
                          <div
                            className={`text-xl font-semibold tracking-tight mb-1 transition-all origin-right ${!getDisplayTeamName(match, "team1") || getDisplayTeamName(match, "team1") === "TBD" ? "text-gray-300 font-medium italic" : match.winner === match.team1Name ? "text-emerald-600" : "text-[#1A1A1A]"}`}
                          >
                          ...
                          <div className="text-[9px] font-bold text-gray-400 uppercase tracking-widest opacity-60">
                            Home Squad
                          </div>
                        </div>

                        <div className="relative shrink-0">
                          <div className="w-12 h-12 bg-[#1A1A1A] rounded-2xl flex items-center justify-center text-white text-[10px] font-bold shadow-sm z-10 relative">
                            VS
                          </div>
                        </div>
                        ...
                        <div className="flex-1 text-center md:text-left">
                          <div
                            className={`text-xl font-semibold tracking-tight mb-1 transition-all origin-left ${!getDisplayTeamName(match, "team2") || getDisplayTeamName(match, "team2") === "TBD" ? "text-gray-300 font-medium italic" : match.winner === match.team2Name ? "text-emerald-600" : "text-[#1A1A1A]"}`}
                          >
                        ...
                        <div className="flex flex-col gap-2 shrink-0 ml-4">
                          {(!isSportsAdminRoute || !isResultEditLocked) && (
                            <button
                              ...
                              className={`p-2 rounded-xl transition-all shadow-sm border group ${
                    ...
                    <div className="space-y-1.5 mb-4">
                      ...
                      {isSportsAdminRoute ? (
                        <div className="w-full h-11 bg-white rounded-xl border border-[#E5E7EB] font-medium text-sm px-4 flex items-center">
                          {resultForm.winner === "TBD"
                            ? "Winner will be auto-declared from score"
                            : resultForm.winner}
                        </div>
                      ) : (
                        <Select
                          ...
                          onValueChange={(val) =>
                            setResultForm((prev) => ({ ...prev, winner: val }))
                          }
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value={match.team1Name || "Team 1"}>
                              {match.team1Name || "Team 1"} Won
                            </SelectItem>
                            <SelectItem value={match.team2Name || "Team 2"}>
                              {match.team2Name || "Team 2"} Won
                            </SelectItem>
          ...
          <div className="bg-white rounded-[30px] w-full max-w-lg p-8 shadow-2xl border border-[#E5E7EB]">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-[12px] text-[#7A7772] font-semibold mt-1 uppercase tracking-widest opacity-70">
                  Set Date, Time & Venue
                </p>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 gap-4 mb-6">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-[#9CA3AF] uppercase tracking-widest pl-1">
                  Date
                </label>
                <input
                  type="date"
          ...
          <div className="bg-white rounded-[40px] w-full max-w-2xl p-10 shadow-2xl animate-scale-in border border-[#E5E7EB]">
            <div className="flex justify-between items-start mb-8">
              <div>
                ...
                <p className="text-[13px] text-[#7A7772] font-semibold mt-1 uppercase tracking-widest opacity-60">
            ...
            <div className="grid grid-cols-2 gap-6 mb-10">
                ...
                <Select
                  ...
                  onValueChange={(val) => {
                    ...
                    setNewMatch({
                      ...
                      sport: selectedSportMeta?.name || "",
                      gender:
                        selectedSportMeta?.gender === "MALE"
                          ? "Boys"
                          : selectedSportMeta?.gender === "FEMALE"
                            ? "Girls"
                            : newMatch.gender,
                    });
                  }}
              ...
              <div className="space-y-1.5 col-span-2 md:col-span-1">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setNewMatch({ ...newMatch, round: val })
                  }
                >
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={
                    allTeams.find((t) => t.id === newMatch.team1Id)?.id ||
                    "adhoc"
                  }
                  onValueChange={(val) => {
                    if (val === "adhoc") {
                      ...
                      setNewMatch({
                        ...newMatch,
                        team1Id: team.id,
                        team1Name: team.teamName || team.name,
                      });
                    }
                  }}
                >
                ...
                {newMatch.team1Id === null && (
                  <input
                    ...
                    onChange={(e) =>
                      setNewMatch({ ...newMatch, team1Name: e.target.value })
                    }
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={
                    allTeams.find((t) => t.id === newMatch.team2Id)?.id ||
                    "adhoc"
                  }
                  onValueChange={(val) => {
                    if (val === "adhoc") {
                      ...
                      setNewMatch({
                        ...newMatch,
                        team2Id: team.id,
                        team2Name: team.teamName || team.name,
                      });
                    }
                  }}
                >
                ...
                {newMatch.team2Id === null && (
                  <input
                    ...
                    onChange={(e) =>
                      setNewMatch({ ...newMatch, team2Name: e.target.value })
                    }
                    className="w-full h-12 bg-white border border-[#E5E7EB] rounded-2xl px-4 text-sm font-bold focus:ring-2 focus:ring-[#1A1A1A]/5 outline-none transition-all mt-2"
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setNewMatch({ ...newMatch, status: val })
                  }
                >
                  <SelectTrigger className="w-full h-12 bg-white rounded-2xl border-[#E5E7EB] font-bold text-[13px]">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 h-14 rounded-2xl border border-[#E5E7EB] text-[11px] font-bold uppercase tracking-widest hover:bg-gray-50 transition-all text-[#1A1A1A]"
              >
                Cancel
              </button>
              <button
                onClick={handleSaveMatch}
                className="flex-2 h-14 bg-[#1A1A1A] text-white rounded-2xl text-[11px] font-bold uppercase tracking-widest shadow-xl hover:bg-black transition-all"
              >
                Update Schedule
              </button>
            </div>
