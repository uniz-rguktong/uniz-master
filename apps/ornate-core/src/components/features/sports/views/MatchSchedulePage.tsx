      ...
      <div className="mb-0">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-6">
          <div className="space-y-1">
            <h1 className="text-xl md:text-[28px] font-semibold text-[#1A1A1A]">Tournament Schedule</h1>
          ...
          <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
            ...
            <div className="flex items-center gap-2 p-1 bg-white border border-[#E5E7EB] rounded-xl shadow-sm h-11 w-full md:w-auto">
              <Select value={selectedSport} onValueChange={setSelectedSport}>
                <SelectTrigger className="w-full md:w-auto md:min-w-[120px] max-w-full md:max-w-[200px] border-0 bg-transparent font-medium text-xs focus:ring-0 h-full [&>span]:truncate flex-1">
              ...
              <Select value={selectedGender} onValueChange={setSelectedGender}>
                <SelectTrigger className="w-full md:w-auto md:min-w-[90px] max-w-full md:max-w-[120px] border-0 bg-transparent font-medium text-xs focus:ring-0 h-full [&>span]:truncate flex-[0.5]">
                ...
                {(() => {
                  ...
                  return (
                    <div className="flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-8 w-full">
                      {/* Status & Round */}
                      <div className="shrink-0 text-left w-full lg:w-36 flex flex-row lg:flex-col items-center lg:items-start justify-between lg:justify-start">
                        <div className="inline-flex items-center gap-2.5 px-3 py-1.5 rounded-xl bg-[#F9FAFB] border border-[#E5E7EB] lg:mb-4 transition-colors">
                        ...
                        <div className="text-xs lg:text-sm font-semibold text-[#1A1A1A] leading-tight pl-2 lg:pl-1 bg-[#F9FAFB] rounded-lg py-1 hover:bg-[#F3F4F6] w-fit">{match.round}</div>
                      </div>

                      {/* Battle Center */}
                      <div className="flex-1 w-full flex flex-row items-center justify-between gap-2 md:gap-6 py-4 md:py-5 px-3 md:px-6 bg-[#F9FAFB] rounded-[20px] border border-[#E5E7EB] transition-all duration-300">
                        <div className="flex-1 text-right">
                          <div className={`text-[13px] sm:text-lg md:text-xl font-semibold tracking-tight mb-1 transition-all origin-right ${!getDisplayTeamName(match, 'team1') || getDisplayTeamName(match, 'team1') === 'TBD' ? 'text-gray-300 font-medium italic' : (match.winner === match.team1Name ? 'text-emerald-600' : 'text-[#1A1A1A]')} break-words`}>
                            {getDisplayTeamName(match, 'team1')}
                          </div>
                          {match.winner === match.team1Name && (
                            <div className="text-[8px] sm:text-[10px] font-bold uppercase tracking-wider text-emerald-600">Winner</div>
                          )}
                          <div className="text-[7px] sm:text-[9px] font-bold text-gray-400 uppercase tracking-widest opacity-60">Home Squad</div>
                        </div>

                        <div className="relative shrink-0 mx-1 sm:mx-2">
                          <div className="w-8 h-8 md:w-12 md:h-12 bg-[#1A1A1A] rounded-xl md:rounded-2xl flex items-center justify-center text-white text-[9px] md:text-[10px] font-bold shadow-sm z-10 relative">VS</div>
                        </div>

                        <div className="flex-1 text-left">
                          <div className={`text-[13px] sm:text-lg md:text-xl font-semibold tracking-tight mb-1 transition-all origin-left ${!getDisplayTeamName(match, 'team2') || getDisplayTeamName(match, 'team2') === 'TBD' ? 'text-gray-300 font-medium italic' : (match.winner === match.team2Name ? 'text-emerald-600' : 'text-[#1A1A1A]')} break-words`}>
                            {getDisplayTeamName(match, 'team2')}
                          </div>
                          {match.winner === match.team2Name && (
                            <div className="text-[8px] sm:text-[10px] font-bold uppercase tracking-wider text-emerald-600">Winner</div>
                          )}
                          <div className="text-[7px] sm:text-[9px] font-bold text-gray-400 uppercase tracking-widest opacity-60">Away Squad</div>
                        </div>
                      </div>

                      {/* Logistics Grid */}
                      <div className="shrink-0 w-full lg:w-auto border-t lg:border-t-0 lg:border-l lg:pl-8 border-[#E5E7EB] pt-4 lg:pt-0">
                        {showScheduleDetails ? (
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 md:gap-x-6 gap-y-2">
                            <div className="space-y-0.5">
                              <div className="flex items-center gap-1.5 text-[8px] md:text-[9px] font-bold text-[#9CA3AF] uppercase tracking-widest">
                                <Calendar className="w-3 h-3 text-orange-500" />
                                Date
                              </div>
                              <div className="text-[11px] md:text-[12px] font-bold text-[#1A1A1A]">{match.date}</div>
                            </div>
                            <div className="space-y-0.5">
                              <div className="flex items-center gap-1.5 text-[8px] md:text-[9px] font-bold text-[#9CA3AF] uppercase tracking-widest">
                              ...
                              <div className="text-[11px] md:text-[12px] font-bold text-[#1A1A1A]">{match.time}</div>
                            </div>
                            <div className="space-y-0.5 col-span-2 sm:col-span-1">
                              ...
                              <div className="text-[11px] md:text-[12px] font-bold text-[#1A1A1A]">{match.venue}</div>
                            </div>
                          </div>
                        ) : (
                          !isBranchSportsAdmin && (
                            <div className="flex items-center w-full">
                              <button
                                ...
                                className="h-10 px-4 rounded-lg border border-[#E5E7EB] text-[10px] font-bold uppercase tracking-wider text-[#1A1A1A] hover:bg-gray-50 transition-colors w-full lg:w-auto"
                              >
                                Schedule Match
                              </button>
                            </div>
                      ...
                      {/* Actions */}
                      {!isBranchSportsAdmin && (
                        <div className="flex flex-row lg:flex-col justify-center items-center gap-3 shrink-0 lg:ml-4 w-full lg:w-auto mt-2 lg:mt-0">
                          {(!isSportsAdminRoute || !isResultEditLocked) && (
                            <button
                              ...
                              title="Update Result"
                            >
                              <Trophy className="w-5 h-5 md:w-4 md:h-4 group-hover:scale-110 transition-transform" />
                            </button>
                          )}
