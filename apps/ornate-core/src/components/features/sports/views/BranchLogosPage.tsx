...
import { Upload, Video, Image as ImageIcon, Play, Eye, Share2, Trash2, X } from 'lucide-react';
import {
    Select,
    SelectContent,
    SelectItem,
...
import { MetricCardSkeleton, VideoCardSkeleton, LogoCardSkeleton } from '@/components/ui/skeleton';
...
export function BranchLogosPage({
    ...
    const handlePublishVideo = async () => {
        ...
        const alreadyExists = promoVideos.some(
            (video) => normalizeUrl(video.url || '') === normalizedInputUrl && (video.platform || '') === videoPlatform
        );
        if (alreadyExists) {
        ...
        try {
            ...
            const res = await createPromoVideo({
                ...
                duration: 'N/A',
                thumbnail: customThumbnailUrl || 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=600&h=400&fit=crop'
            });
        ...
        if (uploadFile) {
            ...
            try {
                ...
                const res = await createBrandLogo({
                    ...
                    url: finalUrl,
                    thumbnail: finalUrl,
                    size: uploadFile ? `${(uploadFile.size / 1024).toFixed(1)} KB` : 'N/A',
                    dimensions: 'N/A'
                });
        ...
        <div className="p-8 animate-page-entrance">
            ...
            <div className="mb-8">
                <div className="flex flex-wrap items-center gap-2 text-sm text-[#6B7280] mb-3">
                    ...
                    <span className="whitespace-nowrap">›</span>
                    <span className="text-[#1A1A1A] font-medium whitespace-nowrap">Assets & Logos</span>
                </div>

                <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
                    <div>
                        <h1 className="text-xl md:text-[28px] font-semibold text-[#1A1A1A] mb-2">{logosLabel} & Assets</h1>
                        <p className="text-sm text-[#6B7280]">Manage {logosLabelLower}, branding assets, and promotional videos</p>
                    </div>
                        ...
                        {tabOrder.map((tab) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-6 py-2 text-sm font-medium rounded-[7px] transition-all duration-200 ${isActive
                                        ? 'bg-white text-[#1A1A1A] shadow-[0_1px_4px_-1px_rgba(0,0,0,0.05)]'
                                        : 'text-[#6B7280] hover:text-[#1A1A1A]'}`}
                                >
                                    {isVideos ? <Video className="w-4 h-4" /> : <ImageIcon className="w-4 h-4" />}
                                    {label}
                                </button>
                            );
                        })}
                ...
                <div className="space-y-6">
                    ...
                    {!isReadOnly && (
                        <button
                            ...
                            className="flex md:hidden items-center justify-center gap-2 px-5 py-3 bg-[#10B981] text-white rounded-lg text-sm font-medium hover:bg-[#059669] transition-colors shadow-sm w-full mb-2">
                            <Upload className="w-5 h-5" />
                            Upload Video
                        </button>
                    )}
                    ...
                    <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-[#E5E7EB] shadow-sm">
                        <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 w-full md:w-auto">
                            ...
                            <div className="flex gap-2 p-1 bg-[#F4F2F0] rounded-lg overflow-x-auto w-full md:w-auto">
                                {['all', 'promo', 'video'].map((cat) => (
                                    <button
                                        key={cat}
                                        onClick={() => setCategoryFilter(cat)}
                                        className={`px-4 py-1.5 text-xs font-medium rounded-md transition-all flex-1 md:flex-none text-center whitespace-nowrap ${categoryFilter === cat
                                            ? 'bg-white text-[#10B981] shadow-sm'
                                            : 'text-[#6B7280] hover:text-[#1A1A1A]'}`}
                                    >
                                        {cat.charAt(0).toUpperCase() + cat.slice(1).replace('all', 'All Content')}
                                    </button>
                                ))}
                            </div>
                        </div>
            ...
            {showUploadVideoModal &&
                <Modal
                    ...
                    <div className="space-y-5 p-8">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-[#1A1A1A] mb-2">Video URL (YouTube/Vimeo) *</label>
                            <input
                                type="text"
                                value={videoUrl}
                                ...
                                {videoThumbnailPreview ? (
                                    <div className="absolute inset-0 flex items-center justify-center bg-gray-50">
                                        ...
                                        <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                                            <p className="text-white text-sm font-medium">Click to change thumbnail</p>
                                        </div>
                                    </div>
                                ) : (
                                    <>
            ...
            {showEditVideoModal && editingVideo &&
                <Modal
                    isOpen={true}
                    onClose={() => { setShowEditVideoModal(false); setUploadFile(null); setLogoPreview(null); }}
                    ...
                    onConfirm={async () => {
                        ...
                        if (res.success) {
                            setPromoVideos(prev => prev.map(v =>
                                v.id === editingVideo.id ? res.video : v
                            ));
            ...
            {showUploadLogoModal &&
                <Modal
                    isOpen={true}
                    onClose={() => {
                        if (!isPublishingLogo && !publishingLogoRef.current) closeLogoModal();
                    }}
                    title="Upload Logo"
                    size="lg"
                    footer={(
                    ...
                    <div className="space-y-5 p-8">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-[#1A1A1A] mb-2">Logo File *</label>
                            <div
                                onClick={() => {
                                    if (!isPublishingLogo && !publishingLogoRef.current) handleLogoFileUpload();
                                }}
                                ...
                                {logoPreview ? (
                                    ...
                                    <>
                                        ...
                                        <p className="text-xs text-[#9CA3AF] mt-1">SVG, PNG, JPG (max. 2MB)</p>
                                    </>
                                )}
                            </div>
                        </div>
            ...
            {showEditLogoModal && editingLogo &&
                <Modal
                    isOpen={true}
                    onClose={() => { setShowEditLogoModal(false); setUploadFile(null); setLogoPreview(null); }}
                    title="Edit Logo"
                    size="lg"
                    onConfirm={async () => {
                        ...
                        if (res.success) {
                            setLogos(prev => prev.map(l => l.id === editingLogo.id ? res.logo : l));
            ...
            <Modal
                ...
                <div className="p-4">
                    <p className="text-sm text-[#6B7280]">
                        Are you sure you want to delete <span className="font-bold text-[#1A1A1A]">{itemToDelete?.title || itemToDelete?.name}</span>? This action cannot be undone.
                    </p>
                </div>
            </Modal>
