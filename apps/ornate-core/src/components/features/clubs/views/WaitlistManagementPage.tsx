...
export function WaitlistManagementPage({ initialData = [], initialEvents = [] }: WaitlistManagementPageProps = {}) {
  ...
  const highPriorityCount = data.filter((r: any) => r.priority === 'high').length;
  const paidWaitlistCount = data.filter((r: any) => r.paymentStatus === 'paid').length;
  ...
  const handleRejectConfirm = async () => {
    try {
      const result = await updateRegistrationStatus(registrationToProcess, 'CANCELLED');
      if (result.success) {
  ...
  const handleReleaseSeats = async () => {
    ...
    if (filterEventId === 'all') {
      showToast('Please select a specific event from filters to release seats.', 'warning');
    ...
    try {
      ...
      const { releaseWaitlistSeats } = await import('@/actions/registrationActions');
    ...
    <div className="p-8 animate-page-entrance">
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            <h1 className="text-xl md:text-[28px] font-semibold text-[#1A1A1A] mb-2">Waitlist Management</h1>
            ...
            <>
              ...
              <div className="animate-card-entrance" style={{ animationDelay: '160ms' }}>
                <MetricCard
                  ...
                  iconBgColor="#FEF3C7"
                  iconColor="#F59E0B"
                  trend={{ value: "0", isPositive: true, comparisonText: "vs yesterday" }}
      ...
      <div className="flex md:hidden flex-col sm:flex-row items-center gap-3 w-full mb-6 relative z-10">
        <button
          onClick={() => setShowReleaseSeatsModal(true)}
          className="flex items-center justify-center gap-2 px-5 py-3 bg-[#10B981] text-white rounded-lg text-sm font-medium hover:bg-[#059669] transition-colors w-full sm:w-auto">
          <CheckCircle className="w-4 h-4" />
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                  ...
                  <div className="flex-1 w-full text-center sm:text-left">
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mb-4">
                      <div className="bg-[#F7F8FA] rounded-lg p-2 sm:p-3 break-all sm:break-normal">
                        <div className="text-[10px] sm:text-xs text-[#6B7280] mb-1">Event</div>
                        <div className="font-medium text-[11px] sm:text-sm text-[#1A1A1A] line-clamp-1">{registration.eventName}</div>
                        <div className="text-[10px] sm:text-xs text-[#6B7280]">{registration.eventType}</div>
                      </div>
                      <div className="bg-[#F7F8FA] rounded-lg p-2 sm:p-3">
                        <div className="text-[10px] sm:text-xs text-[#6B7280] mb-1">Capacity Status</div>
                        <div className="font-medium text-[11px] sm:text-sm text-[#1A1A1A]">
                          {registration.seatsAvailable}/{registration.maxCapacity}
                        </div>
                        <div className="text-[10px] sm:text-xs text-[#EF4444]">Full</div>
                      </div>
                      <div className="bg-[#F7F8FA] rounded-lg p-2 sm:p-3">
                        <div className="text-[10px] sm:text-xs text-[#6B7280] mb-1">Registration Date</div>
                        <div className="font-medium text-[11px] sm:text-sm text-[#1A1A1A]">
                          {new Date(registration.registrationDate).toLocaleDateString()}
                        </div>
                      </div>
                      <div className="bg-[#F7F8FA] rounded-lg p-2 sm:p-3">
                        <div className="text-[10px] sm:text-xs text-[#6B7280] mb-1">Payment Status</div>
                        {registration.paymentAmount ? (
                          <div>
                            <div className="font-medium text-[11px] sm:text-sm text-[#1A1A1A]">₹{registration.paymentAmount}</div>
                            <div className={`text-[10px] sm:text-xs ${registration.paymentStatus === 'paid' ? 'text-[#10B981]' : 'text-[#F59E0B]'}`}>
                              {registration.paymentStatus}
                            </div>
                          </div>
                        ) : (
                          <div className="font-medium text-[11px] sm:text-sm text-[#6B7280]">Free Event</div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3">
                      <div className="grid grid-cols-2 gap-2 w-full sm:flex sm:w-auto sm:gap-3">
                        <button
                          ...
                          className="w-full sm:w-auto flex flex-row items-center justify-center gap-1 sm:gap-2 px-2 py-2 sm:px-4 bg-white border border-[#E5E7EB] rounded-lg text-[11px] sm:text-sm font-medium hover:bg-[#F7F8FA] transition-colors"
                        >
                        ...
                        <button
                          ...
                          className="w-full sm:w-auto flex flex-row items-center justify-center gap-1 sm:gap-2 px-2 py-2 sm:px-4 bg-[#10B981] text-white rounded-lg text-[11px] sm:text-sm font-medium hover:bg-[#059669] transition-colors"
                        >
                          <CheckCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" /> <span className="truncate">Approve Reg</span>
                        </button>
                      </div>
                      <div className="grid grid-cols-1 w-full sm:flex sm:w-auto sm:gap-3">
                        <button
                          onClick={() => handleRejectClick(registration.id)}
                          className="w-full sm:w-auto flex flex-row items-center justify-center gap-1 sm:gap-2 px-2 py-2 sm:px-4 bg-[#EF4444] text-white rounded-lg text-[11px] sm:text-sm font-medium hover:bg-[#DC2626] transition-colors"
                        >
                          <XCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" /> <span className="truncate">Remove</span>
                        </button>
                      </div>
                    </div>
                  </div>
      ...
      <Modal
        ...
        <div className="space-y-4">
          <p className="text-sm text-[#6B7280]">
            Enter the number of seats to release. Top priority students from the waitlist will be alerted and moved to confirmed status.
          </p>
          <div>
            <label className="block text-sm font-medium text-[#1A1A1A] mb-2">Number of Seats</label>
            <input
              type="number"
              min="1"
