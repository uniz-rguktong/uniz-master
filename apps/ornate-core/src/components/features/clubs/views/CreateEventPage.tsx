...
export function CreateEventPage({ onNavigate, mode = 'create', initialData }: CreateEventPageProps = {}) {
    ...
    try {
      ...
      if (!result.success) {
        ...
        const stepErrors: Record<number, string[]> = {
          ...
          2: ['venue', 'startDate', 'startTime', 'endDate', 'endTime', 'coordinators', 'rules'],
          3: ['registrationStatus', 'maxParticipants', 'fee']
        };

        const currentStepErrors = stepErrors[step]?.filter(field => formattedErrors[field]);
        if (currentStepErrors && currentStepErrors.length > 0) {
          const firstFieldWithError = currentStepErrors[0];
          if (firstFieldWithError) {
            showToast(`Validation Failed: ${formattedErrors[firstFieldWithError]}`, 'error');
            return false;
          }
        }
        return false;
  ...
  const saveEvent = async (status = 'PUBLISHED') => {
    ...
    try {
      ...
      // Zod schema will handle validation for publishing
      if (status === 'PUBLISHED') {
