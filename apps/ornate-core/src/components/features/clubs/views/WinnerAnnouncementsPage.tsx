...
interface WinnerAnnouncementsPageProps {
  initialWinners?: Array<Record<string, any>>;
  availableEvents?: Array<Record<string, any>>;
}

export function WinnerAnnouncementsPage({ initialWinners = [], availableEvents = [] }: WinnerAnnouncementsPageProps) {
  ...
  useEffect(() => {
    ...
    const transformed = initialWinners.map(w => ({
      ...
      categoryColor: '#6B7280', // Placeholder or use a helper
    ...
    <div className="p-8 animate-page-entrance">
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
          <div>
            <h1 className="text-xl md:text-[28px] font-semibold text-[#1A1A1A] mb-2">Winner Announcements</h1>
            <p className="text-sm text-[#6B7280]">Announce and manage event winners and prizes</p>
          </div>
            ...
            <>
              <div className="bg-[#F4F2F0] rounded-[18px] p-2 pb-6">
                <div className="bg-white rounded-[14px] p-4 flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-1 text-sm text-[#6B7280]">Total Events <InfoTooltip text="Total events with winner announcements" /></div>
              ...
              <div className="bg-[#F4F2F0] rounded-[18px] p-2 pb-6">
                <div className="bg-white rounded-[14px] p-4 flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-1 text-sm text-[#6B7280]">Published <InfoTooltip text="Announcements currently visible to users" /></div>
          ...
          <div key={event.id} className="bg-white rounded-xl border border-[#E5E7EB] overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
              {event.positions.map((pos: any) => (
                <div key={pos.rank} className="p-4 border rounded-xl bg-white flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <div className="mt-2 text-sm font-semibold text-[#10B981]">{pos.prize}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            {[0, 1, 2].map(idx => (
              <div key={idx} className="p-4 border rounded-lg bg-gray-50 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input placeholder="Name/Team" value={formData.positions[idx].teamName} onChange={e => {
                    const newPos = [...formData.positions];
                    newPos[idx].teamName = e.target.value;
                    setFormData({ ...formData, positions: newPos });
                  }} className="px-3 py-2 border rounded-lg text-sm" />
                  <input placeholder="Prize" value={formData.positions[idx].prize} onChange={e => {
                    const newPos = [...formData.positions];
                    newPos[idx].prize = e.target.value;
                    setFormData({ ...formData, positions: newPos });
                  }} className="px-3 py-2 border rounded-lg text-sm" />
                </div>
                <input placeholder="Members (Gmail/Email required for certificate)" value={formData.positions[idx].members} onChange={e => {
                  const newPos = [...formData.positions];
