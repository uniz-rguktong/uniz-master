...
import { X, Upload, Check, Search, Download, Trash2, Grid3x3, Eye, Image as ImageIcon, AlertCircle } from 'lucide-react';
...
export function UploadPhotosModal({ categories, uploadedPhotos, setUploadedPhotos, onDeletePhotos, initialCategory = 'All Categories', hideTargetAlbumDropdown = false, onClose, onSuccess }: UploadPhotosModalProps) {
    ...
    try {
      ...
      if (uploadedUrls.length > 0) {
        ...
        if (res.success) {
          ...
          setStep('manage');
          showToast(`Successfully uploaded ${uploadedUrls.length} photos`, "success");
      ...
      if (failedFiles.length > 0) {
        showToast(`Failed to upload ${failedFiles.length} files. Check CORS/Network settings.`, "warning");
      }
      ...
      <div className="bg-[#F4F2F0] rounded-[24px] p-[10px] max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in duration-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#6B7280]">
                {step === 'upload' ? 'Add new assets to your gallery' : 'Review and manage your uploads'}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="bg-white rounded-[20px] flex-1 overflow-hidden flex flex-col shadow-inner">
              ...
              <div className="p-8">
                ...
                <div className="mb-8 flex flex-col sm:flex-row items-center justify-between p-5 bg-[#F7F8FA] rounded-2xl border border-[#E5E7EB] gap-4">
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-[#1A1A1A]">Target Album</h4>
                      <p className="text-xs text-[#6B7280]">Select where these photos will be stored</p>
                    </div>
                  </div>
                  {hideTargetAlbumDropdown ? (
                    ...
                    <Select value={selectedCategory} onValueChange={setSelectedCategory}>
                      ...
                      <SelectContent className="rounded-xl border-[#E5E7EB] shadow-xl">
                        <SelectItem value="All Categories" disabled className="text-[#9CA3AF]">Choose an Album...</SelectItem>
                        {categories.map(cat => (
                          <SelectItem key={cat.id} value={cat.name} className="py-2.5">{cat.name}</SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  )}
                  ...
                  <div
                    ...
                    <div className="flex flex-col items-center relative z-10">
                      ...
                      <p className="text-sm text-[#6B7280] mb-8 max-w-sm mx-auto leading-relaxed">
                        Drag and drop your photos here, or click to browse. Supports JPG, PNG and WebP.
                      </p>
                      <button
                        type="button"
                    ...
                    <div className="bg-white border border-[#E5E7EB] rounded-[24px] overflow-hidden shadow-md">
                      <div className="px-6 py-5 bg-[#F7F8FA] border-b border-[#E5E7EB] flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <h4 className="text-sm font-bold text-[#1A1A1A]">In Queue</h4>
                        </div>
                        <button
                          onClick={() => setQueue([])}
                      ...
                      <div className="divide-y divide-[#E5E7EB] max-h-[360px] overflow-y-auto custom-scrollbar">
                        {queue.map((file: any) => (
                          <div key={file.id} className="px-6 py-4 flex items-center justify-between group hover:bg-[#F7F8FA]/50 transition-colors">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <div className="text-sm font-bold text-[#1A1A1A] truncate max-w-[200px]">{file.name}</div>
                                <div className="text-[10px] font-medium text-[#6B7280]">{file.size}</div>
                              </div>
                            </div>
                            {isUploading ? (
                              <div className="flex items-center gap-3">
                                <span className="text-xs font-bold text-[#10B981] tabular-nums">{uploadProgress}%</span>
                      ...
                      <div className="space-y-4 bg-[#F7F8FA] p-6 rounded-[24px] border border-[#E5E7EB]">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-bold text-[#1A1A1A]">{processingMessage || 'Processing uploads...'}</span>
                          </div>
                          <span className="tabular-nums font-black text-[#10B981] text-lg">{uploadProgress}%</span>
                        </div>
              ...
              <div className="p-4 md:p-8">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 gap-6 bg-[#F7F8FA] p-4 rounded-2xl border border-[#E5E7EB]">
                  <div className="flex items-center gap-4">
                    <label className="flex items-center gap-3 cursor-pointer group">
                      <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${selectedPhotos.size === filteredPhotos.length && filteredPhotos.length > 0 ? 'bg-[#10B981] border-[#10B981]' : 'bg-white border-[#D1D5DB] group-hover:border-[#10B981]'}`}>
                        <input
                          type="checkbox"
                          checked={selectedPhotos.size === filteredPhotos.length && filteredPhotos.length > 0}
                          onChange={toggleSelectAll}
                          className="hidden" />
                        {selectedPhotos.size === filteredPhotos.length && filteredPhotos.length > 0 && <Check className="w-3 h-3 text-white" />}
                      </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      <Select value={sortOrder} onValueChange={(value) => setSortOrder(value as GallerySortOrder)}>
                ...
                {filteredPhotos.length === 0 ? (
                  <div className="text-center py-20 bg-[#F7F8FA] rounded-3xl border border-dashed border-[#E5E7EB]">
                    ...
                    <h3 className="text-xl font-bold text-[#1A1A1A] mb-2">No photos found</h3>
                    <p className="text-sm text-[#6B7280]">Try adjusting your filters or upload some new photos.</p>
                  </div>
                ) : (
                  <div className={`grid ${getGridClass()} gap-6`}>
                    {filteredPhotos.map((photo: any) =>
                      <div
                        ...
                        >
                          <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${selectedPhotos.has(photo.id) ? 'bg-[#10B981] border-[#10B981] shadow-lg scale-110' : 'bg-white/80 border-white backdrop-blur-sm opacity-0 group-hover:opacity-100'}`}>
          ...
          <div className="border-t border-[#E5E7EB] px-6 py-5 bg-[#F7F8FA] flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-[#6B7280] uppercase tracking-wider">Status:</span>
              <span className="text-sm font-bold text-[#1A1A1A]">
                {step === 'manage' ? `${filteredPhotos.length} Items cached` : (queue.length > 0 ? `${queue.length} Ready to upload` : 'System Ready')}
              </span>
            </div>
