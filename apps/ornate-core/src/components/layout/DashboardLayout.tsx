...
export function DashboardLayout({ children, user, onLogout, branding }: DashboardLayoutProps) {
    ...
    return (
        <div className="flex h-screen bg-[#F7F8FA] overflow-hidden text-black">
            ...
            <div className="flex-1 flex flex-col min-w-0 transition-all duration-300 relative">
                ...
                {/* Main Content Area */}
                <main className={`flex-1 overflow-y-auto w-full custom-scrollbar bg-white ${(user as any)?.role === 'EVENT_COORDINATOR' ? 'px-6 md:px-12 lg:px-20 pt-6 md:pt-8' : ''}`}>
                    {children}
                </main>
            </div>
        </div>
