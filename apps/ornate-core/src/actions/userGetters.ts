    ...
    async () => {
        ...
        return admins.map(admin => ({
            ...
            role: admin.role === 'SUPER_ADMIN' ? 'Super Admin' :
                (admin.role === 'EVENT_COORDINATOR' ? 'Event Coordinator' : 'Administrative Entity'),
            status: 'active',
            lastActive: 'Recent',
            initial: admin.name ? admin.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2) : 'A',
...
export async function getCoordinators(): Promise<{ success?: boolean; data?: FormattedCoordinator[]; error?: string }> {
    return executeAction(async () => {
        ...
        type Row = Awaited<ReturnType<typeof prisma.admin.findMany<{ select: typeof selectFields }>>>[number];

        const format = (rows: Row[]): FormattedCoordinator[] =>
            rows.map(c => {
                const assigned = [
                    ...c.assignedEvents.map(e => e.title),
                ];
                return {
                    id: c.id,
                    ...
                    email: c.email,
                    phone: c.phone || 'N/A',
                    assignedEvent: assigned.length > 0 ? assigned.join(', ') : 'Unassigned',
...
export async function getSystemUsers(): Promise<{ success?: boolean; data?: FormattedSystemUser[]; error?: string }> {
    return executeAction(async () => {
        const session = await getServerSession(authOptions);
...
export async function getAuditLogs(): Promise<{ success?: boolean; data?: FormattedAuditLog[]; error?: string }> {
    return executeAction(async () => {
        const session = await getServerSession(authOptions);
        if (!session) throw new Error('Unauthorized');
