...
function extractBranchFromPosition(position: any): string | null {
    ...
    const members = Array.isArray(position?.members)
        ...
        : typeof position?.members === 'string'
            ? position.members.split(',').map((m: string) => m.trim()).filter(Boolean)
            : [];
...
export async function syncBranchPointsForSport(sportId: string) {
    const sport = await prisma.sport.findUnique({
        where: { id: sportId },
        select: { id: true, category: true, winnerPoints: true, runnerUpPoints: true },
    });
    if (!sport) return;
    ...
    if (sport.category === 'TEAM') {
        ...
        const winnerTeamName = final.winner === MatchWinner.TEAM1 ? final.team1Name : final.team2Name;
        const runnerTeamName = final.winner === MatchWinner.TEAM1 ? final.team2Name : final.team1Name;
        ...
        const positions = Array.isArray(announcement.positions) ? announcement.positions : [];
        const firstPlace = positions.find((p: any) => Number(p?.rank) === 1) || positions[0];
        const secondPlace = positions.find((p: any) => Number(p?.rank) === 2) || positions[1];
    ...
    const adjustmentMap = new Map(existing.map(e => [e.branch, { adj: e.manualAdjustment, reason: e.adjustmentReason }]));

    await prisma.branchPoints.deleteMany({ where: { sportId } });

    const rows: Array<{ sportId: string; branch: string; points: number; manualAdjustment: number; adjustmentReason: string | null }> = [];
    if (winnerBranch) {
        const prev = adjustmentMap.get(winnerBranch);
        rows.push({ sportId, branch: winnerBranch, points: sport.winnerPoints, manualAdjustment: prev?.adj || 0, adjustmentReason: prev?.reason || null });
    }
    if (runnerBranch && runnerBranch !== winnerBranch) {
        const prev = adjustmentMap.get(runnerBranch);
        rows.push({ sportId, branch: runnerBranch, points: sport.runnerUpPoints, manualAdjustment: prev?.adj || 0, adjustmentReason: prev?.reason || null });
    }
...
export async function updateBranchPoints(sportId: string, branch: string, points: number, adjustmentReason?: string) {
    const user = await getAuthenticatedUser();
    if (!user) return { success: false, error: 'Unauthorized' };
    ...
    return executeAction(async () => {
        const updated = await prisma.branchPoints.upsert({
            ...
            create: { sportId, branch, points, manualAdjustment: 0, adjustmentReason: adjustmentReason || null },
...
export async function recalculateBranchPoints(sportId: string) {
    ...
    return executeAction(async () => {
        ...
        if (!final || !final.winner) {
            await prisma.branchPoints.deleteMany({ where: { sportId } });
            await revalidateBranchPointsData();
            return { success: true, data: { recalculated: false, reason: 'Final not completed yet' } };
        }
        ...
        const createRows: Array<{ sportId: string; branch: string; points: number; manualAdjustment: number }> = [];
        if (winnerBranch) createRows.push({ sportId, branch: winnerBranch, points: sport.winnerPoints, manualAdjustment: 0 });
        if (runnerBranch) createRows.push({ sportId, branch: runnerBranch, points: sport.runnerUpPoints, manualAdjustment: 0 });
        ...
        await revalidateBranchPointsData();
        return { success: true, data: { recalculated: true, rows: createRows.length } };
