...
const _getCachedStats = unstable_cache(
    async (): Promise<SuperAdminStats> => {
        try {
            ...
            const totalRegistrations = totalRegistrationCount + totalSportRegistrationCount;
            const pendingApprovalsCount = pendingEventCount + pendingSportRegistrationCount;
        ...
        try {
            ...
            const [events, sports] = await Promise.all([
                prisma.event.findMany({
                    ...
                    select: { id: true, title: true, venue: true, date: true, category: true, status: true },
                    orderBy: { date: 'asc' },
                    take: 5
                }),
            ...
            const mappedEvents = events.map(event => ({
                ...
                date: event.date,
                status: event.status,
                time: event.date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }),
            ...
            const mappedSports = sports.map(sport => ({
                ...
                time: sport.createdAt.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }),
                portal: 'Sports'
            }));

            return [...mappedEvents, ...mappedSports].sort((a, b) => a.date.getTime() - b.date.getTime()).slice(0, 10);
        } catch (error) {
        ...
        try {
            const getTrendData = async (type: 'Weekly' | 'Monthly' | 'Yearly') => {
                ...
                return regData.map((item, idx) => ({
                    ...
                    attended: Math.round((item.count + (sportData[idx]?.count || 0)) * 0.8) // Simulated attendance
                }));
            };
...
const _getCachedPortalPerformance = unstable_cache(
    async () => {
        try {
            ...
            return [...eventData, sportsData].sort((a, b) => b.score - a.score).slice(0, 5);
        ...
        try {
            const [events, sports] = await Promise.all([
                prisma.event.findMany({
                    ...
                    select: {
                        id: true, title: true, venue: true, date: true,
            ...
            const mappedEvents = events.map(event => ({
                ...
                startedAt: event.date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }),
            ...
            const mappedSports = sports.map(sport => ({
                ...
                name: sport.name,
                venue: 'Sports Arena',
                startedAt: sport.createdAt.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }),
        ...
        try {
            const [eventWinners, sportWinners] = await Promise.all([
                prisma.winnerAnnouncement.findMany({
                    ...
                    select: {
                        id: true, positions: true, publishedAt: true,
                        event: {
                            select: { title: true, category: true, venue: true, creator: { select: { name: true, branch: true } } }
                        }
                    },
    ...
    async () => {
        try {
            const [events, sports] = await Promise.all([
                prisma.event.findMany({
                    where: {
                        status: { in: [EventStatus.DRAFT, EventStatus.PUBLISHED, EventStatus.ONGOING] },
                        NOT: { category: 'Sports' }
                    },
                    select: { id: true, title: true, date: true, time: true, endTime: true, venue: true, category: true, status: true },
                    orderBy: { date: 'asc' }
                }),
                prisma.sport.findMany({
                    where: { status: { in: ['UPCOMING', 'REGISTRATION_OPEN', 'ONGOING'] as any } },
                    select: { id: true, name: true, createdAt: true, status: true },
        ...
        try {
            const [events, sportRegs] = await Promise.all([
                ...
                prisma.sportRegistration.findMany({
                    ...
                    select: {
                        id: true, studentName: true, createdAt: true, branch: true,
                        sport: { select: { name: true } }
                    },
                    orderBy: { createdAt: 'desc' },
                    take: 10
            ...
            const mappedEvents = events.map(event => ({
                ...
                conflicts: [],
                poster: event.posterUrl || 'https://images.unsplash.com/photo-1501612780327-45045538702b?w=200&h=200&fit=crop'
            }));
        ...
        try {
            ...
            return admins.map(admin => {
                ...
                switch (admin.role) {
                    ...
                    case 'BRANCH_ADMIN': roleDisplay = 'Branch Admin'; break;
                    case 'CLUB_COORDINATOR': roleDisplay = 'Clubs'; break;
                    case 'SPORTS_ADMIN': roleDisplay = 'Sports'; break;
                    case 'BRANCH_SPORTS_ADMIN': roleDisplay = 'Branch Sports'; break;
                    case 'HHO': roleDisplay = 'HHO'; break;
                    case 'EVENT_COORDINATOR': roleDisplay = 'Event Coordinator'; break;
                    default: roleDisplay = admin.role;
                }

                // Portal Mapping
                let portalDisplay = 'All';
                if (admin.role === 'BRANCH_ADMIN') portalDisplay = admin.branch || 'N/A';
                else if (admin.role === 'CLUB_COORDINATOR') portalDisplay = admin.clubId || 'N/A';
                else if (admin.role === 'SPORTS_ADMIN') portalDisplay = 'Sports';
                else if (admin.role === 'BRANCH_SPORTS_ADMIN') portalDisplay = admin.branch || 'Branch Sports';
                ...
                return {
                    ...
                    avatar: admin.profilePicture || `https://ui-avatars.com/api/?name=${encodeURIComponent(admin.name || 'Admin')}&background=random`
                };
            });
        } catch (error) {
...
export async function getRegistrationTrend() {
    if (!(await requireSuperAdmin())) return { Weekly: [], Monthly: [], Yearly: [] };
