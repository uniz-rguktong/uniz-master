...
async function syncWinnerAnnouncementFromRanks(eventId: string) {
  ...
  if (rankedRegistrations.length === 0) {
    return {
      ...
      error:
        "No ranked registrations found. Assign ranks (1/2/3) first in registrations.",
    };
  }
  ...
  if (positions.length === 0) {
    return {
      success: false as const,
      error:
        "No valid winner entries could be generated from ranked registrations.",
    };
  }
...
export async function announceWinnersFromRegistrations(
  eventId: string,
  ...
  return executeAction(async () => {
    ...
    try {
      ...
      return {
        error: "You do not have permission to announce winners for this event.",
      };
    }
    ...
    return {
      success: true,
      message: "Winners announced successfully from registrations.",
    };
  ...
  return executeAction(async () => {
    ...
    if (params.targetType === "registration") {
      ...
      if (team.registrationId) {
        ...
        const fallbackRegistration = await prisma.registration.findFirst({
          where: {
            ...
            OR: [{ email: team.leaderEmail }, { studentName: team.leaderName }],
          },
          select: { id: true },
        });
      ...
      if (!registrationIdToUpdate) {
        return {
          error:
            "Team is not linked to a registration. Please register team leader first.",
        };
      }
    }
    ...
    return {
      success: true,
      message: `${params.rank} prize assigned and winners announcement updated.`,
    };
...
function buildRegistrationEmailHtml(params: {
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 8px;">
        ...
        <p style="color: #6b7280; font-size: 12px; margin-top: 20px;">This is an automated message from Ornate Core.</p>
    </div>
    `;
}
...
export async function deleteRegistration(
  ...
  return executeAction(async () => {
    ...
    if (!registration && admin.role !== "SUPER_ADMIN") {
      return {
        error:
          "Unauthorized. You can only delete registrations for your assigned events.",
      };
    }
...
export async function updateRegistration(
  ...
  return executeAction(async () => {
    ...
    if (!registration && admin.role !== "SUPER_ADMIN") {
      return {
        error:
          "Unauthorized. You can only update registrations for your assigned events.",
      };
    }
...
export async function createRegistration(
  ...
  return executeAction(async () => {
    ...
    if (user.role === "SUPER_ADMIN" || event.creatorId === user.id)
      isAuthorized = true;
    else if (user.role === "HHO" && event.creator?.role === "HHO")
      isAuthorized = true;
    else if (
      (user.role === "SPORTS_ADMIN" || user.role === "BRANCH_SPORTS_ADMIN") &&
      event.creator?.role === "SPORTS_ADMIN"
    )
      isAuthorized = true;
    else if (
      user.role === "BRANCH_ADMIN" &&
      event.creator?.branch === user.branch
    )
      isAuthorized = true;
    else if (
      user.role === "CLUB_COORDINATOR" &&
      event.creator?.clubId === user.clubId
    )
      isAuthorized = true;
    else if (
      user.role === "EVENT_COORDINATOR" &&
      event.assignedCoordinators?.some((c) => c.id === user.id)
    )
      isAuthorized = true;
    ...
    if (!isAuthorized) {
      return {
        error:
          "Unauthorized. You do not have permission to register students for this event.",
      };
    }
...
export async function updateRegistrationStatus(
  ...
  return executeAction(async () => {
    ...
    if (!isAuthorized) {
      return {
        error:
          "Unauthorized. You (Role: " +
          user.role +
          ") can only update registrations for your assigned events.",
      };
    }
    ...
    const recipientEmail =
      registration.email || registration.user?.email || null;
    const studentName =
      registration.studentName || registration.user?.name || "Participant";
    const eventTitle = registration.event?.title || "Event";

    if (recipientEmail) {
      const shouldSendImmediateReject =
        status === "REJECTED" && previousStatus !== "WAITLISTED";
      const shouldSendWaitlistRejectAfterDeadline =
        status === "REJECTED" && previousStatus === "WAITLISTED" && eventEnded;
      const shouldSendConfirmAfterDeadline =
        status === "CONFIRMED" && eventEnded;

      if (
        shouldSendImmediateReject ||
        shouldSendWaitlistRejectAfterDeadline ||
        shouldSendConfirmAfterDeadline
      ) {
        const rejectionReason =
          typeof extraData.rejectionReason === "string"
 ...
 */
export async function releaseWaitlistSeats(
  eventId: string,
  count: number,
): Promise<RegistrationActionResponse> {
  const session = await getServerSession(authOptions);
  ...
  return executeAction(async () => {
    ...
    if (!isAuthorized) {
      return {
        error:
          "Unauthorized. You do not have permission to release seats for this event.",
      };
    }
    ...
    if (waitlisted.length === 0) {
      return {
        success: true,
        count: 0,
        message: "No students currently on the waitlist for this event.",
      };
    }
...
export async function bulkImportRegistrations(
  ...
  return executeAction(async () => {
    ...
    registrationsData.forEach((row: any, index: any) => {
      ...
      const rollNumber =
        row["Roll Number"] || row["Student ID"] || row["Roll No"];
    ...
    if (eventIdsInvolved.size > 0 && rollNumbersToFetch.size > 0) {
      ...
      existing.forEach((r: { eventId: string; studentId: string | null }) =>
        existingRegMap.add(`${r.eventId}:${r.studentId}`),
      );
    }
    ...
    for (const item of validRows) {
      if (existingRegMap.has(`${item.eventId}:${item.rollNumber}`)) {
        failedCount++;
        errors.push(
          `Row ${item.rowNumber}: Student ${item.rollNumber} is already registered for "${item.eventName}"`,
        );
        continue;
      }
