...
export async function createWinnerAnnouncement(data: WinnerAnnouncementPayload): Promise<ActionResponse> {
    ...
    return executeAction(async () => {
        ...
        if (session.user.role !== 'SUPER_ADMIN' && event.creatorId !== session.user.id) {
            throw new Error('Unauthorized. You do not have permission to announce winners for this event.');
        }
...
export async function updateWinnerAnnouncement(id: string, data: WinnerAnnouncementPayload): Promise<ActionResponse> {
    const session = await getServerSession(authOptions);
    if (!session || !session.user) {
        return { error: 'Unauthorized' };
    }
    ...
    return executeAction(async () => {
        if (session.user.role !== 'SUPER_ADMIN') {
            const currentAnn = await prisma.winnerAnnouncement.findUnique({ where: { id }, select: { id: true, event: { select: { creatorId: true } } } });
            if (!currentAnn || currentAnn.event.creatorId !== session.user.id) {
                throw new Error('Unauthorized. You do not have permission to edit this announcement.');
            }
        }
...
export async function deleteWinnerAnnouncement(id: string): Promise<ActionResponse> {
    const session = await getServerSession(authOptions);
    if (!session || !session.user) {
        return { error: 'Unauthorized' };
    }
...
export async function togglePublishWinnerAnnouncement(id: string, isPublished: boolean): Promise<ActionResponse> {
    const session = await getServerSession(authOptions);
    if (!session || !session.user) {
        return { error: 'Unauthorized' };
    }
...
export async function getEventsForWinners(): Promise<ActionResponse<any[]>> {
    ...
    return executeAction(async () => {
        ...
        const data = await prisma.event.findMany({
            where: whereClause,
            select: { id: true, title: true, category: true, date: true, winnerAnnouncement: true },
            orderBy: { date: 'desc' }
        });
        return { success: true, data };
    }, 'getEventsForWinners');
