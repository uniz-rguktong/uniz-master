...
function buildPromoScope(user: ScopeUser): Prisma.PromoVideoWhereInput | Prisma.BrandLogoWhereInput {
...
export async function getPromoVideos(): Promise<PromoActionResponse<FormattedPromoVideo[]>> {
    try {
...
export async function createPromoVideo(data: CreatePromoVideoData): Promise<PromoActionResponse<FormattedPromoVideo>> {
    return executeAction(async () => {
        const user = await getSessionUser();
        const admin = await prisma.admin.findUnique({ where: { email: user.email }, select: { id: true } });
...
export async function updatePromoVideo(id: string, data: UpdatePromoVideoData): Promise<PromoActionResponse<Partial<FormattedPromoVideo>>> {
    return executeAction(async () => {
        const user = await getSessionUser();
        const admin = await prisma.admin.findUnique({ where: { email: user.email }, select: { id: true, role: true } });
        if (!admin) return { success: false, error: 'Admin not found' };
        ...
        const isAuthorized = admin.role === 'SUPER_ADMIN' || videoCheck.creatorId === admin.id;
...
export async function deletePromoVideo(id: string): Promise<PromoActionResponse> {
    return executeAction(async () => {
        const user = await getSessionUser();
        const admin = await prisma.admin.findUnique({ where: { email: user.email }, select: { id: true, role: true } });
        if (!admin) return { success: false, error: 'Admin not found' };
...
export async function getBrandLogos(): Promise<PromoActionResponse<FormattedBrandLogo[]>> {
    try {
        const user = await getSessionUser();
...
export async function createBrandLogo(data: CreateBrandLogoData): Promise<PromoActionResponse<Partial<FormattedBrandLogo>>> {
    return executeAction(async () => {
        const user = await getSessionUser();
        const admin = await prisma.admin.findUnique({ where: { email: user.email }, select: { id: true } });
        if (!admin) return { success: false, error: 'Admin not found' };
...
export async function updateBrandLogo(id: string, data: UpdateBrandLogoData): Promise<PromoActionResponse<Partial<FormattedBrandLogo>>> {
    return executeAction(async () => {
        const user = await getSessionUser();
        const admin = await prisma.admin.findUnique({ where: { email: user.email }, select: { id: true, role: true } });
        if (!admin) return { success: false, error: 'Admin not found' };
        ...
        const isAuthorized = admin.role === 'SUPER_ADMIN' || logoCheck.creatorId === admin.id;
        if (!isAuthorized) return { success: false, error: 'Unauthorized' };
...
export async function deleteBrandLogo(id: string): Promise<PromoActionResponse> {
    return executeAction(async () => {
        const user = await getSessionUser();
        const admin = await prisma.admin.findUnique({ where: { email: user.email }, select: { id: true, role: true } });
        if (!admin) return { success: false, error: 'Admin not found' };
