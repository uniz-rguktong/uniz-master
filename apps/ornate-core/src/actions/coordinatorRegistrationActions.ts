...
interface OfflineTeamRegistrationInput {
  eventId: string;
  teamName: string;
...
interface OfflineIndividualRegistrationInput {
  eventId: string;
  studentName: string;
  studentId: string;
  email?: string;
...
interface UpdateOfflineTeamRegistrationInput {
...
interface UpdateOfflineRegistrationInput {
  registrationId: string;
  eventId: string;
  studentName: string;
  studentId: string;
...
function canAccessEvent(
  ...
  event: {
    creator?: { branch: string | null } | null;
    assignedCoordinators?: Array<{ id: string }>;
  },
) {
...
async function findOrCreateOfflineUser(member: {
  ...
  const fallbackLocalPart = (member.rollNumber || `offline-${Date.now()}`)
    .trim()
    .toLowerCase();
...
export async function createCoordinatorOfflineTeamRegistration(
  data: OfflineTeamRegistrationInput,
  ...
  return executeAction(async () => {
    ...
    if (!members || members.length === 0)
      return { error: "At least one team member is required" };
    ...
    const captain =
      members.find((member) => member.role === "Captain") || members[0];
    ...
    if (!isTeamEventType(event.eventType))
      return { error: "This event supports only individual registrations" };
    ...
    const leaderUser = await findOrCreateOfflineUser({
      ...
      ...(captain.branch || captain.department
        ? { branch: captain.branch || captain.department }
        : {}),
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const registrationStatus =
        activeCount >= (event.maxCapacity || 100)
      ...
      await tx.teamMember.createMany({
        data: members.map((member, index) => ({
          ...
          name: member.name,
          email:
            member.email?.trim() || `temp-${teamCode}-${index}@offline.local`,
          phone: member.phone?.trim() || null,
          rollNumber: member.rollNumber?.trim() || null,
          department:
            member.branch?.trim() || member.department?.trim() || null,
    ...
    await createAuditLog({
      action: "CREATE_OFFLINE_TEAM_EVENT_REGISTRATION",
    ...
    return {
      success: true,
      message: `Team "${normalizedTeamName}" registered successfully`,
    };
  }, "createCoordinatorOfflineTeamRegistration");
}

export async function createCoordinatorOfflineIndividualRegistration(
  data: OfflineIndividualRegistrationInput,
): Promise<RegistrationResponse> {
  const user = await getAuthenticatedUser();
  if (!user || !ALLOWED_ROLES.includes(user.role)) {
    return { error: "Unauthorized" };
  ...
  return executeAction(async () => {
    ...
    if (!normalizedStudentId)
      return { error: "Student ID / Roll Number is required" };
    ...
    if (existingRegistration)
      return {
        error: `Student ${normalizedStudentId} is already registered for this event`,
      };
    ...
    await createAuditLog({
      action: "CREATE_OFFLINE_INDIVIDUAL_EVENT_REGISTRATION",
      entityType: "REGISTRATION",
    ...
    return {
      success: true,
      message: `${normalizedStudentName} registered successfully`,
    };
  }, "createCoordinatorOfflineIndividualRegistration");
}

export async function deleteCoordinatorRegistration(
  registrationId: string,
): Promise<RegistrationResponse> {
  const user = await getAuthenticatedUser();
  if (!user || !ALLOWED_ROLES.includes(user.role)) {
  ...
  return executeAction(async () => {
    ...
    if (!registration.event)
      return { error: "Linked event not found for this registration" };
    ...
    await createAuditLog({
      action: "DELETE_OFFLINE_EVENT_REGISTRATION",
      entityType: "REGISTRATION",
...
export async function updateCoordinatorOfflineTeamRegistration(
  data: UpdateOfflineTeamRegistrationInput,
): Promise<RegistrationResponse> {
  const user = await getAuthenticatedUser();
  if (!user || !ALLOWED_ROLES.includes(user.role)) {
    return { error: "Unauthorized" };
  ...
  return executeAction(async () => {
    ...
    if (!registration) return { error: "Registration not found" };
    if (!registration.team?.id)
      return { error: "Linked team not found for this registration" };
    if (!event) return { error: "Event not found" };
    ...
    const captainStudentId =
      (captain.rollNumber || "").trim() || registration.studentId;
    ...
    if (duplicateRegistration)
      return {
        error: `Captain ${captainStudentId} is already registered for this event`,
      };
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.teamMember.createMany({
        data: members.map((member, index) => ({
          ...
          name: member.name,
          email:
            member.email?.trim() || `temp-${teamId}-${index}@offline.local`,
          phone: member.phone?.trim() || null,
          rollNumber: member.rollNumber?.trim() || null,
          department:
            member.branch?.trim() || member.department?.trim() || null,
    ...
    await createAuditLog({
      action: "UPDATE_OFFLINE_TEAM_EVENT_REGISTRATION",
      entityType: "REGISTRATION",
      entityId: normalizedRegistrationId,
      performedBy: user.id,
      metadata: {
...
export async function updateCoordinatorOfflineRegistration(
  data: UpdateOfflineRegistrationInput,
): Promise<RegistrationResponse> {
  const user = await getAuthenticatedUser();
  if (!user || !ALLOWED_ROLES.includes(user.role)) {
    return { error: "Unauthorized" };
  ...
  return executeAction(async () => {
    ...
    if (duplicateRegistration)
      return {
        error: `Student ID ${normalizedStudentId} is already registered for this event`,
      };
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (registration.team?.id) {
        const teamUpdateData: any = {
          ...
          leaderEmail:
            data.email?.trim() ||
            `${normalizedStudentId.toLowerCase()}@offline.local`,
    ...
    await createAuditLog({
      action: "UPDATE_OFFLINE_EVENT_REGISTRATION",
      entityType: "REGISTRATION",
      entityId: normalizedRegistrationId,
      performedBy: user.id,
      metadata: {
...
export async function getRecentOfflineRegistrationsV2(eventId: string) {
  const user = await getAuthenticatedUser();
  if (!user || !ALLOWED_ROLES.includes(user.role)) {
    return {
      success: false,
...
export async function getRecentOfflineRegistrations(eventId: string) {
  return getRecentOfflineRegistrationsV2(eventId);
}
