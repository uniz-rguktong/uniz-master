...
function buildSportWhereClause(user: AuthUser): Prisma.SportWhereInput {
    if (user.role === 'SUPER_ADMIN' || user.role === 'SPORTS_ADMIN' || user.role === 'BRANCH_SPORTS_ADMIN') {
        return {};
    }
...
async function getCachedSports(userId: string, role: string, branch: string | null, clubId: string | null) {
    const user: AuthUser = {
        id: userId,
        role,
        email: '',
    ...
    return sports.map((sport) => {
        ...
        return {
            ...
            date: firstMatch?.date ? firstMatch.date.toISOString().split('T')[0]! : 'TBD',
...
async function getCachedSportsDashboardStats(userId: string, role: string, branch: string | null, clubId: string | null) {
    const user: AuthUser = {
        id: userId,
        role,
        email: '',
    ...
    const activeTournaments = sports.filter((sport) => sport.status === 'ONGOING').length;
    const totalRegistrations = sports.reduce((sum, sport) => sum + sport._count.registrations, 0);
    ...
    if (sportIds.length > 0) {
        totalMatches = await prisma.match.count({ where: { sportId: { in: sportIds } } });
...
const getCachedSportsCalendarEvents = unstable_cache(
    async (userId: string, role: string, branch: string | null, clubId: string | null) => {
        const user: AuthUser = {
            id: userId,
            role,
            email: '',
        ...
        return sports.map((sport) => {
            ...
            const dateSource = firstMatch?.date || sport.registrationDeadline || sport.createdAt;
...
export async function getSports(): Promise<{ success?: boolean; sports?: FormattedSport[]; error?: string }> {
    return executeAction(async () => {
        const user = await getAuthenticatedUser();
        if (!user) throw new Error('Unauthorized');

        const sports = await getCachedSports(user.id, user.role, user.branch || null, user.clubId || null);
...
export async function getSportsDashboardStats(): Promise<{ success?: boolean; stats?: SportsDashboardStats; error?: string }> {
    return executeAction(async () => {
        const user = await getAuthenticatedUser();
        if (!user) throw new Error('Unauthorized');

        const stats = await getCachedSportsDashboardStats(user.id, user.role, user.branch || null, user.clubId || null);
...
export async function getSportsListForFilter(): Promise<{
    success?: boolean;
    sports?: Array<{ id: string; name: string; gender: 'MALE' | 'FEMALE' | 'MIXED' }>;
    error?: string;
}> {
    return executeAction(async () => {
        const user = await getAuthenticatedUser();
...
export async function getSportById(sportId: string): Promise<{ success?: boolean; data?: Record<string, unknown>; error?: string }> {
    return executeAction(async () => {
        const user = await getAuthenticatedUser();
        if (!user) throw new Error('Unauthorized');
        ...
        return {
            ...
            data: {
                ...
                registrationDeadline: sport.registrationDeadline ? sport.registrationDeadline.toISOString() : null,
...
export async function getSportsCalendarEvents(): Promise<{ success?: boolean; data?: SportCalendarEvent[]; error?: string }> {
    return executeAction(async () => {
        const user = await getAuthenticatedUser();
        if (!user) throw new Error('Unauthorized');

        const events = await getCachedSportsCalendarEvents(user.id, user.role, user.branch || null, user.clubId || null);
