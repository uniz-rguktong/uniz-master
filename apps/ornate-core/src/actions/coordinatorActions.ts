...
const INVITE_AUTHORIZED_ROLES = ['SUPER_ADMIN', 'BRANCH_ADMIN', 'HHO', 'CLUB_COORDINATOR'] as const;
...
export async function inviteCoordinator(
    ...
    const callerContext = session.user as { role?: string; branch?: string | null; clubId?: string | null };
    if (!callerRole || !(INVITE_AUTHORIZED_ROLES as readonly string[]).includes(callerRole)) {
        return { error: 'Unauthorized: Insufficient permissions to invite coordinators' };
    }
    ...
    return executeAction(async () => {
        ...
        if (
            ...
            return { error: 'Unauthorized: You can only invite coordinators for your branch events' };
        }
        ...
        if (
            ...
            return { error: 'Unauthorized: You can only invite coordinators for your club events' };
        }
        ...
        if (existingAdmin) {
            if (
                ...
                if (alreadyAssigned) {
                    return { success: true, message: 'Coordinator already assigned to this event' };
                }
                ...
                const resolvedName = existingDetails?.name || normalizedName || normalizedEmail.split('@')[0];
                await sendEmail({
...
export async function verifyCoordinatorToken(token: string) {
    ...
    return executeAction(async () => {
        const coordinator = await prisma.admin.findUnique({
            ...
            select: { id: true, email: true, name: true, coordinatorTokenExpiry: true }
        });
        ...
        if (!coordinator.coordinatorTokenExpiry || coordinator.coordinatorTokenExpiry < new Date()) {
...
export async function setCoordinatorPassword(token: string, password: string) {
    ...
    return executeAction(async () => {
        ...
        if (!coordinator || !coordinator.coordinatorTokenExpiry || coordinator.coordinatorTokenExpiry < new Date()) {
...
const MANAGE_AUTHORIZED_ROLES = ['SUPER_ADMIN', 'BRANCH_ADMIN', 'HHO', 'CLUB_COORDINATOR'] as const;
...
async function requireManageAuth() {
    ...
    return session.user as { id: string; role: string; branch?: string | null; clubId?: string | null };
}
...
export async function addCoordinatorQuick(data: {
    ...
    return executeAction(async () => {
        ...
        return { success: true, message: 'Coordinator added and invite sent', id: created.id };
    }, 'addCoordinatorQuick');
}
