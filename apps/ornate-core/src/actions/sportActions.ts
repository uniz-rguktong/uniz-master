...
function mapStatus(
  ...
  if (normalized === "REGISTRATION_OPEN" || normalized === "REGISTRATION OPEN")
...
function buildSportInput(formData: FormData) {
  ...
  const raw = {
    ...
    minPlayersPerTeam:
      (formData.get("minPlayersPerTeam") as string | null) || null,
    maxPlayersPerTeam:
      (formData.get("maxPlayersPerTeam") as string | null) || null,
...
async function createUniqueSportCopyName(
  baseName: string,
...
export async function updateSport(
  ...
  return executeAction(async () => {
    ...
    assertPermission(
      user as User,
      "update:sport",
      toPermissionResource(existingSport),
    );
...
export async function duplicateSport(
  sportId: string,
): Promise<ActionResponse<Sport>> {
  const user = await getAuthenticatedUser();
  if (!user) return { success: false, error: "Unauthorized" };
  ...
  return executeAction(async () => {
    ...
    const copyName = await createUniqueSportCopyName(
      existing.name,
      existing.gender,
    );
...
export async function setSportActive(
  sportId: string,
  isActive: boolean,
): Promise<ActionResponse<Sport>> {
  const user = await getAuthenticatedUser();
