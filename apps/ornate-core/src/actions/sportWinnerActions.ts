    ...
    <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:600px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e5e7eb;">
      ...
      <div style="padding:32px 24px;background:${bgColor};">
        ...
        <div style="background:#ffffff;border-radius:12px;padding:20px;margin-bottom:24px;border:2px solid ${rankColor}30;text-align:center;">
          ...
          <p style="color:#1A1A1A;font-size:26px;font-weight:900;margin:0;">${name || branch}</p>
          <p style="color:#9CA3AF;font-size:12px;margin:4px 0 0 0;">${name ? branch + ' Department' : 'Department'}</p>
        </div>
      ...
      <div style="background:#1A1A1A;padding:16px 24px;text-align:center;">
        <p style="color:#9CA3AF;font-size:11px;margin:0;">
          This is an official notification from <strong style="color:#ffffff;">Ornate EMS</strong>. Please do not reply to this email.
        </p>
...
export async function createSportWinnerAnnouncement(sportId: string, positions: unknown) {
    ...
    return executeAction(async () => {
        ...
        const addPoints = (posKey: 'first' | 'second' | 'third', awardPoints: number) => {
        ...
        for (const { key, rank } of placesToNotify) {
            ...
            const branch = winner?.branch?.trim() || (typeof winner === 'string' ? winner : '');
            ...
            const subject = `🏆 Congratulations ${recipientName}! You've won ${rank === 1 ? 'Winner' : rank === 2 ? 'Runner' : '2nd Runner'} in ${sportName} — Ornate 2K26`;
            const html = buildWinnerCertificateEmail(branch, sportName, rank, certId, name);
...
export async function updateSportWinnerAnnouncement(id: string, data: { positions?: unknown; isPublished?: boolean }) {
    const user = await getAuthenticatedUser();
    if (!user) return { success: false, error: 'Unauthorized' };
    ...
    return executeAction(async () => {
        const updated = await prisma.sportWinnerAnnouncement.update({
            where: { id },
            data: {
                ...(data.positions !== undefined ? { positions: toJsonValue(data.positions) } : {}),
...
export async function toggleSportWinnerAnnouncementPublish(id: string, isPublished: boolean) {
    const user = await getAuthenticatedUser();
    if (!user) return { success: false, error: 'Unauthorized' };
...
export async function getSportWinnerAnnouncements() {
    ...
    return executeAction(async () => {
        const whereClause: Prisma.SportWinnerAnnouncementWhereInput =
            user.role === 'SUPER_ADMIN' || user.role === 'SPORTS_ADMIN' || user.role === 'BRANCH_SPORTS_ADMIN'
                ? {}
                : user.branch
                    ? { sport: { isActive: true } }
                    : {};
...
export async function getSportWinnerAnnouncement(sportId: string) {
    ...
    return executeAction(async () => {
        const announcement = await prisma.sportWinnerAnnouncement.findUnique({ where: { sportId } });
        return { success: true, data: announcement };
