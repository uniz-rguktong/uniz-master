...
interface CoordinatorContextType {
  ...
  updateCoordinator: (
    id: string | number,
    updatedData: Partial<Coordinator>,
  ) => void;
...
export function CoordinatorProvider({
  ...
  const addCoordinator = (
    newCoordinator: Omit<Coordinator, "id" | "status">,
  ) => {
    setCoordinators((prev) => {
      // Check if coordinator already exists by email
      const exists = prev.find(
        (c) => c.email.toLowerCase() === newCoordinator.email.toLowerCase(),
      );
...
export function useCoordinators() {
  ...
  if (!context) {
    throw new Error(
      "useCoordinators must be used within a CoordinatorProvider",
    );
  }
  return context;
}
