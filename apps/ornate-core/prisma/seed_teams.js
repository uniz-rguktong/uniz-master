...
async function main() {
    ...
    if (teamEvents.length === 0) {
        console.log('⚠️ No team events found. Please run the main seed script first.');
        return;
    }
    ...
    if (students.length < 20) {
        console.log('⚠️ Not enough students to form teams. Please seed more users.');
        return;
    }

    // Helper arrays for random data
    const teamNamePrefixes = ['Code', 'Robo', 'Cyber', 'Tech', 'Data', 'Web', 'Net', 'System', 'Logic', 'Byte'];
    const teamNameSuffixes = ['Warriors', 'Ninjas', 'Masters', 'Squad', 'Crew', 'Titans', 'Spartans', 'Wizards', 'Gurus', 'Knights'];
    const statuses = ['PENDING', 'CONFIRMED', 'WAITLISTED', 'ATTENDED', 'CANCELLED'];
    ...
    for (const event of teamEvents) {
        ...
        for (let i = 0; i < numTeams; i++) {
            ...
            for (let m = 0; m < teamSize; m++) {
                teamMembers.push(shuffledStudents[studentIndex % shuffledStudents.length]);
                studentIndex++;
            }
            ...
            const payStatus = (status === 'CONFIRMED' || status === 'ATTENDED') ? 'PAID' : random(paymentStatuses);
            const amount = event.price || 0;
            const txnId = payStatus === 'PAID' ? `TXN${Math.floor(Math.random() * 1000000)}` : null;
            ...
            try {
                ...
                await prisma.registration.create({
                    data: {
                        ...
                        studentId: team.members.find(m => m.role === 'LEADER')?.rollNumber || 'Unknown',
                        status: team.status,
                ...
                console.log(`  ✅ Created team: ${team.teamName} (${team.members.length} members)`);
