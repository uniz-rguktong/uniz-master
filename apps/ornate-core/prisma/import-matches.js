...
const matches = JSON.parse(fs.readFileSync(path.join(backupDir, 'matches.json'), 'utf-8'));
...
async function run() {
        ...
        for (const [key, count] of needed) {
            ...
            try {
                const sport = await prisma.sport.create({
                    data: { name, gender, category: 'TEAM', format: 'KNOCKOUT', status: 'UPCOMING' }
                });
                sportByNameGender.set(key, sport.id);
                keywordMap.set(`${name}||${gender}`, sport.id);
                console.log(`  ✅ "${name}" (${gender}) → ${sport.id}  [${count} matches]`);
            } catch (e) {
                if (e.message.includes('Unique constraint')) {
                    const existing = await prisma.sport.findFirst({ where: { name, gender } });
                    if (existing) {
    ...
    for (const row of matches) {
        if (!row.sport || row.sport === 'All') { skip++; continue; }
        const sportId = findSportId(row.sport, row.gender);
        if (!sportId) { skip++; errors.push(`Match ${row.id}: still no sport for "${row.sport}||${row.gender}"`); continue; }
        ...
        try {
            ...
            if (!err.message.includes('Unique constraint')) errors.push(`Match ${row.id}: ${err.message}`);
