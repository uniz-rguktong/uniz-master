...
function mapEventStatus(raw) {
    if (!raw) return 'DRAFT';
    const map = {
        'draft': 'DRAFT', 'published': 'PUBLISHED', 'active': 'PUBLISHED',
        'ongoing': 'ONGOING', 'completed': 'COMPLETED', 'cancelled': 'CANCELLED', 'canceled': 'CANCELLED',
    };
...
function safeInt(v) { if (v == null) return null; const n = parseInt(String(v)); return isNaN(n) ? null : n; }
...
async function run() {
    ...
    for (const ev of events) {
        ...
        if (status !== 'DRAFT') {
            try {
                ...
                fixCount++;
            } catch (e) { errors.push(`EventStatus ${ev.id}: ${e.message}`); }
        }
    }
    ...
    const sportsAdminIds = new Set(
        admins.filter(a => ['SPORTS_ADMIN', 'SPORTS_COORDINATOR'].includes(a.role)).map(a => a.id)
    );
    const sportEvents = events.filter(e =>
        (e.category && e.category.toLowerCase() === 'sports') || sportsAdminIds.has(e.creatorId)
    );
    console.log(`   Found ${sportEvents.length} sport events`);

    const eventToSportId = new Map();
    for (const event of sportEvents) {
        const cf = (event.customFields && typeof event.customFields === 'object' && !Array.isArray(event.customFields))
            ? event.customFields : {};
