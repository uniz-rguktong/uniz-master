...
console.log(
  ...
  (process.env.DIRECT_URL || process.env.DATABASE_URL).split("@")[1],
); // Log host for debugging (safe-ish)
...
async function main() {
  ...
  const clubCoordinators = [
    {
      ...
      email: "cultural@clubs.com",
      clubId: "cultural",
    },
    {
      name: "English Communication Coordinator",
      email: "english@clubs.com",
      clubId: "english",
    },
    {
      name: "Competitive Exams Coordinator",
      email: "exams@clubs.com",
      clubId: "exams",
    },
    {
      name: "Coding/Technical Coordinator",
      email: "coding@clubs.com",
      clubId: "coding",
    },
    {
      name: "Arts/Literature Coordinator",
      email: "arts@clubs.com",
      clubId: "arts",
    },
    {
      name: "Recreation Coordinator",
      email: "recreation@clubs.com",
      clubId: "recreation",
    },
    {
      name: "Social Service Coordinator",
      email: "social@clubs.com",
      clubId: "social",
    },
    {
      name: "TechExcel Coordinator",
      email: "techexcel@clubs.com",
      clubId: "techexcel",
    },
    {
      name: "Sarvasrijana Coordinator",
      email: "sarvasrijana@clubs.com",
      clubId: "sarvasrijana",
    },
    { name: "Artix Coordinator", email: "artix@clubs.com", clubId: "artix" },
    {
      name: "Kaladharani Coordinator",
      email: "kaladharani@clubs.com",
      clubId: "kaladharani",
    },
    {
      name: "KhelSaathi Coordinator",
      email: "khelsaathi@clubs.com",
      clubId: "khelsaathi",
    },
    { name: "ICRO Coordinator", email: "icro@clubs.com", clubId: "icro" },
    {
      name: "Pixelro Coordinator",
      email: "pixelro@clubs.com",
      clubId: "pixelro",
    },
  ];
  ...
  const usedPairs = new Set(
    existingRegs.map((r) => `${r.userId}-${r.eventId}`),
  );
  ...
  console.log(
    `Preparing to insert ${registrationsToCreate.length} new registrations...`,
  );
  ...
  for (let i = 0; i < registrationsToCreate.length; i += chunkSize) {
    ...
    try {
      await Promise.all(
        chunk.map((data) => prisma.registration.create({ data })),
      );
      if (i % 50 === 0)
        console.log(`  Inserted ${i} / ${registrationsToCreate.length}...`);
    } catch (err) {
  ...
  console.log(
    "✅ Seeding complete! Generated ${students.length} students and ${registrationsToCreate.length} registrations.",
  );
}
