...
async function main() {
  ...
  if (!fs.existsSync(adminsPath)) {
    console.error(
      "❌ Admin backup file not found in prisma/backups/latest. Please run exported-data.js first.",
    );
    return;
  }
  ...
  for (const branchCode of targetBranches) {
    ...
    if (!admin) {
      ...
      if (jsonAdmin) {
        try {
          ...
          console.error(
            `  -> Failed to create admin ${adminEmail}: ${e.message}`,
          );
          continue;
        }
      } else {
        ...
        const fallbackPassword =
          admins[0]?.password ||
          "$2b$10$Pg1mDaz2Yysjgxu.w90yOeI7/hiDt/GE.0osooLyMARjsTcAz33rC";
    ...
    console.log(
      `  -> Found ${branchStudents.length} students for ${branchName}`,
    );
    ...
    for (const template of templates) {
      ...
      const eventDate = new Date(
        `2026-03-${String(randomDay).padStart(2, "0")}`,
      );
      ...
      const poster =
        genericPosters[Math.floor(Math.random() * genericPosters.length)];
      ...
      const newEvent = await prisma.event.create({
        data: {
          ...
          date: eventDate,
          venue: ["Auditorium", "Seminar Hall", "Lab 1", "Conference Room"][
      ...
      if (branchStudents.length > 0) {
        const numRegs =
          Math.floor(Math.random() * Math.min(20, branchStudents.length)) + 5;
