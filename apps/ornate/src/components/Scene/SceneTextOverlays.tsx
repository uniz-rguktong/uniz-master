...
/** Shared props for every SplitText call in this overlay layer */
const splitBase = {
    delay: 60,
    duration: 0.6,
    ease: 'power4.out',
        ...
        <div className={`absolute inset-0 z-10 grid place-items-center pointer-events-none transition-opacity duration-[50ms] ${isWarpingActive ? 'opacity-0' : 'opacity-100'}`}>
            ...
            {showScene1aText && (
                <div className="scene-1a-text col-start-1 row-start-1 place-self-start mt-[20vh] sm:mt-[15vh] ml-[4vw] sm:ml-[8vw] max-w-[90vw] sm:max-w-2xl px-4">
                    <SplitText
                        {...splitBase}
                        text="College fests… feel kinda familiar, right?"
                        className="text-left text-3xl sm:text-5xl md:text-7xl font-bold tracking-tighter text-white leading-[1.1]"
                        textAlign="left"
                    />
                </div>
            )}
            ...
            {showScene1bText && (
                <div className="scene-1b-text col-start-1 row-start-1 place-self-end mb-[20vh] sm:mb-[15vh] mr-[4vw] sm:mr-[8vw] max-w-[90vw] sm:max-w-2xl px-4 text-right">
                    <SplitText
                        {...splitBase}
                        text="Same stage. Same crowd. Same samosas."
                        className="text-right text-3xl sm:text-5xl md:text-7xl font-bold tracking-tighter text-white leading-[1.1]"
                        textAlign="right"
                    />
                </div>
            )}
            ...
            {showScene3Text && (
                <div className="scene-3-text col-start-1 row-start-1 place-self-center max-w-[90vw] sm:max-w-3xl px-4 sm:px-8">
                    <SplitText
                        {...splitBase}
                        text="So we thought… why not try something bigger?"
            ...
            {showScene7Text && (
                <div className="scene-7-text col-start-1 row-start-1 self-end justify-self-center mb-[20vh] sm:mb-[15vh] max-w-[90vw] sm:max-w-3xl px-4 sm:px-8">
                    <SplitText
                        {...splitBase}
                        text="Planets for each branch, stars for your talents, and fun that's out of this world."
                        className="text-center text-3xl sm:text-5xl md:text-7xl font-bold tracking-tighter text-white leading-[1.1]"
                        textAlign="center"
                    />
                </div>
