...
import { SIDEBAR_LINKS, SEARCH_INDEX } from '@/lib/nav-config';
...
export const HomeSidebar: React.FC<HomeSidebarProps> = ({
    isOpen: propIsOpen,
    onClose: propOnClose,
    sidebarSearch: propSidebarSearch,
    setSidebarSearch: propSetSidebarSearch,
    handleSidebarSearch: propHandleSidebarSearch,
    ...
    // Sync state: prefer props if provided
    const isOpen = propIsOpen !== undefined ? propIsOpen : internalIsOpen;
    const sidebarSearch = propSidebarSearch !== undefined ? propSidebarSearch : internalSearch;
    ...
    const setSidebarSearch = (val: string) => {
        if (propSetSidebarSearch) propSetSidebarSearch(val);
        else setInternalSearch(val);
    };

    const handleSidebarSearch = () => {
        if (propHandleSidebarSearch) {
            propHandleSidebarSearch();
            return;
        }
    ...
    React.useEffect(() => {
        ...
        window.addEventListener('open-ornate-sidebar', handleOpen);
        window.addEventListener('toggle-ornate-sidebar', handleToggle);

        return () => {
            window.removeEventListener('open-ornate-sidebar', handleOpen);
            window.removeEventListener('toggle-ornate-sidebar', handleToggle);
        };
    }, []);
