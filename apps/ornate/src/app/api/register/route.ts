...
export async function POST(req: NextRequest) {
  ...
  const ip =
    typeof forwarded === "string" ? forwarded.split(",")[0] : "127.0.0.1";
  ...
  try {
    ...
    const verified = redis ? await redis.get(`verified:${emailLower}`) : null;
    if (!verified) {
      return NextResponse.json(
    ...
    sendWelcomeEmail(user.email, user.name || "Cadet", stdid).catch(
      console.error,
    );
